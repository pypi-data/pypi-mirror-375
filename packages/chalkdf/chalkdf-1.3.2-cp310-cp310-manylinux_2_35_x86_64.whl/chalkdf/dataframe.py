...
class DataFrame:
    ...
    A :class:`DataFrame` wraps a :class:`~chalkdf.libchalk.chalktable.ChalkTable`
    ...
    def scan_parquet(
        ...
        *,
        ...
        normalized = [
            (p if isinstance(p, str) and "://" in p else Path(p).resolve().as_uri())  # type: ignore[arg-type]
            for p in input_uris
        ]
    ...
    def with_columns(self, dict: typing.Mapping[str, Expr]) -> DataFrame:
    ...
    def filter(self, expr: Expr) -> DataFrame:
        ...
        new_plan = self._plan.filter(expr)
        return self._apply_function(new_plan)
    ...
    def slice(self, start: int, length: int | None = None) -> DataFrame:
        ...
        new_plan = self._plan.limit(length or (2**63), start)
        return self._apply_function(new_plan)
    ...
    def project(self, columns: typing.Mapping[str, Expr]) -> DataFrame:
        """Project to the provided column expressions."""

        projections = {k: v for k, v in columns.items()}
    ...
    def join(self, other: DataFrame, on: dict[str, str] | typing.Sequence[str], how: str = "inner") -> DataFrame:
        ...
        :param on: Column names or mapping of right->left join keys.
        :param how: Join type (e.g. ``"inner"`` or ``"left"``).
        :return: Resulting ``DataFrame`` after the join.
        """

        if isinstance(on, dict):
            on_right = list(on.keys())
            on_left = [on[r] for r in on_right]
        else:
    ...
    def agg(self, by: typing.Sequence[str], *aggregations: typing.Sequence[AggExpr]) -> DataFrame:
        """Group by ``by`` columns and apply aggregation expressions."""

        new_plan = self._plan.aggregate_exprs([*by], [*aggregations])
        return self._apply_function(new_plan)
