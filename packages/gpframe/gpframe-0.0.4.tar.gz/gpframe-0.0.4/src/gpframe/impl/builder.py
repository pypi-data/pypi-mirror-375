...
class _FrameSynchronization(Generic[R]):
    ...
    environment_map: SynchronizedMap
    request_map: SynchronizedMap
    event_msg_map: SynchronizedMap
    routine_msg_map: SynchronizedMap
...
class _Updater:
    ...
    def create_routine_synchronization(
        ...
        environment_map = SynchronizedMap(lock, map_factory(environments), validate_accessable_phase)
        request_map = SynchronizedMap(lock, map_factory(requests), validate_accessable_phase)
        event_msg_map = SynchronizedMap(lock, map_factory(), validate_accessable_phase)
        routine_msg_map = SynchronizedMap(lock, map_factory(), validate_accessable_phase)
...
def create_builder_role(routine: Routine[R]) -> _Role[R]:
    ...
    class _Interface(FrameBuilderType):
        ...
        def set_exception_handler(self, handler: ExceptionHandler[R]):
            def fn():
        ...
        def set_redo_handler(self, handler: RedoHandler[R]):
            def fn():
