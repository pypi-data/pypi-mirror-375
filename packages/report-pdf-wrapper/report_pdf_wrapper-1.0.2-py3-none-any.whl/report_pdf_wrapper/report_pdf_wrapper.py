...
class PdfWrapper:
    ...
    def draw_work_summary(self):
        # Title
        self.page.insert_text(Point(62, 431), "Work Summaryyyy", fontsize=12)
        # Bounding box for the summary area
        self.draw_box(Point(60, 435), Point(520, 435), Point(520, 550), Point(60, 550))
        # Create a multiline text widget that auto-adjusts font size to fit
        field = fitz.Widget()
        field.field_name = "work_summary_field"
        field.field_type = fitz.PDF_WIDGET_TYPE_TEXT
        # Auto-fit font size: PDF viewers will shrink the font as content grows
        field.text_fontsize = 0
        field.text_color = (0, 0, 0)
        field.text_maxlen = 5000
