...
# 更嚴謹的 email：避免把逗號/標點吃進去，減少 Google API 400
...
# 僅數字（可能是「1.5小時」被吃掉單位後的殘留）
_TITLE_NUMERIC_ONLY = re.compile(r'^\s*\d+(?:\.\d+)?\s*$')

# 開頭殘留的「+ 時長」：例如「+ 90 min focus block」或「＋90分鐘 專注」
_TITLE_LEADING_PLUS = re.compile(
    r'^(?:\s*[+＋]\s*\d+(?:\.\d+)?\s*(?:minutes?|mins?|hours?|hrs?|分鐘|小時)\b[ ,，：:;；\-\u2013\u2014]*)+',
    re.I
)
# 開頭殘留的「純時長」：避免 \b 在中文結尾失效，改用 lookahead 檢查邊界
_TITLE_LEADING_DURATION = re.compile(
    r'^(?:\s*(?:\d+(?:\.\d+)?\s*(?:minutes?|mins?|hours?|hrs?)|[一二兩三四五六七八九十\d]+(?:\.\d+)?\s*(?:分鐘|小時))(?=$|[\s,，：:;；\-\u2013\u2014!！]))+',
    re.I
)

# now + XX / XX from now / XX later：用於 tidy 掉殘影
_TITLE_NOW_ADD = re.compile(
    r'(?i)\bnow\s*[\+\uFF0B]\s*\d+(?:\.\d+)?\s*(?:minutes?|mins?|hours?|hrs?)\b'
)
_TITLE_FROM_NOW_LATER = re.compile(
    r'(?i)'
    r'\b\d+(?:\.\d+)?\s*(?:minutes?|mins?|hours?|hrs?)\s*(?:from\s+now|later)\b'
    r'|[一二兩三四五六七八九十\d]+(?:\.\d+)?\s*(?:分鐘|小時)後'
...
def tidy_title(title: str, idea_text: str = "") -> str:
    ...
    # 去除 now + XX / XX from now / XX later / 中文「…後」殘影
    ...
    # 括號殘留與全形括號
    s = re.sub(r"[\(\)（）\[\]【】{}]", "", s)
    # 去除殘留 now/現在/此刻 單字與加號
    s = re.sub(r"(?i)\bnow\b|現在|此刻", "", s)
    # 最後保險掃兩回合：前面的取代可能讓「+ 時長 / 純時長」又浮到字首
    for _ in range(2):
        s = _TITLE_LEADING_PLUS.sub("", s)
        s = _TITLE_LEADING_DURATION.sub("", s)
    # 把殘留加號標準化（若還有的話）
    s = s.replace("+", " ").replace("＋", " ")
    # 也把半形句點一起清掉，避免殘留 '1.' 這種碎片
    s = s.strip(" .,，。．;；-–—!！")
    # 若清完只剩數字（例如「1」或「1.5」），視為無語意 → 直接歸類 Meeting
    if _TITLE_NUMERIC_ONLY.fullmatch(s):
        s = "Meeting"
    # 若清完只剩極短停用詞或標點，且原句屬相對「now / later / …後」
...
# NEW (Milestone A): 支援「now + 90 min / 90 min from now / 現在+90分鐘 / 90分鐘後」
...
def parse_event(text: str, tz: str = DEFAULT_TIMEZONE, default_minutes: int = 60) -> Dict:
    ...
    now = datetime.now(pytz.timezone(tz))
    used_labeled = False
    # 順延的寬限（分鐘）：避免「對齊到整分」比現在小幾十秒就被視為過去
    ROLL_GRACE_MINUTES = 2
    # NEW (Milestone A): 若出現「now + X」語型，強制以 now 為開始時間，X 視為時長提示
    ...
    if m_now:
        try:
            if m_now.re is ZH_NOW_PLUS:
                raw = m_now.group(1)
                unit = m_now.group(2)
                # 支援中文小數與阿拉伯數字：1.5 小時 / 90 分鐘
                if re.fullmatch(r"\d+(?:\.\d+)?", raw):
                    valf = float(raw)
                    hinted_minutes = int(round(valf)) if unit == "分鐘" else int(round(valf * 60))
                else:
                    ival = _zh_to_int(raw)
                    hinted_minutes = ival if unit == "分鐘" else ival * 60            
            else:
                val = float(m_now.group(1))
                unit = (m_now.group(2) or "").lower()
                hinted_minutes = int(round(val * 60)) if unit.startswith(("hour", "hr")) else int(round(val))
            if hinted_minutes and hinted_minutes > 0:
                force_now_start = True
        except Exception:
            pass
    else:
        # 「… later / … 後」也視為 now 開始＋時長
        if LATER_EN.search(s) or LATER_ZH.search(s) or LATER_ZH_HALF.search(s):
            mins = _extract_duration_minutes(s)
            if mins and mins > 0:
                hinted_minutes = mins
                force_now_start = True
    ...
    if not (start_dt and end_dt) and not has_explicit_date(s) and not force_now_start:
        # 若字串含小數（如 1.5），避免把它拆成兩個小時 token
        if not re.search(r"\d+\.\d+", s):
            toks = list(_TIME_TOK.finditer(s))
            # 至少兩個 token，且至少一個 token 帶「分鐘」或 am/pm，才啟動
            def _has_detail(m): return bool(m.group(2) or m.group(3))
            if len(toks) >= 2 and (_has_detail(toks[0]) or _has_detail(toks[1])):
    ...
    if not (start_dt or end_dt):
        # Milestone A：若命中「now + X」語型，直接以 now 作為開始，避免被 dateparser 誤解成年份
        if force_now_start:
    ...
    # 若偵測到「now + X」語型，優先使用 hinted_minutes 當作時長
    dur = hinted_minutes or _extract_duration_minutes(s) or default_minutes
    ...
    if start_dt:
        ...
        if (not has_explicit_date(s) or re.search(r"(?:\b(today)\b|今天)", s, re.I)):
            # 例外：相對「now」語型不順延；或在寬限內也不順延
            if not force_now_start:
                if start_dt < now:
                    if (now - start_dt) > timedelta(minutes=ROLL_GRACE_MINUTES):
