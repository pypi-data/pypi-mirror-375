...
from .model import ADR, PolicyModel, ImportPolicy, BoundaryPolicy, PythonPolicy, BoundaryLayer, BoundaryRule
...
class PolicyPatternExtractor:
    ...
    def _extract_boundary_policies(self, content: str) -> Optional[BoundaryPolicy]:
        ...
        if rules:
            return BoundaryPolicy(rules=rules)
...
class PolicyExtractor:
    ...
    def _merge_policies(self, structured: Optional[PolicyModel], pattern: Optional[PolicyModel]) -> PolicyModel:
    ...
    def has_extractable_policy(self, adr: ADR) -> bool:
        ...
        return (
            (policy.imports and (policy.imports.disallow or policy.imports.prefer)) or
            (policy.boundaries and (policy.boundaries.layers or policy.boundaries.rules)) or
            (policy.python and policy.python.disallow_imports) or
            bool(policy.rationales)
        )
