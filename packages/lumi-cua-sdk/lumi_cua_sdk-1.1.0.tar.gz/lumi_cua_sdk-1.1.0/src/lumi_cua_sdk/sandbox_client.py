...
class Sandbox:
    ...
    async def get_stream_url(self) -> str:
        ...
        response = await self.sandbox_manager.describe_sandbox_terminal_url(self.id)
        stream_url = response.get('Result', {}).get('Url')

        if self.os_type == SandboxOsType.WINDOWS.value:
            if not stream_url:
                logger.warning(
                    "Get empty stream url from ecs manager. You may not have filled in AK/SK when creating the CUA application.")
            else:
                # Construct Guacamole-style URL with parameters
                result = response.get('Result', {})
                windows_key = result.get('WindowsKey', self.id)  # Use sandbox ID as fallback password
                
                # Extract base URL from sandbox_manager_url
                base_url = ""
                if self.sandbox_manager_url.endswith("/mgr/"):
                    base_url = self.sandbox_manager_url.split("/mgr/")[0]
                elif get_config().web_ui_url:
                    base_url = get_config().web_ui_url
                else:
                    logger.warning("No web ui endpoint provided")

                # Construct the full Guacamole URL
                guac_url = f"{base_url}/guac/index.html?url={stream_url}&instanceId={self.id}&ip={self.ip_address}&password={windows_key}&locale=zh"
                return guac_url
        else:
            if not stream_url:
                logger.warning("Get empty stream url from ecs manager. In the case of non-standard gateways, "
                               "linux sandboxes currently do not support obtaining remote stream url in .")
                ...
                stream_url = (f"https://{apig_url}/novnc/vnc.html?host={apig_url}:443&"
