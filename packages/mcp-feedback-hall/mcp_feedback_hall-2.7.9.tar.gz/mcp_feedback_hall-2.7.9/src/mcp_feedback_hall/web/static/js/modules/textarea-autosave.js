...
(function() {
    ...
    function TextareaAutoSave(options) {
        ...
        this.autoSaveInterval = 0; // 停用定期自動保存以減少CPU消耗
    ...
    TextareaAutoSave.prototype.setupEventListeners = function() {
        ...
        logger.debug('事件監聽器設置完成（已移除自動保存功能）');
    };
     ...
     * 手動還原暫存內容
     */
    TextareaAutoSave.prototype.manualRestore = function() {
        if (!this.textarea) {
            logger.warn('找不到 textarea 元素，無法還原');
            return false;
        }
        ...
        try {
            ...
            logger.error('手動還原 textarea 內容失敗：', error);
            return false;
        }
    };
     ...
     * 檢查是否有可用的備份
     */
    TextareaAutoSave.prototype.hasBackup = function() {
        try {
            const savedData = localStorage.getItem(this.storageKey);
            if (!savedData) {
                return false;
     ...
     * 取得備份資訊
     */
    TextareaAutoSave.prototype.getBackupInfo = function() {
        try {
            const savedData = localStorage.getItem(this.storageKey);
            if (!savedData) {
                return null;
            ...
            return {
                ...
                contentLength: saveData.content.length,
                savedTime: new Date(saveData.timestamp).toLocaleString(),
                preview: saveData.content.substring(0, 100) + (saveData.content.length > 100 ? '...' : '')
            };

        } catch (error) {
            logger.error('取得備份資訊失敗：', error);
            return null;
        }
    };
     ...
     * 手動觸發保存
     */
    TextareaAutoSave.prototype.manualSave = function() {
        logger.info('手動觸發保存 textarea 內容');
        this.saveContent();
    };
    ...
    TextareaAutoSave.prototype.clearAfterSubmit = function() {
        logger.info('手動提交成功，但保留暫存內容（永不刪除備份）');
        // 註解: 永遠不清除暫存，保留備份
        // this.clearSavedContent();
    };

    /**
     * 啟動定期自動保存 (已停用以減少CPU消耗)
     */
    TextareaAutoSave.prototype.startAutoSave = function() {
        logger.debug('定期自動保存已停用以減少CPU消耗');
        // 原功能已停用以優化效能
