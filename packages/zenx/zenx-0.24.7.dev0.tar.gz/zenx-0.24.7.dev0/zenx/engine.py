...
class Engine:
    ...
    async def _execute_spider(self, spider_name: str, settings: Settings) -> None:
        ...
        if settings.APP_ENV == "dev":
            logger.warning("dev", pipelines=spider_cls.pipelines[:1], db="memory", spider=spider_name)
        ...
        monitor = Monitor.get_monitor(spider_cls.monitor_name)(logger=logger, settings=settings)
        if monitor:
            await monitor.open()
    ...
    async def _execute_listener(self, listener_name: str, settings: Settings) -> None:
        ...
        if settings.APP_ENV == "dev":
            logger.warning("dev",  pipelines=listener_cls.pipelines[:1], db="memory", listener=listener_name)
        ...
        monitor = Monitor.get_monitor(listener_cls.monitor_name)(logger=logger, settings=settings)
        if monitor:
            await monitor.open()
