...
from .models import (
    GuardrailConfig, FragmentTarget, FragmentType, ConfigFragment, 
    ApplyResult, ApplicationStatus, ConfigTemplate
)
...
class GuardrailManager:
    ...
    def _create_default_config(self) -> GuardrailConfig:
        ...
        targets = [
            FragmentTarget(
                file_path=Path(".eslintrc.adrs.json"),
...
{disallow_imports}
    ...
    def _generate_eslint_fragment(self, contract) -> Optional[ConfigFragment]:
        ...
        if not contract.constraints.imports or not contract.constraints.imports.disallow:
            return None
  ...
  "rules": {{
    ...
    def _generate_ruff_fragment(self, contract) -> Optional[ConfigFragment]:
        ...
        if not contract.constraints.python or not contract.constraints.python.disallow_imports:
            return None
...
{",".join(banned_imports)}
    ...
    def _generate_import_linter_fragment(self, contract) -> Optional[ConfigFragment]:
        ...
        if not contract.constraints.boundaries or not contract.constraints.boundaries.rules:
            return None
        ...
        for i, rule in enumerate(contract.constraints.boundaries.rules):
            ...
            contracts.append(f'''[contracts.{contract_name}]
...
forbidden_modules = ["{rule.forbid}"]''')
    ...
    def _log_policy_changes(self, changes: List[ChangeEvent], results: List[ApplyResult]):
        ...
        success_count = len([r for r in results if r.status == ApplicationStatus.SUCCESS])
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        for target in self.config.targets:
            target_status[str(target.file_path)] = {
                ...
                "has_managed_section": self.config_writer.has_managed_section(target) if target.file_path.exists() else False,
