...
if not os.getenv("COVERAGE_FILE"):
    ...
    cov_file = Path(tempfile.gettempdir()) / f".coverage.pytest.{os.getpid()}.{uuid.uuid4().hex}"
...
def _remove_path(p: Path) -> None:
    try:
        ...
        print(f"[conftest] Warning: failed to remove {p}: {exc}", file=sys.stderr)
...
def _remove_coverage_files(bases: Iterable[Path]) -> None:
    for base in bases:
        ...
        # Any leftover parallel/previous data artifacts like .coverage.hostname.pid.*
...
def pytest_sessionstart(session: Any) -> None:
    ...
    try:
        ...
        cfg_tmp = Path(tempfile.gettempdir()) / f"g2g-empty-config-{os.getpid()}.ini"
