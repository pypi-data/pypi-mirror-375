...
class Diffusion(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            ...
            ResConvBlock(c, c, c),
            SkipBlock([
                nn.image.Downsample2d(),  # 28x28 -> 14x14
                ResConvBlock(c, c * 2, c * 2),
                ResConvBlock(c * 2, c * 2, c * 2),
                SkipBlock([
                    nn.image.Downsample2d(),  # 14x14 -> 7x7
                    ResConvBlock(c * 2, c * 2, c * 2),
                    ResConvBlock(c * 2, c * 2, c * 2),
                    ResConvBlock(c * 2, c * 2, c * 2),
                    ResConvBlock(c * 2, c * 2, c * 2),
                ...
                nn.Conv2d(c*2, c, 3, padding=1),  # Just use a post convolution to reduce the channels.
            ]),
...
def load_opt_state_dict(model, state_dict):
    def load(x):
        if isinstance(opt_state, StateDict):
            px = ParamState()
            model.load_state_dict(px, x)
            return px
        return x
    jax.tree_util.tree_map(load, state_dict, is_leaf=lambda x: isinstance(x, StateDict))

# Load checkpoint
if os.path.exists('mnist_diffusion.pth'):
    state_dict = jaxtorch.pt.load('mnist_diffusion.pth')
    model.load_state_dict(params, state_dict['model'])
    model.load_state_dict(params_ema, state_dict['model_ema'])
    load_opt_state_dict(model, state_dict['opt'])
...
def demo():
    ...
    fakes = jax.random.normal(rng.split(), [100, 1, 28, 28])
    fakes_classes = jnp.arange(10).reshape([10,1]).broadcast_to([10,10]).reshape([100])
    ts = jnp.ones([100])
