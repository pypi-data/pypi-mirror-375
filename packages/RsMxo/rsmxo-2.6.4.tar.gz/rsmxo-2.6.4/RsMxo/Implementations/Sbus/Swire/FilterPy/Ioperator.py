...
class IoperatorCls:
	...
	def set(self, frame: str, field: str, operator: enums.OperatorA, serialBus=repcap.SerialBus.Default) -> None:
		"""SBUS<*>:SWIRe:FILTer:IOPerator \n
		Snippet: driver.sbus.swire.filterPy.ioperator.set(frame = 'abc', field = 'abc', operator = enums.OperatorA.ANY, serialBus = repcap.SerialBus.Default) \n
		...
		self._core.io.write(f'SBUS{serialBus_cmd_val}:SWIRe:FILTer:IOPerator {param}'.rstrip())

	# noinspection PyTypeChecker
	def get(self, serialBus=repcap.SerialBus.Default) -> enums.OperatorA:
		"""SBUS<*>:SWIRe:FILTer:IOPerator \n
		Snippet: value: enums.OperatorA = driver.sbus.swire.filterPy.ioperator.get(serialBus = repcap.SerialBus.Default) \n
		Sets the operator for the index in the selected field of the selected frame. \n
			:param serialBus: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sbus')
			:return: operator: No help available"""
		serialBus_cmd_val = self._cmd_group.get_repcap_cmd_value(serialBus, repcap.SerialBus)
		response = self._core.io.query_str(f'SBUS{serialBus_cmd_val}:SWIRe:FILTer:IOPerator?')
