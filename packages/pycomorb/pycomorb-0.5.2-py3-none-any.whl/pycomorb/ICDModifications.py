...
def get_icd10gm(
    ...
    transfer_file_path = Path(__file__).parent / "germanmodification/icd10gm.csv" # fmt: skip
    ...
    transfer_data = (
        ...
        .cast({year_col: int})
        .rename({str(target_year): f"icd10gm_{target_year}"})
    )
    ...
    return (
        ...
        .with_columns(
            pl.coalesce(
                pl.col(f"icd10gm_{target_year}"), pl.col(code_col)
            ).alias(f"icd10gm_{target_year}")
        )
    )
