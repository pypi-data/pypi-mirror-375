...
class StatusFlags:
    ...
    def __eq__(self, to_compare):
        ...
        try:
            ...
            # StatusFlags object
            return (
                (self.healthy == to_compare.healthy)
                and (self.fully_retracted == to_compare.fully_retracted)
                and (self.moving == to_compare.moving)
                and (self.clutch_engaged == to_compare.clutch_engaged)
                and (self.locked == to_compare.locked)
                and (self.dropping == to_compare.dropping)
                and (self.arresting == to_compare.arresting)
                and (self.ground_sense == to_compare.ground_sense)
                and (self.retracting == to_compare.retracting)
                and (self.redeliver == to_compare.redeliver)
                and (self.abandon_line == to_compare.abandon_line)
                and (self.locking == to_compare.locking)
                and (self.load_line == to_compare.load_line)
                and (self.load_payload == to_compare.load_payload)
            )
    ...
    def __eq__(self, to_compare):
        ...
        try:
            ...
            return (
                ...
                and (self.line_length_m == to_compare.line_length_m)
                and (self.speed_m_s == to_compare.speed_m_s)
                and (self.tension_kg == to_compare.tension_kg)
                and (self.voltage_v == to_compare.voltage_v)
                and (self.current_a == to_compare.current_a)
                and (self.temperature_c == to_compare.temperature_c)
