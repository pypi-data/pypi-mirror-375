...
class TemplateRenderer:
    ...
    def _check_manual_section_ids(self, data_string: str, data_name: str, context: RenderContext = None) -> List[str]:
        ...
        """
        # Use the actual filename from context if available, otherwise fall back to data_name
        actual_filename = context.filename if context else data_name
        ...
        if len(possible_iter) != len(sections_iter):
            ...
            raise ManualSectionError(actual_filename, lineno, f"{data_name} has invalid section")
        ...
        if duplicates:
            ...
            if context:
                context.update(lineno=lineno)
            raise ManualSectionError(actual_filename, lineno, f"{data_name} has duplicated id: {duplicates}")
        # return list of ids
        return sids
    ...
    def _check_manual_section_structure(self, data_string: str, data_name: str, context: RenderContext = None) -> None:
        ...
        for m in matches:
            ...
            if section.count(self.MANUAL_SECTION_START) > 1 or section.count(self.MANUAL_SECTION_END) > 1:
                ...
                if context:
                    context.update(lineno=lineno)
                raise ManualSectionError(actual_filename, lineno, f"Nested section in {data_name}: {section}")
        ...
        if prev_rendered:
            ...
            for sid in prev_sids:
                if sid not in curr_sids:
                    ...
                    if context:
                        context.update(lineno=lineno)
                    actual_filename = context.filename if context else "template"
                    raise ManualSectionError(actual_filename, lineno, f"New template lost manual section: {sid}")
