...
def main():
    ...
    if args.action == "download":
        # 获取下载任务的csv路径
        url_list = client.get_url_list(args.dataset, args.task
        ...
        for episode in url_list['data']:
            ...
            path = os.path.join(output_dir, tid, str(eid) + '.tar')
            os.makedirs(os.path.dirname(path), exist_ok=True)
            download_file_with_progress(url, save_path=path, chunk_size=1024*1024, max_retries=3)