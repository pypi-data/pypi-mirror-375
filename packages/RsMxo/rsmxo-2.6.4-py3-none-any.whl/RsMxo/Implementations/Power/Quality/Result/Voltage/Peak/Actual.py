...
class ActualCls:
	...
	def get(self, power=repcap.Power.Default) -> float:
		"""POWer<*>:QUALity:RESult:VOLTage:PEAK[:ACTual] \n
		Snippet: value: float = driver.power.quality.result.voltage.peak.actual.get(power = repcap.Power.Default) \n
		...
		response = self._core.io.query_str(f'POWer{power_cmd_val}:QUALity:RESult:VOLTage:PEAK:ACTual?')
