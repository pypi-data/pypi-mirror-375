...
class FlatBackground(BackgroundLayer):
    ...
    def map_contexts(self, index):
        """
        The purpose of this routine is to pre-cache a mapping of local "naive contexts" known by the original data
        source, to canonical contexts returned by the query's governing term manager. This is vital because it is
        the exchange specifications generated by the background that become the inputs to LCIA later on.

        Now, it seems possible that the context lookup could be handled by the implementation, which already has an
        _index implementation which is usually a catalog query. So- it's possible this is a case of optimization
        rather than a true necessity. It does break convention a bit. Someday perhaps I will be afforded the time
        to refactor this implementation.
        ...
        for k in self._ex:
            if k.term_ref not in self.context_map:
                ...
                naive_context = index.get_context(term)  # always accesses the data source's contexts
                canonical_context = index._tm[naive_context]  # LciaEngine will retrieve canonical contexts
