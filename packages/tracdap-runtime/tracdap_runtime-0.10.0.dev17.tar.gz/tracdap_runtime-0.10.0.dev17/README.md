...
Using TRAC is a quick and easy way to build production-grade models in Python.
Simply define the parameters, inputs and outputs of a model and TRAC will manage them for you,
so all you need to worry about is the model code. As well as saving time TRAC provides
excellent performance and robust validation, guaranteeing model inputs and outputs always
agree with the defined schema. Most common data formats and storage protocols
are supported out of the box, with an extension mechanism to add custom ones if needed.

The TRAC runtime allows models to be launched anywhere, providing a simple way to ship,
share and execute models that guarantees predicable behavior with no additional effort.
For maximum benefit models can be loaded into the TRAC platform, providing a no-code
route-to-live and a controlled execution environment suitable for the highest governance
requirements.

For a complete guide to writing models in TRAC, see the
...
class QuickStartModel(trac.TracModel):
    ...
    def define_outputs(self):
        ...
        loans_by_region = trac.define_input_table(
            trac.F("region", trac.STRING, label="Customer home region", categorical=True),
            trac.F("loan_amount", trac.DECIMAL, label="Principal loan amount"),
...
# For local development, models can be launched and debugged in an IDE or notebook
...
for a complete guide to writing models in TRAC, including tutorials and an API reference.
