...
def calculate_fundamental_metrics(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        metrics = {
            ...
            "fifty_two_week_low": company_info.get("fifty_two_week_low"),
        ...
        # Compute PEG ratio if possible: PEG = PE / (EPS growth rate as percentage)
        try:
            pe = metrics.get("pe_ratio")
            eg = metrics.get("earnings_growth")
            if pe is not None and eg is not None and eg > 0:
                # earnings_growth from yfinance is typically a decimal (e.g., 0.15 for 15%)
                metrics["peg_ratio"] = pe / (eg * 100)
        except Exception:
            # Best-effort calculation; ignore if any parsing error
            pass
