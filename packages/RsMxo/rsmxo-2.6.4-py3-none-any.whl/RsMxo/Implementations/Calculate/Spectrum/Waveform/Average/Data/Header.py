...
class HeaderCls:
	...
	class GetStruct(StructBase):
		"""Response structure. Fields: \n
			- 1 Xstart: float: 1. header value: start time XStart in s
			- 2 Xstop: float: 2. header value: end time XStop in s
			- 3 Record_Length: int: 3. header value: record length of the waveform in samples
			- 4 Vals_Per_Smp: int: 4. header value: the number of values per sample interval is always = 1."""
		__meta_args_list = [
	...
	def get(self, spectrum=repcap.Spectrum.Default) -> GetStruct:
		"""CALCulate:SPECtrum<*>:WAVeform:AVERage:DATA:HEADer \n
		Snippet: value: GetStruct = driver.calculate.spectrum.waveform.average.data.header.get(spectrum = repcap.Spectrum.Default) \n
		...
		return self._core.io.query_struct(f'CALCulate:SPECtrum{spectrum_cmd_val}:WAVeform:AVERage:DATA:HEADer?', self.__class__.GetStruct())
