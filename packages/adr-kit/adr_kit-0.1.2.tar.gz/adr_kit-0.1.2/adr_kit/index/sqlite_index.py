...
class ADRSQLiteIndex:
    ...
    def _create_tables(self) -> None:
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_adr_links_from ON adr_links (from_adr_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_adr_links_to ON adr_links (to_adr_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_adr_links_type ON adr_links (link_type)")
    ...
    def index_adr(self, adr: ADR) -> None:
        ...
        date_str = adr.front_matter.date.isoformat() if isinstance(adr.front_matter.date, date) else str(adr.front_matter.date)
        status_str = adr.front_matter.status.value if isinstance(adr.front_matter.status, ADRStatus) else str(adr.front_matter.status)
        ...
        cursor.execute("DELETE FROM adr_deciders WHERE adr_id = ?", (adr.front_matter.id,))
        cursor.execute("DELETE FROM adr_tags WHERE adr_id = ?", (adr.front_matter.id,))
        cursor.execute("DELETE FROM adr_links WHERE from_adr_id = ?", (adr.front_matter.id,))
    ...
    def build_index(self, adr_directory: Union[Path, str] = "docs/adr", validate: bool = True) -> Dict[str, Any]:
    ...
    def _update_metadata(self, stats: Dict[str, Any], adr_directory: Union[Path, str]) -> None:
        """Update index metadata."""
        if not self.connection:
            raise RuntimeError("Database not connected")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Tag distribution
        cursor.execute("SELECT tag, COUNT(*) FROM adr_tags GROUP BY tag ORDER BY COUNT(*) DESC LIMIT 10")
...
def generate_sqlite_index(adr_directory: Union[Path, str] = "docs/adr",
