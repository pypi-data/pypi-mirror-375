...
class MutualFundAnalyzer:
    ...
    def run_analysis(self, fund_names: Optional[List[str]] = None, scheme_codes: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
        Analyzes a list of mutual funds by name and/or scheme code.
        """
        ...
        # Analyze by name and get their codes
        for name in fund_names:
            fund_info = self.fetcher.find_fund_by_name(name)
            if fund_info and fund_info.get('schemeCode') not in processed_codes:
                scheme_code = fund_info['schemeCode']
                analysis = self.analyze_fund_by_code(scheme_code)
                if analysis:
                    all_summaries.append(self.generate_fund_summary(analysis))
                    processed_codes.add(scheme_code)

        # Analyze by code, avoiding duplicates
        for code in scheme_codes:
            if code not in processed_codes:
                analysis = self.analyze_fund_by_code(code)
                if analysis:
                    all_summaries.append(self.generate_fund_summary(analysis))
    ...
    def analyze_fund(self, fund_name: str) -> Optional[Dict[str, Any]]:
        ...
        logger.debug(f"Starting analysis for fund name: '{fund_name}'")
        fund_info = self.fetcher.find_fund_by_name(fund_name)
        if not fund_info:
            logger.error(f"Mutual fund '{fund_name}' not found.")
            return None
        
        logger.debug(f"Found fund info from fetcher: {fund_info}")
    ...
    def analyze_fund_by_code(self, scheme_code: str) -> Optional[Dict[str, Any]]:
        ...
        logger.debug(f"Analyzing fund with scheme_code: '{scheme_code}' (type: {type(scheme_code)})")
        ...
        latest_nav_date = nav_df.iloc[-1]['date']
        ...
        analysis_results = {
            ...
            'nav_date': latest_nav_date.strftime('%d-%b-%Y')
        }
    ...
    def _calculate_technical_indicators(self, nav_df: pd.DataFrame) -> Tuple[Dict[str, Any], pd.DataFrame]:
        ...
        if len(df) >= 20:
            ...
            # Add to indicators
            indicators['ma_20'] = round(df['20d_ma'].iloc[-1], 2) if not pd.isna(df['20d_ma'].iloc[-1]) else None
    ...
    def generate_fund_summary(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'nav_date': analysis.get('nav_date', 'N/A'),
