...
def _write_arrow(table: pa.Table, path: str | Path, delimiter: bytes) -> None:
...
def fast_write_space(df: pd.DataFrame, path: str | Path) -> None:
    """Write DataFrame as space-delimited CSV (no header, no index)."""
    _write_arrow(_to_arrow_table(df), path, delimiter=b" ")


def fast_write_tsv(df: pd.DataFrame, path: str | Path) -> None:
