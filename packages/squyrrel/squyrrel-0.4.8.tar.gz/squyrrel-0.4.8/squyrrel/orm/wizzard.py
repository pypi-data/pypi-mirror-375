...
from squyrrel.orm.utils import sanitize_id_array, m2m_aggregation_subquery_alias, field_to_sql_data_type, \
    extract_entities_without_id, extract_entity_with_id
...
class QueryWizzard:
    ...
    def build_m2m_update_queries(self, model, instance_id, relation, current_ids, new_ids):
        ...
        for id_to_remove in ids_to_remove:
            condition = And.concat([
                ...
                Equals(ColumnReference(foreign_id_field_name, table), Parameter(id_to_remove))
            ])
    ...
    def build_one_to_many_update_queries(self, model, instance_id, relation: OneToMany, current_entities, new_related_entities):
        model = self.get_model(model)
        foreign_model = self.get_model(relation.foreign_model)
        foreign_id_field_name = foreign_model.id_field_name()
        queries = []
        new_entities = extract_entities_without_id(foreign_model, new_related_entities)
        current_ids = set(sanitize_id_array(foreign_model, current_entities))
        update_ids = set(sanitize_id_array(foreign_model, new_related_entities))
        ids_to_remove = current_ids - update_ids
        table = foreign_model.table_name

        for id_to_remove in ids_to_remove:
            condition = Equals(ColumnReference(foreign_id_field_name, table),
                               Parameter(id_to_remove))
            queries.append(DeleteQuery(table, condition))

        for update_id in update_ids:
            filter_condition = Equals.id_as_parameter(foreign_model, update_id)
            prev_data = extract_entity_with_id(foreign_model, current_entities, update_id)
            data = extract_entity_with_id(foreign_model, new_related_entities, update_id)
            queries.extend(self.update_queries(foreign_model, filter_condition, update_id, prev_data, data, False))

        for new_entity in new_entities:
            id_field_name = model.id_field_name()
            new_entity[id_field_name] = instance_id
            queries.append(self.insert_query(foreign_model, new_entity))
    ...
    def insert_query(self, model, data):
        ...
        for column, value in data.items():
            field = model.get_field(column)
            # TODO: compare with logic in update_queries
            if field is None:
                if not value:
                    continue
                relation = model.get_relation(column)
                if isinstance(relation, ManyToOne):
                    fk_id = self.get_fk_id(relation, value, relation.insert_search_column)
                    if fk_id:
    ...
    def get_fk_id(self, relation, value, search_column):
        if isinstance(value, dict):
            relation_model = self.get_model(relation.foreign_model)
            fk_id = value[relation_model.id_field_name()]
        else:
            fk_id = self.get_m21_value(relation, search_column, value)
        return fk_id
    ...
    def update_queries(self, model, filter_condition, instance_id, prev_data, data, include_to_many_queries=True):
        ...
        m2m_update_queries = []
        one_to_many_queries = []
        if not include_to_many_queries:
        ...
        for column, value in data.items():
            ...
            if isinstance(relation, ManyToOne):
                if value is None:
                    updates[relation.foreign_key_field] = None
                else:
                    fk_id = self.get_fk_id(relation, value, relation.update_search_column)
                    if fk_id:
                        updates[relation.foreign_key_field] = fk_id
            elif isinstance(relation, ManyToMany) and include_to_many_queries:
                # compare difference
            ...
            elif isinstance(relation, OneToMany) and include_to_many_queries:
                print('current_entities:', prev_data.get(column))
                # in value the id is missing!
                print('new_related_entities:', value)
                one_to_many_queries.extend(
                    self.build_one_to_many_update_queries(
                        model=model,
                        instance_id=instance_id,
                        relation=relation,
                        current_entities=prev_data.get(column),
                        new_related_entities=value
                    )
                )
        ...
        return [update_query] + m2m_update_queries + one_to_many_queries

    def update(self, model, filter_condition, instance_id, prev_data, data):
        queries = self.update_queries(model, filter_condition, instance_id, prev_data, data)
        self.execute_queries_in_transaction(queries)
