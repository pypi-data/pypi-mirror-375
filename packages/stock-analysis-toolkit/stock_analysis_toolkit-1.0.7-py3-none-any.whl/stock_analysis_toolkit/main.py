...
class StockAnalyzer:
    ...
    def send_email_report(
        ...
        """Sends the analysis report via email with embedded charts."""
        if not self.email_sender:
            logger.warning("Email sending is disabled or not configured.")
            return

        # Add mutual fund charts to the attachments list
        mf_attachments = []
        for summary in mutual_fund_summaries:
            if "charts" in summary and summary["charts"]:
                for chart_path in summary["charts"]:
                    p = Path(chart_path)
                    mf_attachments.append({"path": p, "inline": True, "content_id": p.stem})

        # Combine stock and MF attachments
        all_attachments = attachments + mf_attachments
        ...
        success = self.email_sender.send_email(
            ...
            from_name="Stock Analysis Pro",
        )

        if success:
            logger.info(f"Email report sent successfully to {recipients}")
        else:
            logger.error(f"Failed to send email report to {recipients}")
