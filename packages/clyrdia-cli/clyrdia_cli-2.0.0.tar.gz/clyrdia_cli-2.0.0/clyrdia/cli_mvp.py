    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            if licensing_manager.is_authenticated():
                ...
                # User is not authenticated - show offline mode message
                console.print(Panel.fit(
                    "[bold red]🔒 Offline Mode - Authentication Required[/bold red]\n"
                    "Benchmarks are not allowed without authentication.\n"
                    "Please run 'clyrdia-cli login' to authenticate first.",
                    border_style="red",
                    padding=(1, 2),
                    title="Access Denied",
                    title_align="center"
...
def version():
    ...
    console.print(logo)
    console.print(f"[bold cyan]Clyrdia CLI v2.0.0[/bold cyan]")
...
def login(api_key: Optional[str] = typer.Option(None, "--api-key", "-k", help="Your Clyrdia API key")):
    ...
    if api_key:
        ...
        try:
            asyncio.run(licensing.login(api_key))
            console.print(Panel.fit(
                "[bold green]✅ Login Successful![/bold green]\n\n"
                "You are now authenticated with Clyrdia.\n"
                "Run [bold]clyrdia-cli status[/bold] to see your account details.",
                border_style="green",
                padding=(1, 2),
                title="Welcome",
                title_align="center"
    ...
    # No API key provided - show instructions
    console.print(Panel.fit(
    ...
    # Try to open browser
    try:
        webbrowser.open("https://clyrdia.com/auth")
        console.print("\n[green]🌐 Opened browser to clyrdia.com/auth[/green]")
    except Exception:
        console.print("\n[yellow]⚠️  Could not open browser automatically[/yellow]")
        console.print("Please visit https://clyrdia.com/auth manually")
...
def init(
    ...
    console.print(Panel.fit(
        ...
        f"[bold]Test Cases:[/bold] 3 focused on code quality\n"
        f"[bold]Quality Gates:[/bold] Configured for CI/CD\n\n"
        f"[bold]Next Steps:[/bold]\n"
        f"1. Run: [cyan]clyrdia-cli benchmark[/cyan]\n"
        f"2. Check results: [cyan]clyrdia-cli dashboard[/cyan]\n"
        f"3. Integrate with CI/CD: [cyan]clyrdia-cli cicd generate[/cyan]",
        border_style="green",
        padding=(1, 2),
