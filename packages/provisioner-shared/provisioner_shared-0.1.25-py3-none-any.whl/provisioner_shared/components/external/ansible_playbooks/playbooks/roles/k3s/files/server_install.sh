...
# Description   Install/uninstall a K3s server on a local/remote machine
#==============================================================================
...
is_k3s_server_system_service_installed() {
  log_info "Checking for previously installed K3s server system service..."
  cmd_run "systemctl list-units --full -all | grep -Fq k3s.service"
}

is_k3s_server_binary_installed() {
  log_info "Checking for previously installed K3s server binary..."
  is_file_exist "$(get_local_bin_folder)/$(get_k3s_binary_name)"
}

install_k3s_server_system_service() {
  if ! is_dry_run && is_k3s_server_system_service_installed; then
    log_warning "K3s server system service is already installed and running."
  else
    log_info "Installing K3s server system service. os: ${ENV_K3S_OS}, version: $(get_k3s_version)"
    new_line
    cmd_run "curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION=\"$(get_k3s_version)\" INSTALL_K3S_BIN_DIR=\"$(get_local_bin_folder)\" sh -s - $(get_additional_cli_args) --token $(get_k3s_token)"
  fi
}

install_k3s_server_binary() {
  if ! is_dry_run && is_k3s_server_binary_installed; then
    log_warning "K3s server binary is already installed."
  else
    log_info "Installing K3s server binary. os: ${ENV_K3S_OS}, version: $(get_k3s_version)"
    new_line
    cmd_run "curl -sfL https://get.k3s.io | INSTALL_K3S_SKIP_ENABLE=true INSTALL_K3S_SKIP_START=true INSTALL_K3S_VERSION=\"$(get_k3s_version)\" INSTALL_K3S_BIN_DIR=\"$(get_local_bin_folder)\" sh -s - $(get_additional_cli_args) --token $(get_k3s_token)"
  fi
}
...
verify_mandatory_variables() {
  ...
  if ! is_dry_run && is_k3s_os_darwin && ! is_install_as_binary; then
