...
class ItemData(TypedDict):
    ...
    mto_precio_unitario: float          # REQUIRED: Unit price WITH ALL taxes
...
def create_invoice_data(
    ...
    for item in items:
        detail = SaleDetail(
            ...
            mto_precio_unitario=item.get('mto_precio_unitario', item['mto_valor_unitario']),  # CRÍTICO: Precio con TODOS los impuestos
            ...
            # Campos tributarios adicionales
            mto_base_igv=item.get('mto_base_igv'),
            mto_base_isc=item.get('mto_base_isc'),
            mto_base_ir=item.get('mto_base_ir'),
            mto_base_icbper=item.get('mto_base_icbper'),
            mto_base_otro=item.get('mto_base_otro'),
            porcentaje_isc=item.get('porcentaje_isc'),
            porcentaje_ir=item.get('porcentaje_ir'),
            porcentaje_otro=item.get('porcentaje_otro'),
            factor_icbper=item.get('factor_icbper'),
            isc=item.get('isc'),
            ir=item.get('ir'),
            icbper=item.get('icbper'),
            otro_tributo=item.get('otro_tributo'),
            tip_sis_isc=item.get('tip_sis_isc'),
            tip_afe_ir=item.get('tip_afe_ir'),
            tip_afe_otro=item.get('tip_afe_otro'),
            mto_precio_unitario_final=item.get('mto_precio_unitario_final'),
            mto_valor_gratuito=item.get('mto_valor_gratuito')
        )
        details.append(detail)
    
    # Calcular valor_venta automáticamente (suma de mto_valor_venta de todos los items)
    valor_venta = sum(item.get('mto_valor_venta', 0) for item in items)
...
def send_invoice(credentials: SunatCredentials, invoice_data: InvoiceDataComplete) -> SunatResponse:
    ...
    try:
        ...
        optional_fields = [
            ...
            'mto_igv', 'mto_isc', 'mto_ivap', 'mto_otros_tributos', 'mto_total_tributos',
            'mto_base_isc', 'mto_base_ivap', 'mto_base_otros_tributos', 'mto_impventa',
