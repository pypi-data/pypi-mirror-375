...
"""Tool for listing available RDS DB Clusters (converted from resource)."""
...
class ClusterSummary(BaseModel):
    ...
    db_cluster_arn: Optional[str] = Field(
        None,
        description='The ARN of the DB cluster',
    )
    db_cluster_resource_id: Optional[str] = Field(
        None,
        description='The resource ID of the DB cluster',
    )
    ...
    availability_zones: List[str] = Field(
        ...
        description='The AZs where the cluster instances can be created',
    )
    multi_az: bool = Field(
        description=(
...
LIST_CLUSTERS_TOOL_DESCRIPTION = (
    ...
    'This tool returns information about all RDS database clusters in your '
    'AWS account, including both Aurora clusters and Multi-AZ DB clusters.'
)


@mcp.tool(
    name='ListDBClusters',
    description=LIST_CLUSTERS_TOOL_DESCRIPTION,
)
@handle_exceptions
async def list_db_clusters(
    ...
    """List all RDS clusters in the selected AWS region.

    Args:
        aws_region: Region to target for this call (e.g., 'us-east-1').
    """
    logger.info('Listing RDS clusters (tool)')
