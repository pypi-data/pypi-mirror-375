...
def error(message):
    ...
    exit('yals: ERROR: ' + explain(message))
...
def warning(message):
    ...
    if arg.verbose:
        var.num_warning += 1
        print(f'yals: WARNING {var.num_warning}: ' + explain(message), file=stderr)

def YmdHMS(time):
    "convert time as seconds since epoch into 'YYYY-mm-dd_HH:MM:SS' format"
    return '%04d-%02d-%02d_%02d:%02d:%02d' % tuple(localtime(time)[:6])
...
def yals():
    ...
    def get_arguments():
        parser = ArgumentParser(prog='yals', formatter_class=RawDescriptionHelpFormatter, description=DESCRIPTION)
...
##            help='browse the PDF FFDD User Manual and exit')
        ...
        parser.add_argument('-F','--files-if', type=str, default='True',
            help=f"Python boolean expression for selection of files, default: {F_default!r}, "
                 f"domain: {show(F_domain)}, syntax: standard Python3 syntax plus the YARE match operators '/' (case-sensitive) and '//' (case-insensitive)")
        ...
        parser.add_argument('-D','--dirs-if', type=str, default='True',
            help=f"Python boolean expression for selection of dirs, default: {D_default!r}, "
                 f"domain: {show(D_domain)}, syntax: standard Python3 syntax plus the YARE match operators '/' (case-sensitive) and '//' (case-insensitive)")
            ...
            for name in names:
                if yarecsmatch(name, var.file_pattern):
                    ...
                    try:
                        ...
                        warning(f'No user: {path!r}')
                        u = '????'
                    try:
                        g = getpwuid(stat_path.st_gid).pw_name
                    except KeyError:
                        warning(f'No group: {path!r}')
                        g = '????'
                    s = stat_path.st_size
