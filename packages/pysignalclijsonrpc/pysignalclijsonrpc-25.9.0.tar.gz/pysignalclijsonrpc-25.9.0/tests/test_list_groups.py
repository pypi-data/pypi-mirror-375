...
def test_list_groups_ok():
    ...
    assert isinstance(res, list)  # nosec
    assert isinstance(res[0], dict)  # nosec
    assert len(res) == 2  # nosec
    pook.reset()
...
def test_list_groups_error():
    ...
    assert "Specified account does not exist" in str(exc_info.value)  # nosec
    pook.reset()
