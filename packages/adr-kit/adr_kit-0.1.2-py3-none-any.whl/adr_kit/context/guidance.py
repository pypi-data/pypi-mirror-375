...
class ContextualPromptlet(object):
    ...
    def __init__(self, message: str, guidance_type: GuidanceType, priority: str = "medium"):
...
class GuidanceGenerator:
    ...
    def generate_guidance(
        ...
        if task_context.task_type.value in ["dependency", "feature", "refactor", "integration"]:
        ...
        if constraints_contract and not constraints_contract.constraints.is_empty():
            ...
            constraint_guidance = self._generate_constraint_guidance(constraints_contract, task_context)
            guidance.extend(constraint_guidance)
        
        # 3. ADR-specific guidance
        adr_guidance = self._generate_adr_guidance(relevant_adrs, relevance_scores, task_context)
        guidance.extend(adr_guidance)
        
        # 4. Task-type specific guidance
        task_guidance = self._generate_task_specific_guidance(task_context, relevant_adrs)
        guidance.extend(task_guidance)
        
        # 5. Pattern and reference guidance
        if relevant_adrs:
            guidance.append(self.promptlets["cite_adrs"].to_guidance([adr.id for adr in relevant_adrs[:3]]))
        
        # Sort by priority and return top guidance items
        priority_order = {"critical": 0, "high": 1, "medium": 2, "low": 3}
        guidance.sort(key=lambda g: priority_order.get(g.priority, 3))
        
        return guidance[:8]  # Limit to top 8 guidance items to keep token count reasonable
    ...
    def _generate_constraint_guidance(
        ...
        if constraints.imports:
            if constraints.imports.disallow and task_context.task_type.value in ["dependency", "feature"]:
                disallowed = ', '.join(constraints.imports.disallow[:3])  # Show first 3
                guidance.append(PlanningGuidance(
            ...
            if constraints.imports.prefer and task_context.task_type.value in ["dependency", "feature"]:
        ...
        if constraints.boundaries and constraints.boundaries.rules:
            if any(word in task_context.keywords for word in ["architecture", "component", "service"]):
                guidance.append(PlanningGuidance(
    ...
    def _generate_adr_guidance(
        ...
        high_relevance_adrs = [adr for adr in relevant_adrs if adr.relevance_score >= 0.7]
        ...
        for adr in high_relevance_adrs[:3]:  # Top 3 most relevant
            ...
            if adr.key_constraints:
                constraints_text = ', '.join(adr.key_constraints[:2])  # Top 2 constraints
                guidance.append(PlanningGuidance(
                    guidance_type=GuidanceType.CONSTRAINT.value,
                    priority="high",
    ...
    def generate_summary_guidance(
        ...
        if task_type == "dependency":
            ...
            lines.append("Follow established patterns and consider architectural impact.")
        elif task_type == "refactor":
        ...
        if constraints_contract and not constraints_contract.constraints.is_empty():
            ...
            if constraints_contract.constraints.imports:
                if constraints_contract.constraints.imports.disallow:
                    constraint_count += len(constraints_contract.constraints.imports.disallow)
                if constraints_contract.constraints.imports.prefer:
                    constraint_count += len(constraints_contract.constraints.imports.prefer)
            
            if constraint_count > 0:
                lines.append(f"Follow {constraint_count} active import/dependency constraints.")
        
        # ADR reference
        if relevant_adrs:
            accepted_adrs = [adr for adr in relevant_adrs if adr.status.value == "accepted"]
            if accepted_adrs:
                lines.append(f"Reference {len(accepted_adrs)} accepted ADRs for detailed guidance.")
        
        # Default guidance
        if not lines:
            lines.append("Follow existing architectural decisions and get approval for new ones.")
