...
class FolderClassifier:
    def __init__(self, app_name: str, deployment: str, model: str,  max_tokens: Optional[int] = 1024,
                 temperature: Optional[float] = 0.2, top_p: Optional[float] = 0.8, timeout: Optional[int] = 10,
    ...
    async def run_chat_completion(self, chat_completion_request: Dict[str, Any]) -> Dict[str, Any]:
        result = None
        try:
            response = await asyncio.wait_for(self.model_handle.create_chat_completion_internal.remote(chat_completion_request),
            ...
            if self.fallback_config:
                ...
                chat_completion_request["model"] = self.fallback_config.model
