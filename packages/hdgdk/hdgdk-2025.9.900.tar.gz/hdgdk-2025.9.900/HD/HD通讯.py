...
def HD通讯_获取绑定窗口序号EX(进程PID: int,winIndex: int,checkFunc: int) -> int:
    """
    通过进程PID尝试拿到绑定的窗口序号
    ...
    Returns:
        int: 绑定的窗口序号，小于等于0表示不存在PID，其他值正常操作
             没有进程PID返回-1178，没有绑定过返回-1179
    """
    hd = Config.get_hd()
    
    HCInject_GetPreWinIndex = hd.HCInject_GetPreWinIndexEx
    HCInject_GetPreWinIndex.restype = ctypes.c_int64
    HCInject_GetPreWinIndex.argtypes = [
        ctypes.c_int32,
        ctypes.c_int32,
    ...
    ret = HCInject_GetPreWinIndex(
        ctypes.c_int32(进程PID),
        ctypes.c_int32(winIndex),
        ctypes.c_int64(checkFunc)
    )
    return ret
