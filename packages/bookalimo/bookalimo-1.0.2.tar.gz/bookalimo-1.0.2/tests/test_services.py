...
class TestAsyncPricingService:
    ...
    async def test_quote_basic(
        ...
        sample_pickup_location: Location,
        sample_dropoff_location: Location,
    ) -> None:
        """Test basic quote functionality."""
        mock_response = PriceResponse(token="test-token-123", prices=[])

        with patch.object(
            pricing_service._transport,
            "post",
    ...
    async def test_quote_excludes_none_optional_fields(
        ...
        with patch.object(
            ...
            assert (
                "carClassCode" not in request_dict
                or request_dict["carClassCode"] is None
            )
            assert request_dict["customerComment"] == "Valid comment"

    @pytest.mark.asyncio
    async def test_update_details_basic(
        self, pricing_service: AsyncPricingService
    ) -> None:
        ...
        with patch.object(
            ...
            result = await pricing_service.update_details(
                DetailsRequest(token="original-token-123", car_class_code="LUXURY")
            )
    ...
    async def test_update_details_multiple_fields(
        self, pricing_service: AsyncPricingService, sample_pickup_location: Location
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_update_details_excludes_none_values(
        self, pricing_service: AsyncPricingService
    ) -> None:
...
class TestSyncPricingService:
    ...
    def test_quote_basic(
        ...
        with patch.object(
            pricing_service._transport, "post", new=Mock(return_value=mock_response)
        ) as mock_post:
    ...
    def test_update_details_basic(self, pricing_service: PricingService) -> None:
        ...
        with patch.object(
            ...
            result = pricing_service.update_details(
                DetailsRequest(token="sync-original-token-123", car_class_code="LUXURY")
            )
...
class TestAsyncReservationsService:
    ...
    @pytest.mark.asyncio
    async def test_list_reservations_default(
        self, reservations_service: AsyncReservationsService
    ) -> None:
        """Test list reservations with default parameters."""
        mock_response = ListReservationsResponse(success=True, reservations=[])

        with patch.object(
            reservations_service._transport,
            "post",
            new=AsyncMock(return_value=mock_response),
        ) as mock_post:
    ...
    @pytest.mark.asyncio
    async def test_list_reservations_archived(
        self, reservations_service: AsyncReservationsService
    ) -> None:
    ...
    async def test_get_reservation(
        ...
        mock_response = GetReservationResponse(
            reservation=ReservationData(confirmation="TEST123"),
            is_editable=True,
    ...
    @pytest.mark.asyncio
    async def test_edit_reservation_cancel(
        self, reservations_service: AsyncReservationsService
    ) -> None:
        ...
        with patch.object(
            ...
            result = await reservations_service.edit(
                EditReservationRequest(confirmation="TEST123", is_cancel_request=True)
            )
            ...
            request = call_args[0][1]
            assert isinstance(request, EditReservationRequest)
            assert request.confirmation == "TEST123"
            assert request.is_cancel_request is True

    @pytest.mark.asyncio
    async def test_edit_reservation_modify(
        self, reservations_service: AsyncReservationsService
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_edit_reservation_excludes_none_changes(
        self, reservations_service: AsyncReservationsService
    ) -> None:
    ...
    async def test_book_with_credit_card(
        self,
        reservations_service: AsyncReservationsService,
        ...
        with patch.object(
            ...
            result = await reservations_service.book(
                BookRequest(token="booking-token-123", credit_card=sample_credit_card)
            )
    ...
    @pytest.mark.asyncio
    async def test_book_with_charge_method(
        self, reservations_service: AsyncReservationsService
    ) -> None:
        ...
        with patch.object(
            ...
            result = await reservations_service.book(
                BookRequest(token="booking-token-456", method="charge")
            )
    ...
    @pytest.mark.asyncio
    async def test_book_with_promo_code(
        self,
        reservations_service: AsyncReservationsService,
        sample_credit_card: CreditCard,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_book_missing_payment_method_raises_error(
        self, reservations_service: AsyncReservationsService
    ) -> None:
        ...
        with pytest.raises(
            ...
            # Error should now be raised when creating BookRequest
            BookRequest(token="booking-token-999")

    @pytest.mark.asyncio
    async def test_book_charge_method_without_credit_card(
        self, reservations_service: AsyncReservationsService
    ) -> None:
        ...
        with patch.object(
            ...
            result = await reservations_service.book(
                BookRequest(token="booking-token-charge", method="charge")
            )
...
class TestSyncReservationsService:
    ...
    def test_book_sync_uses_model_validate(
        self, reservations_service: ReservationsService, sample_credit_card: CreditCard
    ) -> None:
        ...
        with patch.object(
            ...
            result = reservations_service.book(
                BookRequest(token="sync-booking-token", credit_card=sample_credit_card)
            )
