...
class TelegramCapcha:
    ...
    # Проверка подписки
        sub_info = self.api.get_subscription_info()
        self.subscription_type = sub_info.get('type', 'free')
        self.subscription_limits = sub_info.get('limits', {})
    
    # Устанавливаем complexity с учетом ограничений подписки
        initial_complexity = max(1, min(complexity, 10))
        if self.subscription_type == 'free' and initial_complexity > 3:
            self.complexity = 3
            print("Warning: Complexity limited to 3 for free users")
        else:
            self.complexity = initial_complexity
    
    # Устанавливаем captcha_type с учетом ограничений подписки
        initial_captcha_type = captcha_type.lower()
        if self.subscription_type == 'free' and initial_captcha_type not in ["text", "auto"]:
            self.captcha_type = "text"
            print("Warning: Only text captcha available for free users")
        else:
            self.captcha_type = initial_captcha_type
