...
increment_default = [KeyCode.from_char(ch) for ch in "1234567890asdfghjkl;"]
decrement_default = [KeyCode.from_char(ch) for ch in "qwertyuiopzxcvbnm,./"]
...
def main():
    ...
    while(1):
        if active:
            while not key_queue.empty(): # key handling in main thread this time
                actual_print("Rec")
                key = key_queue.get()
                if key in increment:
                    increment_val(key)
