...
class MSReader(object):
    ...
    def summary(self, show=True):
        ...
        for value in summary['ms_levels'].values():
            ...
            value['activation_energies'] = tuple(set(map(lambda d: int(round(d)), filter(None, value['activation_energies']))))
        ...
        if show:
            ...
            for level, info in summary['ms_levels'].items():
                ...
                print("\tMass Ranges: %s" % (info['mass_ranges_count'] if info['mass_ranges_count'] > 10 else str(info['mass_ranges'])))
