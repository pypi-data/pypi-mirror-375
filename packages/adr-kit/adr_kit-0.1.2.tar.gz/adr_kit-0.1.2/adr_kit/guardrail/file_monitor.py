...
class FileMonitor:
    ...
    def _update_baseline(self):
        ...
        for file_path in adr_files:
            try:
                ...
                if adr:
                    status_value = adr.front_matter.status.value if hasattr(adr.front_matter.status, 'value') else str(adr.front_matter.status)
            ...
            if file_path not in self._file_hashes:
                ...
                try:
                    ...
                    if adr:
                        ...
                        new_status = adr.front_matter.status.value if hasattr(adr.front_matter.status, 'value') else str(adr.front_matter.status)
    ...
    def get_policy_relevant_changes(self, changes: List[ChangeEvent]) -> List[ChangeEvent]:
