...
class WebApplication():
    ...
    def create_proxy_mcp(self, 
                            name:str, 
                            config_file:str,
                            path:str='/mcp_proxy'):
        
        """
        if not self.__mcp_instance.create_proxy_from_client(name=name,
    ...
    def add_method(self, 
        ...
        if self.__json_rpc:
            if not name:
                self.__json_rpc.add_method(func=func, name=func.__name__, tags=tags)
            else:
                self.__json_rpc.add_method(func=func, name=name, tags=tags)
        else:
