...
def create_flower_trace(
    ...
    Args:
        ...
        show_hover: 是否显示悬停提示
        animate: 是否启用动画
        show_markers: 是否显示标记点（覆盖配置）
        marker_size: 标记点大小（覆盖配置）
        marker_color: 标记点颜色（覆盖配置）

    Returns:
        plotly Scatter对象
    """
    # 使用配置默认值
    show_markers = show_markers if show_markers is not None else config.show_markers
    marker_size = marker_size if marker_size is not None else config.marker_size
    marker_color = (
        marker_color if marker_color is not None else (config.marker_color or color)
    )

    # 设置模式
    mode = "lines"
    # 只在明确要求显示标记点时才显示，或者在动画中且配置允许时显示
    should_show_markers = show_markers or (animate and config.show_markers_in_animation)
    ...
    if should_show_markers:
        trace_config["marker"] = dict(
            size=marker_size, symbol="circle", color=marker_color
        )
...
def n_flower_petal(
    ...
    try:
        ...
        if animate:
            ...
            # 优化帧数量，确保不超过50帧以保持性能
            total_frames = min(50, len(x) // config.animation_frame_gap)
            step_size = max(config.animation_frame_gap, len(x) // total_frames)

            for k in range(2, len(x), step_size):
                # 使用create_flower_trace函数创建一致的轨迹
                frame_trace = create_flower_trace(
                    x=x[:k],
                    y=y[:k],
                    color=color,
                    width=width,
                    ...
                    opacity=opacity,
                    name=f"Frame {k}",
                    show_hover=config.show_hover,
                    animate=animate,
                    show_markers=show_markers,
                )
                frames.append(go.Frame(data=[frame_trace]))

            # 添加最后一帧（完整图形）
            final_trace = create_flower_trace(
                x=x,
                y=y,
                color=color,
                width=width,
            ...
            frames.append(go.Frame(data=[final_trace]))
    ...
    try:
        ...
        if animate:
            ...
            for trace in fig.data:
                x = trace.x
                y = trace.y
                all_traces.append((x, y, trace.line.color, trace.opacity, trace.name))

            frames = []
            max_points = max(len(x) for x, y, _, _, _ in all_traces)

            # 优化帧数量，确保不超过50帧以保持性能
            total_frames = min(50, max_points // config.animation_frame_gap)
            step_size = max(config.animation_frame_gap, max_points // total_frames)

            for k in range(2, max_points, step_size):
                frame_data = []
                for idx, (x, y, trace_color, trace_opacity, trace_name) in enumerate(
                    all_traces
                ):
