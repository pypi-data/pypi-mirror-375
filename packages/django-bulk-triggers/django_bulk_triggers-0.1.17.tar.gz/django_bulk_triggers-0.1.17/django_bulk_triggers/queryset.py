        ...
        if has_subquery and instances and not current_bypass_triggers:
            ...
            for instance in instances:
                if instance.pk is not None:
                    ...
                    for field in model_cls._meta.fields:
                        if field.name != "id":
                            pre_after_trigger_values[field.name] = getattr(instance, field.name, None)
            ...
            for instance in instances:
                if instance.pk in pre_after_trigger_state:
                    ...
                    for field_name, pre_after_trigger_value in pre_after_trigger_values.items():
                        current_value = getattr(instance, field_name)
            ...
            if after_trigger_modified_fields:
                ...
                result = model_cls.objects.bulk_update(
                    instances, bypass_triggers=True, **{f.name: f for f in after_trigger_modified_fields}
                )
                logger.debug(f"FRAMEWORK DEBUG: AFTER_UPDATE bulk_update result = {result}")
