...
class ForevaAgent:
    ...
    def deactivate(self) -> Dict[str, Any]:
        """
        Deactivate this agent (free a seat).
        Returns server response dict.
        """
        payload = {"phone_number": self.phone_number}
        result = self._client.deactivate_agent(payload)
        # Update local state
        self._is_activated = False
        return result
    ...
    def get_partner_limits(self) -> Dict[str, Any]:
        """
        Fetch partner-level limits (seats and minutes)
        """
        resp = self._client.get_partner_limits()
