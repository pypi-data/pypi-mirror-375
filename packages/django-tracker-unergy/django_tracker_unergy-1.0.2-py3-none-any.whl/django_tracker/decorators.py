...
def auditable(
    ...
    def decorator(cls):
        ...
        if audit_config.audit_m2m_changes:
            _setup_m2m_signals_safe(cls, audit_config)
...
def _setup_m2m_signals_safe(model_class, audit_config: AuditConfig):
    """Configura señales M2M de forma segura, manejando errores de inicialización"""
    try:
        _setup_m2m_signals(model_class, audit_config)
    except Exception as e:
        import logging
        logger = logging.getLogger('django_tracker')
        logger.warning(f"Could not setup M2M signals for {model_class.__name__} during model loading: {e}")
        logger.warning("M2M auditing will be skipped for this model.")
...
def _setup_m2m_signals(model_class, audit_config: AuditConfig):
    ...
    for field_name in m2m_fields:
        try:
            ...
            # Verificar que el campo tiene un through model disponible
            if not hasattr(field, 'through') or field.through is None:
                import logging
                logger = logging.getLogger('django_tracker')
                logger.warning(f"M2M field {field_name} on {model_class.__name__} doesn't have through model ready yet")
                continue
            ...
            import logging
            logger = logging.getLogger('django_tracker')
            logger.warning(f"Could not setup M2M signal for field {field_name} on {model_class.__name__}: {e}")
