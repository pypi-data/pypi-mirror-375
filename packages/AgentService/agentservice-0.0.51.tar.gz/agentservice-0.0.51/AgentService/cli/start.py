...
from AgentService.agent.file_search import search_in_files
...
def start_project():
    ...
    connector = AgentConnector(
        endpoint=f"http://{config.app_host}:{config.app_port}"
    )
    connector.bind_tool_output("file_search", search_in_files)
