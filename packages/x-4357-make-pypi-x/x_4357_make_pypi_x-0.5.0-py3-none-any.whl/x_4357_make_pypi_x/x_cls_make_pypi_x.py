...
class x_cls_make_pypi_x(BaseMake):
    ...
    def _pep561_enhance(self, build_dir: str, package_dir: str, ancillary_files: list[str] | None = None) -> None:  # type: ignore[override]
        ...
        minimal pyproject.toml containing project metadata plus explicit
        package discovery so wheels include ancillary files.
        """
        try:
            ...
            for a in ancillary_files or []:
                try:
                    ap = _P(a)
                    if ap.is_file() and ap.parent == pkg_path and ap.suffix.lower() in {'.txt', '.md', '.rst'}:
