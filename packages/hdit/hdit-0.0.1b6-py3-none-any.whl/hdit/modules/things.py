...
from .attention import GlobalTransformerLayer, ShiftedWindowTransformerLayer, NeighborhoodTransformerLayer
...
class SelectiveShiftedWindowTransformerLayer(ShiftedWindowTransformerLayer):
    def __init__(self, d_model, d_ff, d_head, cond_features, window_size, window_shift, dropout = 0, cond_index = None):
        super().__init__(d_model, d_ff, d_head, cond_features, window_size, window_shift, dropout)
        self.cond_index = cond_index
...
shifted_window_builders = {
    0: lambda context: SelectiveShiftedWindowTransformerLayer(context.width, context.width, 64, context.mapping_width, 8, 8 // 2 if context.block_index % 2 == 1 else 0, 0., cond_index=None),
    1: lambda context: SelectiveShiftedWindowTransformerLayer(context.width, context.width, 64, context.mapping_width, 8, 8 // 2 if context.block_index % 2 == 1 else 0, 0., cond_index=None),
    2: lambda context: SelectiveGlobalTransformerLayer(context.width, context.width, 64, context.mapping_width, 0., cond_index=None),
    3: lambda context: SelectiveShiftedWindowTransformerLayer(context.width, context.width, 64, context.mapping_width, 8, 8 // 2 if context.block_index % 2 == 1 else 0, 0., cond_index=None),
    4: lambda context: SelectiveShiftedWindowTransformerLayer(context.width, context.width, 64, context.mapping_width, 8, 8 // 2 if context.block_index % 2 == 1 else 0, 0., cond_index=None),
}


if NeighborhoodTransformerLayer is None:
    default_block_builders = shifted_window_builders
    neighborhood_builders = None
else:
    ...
    default_block_builders = neighborhood_builders
