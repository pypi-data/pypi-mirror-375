...
class XmlSigner:
    ...
    def sign_xml(self, xml_content: str) -> str:
        ...
        Enfoque simplificado: Firma con SHA-256 (compatible con SUNAT).
        ...
        try:
            ...
            # Configurar firmador con SHA-256 (más seguro y compatible con SUNAT)
            signer = XMLSigner(
            ...
            print("[XmlSigner] Usando SHA-256")
            ...
            signature_elem.set('Id', 'CpeEngineSign')
    ...
    @staticmethod
    def extract_digest_value(signed_xml: str) -> str:
        """
        Extrae el valor DigestValue del XML firmado.
        
        Args:
            signed_xml: XML firmado como string
            
        Returns:
            Valor del DigestValue (hash SHA-256)
            
        Raises:
            XmlSignerError: Si no se puede extraer el DigestValue
        """
        try:
            # Parsear XML firmado
            signed_doc = etree.fromstring(signed_xml.encode('utf-8'))
            
            # Buscar el DigestValue
            digest_value_elem = signed_doc.find('.//{http://www.w3.org/2000/09/xmldsig#}DigestValue')
            
            if digest_value_elem is None or digest_value_elem.text is None:
                raise XmlSignerError("No se encontró DigestValue en el XML firmado")
            
            digest_value = digest_value_elem.text.strip()
            print(f"[XmlSigner] DigestValue extraído: {digest_value}")
            return digest_value
            
        except Exception as e:
            error_msg = f"Error extrayendo DigestValue: {e}"
            print(f"[XmlSigner] ERROR: {error_msg}")
            raise XmlSignerError(error_msg)