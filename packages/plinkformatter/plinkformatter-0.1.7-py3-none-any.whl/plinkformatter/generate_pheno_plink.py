...
def fast_write_space(df: pd.DataFrame, path: str):
    """
    Write a DataFrame with single-space delimiter, no header, no index.
    Intended for PED / PHENO files.
    """
    table = pa.Table.from_pandas(df, preserve_index=False)
    opts = pv.WriteOptions(
        include_header=False,
        delimiter=b" ",
        quoting_style=pv.QuotingStyle.NONE,
    )
    pv.write_csv(table, path, write_options=opts)


def fast_write_tsv(df: pd.DataFrame, path: str):
    """
    Write a DataFrame with tab delimiter, no header, no index.
    Intended for MAP files.
    """
    table = pa.Table.from_pandas(df, preserve_index=False)
    opts = pv.WriteOptions(
        include_header=False,
...
def generate_pheno_plink(
    ...
    def write_files_fast(measnum):
        ...
        if "f" in df["sex"].unique():
            ...
            # PHENO
            fast_write_space(
                df_f[["FID", "IID", "zscore", "value"]],
                os.path.join(outdir, f"{measnum}.f.pheno"),
            )

            # PED
            allele_df_f = column_splitter_vec(df_f[[c for c in df_f.columns if isinstance(c, int)]])
            fast_write_space(
                allele_df_f,
                os.path.join(outdir, f"{measnum}.f.ped"),
            )

            # MAP
            fast_write_tsv(
                map_df,
                os.path.join(outdir, f"{measnum}.f.map"),
            )
        ...
        if "m" in df["sex"].unique():
            ...
            fast_write_space(
                df_m[["FID", "IID", "zscore", "value"]],
                os.path.join(outdir, f"{measnum}.m.pheno"),
            )

            allele_df_m = column_splitter_vec(df_m[[c for c in df_m.columns if isinstance(c, int)]])
            fast_write_space(
                allele_df_m,
                os.path.join(outdir, f"{measnum}.m.ped"),
            )

            fast_write_tsv(
                map_df,
                os.path.join(outdir, f"{measnum}.m.map"),
            )
