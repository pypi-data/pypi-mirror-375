...
@click.option(
    ...
    help=(
        ...
        "In your code, the COILED_INPUTS_PATH environment variable with give path to directory containing "
        "the transferred file or files."
    ),
)
@click.option(
    "--download",
...
def _batch_run(default_kwargs, logger=None, from_cli=False, **kwargs) -> dict:
    ...
    with coiled.Cloud(workspace=kwargs["workspace"]) as cloud:
        ...
        if (
            ...
            filestores = FilestoreManager.create_filestores(
                name=kwargs["name"] or f"batch-job-{job_id}",
                workspace=job_spec["workspace"],
                region=kwargs["region"],
            )
            upload_to_filestore_with_ui(
                fs=filestores.get("input"), local_dir=kwargs.get("local_upload_path") or kwargs.get("local_sync_path")
            )
        ...
        if filestores:
            FilestoreManager.attach_filestores_to_cluster(
                ...
                in_name=filestores["input"]["name"],
                out_name=filestores["result"]["name"],
            )
