...
class TextPreprocessor:
    ...
    def load_stop_words(self):
        ...
        try:
            ...
            stop_words_dir = current_dir
            
            # 查找停用词文件
            stop_word_filepath_list = glob(stop_words_dir + FilePatterns.STOPWORDS_PATTERN)
            
            if not stop_word_filepath_list:
                logging.warning(LogMessages.STOPWORDS_FILE_NOT_FOUND.value.format(directory=stop_words_dir))
                # 返回基本停用词集合
            ...
            for stop_word_filepath in stop_word_filepath_list:
                try:
                    with open(stop_word_filepath, 'r', encoding='utf-8') as fp:
                        file_stopwords = [line.rstrip() for line in fp if line.strip()]
