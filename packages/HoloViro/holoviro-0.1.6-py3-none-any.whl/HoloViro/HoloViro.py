...
class HoloViro:
    ...
    def _projectFromSpec(self, spec: str) -> str:
        """
        Extract the distribution (project) name from a pip requirement string.
        Uses packaging.Requirement when available; falls back to a simple split.
        Examples:
          'python-dotenv>=1.0.1' -> 'python-dotenv'
          'google-genai[all]'    -> 'google-genai'
          'openai ; python_version>="3.10"' -> 'openai'
        """
        try:
            from packaging.requirements import Requirement  # optional
            return Requirement(spec).name
        except Exception:
            s = str(spec).strip()
            s = s.split(';', 1)[0]        # drop markers
            s = s.split('[', 1)[0]        # drop extras
            # split on first version operator (===, ==, >=, <=, ~=, !=, >, <)
            m = re.split(r'(?:===|==|>=|<=|~=|!=|>|<)', s, maxsplit=1)
            return m[0].strip()

    def _versionSatisfied(self, spec: str) -> bool:
        """
        Return True if the venv already has a distribution matching `spec`.
        If `packaging` is installed, enforce version specifiers; otherwise,
        just checks presence of the distribution.
        """
        name = self._projectFromSpec(spec)
        # search only inside the venv site-packages
        for dist in im.distributions(path=[self.sitePath]):
            if (dist.metadata.get('Name') or '').lower() == name.lower():
                # optional: enforce version constraints if packaging is available
                try:
                    from packaging.requirements import Requirement
                    req = Requirement(spec)
                    return (not req.specifier) or req.specifier.contains(dist.version, prereleases=True)
                except Exception:
                    return True  # packaging not available -> presence is good enough
        return False

    def pipInstall(self, package: str | list[str], debug=False) -> None:
        packages = [package] if isinstance(package, str) else (package or [])
        for spec in packages:
            if self._versionSatisfied(spec):
                if debug:
                    print(f"[venv] already satisfied: {spec}")
                logger.info("Package already satisfied in venv: %s", spec)
            else:
                if debug:
                    print(f"[venv] installing: {spec}")
                logger.info("Installing '%s' to virtual environment...", spec)
                self.installPackage(str(spec))
