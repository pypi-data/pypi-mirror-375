...
class StartCls:
	...
	def get(self, spectrum=repcap.Spectrum.Default) -> float:
		"""CALCulate:SPECtrum<*>:GATE:USED:STARt \n
		Snippet: value: float = driver.calculate.spectrum.gate.used.start.get(spectrum = repcap.Spectrum.Default) \n
		...
		response = self._core.io.query_str(f'CALCulate:SPECtrum{spectrum_cmd_val}:GATE:USED:STARt?')
