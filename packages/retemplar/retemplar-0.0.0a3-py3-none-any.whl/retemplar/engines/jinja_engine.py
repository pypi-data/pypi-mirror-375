...
def process_files(
    ...
    options = JinjaEngineOptions.model_validate(engine_options)
    ...
    for file_path, content in files_to_process.items():
        try:
            ...
            if '{{' in relative_path and '}}' in relative_path:
                ...
                output_path = template.render(**options.variables)

            # Remove .j2 extension if present
            output_path = output_path.removesuffix('.j2')

            # Apply jinja_dst prefix if specified
            if options.jinja_dst:
                dst_prefix = str(options.jinja_dst).rstrip('/')
            ...
            if isinstance(content, bytes):
                ...
                if should_template:
                    ...
                    processed_content = template.render(**options.variables)
