...
# ///////////////////////////////////////////////////////////////////////////
# $Id: Electrodes.py 9041 2025-09-09 20:29:54Z jhill $
...
from . import USE_NUMPY_MATRIX # switch on/off in __init__.py
if USE_NUMPY_MATRIX:
	...
	ChannelSetSuperclass = numpy.matrix
	ChannelSetArrayPriority = numpy.matrix.__array_priority__ + 1.0
	MakeMatrix = numpy.matrix
	AsMatrix = numpy.asmatrix
else:
	ChannelSetSuperclass = numpy.ndarray
	ChannelSetArrayPriority = 11.0
	def MakeMatrix(x, copy=True, **kwargs):
		x = numpy.array(x, copy=copy, ndmin=2, **kwargs)
		if x.ndim != 2: raise ValueError('matrix must be 2-dimensional')
		return x
	def AsMatrix(x, copy=False, **kwargs):
		return MakeMatrix(x, copy=copy, **kwargs)
...
class ChannelSet(ChannelSetSuperclass):
	"""
	...
	A ChannelSet is a numpy.@SUPERCLASS@ subclass, with the channels arranged in a column.
	Pre-multiplication by a spatial filtering matrix (each row is a filter) works as it should.
		
	"""
	__array_priority__ = ChannelSetArrayPriority
	def __new__(cls, s=None, ref=None, gnd=None):
		...
		self = MakeMatrix(self, dtype=object).view(cls)
	...
	def plot(self, *pargs, **kwargs):
		"""
		See documentation for the `trodeplot()` function
		(ignoring the initial `channels` argument).
		"""
	...
	def BCI2000SpatialFilterParameters( self, spatialFilterMatrix, filters_as='columns', outputFileName=None, fullFormat=False ):
		"""
		For example::
		...
		if fullFormat:
			p = """
				Filtering int SpatialFilterType= 1 // full matrix format 
				Filtering matrix SpatialFilter= { %s } { %s } %s
			""" % (
				' '.join( self.get_labels() ),
				' '.join( self.get_labels() ),
				' '.join( str( x ) for x in spatialFilterMatrix.flat ),   # ( '%.10f' % x ).rstrip( '0' ).rstrip( '.' ) if BCI2000 chokes on the scientific notation (may have been true in earlier versions)
			)
			p = '\n'.join( line.strip() for line in p.strip().split( '\n' ) )
		else:
			sparse = []
	...
	def ReadBCI2000SpatialFilter( self, prm, filters_as='columns', containerForOutputLabels=None ):
		...
		if spatialFilterType == 0: # disable spatial filtering
			...
			m = MakeMatrix( prm.NumericValue, dtype=float ).T # work with filters_as='columns' for now, until the end
...
try: ChannelSet.__doc__ = ChannelSet.__doc__.replace( '@SUPERCLASS@', ChannelSetSuperclass.__name__ )
except: pass  # TODO: in Python 2.7, class __doc__ is not writable - maybe convey this info somewhere else?
...
def trodeplot(channels=(), act='connected',
	...
	if surf or contour:
		...
		if contour and numpy.ptp(chzi.ravel()):
