...
class Deploy(BaseModel):
    ...
    systemd: StrictBool = Field(default=True, description="Should Systemd service be deployed, only for local single-node deployments with podman kube play")
    ...
    development: StrictBool = Field(default=True, description="Should development features be enabled")
    logging: StrictBool = Field(default=True, description="Should logging with Promtail be enabled")
...
class Prometheus(BaseModel):
    ...
    cloudGrafana: StrictBool = Field(default=True, description="Should remote write to Grafana Cloud be enabled")
...
class Systemd(BaseModel):
    ...
    hostSshPort: StrictInt = Field(default=22, ge=1, le=65535, description="We connect to host from inside the container over ssh, this is host ssh port where sshd is listening")
...
class App(BaseModel):
    ...
    hostPortHttps: StrictInt = Field(default=0, ge=0, le=65535, description="Change app host port for HTTPS if needed. NOTE: common.requireHttps must be set to true to enable this port")
    licenceBase64: Optional[str] = Field(default="", description="Base64 encoded licence string, for development purposes only")
    dataGi: StrictInt = Field(default=20, ge=1, description="Size of persistent volume claim for app data in Gi")
...
class Link(BaseModel):
    ...
    image: Image = Field(default=Image(registry="registry.gitlab.com/griddot/linkgrid", version="v2025.1.0"), description="")
    postgres: Postgres = Field(default=Postgres(), description="")
...
if __name__ == "__main__":
    ...
    with open(f"{folder}/values.yaml", "w") as f:
        f.write(values.to_str())

    # values.landis-gyr.yaml
    TOP_LEVEL_COMMENT = ""
    landis_base = ValuesYaml(common=Common(reseller="landis-gyr"), deploy=Deploy(development=False, systemd=False, link=False), prometheus=Prometheus(scrapeInterval="30s"))
    with open(f"{folder}/values/values.landis-gyr.base.yaml", "w") as f:
        f.write(landis_base.to_str(exclude_defaults=True))

    landis = ValuesYaml(
        sync=Sync(image=Image(registry="registry.gitlab.com/griddot/resellers/landis-gyr/syncgrid")),
        prometheus=Prometheus(nodeExporter=False, databaseExporter=False)
    )
    ...
    with open(f"{folder}/values/values.landis-gyr.yaml", "w") as f:
    ...
    # values.enerdat.yaml
    enerdat_base = ValuesYaml(common=Common(reseller="enerdat"), deploy=Deploy(development=False), prometheus=Prometheus(scrapeInterval="30s"))
    with open(f"{folder}/values/values.enerdat.base.yaml", "w") as f:
        f.write(enerdat_base.to_str(exclude_defaults=True))

    enerdat = ValuesYaml(
        sync=Sync(image=Image(registry="registry.gitlab.com/griddot/resellers/enerdat/syncgrid")),
        link=Link(image=Image(registry="registry.gitlab.com/griddot/resellers/enerdat/linkgrid")),
        prometheus=Prometheus(nodeExporter=False, databaseExporter=False)
    )

    with open(f"{folder}/values/values.enerdat.yaml", "w") as f:
        f.write(enerdat.to_str(exclude={
            "deploy": {
                "development",
            },
            **default_exclude
