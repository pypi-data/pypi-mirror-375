...
class RejectionReason(Enum):
    ...
    def translate_from_rpc(rpc_enum_value):
        ...
        if (
            rpc_enum_value
            == arm_authorizer_server_pb2.REJECTION_REASON_INVALID_WAYPOINT
        ):
...
class ArmAuthorizerServerResult:
    ...
    class Result(Enum):
        ...
        def translate_to_rpc(self):
            if self == ArmAuthorizerServerResult.Result.UNKNOWN:
                return (
                    arm_authorizer_server_pb2.ArmAuthorizerServerResult.RESULT_UNKNOWN
                )
            if self == ArmAuthorizerServerResult.Result.SUCCESS:
                return (
                    arm_authorizer_server_pb2.ArmAuthorizerServerResult.RESULT_SUCCESS
                )
        ...
        def translate_from_rpc(rpc_enum_value):
            ...
            if (
                rpc_enum_value
                == arm_authorizer_server_pb2.ArmAuthorizerServerResult.RESULT_FAILED
            ):
    ...
    def translate_from_rpc(rpcArmAuthorizerServerResult):
        ...
        return ArmAuthorizerServerResult(
            ArmAuthorizerServerResult.Result.translate_from_rpc(
                rpcArmAuthorizerServerResult.result
            ),
...
class ArmAuthorizerServer(AsyncBase):
    ...
    def _setup_stub(self, channel):
        """Setups the api stub"""
        self._stub = arm_authorizer_server_pb2_grpc.ArmAuthorizerServerServiceStub(
            channel
        )
    ...
    def _extract_result(self, response):
        ...
        return ArmAuthorizerServerResult.translate_from_rpc(
            response.arm_authorizer_server_result
        )
