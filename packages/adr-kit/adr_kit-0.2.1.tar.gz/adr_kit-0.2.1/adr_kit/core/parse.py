...
def parse_front_matter(content: str, file_path: Union[Path, str, None] = None) -> Tuple[Dict[str, Any], str]:
    ...
    if not match:
        raise ParseError("No YAML front-matter found. ADR files must start with ---", file_path)
...
def parse_adr_content(content: str, file_path: Union[Path, str, None] = None, strict: bool = True) -> ADR:
...
def find_adr_files(directory: Union[Path, str] = "docs/adr", pattern: str = "ADR-*.md") -> List[Path]:
