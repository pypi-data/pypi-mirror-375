...
***v5版本最新加入了usecols参数，新增支持读取excel中指定的列***
usecols支持输入：
    usecols="B:D",
    usecols=["姓名", "年龄", "城市"]
    usecols=[1, 2, 3]

***v6版本最新加入了Del_blankRow参数，支持留取某列带空数据的行
del_blankRow=True: 只要某行中有一列为空数据就删除（默认值）
del_blankRow=False：只要某行中有一列的数据非空就不删除
'''
def readExcel2dataframe(sheetname,filepath,skiprows=0,nrows=0,usecols=None, del_blankRow=True):
    if nrows == 0: #默认将全部excel文件读入
        df = pd.read_excel(filepath, sheet_name=sheetname, skiprows=skiprows, usecols=usecols)
    elif nrows < 0:
        warnings.warn("readExcel2dataframe函数在调用时传入的参数nrows不能为负值")
        exit(1)
    else: #指定读取excel中指定行数的数据
        df = pd.read_excel(filepath, sheet_name=sheetname, skiprows=skiprows, nrows=nrows, usecols=usecols)

    if df.size ==0: #空dataframe
        return df,None,None

    # 处理同名列名excel自动加编号问题： 去除列名"日期.1","日期.2"后面的数字，将列名改成"日期"
    # 当用程序写入excel文件，包含同名列的时候，excel会自动在列名后加编号，打开excel文件时看不出来，但是程序读列名时会有编号
    # 去除掉所有同名列后面自动添加的编号
    ...
    # 只处理要当索引的列：【日期】列
    df.columns = [
        col.split('.')[0] if col.startswith('日期') else col
    ...
    # 删除全部列为空值的行 or 只要有列为空值就删除该行
    if del_blankRow is True:
...
'''
v6版本增加了对dataframe中包含数据缺失的所有列手动禁止线性插值这个功能，
因为在该版本中增加了对显示空数据点的支持，为了如实显示空数据，增加了interpolate参数进行设置
默认情况下interpolate=True，即当要画图的dataframe指定列中的某行出现空数据时，该函数会先自动进行插值，再画图。
若用户想显示该空数据，则需要手动设置interpolate=False
'''
def matplot_draw(df,ax,fig,title,**kwargs):
    ...
    if "ax_twin" in kwargs:  #传入副坐标轴
        ...
        # 分别为主副轴设置颜色循环
        custom_colors_ax = [
            'blue', 'coral', 'green', 'orange', 'purple',
            'red', 'gold', 'yellow', 'azure', 'beige',
            'brown', 'fuchsia', 'navy', 'olive', 'orangered',
            'peachpuff', 'tan', 'wheat'
        ]
        ax.set_prop_cycle(cycler('color', custom_colors_ax))

        custom_colors_ax_twin = [
            'grey', 'violet', 'cyan', 'lightblue', 'black',
            'chocolate', 'teal', 'yellow', 'magenta', 'mistyrose'
            # 非 'tab:'，但有效的 HTML 名称
        ]
        ax_twin.set_prop_cycle(cycler('color', custom_colors_ax_twin))
    ...
    # 默认画图函数会对数据中的空值根据前后数据自动进行插值，若希望显示此空值则需要在调用此函数时手动设置interpolate=False
    if "interpolate" in kwargs:
        interpolate_flag=kwargs["interpolate"]
    else:
    ...
    if has_missing_values:
        ...
        if interpolate_flag is True: #对包含数据缺失的所有列进行线性插值
    ...
    for column in df.columns:
        if "【副轴】"in column:
            ...
            if "【柱状图】" in column:
                ...
                bar=ax_twin.bar(x, df[new_colname], width=10, label=new_colname)
                lines.append(bar)
                ...
                if "【虚线】" in column:
                    ...
                    line,=ax_twin.plot(x, df[new_colname], label=new_colname, linestyle='--')
                    lines.append(line)
                else:
                    line, = ax_twin.plot(x, df[column], label=column)
                    lines.append(line)
