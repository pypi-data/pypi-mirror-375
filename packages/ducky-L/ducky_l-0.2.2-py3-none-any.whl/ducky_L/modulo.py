...
import qrcode
def crear_qr(datos, nombre_archivo="qr.png"):
    try:
        ...
        qr = qrcode.QRCode(
            ...
            box_size=30, # Tamaño de cada 'caja' o píxel del QR
            border=5, # Tamaño del borde en cuadros
        )
        
        # Añade los datos y los procesa
        qr.add_data(datos)
        qr.make(fit=True)
...
import time
def anima(texto, delay=0.05):
    for letra in texto:
        print(letra, end="", flush=True)
        time.sleep(delay)
    print()

def color(texto, color='rojo'):
    colores = {
        'negro': '\033[30m',
        'rojo': '\033[31m',
        'verde': '\033[32m',
