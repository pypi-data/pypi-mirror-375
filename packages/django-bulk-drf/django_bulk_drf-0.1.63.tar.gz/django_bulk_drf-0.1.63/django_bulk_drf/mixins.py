        ...
        if instances_to_upsert:
            try:
                ...
                if unique_values_list:
                    ...
                    existing_records = {tuple(getattr(obj, field) for field in unique_fields): obj for obj in existing_queryset}
                ...
                if instances_to_create:
                    ...
                    created_instances = model_class.objects.bulk_create(instances_to_create, batch_size=1000)

                # Step 5: Perform individual updates for existing records
                updated_instances = []
                if instances_to_update:
                    print(
                        f"BulkOperationsMixin._handle_array_upsert_direct() - Updating {len(instances_to_update)} existing records",
                        file=sys.stderr,
                    )
                        ...
                        if update_fields:
                            for field in update_fields:
                                if hasattr(new_data_instance, field):
                                    setattr(existing_instance, field, getattr(new_data_instance, field))
                        else:
                            # Update all non-unique fields
                            for field in model_class._meta.fields:
                                if field.name not in unique_fields and not field.primary_key and not field.auto_now:
                                    if hasattr(new_data_instance, field.name):
                                        setattr(existing_instance, field.name, getattr(new_data_instance, field.name))
