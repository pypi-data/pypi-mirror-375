...
for reentry_vel in reentry_velocities:
    ...
    for height in heights:
        print(
            f"Running simulation for height {height:.2e} rad and velocity {reentry_vel:.2e} m/s"
        )
    ...
    plt.plot(heights, ceps, label=f"v={reentry_vel / 1e3:.1f} km/s")
