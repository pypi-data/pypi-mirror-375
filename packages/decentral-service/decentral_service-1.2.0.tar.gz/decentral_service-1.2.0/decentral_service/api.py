...
class APIServer:
    ...
    def __init__(self, host: str = "localhost", port: int = 6379, service=None, debug: bool = False):
        self.host = host
        self.port = port
    ...
    def _setup_routes(self):
        ...
        @self.app.route('/tasks', methods=['POST', 'GET'])
        def tasks_handler():
            """Handle task submission (POST) and listing (GET)"""
            if not self.service:
                return jsonify({"error": "Service not available"}), 500
            ...
            if request.method == 'POST':
                ...
                try:
                    if self.debug:
                        self.logger.debug("Task submission requested")
                    data = request.get_json()
                    if data is None:
                        return jsonify({"error": "JSON data required"}), 400
            ...
            try:
                debug_info = {
                    ...
                    "service_config": {
                        ...
                        "enable_api": self.service.enable_api,
