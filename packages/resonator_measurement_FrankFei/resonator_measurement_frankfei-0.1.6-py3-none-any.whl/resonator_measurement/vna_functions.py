...
from . import calculation_functions as calc
...
class VNA_Functions:
    def __init__(self, instrument_file_path):
        ...
        self.vna = VNABackend(device=instruments["vna"])
        self.exe = self.vna.download()
        print(self.exe)
    ...
    def append_row_to_csv(self, filename, new_row):
    ...
    def power_sweep(self, file_path, index,center_freq, span=6e5, points=501, low=-80, high=20, num=101, ave=150):
        ...
        for i, p in enumerate(track(powers)):
            ...
            self.append_row_to_csv(file_path_params, parameters)

            ### save Q_i
            file_path_Q_i = f'Q_i/R{index}_power_{p}_Qis.csv'
            Qi = fit.params["Q_i"]
            Qi_lst = [Qi.value, Qi.stderr]
            self.append_row_to_csv(file_path_Q_i, Qi_lst)
