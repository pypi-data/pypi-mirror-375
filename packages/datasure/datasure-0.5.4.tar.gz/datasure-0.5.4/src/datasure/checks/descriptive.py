...
def plot_date_distribution(data: pd.DataFrame, date_col: str) -> None:
    ...
    def display_table_date_distribution(
        ...
        # Adjust max_elements if data size exceeds 390,000
        styler = prepped_date_data.style.format(
            {
                "Frequency": "{:,.0f}",
                "Percentage": "{:,.2f}%",
            }
        ...
        if prepped_date_data.size > 390000:
            styler = styler.set_table_attributes('style="max-elements: none;"')
...
def plot_categorical_distribution(data: pd.DataFrame, categorical_col: str) -> None:
    ...
    def display_one_way_table() -> None:
        ...
        cmap = sns.light_palette("pink", as_cmap=True)
        # Adjust max_elements if data size exceeds 390,000
        styler = one_way_table.style.format(
            {
                "Frequency": "{:,.0f}",
                "Percentage": "{:,.2f}%",
            }
    ...
    def display_two_way_table() -> None:
        ...
        cmap = sns.light_palette("pink", as_cmap=True)
        # Adjust max_elements if data size exceeds 390,000
        styler = two_way_table.style.background_gradient(subset=format_cols, cmap=cmap)
        if two_way_table.size > 390000:
            styler = styler.set_table_attributes('style="max-elements: none;"')
