...
class EvtCountCls:
	...
	def get(self, power=repcap.Power.Default) -> int:
		"""POWer<*>:QUALity:RESult:VOLTage:RMS:EVTCount \n
		Snippet: value: int = driver.power.quality.result.voltage.rms.evtCount.get(power = repcap.Power.Default) \n
		...
		response = self._core.io.query_str(f'POWer{power_cmd_val}:QUALity:RESult:VOLTage:RMS:EVTCount?')
