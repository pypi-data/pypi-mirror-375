...
def response_format_from_msgspec_struct(
    ...
    typevar_default_schema: Dict[str, Any] = {"type": "string"}
) -> Dict[str, Any]:
    ...
    def _patch_empty_typevar_schemas(
        schema_node: Any, default_schema: Dict[str, Any]
    ) -> None:
        """Iterates through the schema and replaces empty schemas ({}),
        typical of unresolved TypeVars, with a default schema.
        """
        if isinstance(schema_node, dict):
            # Checks if we are inside an object with properties
            if "properties" in schema_node and isinstance(schema_node["properties"], dict):
                props = schema_node["properties"]
                for key, value in props.items():
                    # If a property's schema is an empty dict, replace!
                    if value == {}:
                        props[key] = default_schema
            
            # Continue recursion for other values
            for value in schema_node.values():
                _patch_empty_typevar_schemas(value, default_schema)
                
        elif isinstance(schema_node, list):
            for item in schema_node:
                _patch_empty_typevar_schemas(item, default_schema)
    ...
    # Fix empty TypeVars schemas
    _patch_empty_typevar_schemas(inlined_schema, typevar_default_schema)
