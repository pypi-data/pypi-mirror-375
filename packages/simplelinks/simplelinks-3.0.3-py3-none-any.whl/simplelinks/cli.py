...
def client_cli():
    ...
    join_parser.add_argument(
        '--enable-p2p', action='store_true',
        help='Enable TCP P2P direct connections (experimental)'
    )
    ...
    if args.command == 'join':
        try:
            ...
            if args.enable_p2p:
                # Use P2P-enabled client
                from .client_v3 import P2PClient
                p2p_client = P2PClient(
                    server_host=f"wss://{args.host}:{args.port}",
                    secret=args.secret,
                    virtual_ip="10.254.254.2",  # Will be dynamically assigned by server
                    enable_p2p=True
                )
                print(f"🚀 Starting SimpleLinks Client with P2P support...")
                print(f"  Server: {args.host}:{args.port}")
                print(f"  P2P: Enabled (TCP port 20002)")
                print(f"  Secret: {args.secret[:8]}...")
                print()
                
                import asyncio
                asyncio.run(p2p_client.start())
            else:
                # Use standard client
