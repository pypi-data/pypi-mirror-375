...
class TestForkliftExceptions:
    ...
    def test_forklift_error_base_exception(self):
        """Test base ForkliftError exception."""
        error = ForkliftError("Test error", exit_code=5)
        assert str(error) == "Test error"
        assert error.exit_code == 5

    def test_forklift_error_default_exit_code(self):
        """Test ForkliftError with default exit code."""
        error = ForkliftError("Test error")
        assert error.exit_code == 1

    def test_configuration_error(self):
        """Test ForkliftConfigurationError."""
        error = ForkliftConfigurationError("Config error")
        assert str(error) == "Config error"
        assert error.exit_code == 2

    def test_validation_error(self):
        """Test ForkliftValidationError."""
        error = ForkliftValidationError("Validation error")
        assert str(error) == "Validation error"
        assert error.exit_code == 3

    def test_network_error(self):
        """Test ForkliftNetworkError."""
        error = ForkliftNetworkError("Network error", operation="test_op")
        assert str(error) == "Network error"
        assert error.exit_code == 4
        assert error.operation == "test_op"

    def test_authentication_error(self):
        """Test ForkliftAuthenticationError."""
        error = ForkliftAuthenticationError("Auth error")
        assert str(error) == "Auth error"
        assert error.exit_code == 5

    def test_output_error(self):
        """Test ForkliftOutputError."""
        error = ForkliftOutputError("Output error")
        assert str(error) == "Output error"
        assert error.exit_code == 6

    def test_unicode_error(self):
        """Test ForkliftUnicodeError."""
        error = ForkliftUnicodeError("Unicode error", text="test text")
...
class TestErrorHandler:
    ...
    def test_handle_error_forklift_error(self, mock_exit):
        """Test handling ForkliftError."""
        handler = ErrorHandler(debug=False)
        error = ForkliftValidationError("Validation failed")
    ...
    def test_validate_unicode_text_failure(self):
        ...
        with patch.object(handler, 'validate_unicode_text') as mock_validate:
            mock_validate.side_effect = ForkliftUnicodeError("Cannot process Unicode text in test context")
...
class TestErrorHandlingIntegration:
    ...
    def test_error_handler_context_preservation(self):
        ...
        forklift_error = ForkliftOutputError(f"Wrapped: {original_error}")
    ...
    def test_unicode_error_with_text_truncation(self):
        ...
        long_text = "x" * 200
        error = ForkliftUnicodeError("Unicode error", text=long_text)
    ...
    def test_network_error_with_operation_context(self):
        ...
        error = ForkliftNetworkError("Connection failed", operation="fetch_forks")
