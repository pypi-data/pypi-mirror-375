...
class Binder:
    ...
    def has_explicit_binding_for(self, interface: type) -> bool:
        return self.has_binding_for(interface) and not isinstance(
            self._bindings[interface], ImplicitBinding
        )
...
def _punch_through_alias(type_: Any) -> type:
    if (
        ...
        isinstance(type_, _AnnotatedAlias)
...
class SingletonScope(Scope):
    ...
    def _get_instance(
        self, key: Type[T], provider: Provider[T], injector: "Injector"
    ) -> T:
    ...
    def _get_instance_from_parent(
        self, key: Type[T], provider: Provider[T], parent: "Injector"
    ) -> T:
...
class Injector:
    ...
    @synchronized(lock)
    def get(
        self, interface: Type[T], scope: Union[ScopeDecorator, Type[Scope], None] = None
    ) -> T:
...
class _NoReturnAnnotationProxy:
    ...
    def __getattribute__(self, name: str) -> Any:
        ...
        if name == "__annotations__":
            ...
            return {
                name: value for (name, value) in annotations.items() if name != "return"
            }
...
def _infer_injected_bindings(
    ...
    def _is_injection_annotation(annotation: Any) -> bool:
        return _is_specialization(annotation, Annotated) and (
            _inject_marker in annotation.__metadata__
            or _noinject_marker in annotation.__metadata__
        )
    ...
    try:
        ...
        bindings = get_type_hints(
            cast(Callable, _NoReturnAnnotationProxy(callable)), include_extras=True
        )
    except NameError as e:
