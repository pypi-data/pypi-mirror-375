...
### Analysis Command Flags (`#sk_analyze`)
| Flag | Purpose | Values |
|------|---------|--------|
...
### Improve Command Flags (`#sk_improve`)
| Flag | Purpose | Values |
|------|---------|--------|
...
### Workflow Command Flags (`#sk_workflow`)
| Flag | Purpose | Values |
|------|---------|--------|
...
### Troubleshoot Command Flags (`#sk_troubleshoot`)
| Flag | Purpose | Values |
|------|---------|--------|
...
### Cleanup Command Flags (`#sk_cleanup`)
| Flag | Purpose | Values |
|------|---------|--------|
...
### Estimate Command Flags (`#sk_estimate`)
| Flag | Purpose | Values |
|------|---------|--------|
...
```bash
#sk_implement "responsive dashboard" --magic --c7
#sk_design component-library --type component --format code
#sk_test ui-components/ --magic --play
#sk_improve legacy-ui/ --magic --morph --validate
```

### Backend Development
```bash
#sk_analyze api/ --focus performance --seq --think
#sk_design payment-api --type api --format spec
#sk_troubleshoot "API timeout" --type performance --trace
#sk_improve auth-service --type security --validate
```

### Large Projects
```bash
#sk_analyze . --ultrathink --all-mcp --safe-mode
#sk_workflow enterprise-system --strategy enterprise --depth deep
#sk_cleanup . --type all --safe --interactive
#sk_estimate "migrate to microservices" --type complexity --breakdown
```

### Quality & Maintenance
```bash
#sk_improve src/ --type quality --safe --interactive
#sk_cleanup imports --type imports --preview
#sk_reflect --type completion --validate
#sk_git commit --smart-commit
```
...
### Debug Flags
```bash
#sk_analyze . --verbose                      # Shows decision logic and flag activation
#sk_select_tool "operation" --explain        # Explains tool selection process
#sk_reflect --type session --analyze         # Reviews current session decisions
```

### Quick Fixes
```bash
#sk_analyze . --help                         # Shows available flags for command
#sk_analyze . --no-mcp                       # Native execution only
#sk_cleanup . --preview                      # Shows what would be cleaned
```
