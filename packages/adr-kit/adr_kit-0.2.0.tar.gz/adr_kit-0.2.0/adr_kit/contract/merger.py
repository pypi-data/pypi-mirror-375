...
class PolicyMerger:
    ...
    def merge_policies(self, accepted_adrs: List[ADR]) -> MergeResult:
        ...
        merged_boundaries = BoundaryPolicy()
        merged_python = PythonPolicy()
        provenance = {}
        ...
        for adr in sorted_adrs:
            ...
            effective_date = datetime.combine(adr.front_matter.date, datetime.min.time())
            ...
            if policy.boundaries:
                merged_boundaries, boundary_provenance = self._merge_boundary_policy(
                    merged_boundaries, policy.boundaries, adr_id, adr_title, effective_date
                )
        ...
        constraints = MergedConstraints(
            imports=merged_imports if (merged_imports.disallow or merged_imports.prefer) else None,
            boundaries=merged_boundaries if (merged_boundaries.layers or merged_boundaries.rules) else None,
