...
def serialDemo():
    ...
    """
    g_pkt = zlb.ZlBusUnPack(tracker_nums = 6, user_id = 0xFF, fifoMaxSize = 30)

    # 手动设置上传数据流水号编码, 默认FLOW_ID_FORMAT_8,可忽略该步骤
    # g_pkt.setFlowIdFormat(zlapi.e_Tk_FLOW_FORMAT.TK_FLOW_ID_FORMAT_8)
    ...
    if uartStr:
        ...
        # 获取上传数据格式，用于上传数据解析
        serialWrite(zlapi.ul_getDataFormat())

        time.sleep(1)

        # 将姿态数据调整为串口输出 (开发板确保，接口模式包含UART)
        # write(zlapi.hl_configOutDataPort(zlapi.e_DataOutPort.TK_RF_PORT | zlapi.e_DataOutPort.TK_UART_PORT))
        serialWrite(zlapi.hl_configOutDataPort(zlapi.e_DataOutPort.TK_UART_PORT))
