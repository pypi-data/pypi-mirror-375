...
def compile_libmplabml_shared_library(path):
    ...
    try:
        ...
        if os.path.exists("libtensorflow-microlite.a"):
            ...
            subprocess.call(
                "g++ --shared -o libmplabml.so -Wl,--whole-archive libtensorflow-microlite.a libmplabml.a  -Wl,--no-whole-archive",
                shell=True,
            )
        else:
    ...
    def __init__(self, path: str, class_map: Optional[Dict] = None):
        ...
        if path[-3:] == "zip":
            ...
            with zipfile.ZipFile(path, "r") as zip_ref:
                ...
                lib_path = os.path.join(unzip_dir, "knowledgepack", "mplabml", "lib")
                model_json_path = os.path.join(unzip_dir, "knowledgepack")

        if os.name == "nt":
            if not os.path.exists(os.path.join(lib_path, "libmplabml.dll")):
                print("ERROR: libmplabml.dll as not found in {}.".format(lib_path))
                raise ImportPathException(
                    "ERROR: libmplabml.dll as not found in {}".format(lib_path)
                )

            clf_lib = CDLL(os.path.join(lib_path, "libmplabml.dll"))

        if os.name == "posix":
            if not os.path.exists(os.path.join(lib_path, "libmplabml.so")):
                print(
                    "Warning: libmplabml.so as not found in {}. Attempting".format(
                        lib_path
                    )
                )
                if not os.path.exists(os.path.join(path, "libmplabml.a")):
                    print("ERROR: libmplabml.a as not found in {}.".format(lib_path))

                print("Attempting to compile libmplabml.so")

                compile_libmplabml_shared_library(lib_path)

                if not os.path.exists(os.path.join(lib_path, "libmplabml.so")):
                    raise ImportPathException(
                        "ERROR: libmplabml.so as not found in {}".format(lib_path)
                    )

            clf_lib = CDLL(os.path.join(lib_path, "libmplabml.so"))
