...
class ProductionRequest:
    ...
    def _buildProduction(
        ...
        filetypes_joined = "".join([x.split(".")[0] for x in fTypeIn])
        name = f"{self.requestID}_{prodType}_{self.prodGroup}_EventType_{self.eventType}_{filetypes_joined}"
        # If the name is more than 200 characters, append a hash to stay under the 255 character limit
        if len(name) > 200:
            digest = hashlib.sha256(name.encode()).hexdigest()[:8]
            name = f"{name[:200]}_{digest}"
        name = f"Request_{name}_{self.appendName}"

        prod.LHCbJob.workflow.setName(name)
