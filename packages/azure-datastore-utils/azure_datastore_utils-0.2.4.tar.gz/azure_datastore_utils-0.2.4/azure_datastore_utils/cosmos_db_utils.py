...
class CosmosDBUtils:
    ...
    def __init__(self, database_name: str, collection: str, auth_strategy: CosmosDbAuthStrategy = 'ConnectionString'):
    ...
    def get_single_item(self, item_id: str, partition_key: Any | None = None) -> Dict[str, Any] | None:
        ...
        try:
            item_response = container_proxy.read_item(item=item_id, partition_key=partition_key)
            ...
            raise Exception(f"Cosmos DB request failed with status {e.status_code}, message: {e.message}") from e
        except Exception as f:
            raise Exception(f"Unexpected error from Cosmos DB, details : {f}") from f
