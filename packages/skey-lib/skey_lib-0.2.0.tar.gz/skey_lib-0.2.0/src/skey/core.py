...
class SKey:
    ...
    def md5(self):
        ...
        regs = self.digest_to_regs(digest)

        tmpseq = self.seq
        while tmpseq + 1 > 0:
            self.hash = self.otpfoldregs(regs)
    ...
    def wordlist(self):
        ...
        for i in range(4, -1, -1):
            tmpstr += SKey.btoe(((wi >> (i * 11 + 9)) & 0x7ff)) + " "
        tmpstr += SKey.btoe(((wi << 2) & 0x7fc) | (parity & 0x03))
        return tmpstr

    @staticmethod
    def btoe(index):
        if 0 <= index < len(SKey.words):
            return SKey.words[index]
        return "bogus"
