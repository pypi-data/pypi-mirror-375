...
def analyze_workflow_structure(workflow) -> Dict[str, Any]:
    ...
    metrics["thoughtful.workflow.total_steps"] = total_steps

    metrics["thoughtful.workflow.top_level_steps"] = len(workflow)

    step_ids = extract_step_ids(workflow)
    metrics["thoughtful.workflow.step_ids"] = step_ids

    max_depth = calculate_max_depth(workflow)
    metrics["thoughtful.workflow.max_depth"] = max_depth
