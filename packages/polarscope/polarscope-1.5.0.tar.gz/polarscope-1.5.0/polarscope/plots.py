...
def _numeric_columns(df: pl.DataFrame) -> list[str]:
    """Optimized numeric column detection using Polars selectors."""
    try:
        # Use Polars selectors for optimal performance
...
def _ensure_columns(df: pl.DataFrame, columns: Iterable[str] | None) -> list[str]:
    """Optimized column validation using Polars selectors."""
    if columns is None:
        return _numeric_columns(df)
    else:
        # Filter to existing columns first
        valid_cols = [c for c in columns if c in df.columns]
        if not valid_cols:
            return []
        
        # Use Polars selectors to get numeric columns efficiently
        try:
            numeric_cols = set(df.select(cs.numeric()).columns)
            return [c for c in valid_cols if c in numeric_cols]
        except Exception:
            # Fallback for older Polars versions
...
def _corr_matrix(df: pl.DataFrame, cols: Sequence[str]) -> List[List[float]]:
    """Optimized correlation matrix calculation using Polars built-in corr() method."""
    # Use Polars built-in correlation matrix calculation (much faster)
    corr_df = df.select(cols).corr()
    return corr_df.to_numpy().tolist()
...
def convert_datatypes(
    ...
    *,
    ...
    """
    Optimize DataFrame data types to reduce memory usage using vectorized operations.
    ...
    # Get column statistics in batch for better performance
    col_stats = {}
    for col in df.columns:
        ...
        col_stats[col] = {
            ...
            'min': series.min() if series.dtype.is_numeric() else None,
            'max': series.max() if series.dtype.is_numeric() else None,
        }
    ...
    for col, stats in col_stats.items():
        ...
        # Skip if contains nulls (for simplicity)
