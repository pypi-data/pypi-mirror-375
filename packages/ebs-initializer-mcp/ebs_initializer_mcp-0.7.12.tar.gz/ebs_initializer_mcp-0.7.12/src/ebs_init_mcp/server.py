...
@mcp.tool()
def suggest_initialization_for_instance(instance_id: str, region: str = DEFAULT_REGION) -> str:
    """
    Analyze an EC2 instance and suggest EBS volume initialization if needed.
    Perfect for use after launching new EC2 instances.
    
    Args:
        instance_id: EC2 instance ID to analyze (e.g., i-1234567890abcdef0)
        region: AWS region name (default: from AWS_DEFAULT_REGION or AWS_REGION env var, fallback to us-east-1)
    
    Returns:
        Text suggestion with initialization recommendation
    """
    try:
        ec2 = get_ec2_client(region)
        ...
        if instance_state not in ['running', 'stopped']:
            return f"ℹ️ Instance {instance_id} is in '{instance_state}' state. EBS initialization is typically done when instance is running."
        ...
        if not response['Volumes']:
            return f"ℹ️ No EBS volumes found attached to instance {instance_id}."
        
        # Analyze volumes for initialization needs
        volumes_needing_init = []
        volumes_not_needing_init = []
        total_gb_needing_init = 0
            ...
            for attachment in volume['Attachments']:
                if attachment['InstanceId'] == instance_id:
                    ...
                    if needs_initialization(volume):
                        volumes_needing_init.append(volume_summary)
                        total_gb_needing_init += volume['Size']
                    else:
                        volumes_not_needing_init.append(volume_summary)
        
        # Generate suggestion message
        total_volumes = len(volumes_needing_init) + len(volumes_not_needing_init)
        
        if not volumes_needing_init:
            return f"""🟢 No EBS Volume Initialization Needed
            
Instance: {instance_id} ({instance_type})
Status: {instance_state}
Total Volumes: {total_volumes}
✅ All volumes are blank (no snapshots) - no initialization required
            
ℹ️ Blank EBS volumes don't need initialization as they're already at full performance."""

        # Calculate estimated time
        instance_max_throughput = get_instance_ebs_throughput(instance_type, region)
        volumes_for_estimation = [
            {'size_gb': vol['size_gb'], 'max_throughput_mbps': vol.get('throughput', 1000)}
            for vol in volumes_needing_init
        ]
        ...
        suggestion = f"""🔥 EBS Volume Initialization Suggested!
...
📊 Volume Analysis:
• Needs initialization: {len(volumes_needing_init)} volumes ({total_gb_needing_init}GB)
• Already optimized: {len(volumes_not_needing_init)} volumes

⚡ Volumes requiring initialization:"""
        
        for i, vol in enumerate(volumes_needing_init, 1):
            suggestion += f"\n  {i}. {vol['volume_id']} - {vol['size_gb']}GB {vol['volume_type']}"
            if vol['snapshot_id']:
                suggestion += f" (from snapshot)"
        
        if estimated_minutes > 0:
            estimated_time = format_estimated_time(estimated_minutes)
            suggestion += f"\n\n⏱️ Estimated completion time: {estimated_time}"
        
        suggestion += f"""

💡 Recommendation:
These volumes were created from snapshots and need initialization to achieve full performance.
Without initialization, they may experience reduced IOPS until all blocks are accessed.

🚀 To proceed with initialization, use:
initialize_all_volumes(instance_id="{instance_id}", method="fio", region="{region}")

📋 Benefits of initialization:
• Immediate full IOPS performance
• Predictable I/O latency
• Optimal application performance from start"""
        
        return suggestion
        
    except Exception as e:
        return f"❌ Failed to analyze instance {instance_id}: {str(e)}"
