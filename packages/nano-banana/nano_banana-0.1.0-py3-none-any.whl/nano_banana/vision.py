...
class NanoBanana:
    ...
    def text_to_image(self, prompt: str) -> str:
        ...
        # 构建消息内容
        content = [{"type": "text", "text": f"请生成一张图片：{prompt}"}]
    ...
    def image_to_image(self, prompt: str, reference_images: Union[str, Path, List[Union[str, Path]]]) -> str:
        """图片到图片的转换/编辑/创作"""
        
        # 构建消息内容
        content = [{"type": "text", "text": f"请参考提供的图片，按照要求：{prompt},生成一张新的图片"}]
...
def image_to_image(prompt: str, reference_images: Union[str, Path, List[Union[str, Path]]]) -> str:
    """图片到图片的转换/编辑/创作"""
