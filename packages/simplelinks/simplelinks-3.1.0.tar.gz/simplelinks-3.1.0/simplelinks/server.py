...
class SimpleLinksServer:
    ...
    async def register_client(self, websocket, message: dict) -> dict:
        ...
        try:
            ...
            # 在客户端注册成功后，向所有客户端推送更新的peer列表
            asyncio.create_task(self.broadcast_peer_list(subnet))
    ...
    async def broadcast_peer_list(self, subnet: str = None):
        """向所有在线客户端广播更新的P2P peer列表"""
        try:
            # 获取所有在线客户端
            online_clients = []
            for session_id, websocket in self.clients.items():
                client = self.db.get_client_by_session_id(session_id)
                if client and client.get('is_online'):
                    peer_info = {
                        'virtual_ip': client['virtual_ip'],
                        'private_ip': client.get('private_ip'),
                        'hostname': client.get('hostname', 'unknown')
                    }
                    # 只包含有效的private_ip
                    if peer_info['private_ip']:
                        online_clients.append(peer_info)
            ...
            peer_message = {
                "type": "peer_list",
                "peers": online_clients
            }
            
            # 广播给所有在线客户端
            broadcast_count = 0
            for session_id, websocket in list(self.clients.items()):
                try:
                    await websocket.send(json.dumps(peer_message))
                    broadcast_count += 1
                except websockets.exceptions.ConnectionClosed:
                    # 客户端已断开，清理
                    await self.cleanup_client(session_id)
                except Exception as e:
                    self.logger.debug(f"Failed to send peer list to client {session_id}: {e}")
            
            if broadcast_count > 0:
                self.logger.info(f"Broadcasted peer list to {broadcast_count} clients ({len(online_clients)} peers)")
    ...
    async def cleanup_client(self, session_id: str):
        ...
        try:
            if session_id in self.clients:
                client = self.db.get_client_by_session_id(session_id)
                if client:
                    subnet = client.get('secret', 'default')  # secret存储的是subnet
