...
class DiceCoefficient:
    ...
    def __call__(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        targets = nn.functional.one_hot(targets.long(), num_classes=inputs.size(1)).permute(0, 4, 1, 2).float() if targets.dim() == 3 else nn.functional.one_hot(targets.long(), num_classes=inputs.size(1)).permute(0, 4, 1, 2, 3).float()
