...
namespace mindspore {
namespace kernel {
OPS_KERNEL_COMMON_API float Scaling(size_t in_size, size_t out_size, bool align_corners);
...
void OPS_KERNEL_COMMON_API ComputeInterpolationWeights(const size_t out_size, const size_t in_size, const float scale,
...
OPS_KERNEL_COMMON_API std::vector<bool> Dec2Bin(const int64_t &mask);
// ===========================New interface==========================================================
OPS_KERNEL_COMMON_API void FillEmptyDims(const std::string &kernel_name, std::vector<int64_t> *begin,
                                         std::vector<int64_t> *end, std::vector<int64_t> *stride,
                                         ShapeVector *input_shape, bool is_gpu_strided = false);
OPS_KERNEL_COMMON_API void ParseStrideSliceMasks(const std::vector<kernel::KernelTensor *> &inputs,
                                                 std::vector<int64_t> *begin, std::vector<int64_t> *end,
                                                 std::vector<int64_t> *stride, const ShapeVector &input_shape);

// ===========================Old interface==========================================================
OPS_KERNEL_COMMON_API void FillEmptyDims(const BaseOperatorPtr &base_operator, std::vector<int64_t> *begin,
                                         std::vector<int64_t> *end, std::vector<int64_t> *stride,
                                         ShapeVector *input_shape, bool is_gpu_strided = false);
...
OPS_KERNEL_COMMON_API void CheckSliceValid(const std::vector<int64_t> &start, const std::vector<int64_t> &stop,
                                           const std::vector<int64_t> &step, const std::vector<int64_t> &input_shape);
OPS_KERNEL_COMMON_API size_t CalOffset(const std::vector<int64_t> &start, const std::vector<int64_t> &stop,
                                       const std::vector<int64_t> &dim_offset);
OPS_KERNEL_COMMON_API std::vector<int64_t> CalDimOffset(const std::vector<int64_t> &input_shape);
OPS_KERNEL_COMMON_API size_t GetCopySize(const std::vector<int64_t> &dim_offset, const std::vector<int64_t> &start,
                                         const std::vector<int64_t> &stop);

OPS_KERNEL_COMMON_API std::pair<MatrixDiag::Alignment, MatrixDiag::Alignment> GetAlignments(
  const std::string &alignment);

namespace broadcast_utils {
OPS_KERNEL_COMMON_API bool AlignedBroadCastShape(size_t align_rank, std::vector<size_t> *broadcast,
