...
class TechnicalChoice(BaseModel):
    ...
    name: str = Field(..., description="Name of the choice (e.g., 'react', 'postgresql')")
...
class DependencyChoice(TechnicalChoice):
    ...
    choice_type: ChoiceType = Field(default=ChoiceType.DEPENDENCY, description="Always dependency")
    package_name: str = Field(..., description="Package/library name")
    version_constraint: Optional[str] = Field(None, description="Version constraint (e.g., '^1.0.0')")
    ...
    replaces: Optional[List[str]] = Field(None, description="Dependencies this replaces")
...
class FrameworkChoice(TechnicalChoice):
    ...
    choice_type: ChoiceType = Field(default=ChoiceType.FRAMEWORK, description="Always framework")
    ...
    migration_required: bool = Field(
        False, 
        description="Whether migration from existing solution is needed"
    )
    current_solution: Optional[str] = Field(
        None, 
        description="What this framework replaces"
    )
...
def create_technical_choice(
    choice_type: Union[str, ChoiceType],
    name: str,
    context: str,
    **kwargs
) -> TechnicalChoice:
