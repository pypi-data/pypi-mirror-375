...
def simple_embeddings_test():
    ...
    try:
        ...
        if response.embeddings and len(response.embeddings) > 0:
            ...
            print("⚠️  No embeddings found in response")

        # Show token usage - Google AI SDK limitation for embeddings
        print("⚠️  Token usage: Not available (Google AI SDK limitation)")
        print(
...
def simple_vertex_ai_embeddings_test():
    ...
    if not revenium_key:
        print("⚠️  REVENIUM_METERING_API_KEY not found - metering will fail")
        ...
        if embeddings and len(embeddings) > 0:
            ...
            if hasattr(embeddings[0], "statistics") and embeddings[0].statistics:
                ...
                if hasattr(stats, "token_count"):
                    ...
                    print("⚠️  Token count not available in statistics")
            else:
                print("⚠️  No statistics available in embedding response")
        else:
            print("⚠️  No embeddings found in response")
...
def main():
    ...
    if provider == "google-ai":
        ...
        print("☁️  VERTEX AI SDK EMBEDDINGS TEST")
        print("=" * 70)
