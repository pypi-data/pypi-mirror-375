...
class TrendsAGIClient:
    ...
    # --- MODIFIED METHOD: Replaced deep_analysis with async task pattern ---
    def queue_deep_analysis(self, query: Optional[str] = None, trend_id: Optional[int] = None) -> models.InsightTaskResponse:
        """
        Queues a deep analysis job for a given query or trend ID.
        Returns a task response object containing the task_id to poll.
        """
        if not query and not trend_id:
            raise ValueError("Either 'query' or 'trend_id' must be provided.")
        payload = {"query": query, "trend_id": trend_id}
        payload = {k: v for k, v in payload.items() if v is not None}
        response_data = self._request('POST', '/api/intelligence/deep-analysis/generate', json=payload)
        return models.InsightTaskResponse.model_validate(response_data)

    # --- NEW METHOD ---
    def get_deep_analysis_status(self, task_id: str) -> models.InsightTaskStatusResponse:
        """
        Checks the status and result of a deep analysis task.
        If successful, the result will contain the full DeepAnalysis object.
        """
        response_data = self._request('GET', f'/api/intelligence/deep-analysis/status/{task_id}')
        return models.InsightTaskStatusResponse.model_validate(response_data)
    ...
    # --- MODIFIED METHOD: Added 'selected_fields' parameter to fix bug ---
    ...
    async def _connect_websocket(self, endpoint: str) -> AsyncGenerator[str, None]:
        ...
        headers = {"X-API-Key": self._session.headers["X-API-Key"]}
