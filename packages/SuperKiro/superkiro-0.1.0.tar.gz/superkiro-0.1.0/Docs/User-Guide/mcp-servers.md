...
#sk_implement "React authentication system"
# → Provides official React patterns

# Manual activation  
#sk_analyze auth-system/ --c7
```
...
```bash
# Automatic activation
#sk_troubleshoot "API performance issues"
# → Enables systematic root cause analysis

# Manual activation
#sk_analyze --think-hard architecture/
```
...
```bash
# Automatic activation
#sk_implement "responsive dashboard component"
...
```bash
# Automatic activation
#sk_test --type e2e "user login flow"
# → Enables browser automation testing

# Manual activation
#sk_test "accessibility compliance" --play
```
...
```bash
# Automatic activation
#sk_improve legacy-codebase/ --focus maintainability
