...
class AbCls:
	...
	def set(self, ab: enums.AdLogic, evnt=repcap.Evnt.Default, logic=repcap.Logic.Default) -> None:
		"""TRIGger:EVENt<*>:PATTern:QUALify:ANALog:LOGic<*>:AB \n
		Snippet: driver.trigger.event.pattern.qualify.analog.logic.ab.set(ab = enums.AdLogic.AND, evnt = repcap.Evnt.Default, logic = repcap.Logic.Default) \n
		...
		self._core.io.write(f'TRIGger:EVENt{evnt_cmd_val}:PATTern:QUALify:ANALog:LOGic{logic_cmd_val}:AB {param}')

	# noinspection PyTypeChecker
	def get(self, evnt=repcap.Evnt.Default, logic=repcap.Logic.Default) -> enums.AdLogic:
		"""TRIGger:EVENt<*>:PATTern:QUALify:ANALog:LOGic<*>:AB \n
		Snippet: value: enums.AdLogic = driver.trigger.event.pattern.qualify.analog.logic.ab.get(evnt = repcap.Evnt.Default, logic = repcap.Logic.Default) \n
		No command help available \n
			:param evnt: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Event')
			:param logic: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Logic')
		...
		evnt_cmd_val = self._cmd_group.get_repcap_cmd_value(evnt, repcap.Evnt)
		logic_cmd_val = self._cmd_group.get_repcap_cmd_value(logic, repcap.Logic)
		response = self._core.io.query_str(f'TRIGger:EVENt{evnt_cmd_val}:PATTern:QUALify:ANALog:LOGic{logic_cmd_val}:AB?')
