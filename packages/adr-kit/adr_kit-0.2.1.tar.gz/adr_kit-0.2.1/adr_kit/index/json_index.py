...
class IndexEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "id": fm.id,
            "title": fm.title,
            "status": fm.status.value if isinstance(fm.status, ADRStatus) else str(fm.status),
            "date": date_str,
...
class ADRIndex:
    ...
    def filter_by_status(self, status: Union[ADRStatus, str, List[Union[ADRStatus, str]]]) -> List[IndexEntry]:
    ...
    def filter_by_tags(self, tags: Union[str, List[str]], match_all: bool = False) -> List[IndexEntry]:
...
def generate_adr_index(adr_directory: Union[Path, str] = "docs/adr",
                      output_path: Union[Path, str] = "docs/adr/adr-index.json",
                      validate: bool = True,
                      indent: int = 2) -> ADRIndex:
