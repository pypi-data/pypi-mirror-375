...
class IoiPathWb:
    ...
    @classmethod
    def read_wb_to_aod(
            cls, io: TyPlFileSrcArr, sheet: TnPlSheets, **kwargs) -> TnAoD:
        _aod: TyAoD = PlDf.to_aod(cls.read_wb_to_df(io, sheet, **kwargs))
        return _aod

    @classmethod
    def read_wb_to_aod_or_doaod(
            cls, io: TyPlFileSrcArr, sheet: TnPlSheets, **kwargs) -> TnAoD_DoAoD:
    ...
    def read_wb_to_doaod(
            cls, io: TyPlFileSrcArr, sheet: TnPlSheets, **kwargs) -> TnDoAoD:
    ...
    @classmethod
    def read_wb_to_df(
            cls, io: TyPlFileSrcArr, sheet: TnPlSheets, **kwargs) -> TnPlDf:
        Io.verify(io)
    ...
    def read_wb_to_df_or_dodf(
            cls, io: TyPlFileSrcArr, sheet: TnPlSheets, **kwargs) -> TnPlDf_DoPlDf:
        Io.verify(io)
        if isinstance(sheet, str):
    ...
    @classmethod
    def read_wb_to_dodf(
            cls, io: TyPlFileSrcArr, sheet: TnPlSheets, **kwargs) -> TnDoPlDf:
        Io.verify(io)
        if isinstance(sheet, str):
            _obj: TnPlDf_DoPlDf = pl.read_excel(
                    io, sheet_id=None, sheet_name=[sheet], **kwargs)
    ...
    def sh_obj_for_iterator(
            cls, io: TyPlFileSrcArr, sheet: Iterator, **kwargs) -> TnPlDf_DoPlDf:
