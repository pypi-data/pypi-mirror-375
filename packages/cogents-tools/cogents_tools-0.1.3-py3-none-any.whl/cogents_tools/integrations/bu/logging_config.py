    ...
    class BrowserUseFormatter(logging.Formatter):
        ...
        def format(self, record):
            ...
            if (
                ...
                and record.name.startswith("cogents_tools.integrations.bu.")
            ):
                ...
                if "Agent" in record.name:
                    ...
                    record.name = "dom"
                elif record.name.startswith("cogents_tools.integrations.bu."):
...
def setup_log_pipes(session_id: str, base_dir: str | None = None):
    ...
    for name in ["cogents_tools.integrations.bu.agent", "cogents_tools.integrations.bu.tools"]:
    ...
    for name in ["bubus", "cogents_tools.integrations.bu.browser.session"]:
        logger = logging.getLogger(name)
