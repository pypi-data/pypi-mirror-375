...
class TestDuplicateDetector:
    ...
    def _create_mock_repo(self, prs: list[Any]) -> Any:
        ...
        repo.get_pull.side_effect = lambda num: next(pr for pr in prs if pr.number == num)
        return repo
...
class TestCheckForDuplicatesFunction:
    ...
    def _create_mock_github_context(self, pr_number: int | None = 123) -> GitHubContext:
    ...
    def test_check_for_duplicates_success(self, mock_get_repo: Any, mock_build_client: Any) -> None:
    ...
    @patch("github2gerrit.duplicate_detection.build_client")
    @patch("github2gerrit.duplicate_detection.get_repo_from_env")
    def test_check_for_duplicates_no_pr_number(self, mock_get_repo: Any, mock_build_client: Any) -> None:
    ...
    @patch("github2gerrit.duplicate_detection.build_client")
    @patch("github2gerrit.duplicate_detection.get_repo_from_env")
    def test_check_for_duplicates_api_failure_doesnt_crash(self, mock_get_repo: Any, mock_build_client: Any) -> None:
...
class TestDependabotScenarios:
    ...
    def test_identical_dependabot_prs(self) -> None:
        ...
        fp1 = ChangeFingerprint("Bump lfit/gerrit-review-action from 0.6 to 0.8")
        fp2 = ChangeFingerprint("Bump lfit/gerrit-review-action from 0.6 to 0.8")
        fp3 = ChangeFingerprint("Bump lfit/gerrit-review-action from 0.6 to 0.8")
    ...
    def test_different_dependabot_versions(self) -> None:
        ...
        fp1 = ChangeFingerprint("Bump lfit/gerrit-review-action from 0.6 to 0.7")
        fp2 = ChangeFingerprint("Bump lfit/gerrit-review-action from 0.6 to 0.8")
        fp3 = ChangeFingerprint("Bump lfit/gerrit-review-action from 0.7 to 0.8")
...
class TestGerritDuplicateDetection:
    ...
    def _create_mock_github_context(self, pr_number: int = 123, repository: str = "org/repo") -> GitHubContext:
        """Create a mock GitHub context."""
        return GitHubContext(
            event_name="pull_request",
            event_action="opened",
    ...
    def test_resolve_gerrit_info_from_local_gitreview(self, mock_read_text: Any, mock_exists: Any) -> None:
...
"""
    ...
    @patch("pathlib.Path.exists")
    def test_resolve_gerrit_info_from_remote_gitreview(self, mock_exists: Any, mock_urlopen: Any) -> None:
...
"""
    ...
    @patch("urllib.request.urlopen")
    def test_check_for_duplicates_with_gerrit_duplicate(self, mock_urlopen: Any) -> None:
        ...
        mock_response = Mock()
        mock_response.read.return_value = json.dumps(gerrit_response).encode()
        ...
        with patch.dict(
            ...
            with pytest.raises(DuplicateChangeError) as exc_info:
                detector.check_for_duplicates(target_pr, allow_duplicates=False, gh=gh)

            assert "subject matches existing Gerrit change(s)" in str(exc_info.value)

    @patch("urllib.request.urlopen")
    def test_check_for_duplicates_with_gerrit_duplicate_allowed(self, mock_urlopen: Any) -> None:
        ...
        with patch.dict(
            ...
            detector.check_for_duplicates(target_pr, allow_duplicates=True, gh=gh)
