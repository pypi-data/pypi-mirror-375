...
class TrackingServerResult:
    ...
    class Result(Enum):
        ...
        def translate_from_rpc(rpc_enum_value):
            ...
            if (
                rpc_enum_value
                == tracking_server_pb2.TrackingServerResult.RESULT_UNKNOWN
            ):
                return TrackingServerResult.Result.UNKNOWN
            if (
                rpc_enum_value
                == tracking_server_pb2.TrackingServerResult.RESULT_SUCCESS
            ):
                return TrackingServerResult.Result.SUCCESS
            if (
                rpc_enum_value
                == tracking_server_pb2.TrackingServerResult.RESULT_NO_SYSTEM
            ):
                return TrackingServerResult.Result.NO_SYSTEM
            if (
                rpc_enum_value
                == tracking_server_pb2.TrackingServerResult.RESULT_CONNECTION_ERROR
            ):
    ...
    def translate_from_rpc(rpcTrackingServerResult):
        ...
        return TrackingServerResult(
            TrackingServerResult.Result.translate_from_rpc(
                rpcTrackingServerResult.result
            ),
...
class TrackingServer(AsyncBase):
    ...
    async def respond_tracking_point_command(self, command_answer):
        ...
        if result.result != TrackingServerResult.Result.SUCCESS:
            raise TrackingServerError(
                result, "respond_tracking_point_command()", command_answer
            )
    ...
    async def respond_tracking_off_command(self, command_answer):
        ...
        if result.result != TrackingServerResult.Result.SUCCESS:
            raise TrackingServerError(
                result, "respond_tracking_off_command()", command_answer
            )
