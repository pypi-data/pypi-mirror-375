                                ...
                                if is_json_passthrough(obj):
                                    ...
                                    if isinstance(value, list) and value:
                                        ...
                                        if list_item_type and hasattr(list_item_type, "__fraiseql_definition__"):
                                            result = []
                                            for item in value:
                                                # JSONPassthrough wraps dict items in more JSONPassthrough objects
                                                # but we need actual typed objects for GraphQL
                                                if is_json_passthrough(item):
                                                    # Extract the underlying dict data from JSONPassthrough
                                                    if hasattr(item, '_data'):
                                                        raw_data = item._data
                                                        # Convert dict to typed object
                                                        if hasattr(list_item_type, "from_dict"):
                                                            result.append(list_item_type.from_dict(raw_data))
                                                        else:
