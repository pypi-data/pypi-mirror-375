...
class RailtownHandler(logging.Handler):
            ...
            payload = [
                {
                    ...
                    "UserProperties": {
                        ...
                        "ClientVersion": "Python-2.0.4",  # VERSION Must match __version__ in __init__.py
                        "Encoding": "utf-8",
    ...
    def _validate_session_format(self, data: dict[str, Any]) -> tuple[bool, list[str]]:
        ...
        """
        required_session_fields = ["session_id", "name", "start_time", "end_time", "runs"]
        ...
        required_run_fields = ["run_id", "name", "start_time", "end_time", "status"]
    ...
    def _enrich_run_with_session_data(self, session_data: dict[str, Any], run_data: dict[str, Any]) -> dict[str, Any]:
        ...
        enriched_data["session_name"] = session_data["name"]  # Session name (stable)
        enriched_data["session_start_time"] = session_data["start_time"]
        enriched_data["session_end_time"] = session_data["end_time"]

        # Add run metadata with renamed fields
        enriched_data["run_id"] = run_data["run_id"]
        enriched_data["run_name"] = run_data["name"]  # Run name
        enriched_data["run_start_time"] = run_data["start_time"]
        enriched_data["run_end_time"] = run_data["end_time"]
        enriched_data["run_status"] = run_data["status"]
