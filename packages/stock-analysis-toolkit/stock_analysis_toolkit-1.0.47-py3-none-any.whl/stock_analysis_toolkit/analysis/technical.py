...
class TechnicalAnalysis:
    ...
    def calculate_all_indicators(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # SMA 20/50 crossover state
            sma20 = indicators.get("sma_20")
            sma50 = indicators.get("sma_50")
            if sma20 is not None and sma50 is not None:
                if sma20 > sma50:
                    indicators["sma_20_50_crossover"] = "bullish"
                elif sma20 < sma50:
                    indicators["sma_20_50_crossover"] = "bearish"
                else:
                    indicators["sma_20_50_crossover"] = "neutral"
            else:
                indicators["sma_20_50_crossover"] = None
            ...
            # Weekly (approx 5 trading days) and Monthly (approx 21 trading days) returns
            try:
                if len(self.data) > 5 and not self.data["Close"].isna().any():
                    prev_5 = float(self.data["Close"].iloc[-6])
                    if prev_5 > 0:
                        indicators["weekly_return"] = round(((indicators["close"] - prev_5) / prev_5) * 100, 2)
                if len(self.data) > 21 and not self.data["Close"].isna().any():
                    prev_21 = float(self.data["Close"].iloc[-22])
                    if prev_21 > 0:
                        indicators["monthly_return"] = round(((indicators["close"] - prev_21) / prev_21) * 100, 2)
            except Exception as e:
                logger.debug(f"Weekly/Monthly return calc failed: {e}")
