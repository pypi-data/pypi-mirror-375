...
from mltable._utils import _validate
...
class TestSchema:
    ...
    def _check_mltable_passes_schema(self, mltable):
        with tempfile.TemporaryDirectory() as td:
            ...
            mltable.save(td, colocated=False, if_err_remove_files=False)
    ...
    def test_query_source(self, get_query_source):
        # load with a dummy query source
        # only care about validation so not using MLTable.load to trigger a remote call
        mltable_file_path = os.path.join(get_query_source, 'MLTable')
        with open(mltable_file_path, 'r') as f:
            mltable_yaml = yaml.safe_load(f)
        _validate(mltable_yaml)
