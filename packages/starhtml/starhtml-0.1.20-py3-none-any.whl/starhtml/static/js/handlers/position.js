...
async function computeFloatingPosition(reference, floating, config, detector) {
  const padding = 10;
  ...
  if (parentPopover) {
    const isHorizontal = config.placement.startsWith("right") || config.placement.startsWith("left");
    if (isHorizontal) {
      const parentRect = parentPopover.getBoundingClientRect();
      const refRect = reference.getBoundingClientRect();
      const distanceToEdge = config.placement.startsWith("right") ? parentRect.right - refRect.right : refRect.left - parentRect.left;
...
var position_default = {
  ...
  onLoad({ el, value, mods, startBatch, endBatch }) {
    ...
    const getTargetElement = () => {
      ...
      const isVertical = config.placement.startsWith("top") || config.placement.startsWith("bottom");
