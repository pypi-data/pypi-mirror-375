...
def read_GdfidL(files, component_type='long', divide_by=None, imp=False):
    """
    Read GdfidL text file format into an Impedance or WakeFunction object.

    Parameters
    ----------
    files : str or list of two str
        Path to the text file to read.
        Single str for wake date and list of str for impedance in the format: [ReZ, ImZ].
    component_type : str, optional
        Type of the Impedance or WakeFunction object to load.
        Default is 'long'.
    divide_by : float, optional
    ...
    """
    if imp:
        df_re = pd.read_csv(files[0],
                            skiprows=11,
                            sep="\s+",
                            names=["Frequency", "Real"])
        df_im = pd.read_csv(files[1],
                            skiprows=11,
                            sep="\s+",
                            names=["Frequency", "Imaginary"])

        if divide_by is not None:
            df_re["Real"] = df_re["Real"] / divide_by
            df_im["Imaginary"] = df_im["Imaginary"] / divide_by

        result = Impedance(variable=df_re["Frequency"],
                           function=df_re["Real"] + 1j * df_im["Imaginary"],
                           component_type=component_type)
    else:
