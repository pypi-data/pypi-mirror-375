# Purpose: Demonstrate API usage for a halfcat style liquid
...
file_prefix = 'nitrous_ethanol'

"""
for gui: button to get new liquid vs new hybrid (block if unsaved)
"""

print('Building combustion chemistry table...')
comb = chem.ChemSolver(hrap_root/'thermo.dat')
...
ox, fu = 'N2O(L),298.15K', 'C2H5OH(L),90%'
internal_state = None
for j, OF in enumerate(chem_OF):
    for i, Pc in enumerate(chem_Pc):
        ...
        flame, internal_state = comb.solve(Pc, {ox: o, fu: 1-o}, max_iters=150, internal_state=internal_state)
