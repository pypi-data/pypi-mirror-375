...
def check_initialization_status(command_id: str, region: str = DEFAULT_REGION) -> str:
    ...
    try:
        ...
        if response['Status'] == 'InProgress' and start_time:
            try:
                ...
                if estimation_data and estimation_data.get('estimated_minutes', 0) > 0:
                    ...
                    logger.info(f"Debug - elapsed_minutes: {elapsed_minutes}, estimated_total_minutes: {estimated_total_minutes}")
        ...
        result = {
            ...
            "debug_comment": comment_debug,  # 디버깅용
        }
        ...
        if response['Status'] == 'Success':
            ...
            if progress_info and 'progress_bar' in progress_info:
                progress_message = f"\n🔄 Volume initialization in progress\n\n{progress_info['progress_bar']}"
                if progress_info.get('estimated_remaining_minutes', 0) > 0:
                    remaining = progress_info['estimated_remaining_minutes']
                    progress_message += f"\n⏱️  Estimated remaining time: {remaining} minutes"
                if progress_info.get('elapsed_minutes', 0) > 0:
                    elapsed = progress_info['elapsed_minutes']
                    progress_message += f" (elapsed: {elapsed} minutes)"
        ...
        # For InProgress status, also add a formatted summary at the end
        json_result = json.dumps(result, indent=2, ensure_ascii=False)
        
        # Add visual progress display for AI agent
        if response['Status'] == 'InProgress' and progress_info and 'progress_bar' in progress_info:
            visual_progress = f"""

=== EBS Volume Initialization Progress ===
{progress_info['progress_bar']}
Elapsed: {progress_info.get('elapsed_minutes', 0)} minutes"""
            
            if progress_info.get('estimated_remaining_minutes', 0) > 0:
                visual_progress += f" | Remaining: ~{progress_info['estimated_remaining_minutes']} minutes"
            
            if progress_info.get('volumes_count', 0) > 0:
                visual_progress += f"\nVolumes: {progress_info['volumes_count']} | Total: {progress_info.get('total_gb', 0)}GB | Method: {progress_info.get('method', 'Unknown')}"
            
            visual_progress += "\n" + "=" * 45
            
            return json_result + visual_progress
