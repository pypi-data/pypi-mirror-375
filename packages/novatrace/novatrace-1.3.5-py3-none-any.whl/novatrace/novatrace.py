...
def _show_docker_build_progress(process, estimated_duration=300):
    """
    Show a progress bar for Docker build process with real-time parsing
    
    Args:
        process: The subprocess.Popen object for the Docker build
        estimated_duration: Estimated build time in seconds (default 5 minutes)
    """
    print("🔨 Building Docker image...")
    print("📦 This process can take ~5 minutes the first time")
    
    # Docker build stages - approximate percentages
    stages = [
        (5, ""),
        (15, "⬇️  Pulling base images..."),
        (30, "📦 Installing system packages..."),
        (45, "� Installing Node.js dependencies..."),
        (65, "⚛️  Building React application..."),
        (80, "📁 Copying application files..."),
        (90, "🏗️  Finalizing image layers..."),
        (95, "🏷️  Tagging image..."),
        (100, "✅ Build complete!")
    ]
    ...
    # Keywords that indicate build progress
    progress_keywords = {
        'FROM': 10,
        'RUN apt-get': 20,
        'RUN npm install': 40,
        'RUN npm run build': 60,
        'COPY': 75,
        'EXPOSE': 85,
        'Successfully tagged': 95
    }
    ...
    try:
        while process.poll() is None:
            ...
            # Try to read output for smarter progress detection
            try:
                if process.stdout and process.stdout.readable():
                    line = process.stdout.readline()
                    if line:
                        line = line.strip()
                        # Check for Docker build keywords
                        for keyword, progress_val in progress_keywords.items():
                            if keyword in line and progress_val > current_progress:
                                current_progress = progress_val
                                break
            except:
                pass  # Continue with time-based progress if parsing fails
            
            # Calculate progress based on time if no keyword progress
            time_progress = min((elapsed / estimated_duration) * 90, 90)
            progress = max(current_progress, time_progress)
            
            # Update stage message
            message = ""
            for i, (stage_progress, stage_message) in enumerate(stages):
                if progress >= stage_progress and i >= stage_idx:
        ...
        print(f"\r   ❌ Build cancelled by user                                   ")
        process.terminate()
        return False
    ...
    if process.returncode == 0:
        bar = '█' * bar_length
        print(f"\r   [{bar}] 100.0% - ✅ Build complete!                    ")
        return True
    else:
        print(f"\r   ❌ Build failed (exit code: {process.returncode})                     ")
        return False
    ...
    def __init__(self, session_name: str = "default", engine_url: str = None, time_zone: pytz.tzinfo = pytz.utc, interface: bool = True, debug: bool = False):
        ...
        try:
            ...
            if self._is_interface_owner:
                ...
                init_database(self.engine, debug=self.debug)
        ...
        def run_web_interface():
            try:
                ...
                try:
                    ...
                    print("\n🍎 macOS:")
                    print("   Download Docker Desktop from: https://docs.docker.com/desktop/install/mac-install/")
                    print("\n🪟 Windows:")
                    print("   Download Docker Desktop from: https://docs.docker.com/desktop/install/windows-install/")
                ...
                if self.debug:
                    ...
                    print(f"🔨 Build command: {' '.join(build_cmd)} (This process can take ~5 mins the first time)")
                else:
                    print("🚀 Starting NovaTrace dashboard...")
                ...
                if not image_exists or force_rebuild:
                    ...
                    if self.debug:
                        ...
                        # Show progress bar for non-debug mode
                        ...
                        success = _show_docker_build_progress(build_process)
                    
                    if not success:
                        if self.debug:
                            print("   ❌ Docker build failed")
                        else:
                            print("❌ Docker build failed. Use debug=True for detailed logs.")
                        return
