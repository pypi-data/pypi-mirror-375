...
async def test_auto_exec_disable_returns_pending_on_custom():
    ...
    completion = await run_with_auto_execution(agent, messages, tools, router, mode='disable')
...
async def test_auto_exec_auto_runs_custom_and_agent_to_final():
    ...
    completion = await run_with_auto_execution(agent, messages, tools, router, mode='auto')
    assert isinstance(completion, dict) and completion.get('object') == 'chat.completion'
    choice = (completion.get('choices') or [{}])[0] or {}
    message = choice.get('message') or {}
...
async def test_auto_exec_agent_mode_auto_runs_agent_only():
    ...
    completion = await run_with_auto_execution(agent, messages, tools, router, mode='agent')
...
async def test_auto_exec_tool_mode_auto_runs_custom_only():
    ...
    completion = await run_with_auto_execution(agent, messages, tools, router, mode='tool')
