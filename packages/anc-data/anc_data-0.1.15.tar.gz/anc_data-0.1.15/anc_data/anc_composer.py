...
class SeqSplitConfig:
    ...
    def __init__(
        self, allow_split=False, split_threshold=None, freeze_batch_interval=None
    ):
...
class AncComposerBuffer:
    ...
    def flush(self):
        ...
        """
        buffer = [
            self.buffer[i] for i in range(len(self.buffer)) if i not in self.delete_idx
        ]
        self.buffer = buffer
        self.delete_idx = set()
...
class AncComposer:
    ...
    def __init__(
        ...
        self.allow_split = (
            seq_split_config.allow_split if seq_split_config is not None else False
        )
        freeze_batch_interval = (
            seq_split_config.freeze_batch_interval
            if seq_split_config is not None
            else None
        )
        if self.allow_split:
            assert (
                split_fn is not None
            ), "Need to provide split_fn to enable allow_split"
        ...
        self.split_threshold = (
            seq_split_config.split_threshold
            if seq_split_config is not None
            else max_seq_len
        )
    ...
    def get_items(self, is_last_call=False, ignore_split_freezing=False):
        ...
        Args:
            ...
            ignore_split_freezing: Whether to bypass freeze mechanism for splits
        Returns:
            list: List of items in the micro batch, the sum of the token length
            of the items should be equal to or less than max_seq_len
        """
        items = []
        if is_last_call:
    ...
    def apply(self, sample, is_last_call=False):
        ...
        """
        if (
            self.buffer._get_token_length(sample) <= self.max_seq_len
            or self.allow_split
        ):
            total_token_length = self.buffer.put(sample)
        else:
            # don't put this sample into buffer, it will stay there forever
            logging.warning(
                f"sample token length {self.buffer._get_token_length(sample)} exceeded max_seq_len, skipping"
            )
        ...
        while total_token_length >= self.max_seq_len * self.ratio:
            ...
            res.append(items)
            new_total = self.buffer.put(None)
            if new_total >= total_token_length:
                # avoid infinite loop when no item can be fetched from buffer
                break
            total_token_length = new_total
        if is_last_call:
...
if __name__ == "__main__":
    ...
    data_generator = create_fake_data(
        min_token_length, max_seq_length, max_token_length
    )
    micro_batch_count = 0
    batch_doc_id = []
    token_lengths = []
