...
class GraphUtils:
    ...
    @staticmethod
    def safe_get_request_data(request, data_type, default_value=None):
        """
        安全获取并验证请求中的JSON数据

        Args:
            request: Flask请求对象
            data_type: 数据类型，'nodeInfo' 或 'metaData'
            default_value: 解析失败时返回的默认值
        
        Returns:
            dict: 解析并处理后的数据字典，或默认值
        """
        # 定义不同数据类型的必要字段
        required_fields_map = {
            'nodeInfo': ["nodeName", "nodeType"],
            'metaData': ["tag", "run"]
        }
        ...
        # 从请求参数中获取数据
        data_str = request.args.get(data_type)
        if data_str is None:
            logger.error(f"{data_type} parameter is missing in request")
            return default_value

        try:
            data_str = str(data_str)
            # 安全解析JSON
            parsed_data = GraphUtils.safe_json_loads(data_str)

            # 验证必要字段是否存在
            for field in required_fields:
                if field not in parsed_data:
                    logger.error(f"Field {field} is missing in {data_type}.")
                    return default_value
            # 特殊字符过滤/转义
            sanitized = {}
            ...
            logger.error(f"{data_type} parameter is not in valid JSON format.")
            return default_value
        except Exception as e:
            logger.error(f"An error occurred while parsing the {data_type} parameter: {str(e)}")
            return default_value
