"""
Environment variable management for SuperKiro
...
def _get_env_tracking_file() -> Path:
    ...
    install_dir = Path.home() / ".kiro"
    install_dir.mkdir(exist_ok=True)
    return install_dir / "superkiro_env_vars.json"
...
def get_superkiro_environment_variables() -> Dict[str, str]:
    """
    Get environment variables that were set by SuperKiro
    
    Returns:
        Dictionary of environment variable names to their current values
    """
    # Load tracking data to get SuperKiro-managed variables
    tracking_data = _load_env_tracking()
    
    found_vars = {}
    for env_var, metadata in tracking_data.items():
        if metadata.get("set_by") == "superkiro":
            value = os.environ.get(env_var)
            if value:
                found_vars[env_var] = value
    
    # Fallback: check known API key environment variables
...
def _create_restore_script(env_vars: Dict[str, str]) -> Optional[Path]:
    ...
    try:
        home = Path.home()
        if os.name == 'nt':  # Windows
            script_path = home / "restore_superkiro_env.bat"
            with open(script_path, 'w') as f:
                f.write("@echo off\n")
                f.write("REM SuperKiro Environment Variable Restore Script\n")
            ...
            script_path = home / "restore_superkiro_env.sh"
            with open(script_path, 'w') as f:
                f.write("#!/bin/bash\n")
                f.write("# SuperKiro Environment Variable Restore Script\n")
...
def _remove_env_var_from_shell_config(shell_config: Path, env_var: str) -> bool:
    ...
    try:
        ...
        for line in lines:
            ...
            if f'export {env_var}=' in line or line.strip() == f'# SuperKiro API Key':
