...
BANDS_LIST = ["B01", "B02", "B03", "B04", "B05", "B06", "B07", "B08", "B8A", "B09", "B10", "B11", "B12"]
UTM_ZONES = {
            19:31979,
            20:31980,
            21:31981,
...
class RasterProcessing:
    ...
    def create_cloud_mask(self, cropped_bands, output_path=None, threshold=0.4, average_over=2, dilation_size=10):
        ...
        # Máscara baseada na B01 (true se > 2200)
        band_01 = cropped_bands.get('B01')
        if band_01 is not None:
            band_01_cloud_mask = band_01.array > 2200
        else:
            band_01_cloud_mask = None

        # Use resolução de 10m para rodar o s2cloudless
        target_resolution = 10

        # Banda de referência (de preferência B05, senão B02)
        ...
        for band_name in s2cloudless_band_order:
            ...
            if band_info is None:
                ...
                if bands_resolutions[band_name] != target_resolution:
                    ...
                    from skimage.transform import resize
                    band_data = resize(
                        band_data, (target_height, target_width),
        ...
        # Gerar máscara de nuvens (s2cloudless)
        ...
        # Combinar com a máscara da banda B01 (>2200)
        if band_01_cloud_mask is not None:
            band_01_cloud_mask_resized = self.pad_or_crop(
                band_01_cloud_mask.astype(np.uint8),
                (target_height, target_width)
            )
            final_mask = np.logical_or(cloud_mask, band_01_cloud_mask_resized)
        else:
            final_mask = cloud_mask

        # Converter para uint8 (0=sem nuvem, 1=nuvem)
        cloud_mask_binary = final_mask.astype(np.uint8)
        ...
        # Salvar se output_path for informado
        if output_path:
