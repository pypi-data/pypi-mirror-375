...
NETCRAFT_API_REPORT_MAIL_ENV_VAR = 'SECRET_NETCRAFT_REPORT_MAIL'

# Options definition
parser = argparse.ArgumentParser(description="version: " + VERSION)
common_group = parser.add_argument_group('common parameters')
common_group.add_argument('-k', '--api-key', help='API key (could either be provided in the "%s" env var)' % NETCRAFT_API_REPORT_MAIL_ENV_VAR, type = str)
...
def main():
    ...
    if not(api_key):
        if NETCRAFT_API_REPORT_MAIL_ENV_VAR in os.environ:
            api_key = os.environ[NETCRAFT_API_REPORT_MAIL_ENV_VAR]
        else:
