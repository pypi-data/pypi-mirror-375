...
def parse_args():
    ...
    parser.add_argument(
        "--meas-dev", default="manual", help="Type of measurement device that is used"
    )
    parser.add_argument(
        "-o",
        "--meas-option",
    ...
    parser.add_argument(
        "--switch", default="manual", help="Type of switch to be used to connect antennas"
    )
    parser.add_argument(
        "--switch-option", default=[], action="append", help="Options that are passed to the switch"
    )
    parser.add_argument(
        "-r", "--rate", type=float, help="Sampling rate at which the calibration is performed"
    )
    parser.add_argument(
        "--store",
    ...
    parser.add_argument(
        "--load",
        metavar="filename.pickle",
        help="If provided, will load intermediate cal data instead of running a " "measurement.",
    )
...
def sanitize_args(usrp, args, default_rate):
    ...
    if args.channels == "*":
        ...
        for chan in channels:
            assert chan in range(
                available_chans
            ), "ERROR: Invalid channel: {}. Should be in 0..{}.".format(chan, available_chans)
    ...
    if args.antenna == "*":
        ...
        for ant in antennas:
            assert ant in available_ants, "Invalid antenna: {}. Should be in {}.".format(
                ant, available_ants
            )
    ...
    print(
        "=== Requested sampling rate: {} Msps, actual rate: {} Msps".format(
            rate / 1e6, actual_rate / 1e6
        )
    )
    if args.dir == "tx" and abs(args.tone_freq) > actual_rate:
        raise ValueError(
            "The TX tone frequency offset of {} kHz is greater than the sampling rate.".format(
                args.tone_freq / 1e3
            )
        )
...
class CalRunner:
    ...
    def __init__(self, usrp, usrp_cal, meas_dev, args):
        ...
        if self.lo_offset:
            print("=== Using USRP LO offset: {:.2f} MHz".format(self.lo_offset / 1e6))
    ...
    def run(self, chan, freq):
        ...
        if abs(actual_freq - freq) > 1.0:
            print(
                "WARNING: Frequency was coerced from {:.2f} MHz to {:.2f} MHz!".format(
                    freq / 1e6, actual_freq / 1e6
                )
            )
    ...
    for chan in channels:
        ...
        for ant in antennas:
            if ant in results[chan]:
                print("=== Using pickled data for channel {}, antenna {}.".format(chan, ant))
                continue
            print("=== Running calibration for channel {}, antenna {}.".format(chan, ant))
            ...
            for freq in freqs:
                try:
                    ...
                    print("ERROR: Stopping calibration due to exception: {}".format(str(ex)))
                    usrp_cal.stop()
                    return 1
