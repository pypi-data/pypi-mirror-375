...
def _get_data_asset_by_asset_uri(asset_uri_match, storage_options=None):
    ...
    if storage_options and STORAGE_OPTION_KEY_AZUREML_CREDENTIAL in storage_options:
        credential = storage_options.get(STORAGE_OPTION_KEY_AZUREML_CREDENTIAL)
    else:
        try:
            ...
            _LoggerFactory.trace(_get_logger(),
                                 f"failed to set auth with error: {error_message}, check if update to latest version")
