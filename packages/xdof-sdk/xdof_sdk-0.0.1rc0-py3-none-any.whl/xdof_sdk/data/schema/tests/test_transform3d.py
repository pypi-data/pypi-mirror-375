...
def test_cycle_consistency():
    ...
    T_inv = Transform3D.from_transform_matrix(np.linalg.inv(T.to_transform_matrix()))  # type: ignore
    I1 = (T @ T_inv).to_transform_matrix()
    I2 = (T_inv @ T).to_transform_matrix()
...
def test_translation_only():
    ...
    assert np.allclose((T.to_transform_matrix())[:3, :3], np.eye(3), atol=TOL)
...
def test_rotation_only():
    ...
    assert np.allclose((T.to_transform_matrix())[:3, :3], expected_R, atol=TOL)
...
def test_rotation_and_translation():
    ...
    M_expected = T1.to_transform_matrix() @ T2.to_transform_matrix()
    assert np.allclose((T1 @ T2).to_transform_matrix(), M_expected, atol=TOL)
