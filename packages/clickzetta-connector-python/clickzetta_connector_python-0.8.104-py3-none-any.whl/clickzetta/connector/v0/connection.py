...
from clickzetta.connector.v0 import _dbapi_helpers, cursor, import_ingestion_api, import_bulkload_v2_api
...
class Connection(object):
    ...
    def create_bulkload_v2_stream(self, schema_name, table_name, options) :
        return import_bulkload_v2_api().create_stream(self._client, schema_name, table_name, options)

    def get_bulkload_v2_stream(
            self, schema: str = None, table: str = None
    ):
        return import_bulkload_v2_api().get_stream(
            self._client, schema=schema, table=table
        )
