...
# ---- 将dataframe按列追加写入excel数据表中，若数据表为空则从在当前数据块的右侧列插入新的dataframe
# 此函数不支持写入index，因此在写入dataframe数据前，需调用dataframe.reset_index()函数将索引列变成普通列
# df: 要插入的dataframe数据
# nextToMaxCol: 指定在当前文件最大列的右侧第几列开始插入新dataframe，设置为1则为在紧邻的下一个列插入，设置为2则会相隔一列插入
def append_excel_by_column(filepath,sheetname,df,nextToMaxCol):
    try:
        # ===== 第一步：加载 Excel，找到第一个 DataFrame 的最右列 =====
        # 打开文件，若文件不存在则创建并打开
        exist_data = True
        ...
        if exist_data:
            # 获取第一个 DataFrame 占用的最大列号（表头所在行）
            max_col_df1 = ws.max_column  # 表示当前已占用的列数
            start_col_df2 = max_col_df1 + nextToMaxCol  # 第二个 DataFrame 从现有最大列的右侧第nextToMaxCol列开始
        else:
            start_col_df2 = 1

        # ===== 第三步：将第二个 DataFrame 写入指定列 =====
        # 写表头
        for col_idx, col_name in enumerate(df.columns, start=start_col_df2):
            ws.cell(row=1, column=col_idx, value=col_name)

        # 写数据
        for row_idx, row in enumerate(df.itertuples(index=False), start=2):
            for col_idx, value in enumerate(row, start=start_col_df2):
                ws.cell(row=row_idx, column=col_idx, value=value)

        # 保存文件
        wb.save(filepath)
        print(f"DataFrame已追加列写入Excel{filepath}")

    except Exception as e:
        print(f"An error occurred in function append_excel_by_column(): {e}")
        exit(1)
...
"""
滑动窗口生成器，每次返回一个窗口内的 DataFrame 切片。

参数：
    df (pd.DataFrame): 输入的 DataFrame
    window_size (int): 窗口大小（行数）
    step (int): 每次滑动的步长，默认 1

返回：
    生成器，每次产出 (start_index, end_index, 窗口DataFrame)
"""
def sliding_window(df, window_size, step=1):
    """
    滑动窗口生成器，每次返回一个窗口内的 DataFrame 切片。
    ...
    if window_size > len(df):
        raise ValueError("窗口大小不能大于 DataFrame 总行数")
...
参数：
    annual_rate: 年化收益率（小数，如 0.03 表示 3%）
    N: 一年天数（默认252为交易日，365为自然日）
    method: 'compound' 表示复利，'simple' 表示单利

返回：
    daily_rate: 日收益率（小数）
"""
def annual_to_daily(annual_rate, N=252, method='compound'):
    """
    将年化收益率转换为日收益率
    ...
    if method == 'compound':
        # 复利法
        daily_rate = (1 + annual_rate) ** (1 / N) - 1
    elif method == 'simple':
        # 单利法
        daily_rate = annual_rate / N
    else:
        raise ValueError("method 参数必须是 'compound' 或 'simple'")
