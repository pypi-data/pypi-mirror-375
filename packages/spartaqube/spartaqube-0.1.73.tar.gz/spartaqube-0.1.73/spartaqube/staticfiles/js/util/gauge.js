...
// Modified by BMY (to add datalabels)
(function() {
      ...
      Gauge.prototype.render = function() {
        ...
        if (this.options.staticZones) {
          ...
          for (j = 0, len = ref.length; j < len; j++) {
            ...
            if(this.displayedValue >= zone.min && this.displayedValue <= zone.max){
              colorZoneGaugeValue = zone.strokeStyle;
            }
            
            // *********************************************************************************************************
            // BMY ADD DATALABELS WITHIN ZONE (BMY 12/05/2024)
            var dataLabels = this.options.dataLabels;
            if(dataLabels){
              if(Object.keys(dataLabels).includes('displayDatalabels')){
                if(dataLabels["displayDatalabels"]){
                  var labelsArr = dataLabels['labels'];
                  if(labelsArr){
                    if(labelsArr.length-1 == len){
                      var angle = this.getAngle((min+max)/2);
                      var radiusLabel = radius
                      var labelText = labelsArr[j]
                      var labelWidth = this.ctx.measureText(labelText).width;
                      var scaleMutateHeight = 0;
                      if (zone.height) {
                        scaleMutateHeight = (this.lineWidth / 2) * (zone.offset || 1 - zone.height);
                        radiusLabel = radiusLabel + scaleMutateHeight
                      }
                      var startX = Math.round(radiusLabel * Math.cos(angle))-labelWidth/2;
                      var startY = Math.round(radiusLabel * Math.sin(angle));
                      // console.log("FONT GAUGE")
                      // console.log(dataLabels['sizeDatalabels']+"px "+dataLabels['familyDatalabels'])
                      this.ctx.font = dataLabels['sizeDatalabels']+"px "+dataLabels['familyDatalabels'];
                      this.ctx.fillStyle = dataLabels['colorDatalabels'];
                      this.ctx.fillText(labelText, startX, startY);
                    }
                  }
                }
              }
          ...
          // ***********************************************************************************************************
          // GAUGE TYPE 2 (BMY 12/05/2024)
          if(Object.keys(this.options).includes('gaugeType')){
            if (this.options['gaugeType'] == 2){
              var dynamicColorGaugeValue = colorZoneGaugeValue;
              this.ctx.strokeStyle = dynamicColorGaugeValue;
              this.ctx.beginPath();
              var min = this.minValue;
              var max = this.displayedValue
              // var lineWidthArcValue = this.canvas.height * this.options['lineWidthArcValue']
              // this.ctx.arc(0, 0, radius - this.lineWidth, this.getAngle(min), this.getAngle(max), false);
              // var r = radius - lineWidthArcValue
              var R = radius - this.lineWidth - 10
              var startAngle = this.getAngle(min)
              var endAngle = this.getAngle(max)
              ...
              if(R > 0){
                ...
                for (var pp=1; pp < 5; pp++){
                  this.ctx.beginPath();
                  var rTmp = R - pp*(this.lineWidth - 5);
                  if(rTmp > 0){
                    this.ctx.arc(0, 0, rTmp, startAngle, endAngle, false);
                    this.ctx.stroke();
                  }
                }
              }
