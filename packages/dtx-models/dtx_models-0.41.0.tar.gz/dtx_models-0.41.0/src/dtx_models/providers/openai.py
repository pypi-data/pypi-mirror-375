...
class BaseProviderConfig(BaseModel):
    ...
    def set_env_var_values(self, env_values: Dict[str, str]):
        """Set multiple environment variable values at once (case-insensitive keys)."""
        canonical_map = {k.lower(): v for k, v in self.env_vars.items()}
        for key, value in env_values.items():
            attr = canonical_map.get(key.lower())
            if attr is not None:
