...
def layout():
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.P("通过花朵图可视化CPR质量", style={"color": "#7f8c8d", "marginTop": "5px"})
        ], style={"marginBottom": "20px", "borderBottom": "1px solid #ecf0f1", "paddingBottom": "10px"}),
        
        # 控制面板
        html.Div([
            html.Div([
                html.Label("选择数据文件", style={"fontWeight": "bold", "marginBottom": "5px"}),
                dcc.Dropdown(
            ...
            html.Div([
                html.Label("选择时间段", style={"fontWeight": "bold", "marginBottom": "5px"}),
                dcc.RangeSlider(
        ...
        ], style={"backgroundColor": "#ffffff", "padding": "15px", "borderRadius": "8px", "boxShadow": "0 2px 5px rgba(0,0,0,0.05)"}),
...
@callback(
    Output("flower-file-dropdown", "options"),
...
@callback(
    [Output("flower-graph", "figure"),
     Output("flower-analysis-results", "children")],
    [Input("flower-data-store", "data"),
     Input("flower-time-range-slider", "value")],
    prevent_initial_call=True
)
        ...
        for i, cycle in enumerate(cycles):
            ...
            if "velocity" in cycle.columns:
                fig.add_trace(go.Scatter(
                    ...
                    mode="lines",
                    name=f"周期 {i+1}",
                    line=dict(width=1, color=f"rgba(31, 119, 180, {0.3 + 0.7*i/len(cycles)})")
                ))
            # 否则使用深度-时间图
            else:
    ...
    try:
        ...
        if len(df) > 1:
            ...
            if time_diff > 0:
                ...
                for i in range(1, len(df) - 1):
                    if df["depth"].iloc[i] > df["depth"].iloc[i-1] and df["depth"].iloc[i] > df["depth"].iloc[i+1]:
                        peaks += 1
        ...
        return html.Div([
            html.Div([
                html.Div([
                    html.H4("花朵一致性"),
                    html.P(calculate_consistency_score(df), style={"fontSize": "24px", "fontWeight": "bold", "color": "#3498db"})
                ], className="feature-card", style={"flex": "1", "padding": "15px", "backgroundColor": "#f8f9fa", "borderRadius": "8px", "margin": "5px", "textAlign": "center"}),
                
                html.Div([
                    html.H4("完全释放率"),
                    html.P(f"{release_percentage:.1f}%", style={"fontSize": "24px", "fontWeight": "bold", "color": "#2ecc71"})
                ], className="feature-card", style={"flex": "1", "padding": "15px", "backgroundColor": "#f8f9fa", "borderRadius": "8px", "margin": "5px", "textAlign": "center"}),
                
                html.Div([
                    html.H4("按压频率"),
                    html.P(f"{frequency:.1f} 次/分钟", style={"fontSize": "24px", "fontWeight": "bold", "color": "#e74c3c"})
                ], className="feature-card", style={"flex": "1", "padding": "15px", "backgroundColor": "#f8f9fa", "borderRadius": "8px", "margin": "5px", "textAlign": "center"})
            ...
            html.Div([
                ...
                html.P([
                    ...
                    f"{avg_depth:.1f} mm ",
                    html.Span("(理想: 50-60 mm)", style={"color": "green" if 50 <= avg_depth <= 60 else "orange"}),
                    html.Br(),
                    html.Span("• 按压频率: ", style={"fontWeight": "bold"}),
                    f"{frequency:.1f} 次/分钟 ",
                    html.Span("(理想: 100-120 次/分钟)", style={"color": "green" if 100 <= frequency <= 120 else "orange"}),
                    html.Br(),
                    html.Span("• 完全释放: ", style={"fontWeight": "bold"}),
                    f"{release_percentage:.1f}% ",
                    html.Span("(理想: >90%)", style={"color": "green" if release_percentage > 90 else "orange"})
                ])
