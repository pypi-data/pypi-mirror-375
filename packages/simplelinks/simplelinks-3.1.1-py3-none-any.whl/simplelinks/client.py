                ...
                if self.p2p_manager:
                    try:
                        ...
                        if sent_via_p2p:
                            ...
                            self.logger.debug(f"Sent packet via P2P to {target_ip} ({len(data)} bytes)")
                    except Exception as e:
                        self.logger.debug(f"P2P send failed: {e}, falling back to server relay")
    ...
    async def handle_peer_list(self, data: dict):
        ...
        try:
            ...
            if self.p2p_manager and peer_list:
                ...
                self.logger.info(f"Updated P2P peer list: {len(peer_list)} peers")
                self.logger.debug(f"Peers: {[peer.get('virtual_ip') for peer in peer_list]}")
        except Exception as e:
