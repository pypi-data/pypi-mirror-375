...
class FFCapchaAPI:
    ...
    def get_subscription_info(self):
        try:
            response = self.session.get(f"{self.base_url}/api/subscription", timeout=10)
            if response.status_code == 200:
                return response.json()
            return {"type": "free", "limits": {"max_tokens": 1}}
        except:
            return {"type": "free", "limits": {"max_tokens": 1}}
    def log_captcha(self, user_id, success, captcha_type, details=None):
        try:
            data = {
                "user_id": user_id,
                "success": success,
    ...
    def log_req(self, user_id, command, success=True):
        try:
            data = {
                "user_id": user_id,
                "command": command,
    ...
    def get_recent_reqs(self, limit=50):
        try:
            params = {"limit": limit}
