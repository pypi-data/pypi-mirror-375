...
class NpeakCls:
	...
	def get(self, power=repcap.Power.Default) -> float:
		...
		Snippet: value: float = driver.power.quality.result.power.reactive.npeak.get(power = repcap.Power.Default) \n
		...
		response = self._core.io.query_str(f'POWer{power_cmd_val}:QUALity:RESult:POWer:REACtive:NPEak?')
