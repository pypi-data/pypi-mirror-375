...
# Import registry to access debug setting
...
# JWT Configuration - Generate random secret key
SECRET_KEY = secrets.token_urlsafe(32)  # Generate a random 32-byte URL-safe token
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = 1440  # 24 hours

# Debug information
registry = get_registry()
debug = registry.get('debug', False)
if debug:
    print(f"🔑 JWT Secret Key (debug mode): {SECRET_KEY}")
    print("⚠️  This key is auto-generated and will change on restart!")
...
@app.get("/api/projects/{project_id}/traces", tags=["Projects"])
async def get_project_traces(
    ...
    """
    Get all traces for a specific project with optional pagination.
    
    Returns complete trace data for a project, useful for detailed analysis.
    Unlike the main project endpoint, this can return all traces without limit.
    
    - **project_id**: The ID of the project to get traces for
    - **limit**: Maximum number of traces to return (default: all traces)
    ...
    try:
        ...
        # Get traces with optional pagination
        query = db.query(Trace).filter(Trace.project_id == project_id).order_by(desc(Trace.created_at))
        ...
        return {
            ...
            "total_traces": db.query(Trace).filter(Trace.project_id == project_id).count(),
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching project traces: {str(e)}")
...
async def get_system_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "free_memory": f"{system_metrics['memory']['available']:.1f} GB",
            "free_disk": f"{system_metrics['disk']['free']:.1f} GB",
            "total_memory": f"{system_metrics['memory']['total']:.1f} GB",
            "total_disk": f"{system_metrics['disk']['total']:.1f} GB",
            "cpu_usage": f"{system_metrics['cpu']['usage']:.1f}%",
            "memory_usage": f"{system_metrics['memory']['percent']:.1f}%",
            "disk_usage": f"{system_metrics['disk']['percent']:.1f}%",
            "platform": system_metrics["system"]["platform"],
            "hostname": system_metrics["system"]["hostname"],
            "statistics": {
