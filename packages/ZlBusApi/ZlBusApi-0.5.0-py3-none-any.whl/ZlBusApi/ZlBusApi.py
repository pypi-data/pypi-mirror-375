...
_dll_name = {
    'Windows': 'zlbus.dll',
    'Linux':   'zlbus.so',
}.get(platform.system(), 'zlbus.so')

def _find_dll_path():
    """查找DLL文件的路径，支持多种可能的位置"""
    # 首先尝试在包目录的bin子目录中查找
    package_bin_path = os.path.join(os.path.dirname(__file__), 'bin', _dll_name)
    if os.path.exists(package_bin_path):
        return package_bin_path
    
    # 如果在包目录中找不到，尝试在site-packages的bin目录中查找
    try:
        import site
        site_packages = site.getsitepackages()
        for site_package in site_packages:
            site_bin_path = os.path.join(site_package, 'bin', _dll_name)
            if os.path.exists(site_bin_path):
                return site_bin_path
    except:
        pass
    
    # 如果还是找不到，尝试在sys.prefix的bin目录中查找
    try:
        import sys
        prefix_bin_path = os.path.join(sys.prefix, 'bin', _dll_name)
        if os.path.exists(prefix_bin_path):
            return prefix_bin_path
    except:
        pass
    
    # 尝试在包目录的上一级目录的bin目录中查找（处理pip安装的情况）
    parent_bin_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'bin', _dll_name)
...
def _call_cmd(cmdType, func, rf_id, dot_id, *args) -> bytes:
    ...
    """
    # 统一缓冲区
    buf, buf_len = _make_buffer(_MAX_FRAME)
...
# 8. RF 功率
# -------------------------------------------------------------
_lib.ul_modifyIcRfPower_id.argtypes = [c_uint8, c_uint8, c_uint8, POINTER(c_uint8), c_uint16]
...
ul_modifyIcRfPower = \
    lambda fmt, rf_id = DEFAULT_RFID, dot_id = DEFAULT_DOTID: _call_cmd(_M_ID, _lib.ul_modifyIcRfPower_id, rf_id, dot_id, fmt)
...
# ---------- 7. 手指 ANT 滤波参数 ----------
_lib.hl_configFingerFilterParam_id.argtypes = [POINTER(FingerFilterParam_t), c_uint8, c_uint8, POINTER(c_uint8), c_uint16]
...
hl_configFingerFilterParam = \
    lambda param, rf_id = DEFAULT_RFID, dot_id = DEFAULT_DOTID: _call_cmd(_M_ID, _lib.hl_configFingerFilterParam_id, rf_id, dot_id, param)
...
dl_modifyDeviceList = \
    lambda blocks, nums, rf_id = DEFAULT_DONGLEID: _call_cmd(_S_ID, _lib.dl_modifyDeviceList_id2, rf_id, None, cast(blocks, POINTER(DeviceBlock_t)), nums)
