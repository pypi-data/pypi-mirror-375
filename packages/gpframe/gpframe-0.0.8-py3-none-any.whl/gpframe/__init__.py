...
A read-only view of a registry.
    ...
    reader.get(key, default=..., typ=...)

- key: key to look up.
- default: optional fallback value. If omitted and key not found, raises KeyError.
- typ: optional type to validate the value against.

MessageUpdater API
------------------
A read-write view of a registry.

- get(key, default=..., typ=...) -> T
  Retrieve a value (same behavior as MessageReader.get).

- update(key, value) -> T
  Store a value under the given key, returning it.

- apply(key, fn, default=...) -> T
  Atomically update a value using a function:

      ctx.event_message.apply("count", lambda c: c + 1, default=0)

- remove(key, default=None) -> T | None
  Remove a key and return its value. If absent, return default.
