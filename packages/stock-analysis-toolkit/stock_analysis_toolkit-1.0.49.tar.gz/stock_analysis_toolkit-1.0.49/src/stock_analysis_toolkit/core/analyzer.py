...
from ..data.models import AnalysisResult, StockData, Recommendation, IndexAnalysisResult
...
class StockAnalyzer:
    ...
    def __init__(self, symbols: Optional[List[str]] = None, indices: Optional[List[str]] = None, days: int = 365):
        """
    ...
    def analyze_stock(self, symbol: str, stock_data: StockData) -> AnalysisResult:
        ...
        try:
            ...
            # Add latest price to the indicators
            if not stock_data.data.empty:
                technical_indicators['latest_price'] = stock_data.data['Close'].iloc[-1]
            else:
                technical_indicators['latest_price'] = None

            # Compute CAGR metrics if sufficient history is available
            try:
                df = stock_data.data
                ...
                def nearest_on_or_before(target):
                    s = df.index.get_indexer([target], method='pad')
                    idx = s[0]
                    if idx == -1:
                        return None
                    return df['Close'].iloc[idx]

                def cagr_for_years(years: int):
                    target_date = last_date - pd.DateOffset(years=years)
                    first_close = nearest_on_or_before(target_date)
                    if first_close is None or first_close <= 0 or last_close is None or last_close <= 0:
                        return None
                    return (pow(last_close / first_close, 1/years) - 1) * 100.0

                technical_indicators['cagr_1y'] = cagr_for_years(1)
                technical_indicators['cagr_3y'] = cagr_for_years(3)
                technical_indicators['cagr_5y'] = cagr_for_years(5)
            except Exception as e:
                logger.debug(f"CAGR computation skipped for {symbol}: {e}")
            ...
            stock_info = self.data_fetcher.get_company_info(symbol)

            # Generate recommendation
            recommendation = self.generate_recommendation(symbol, technical_indicators, fundamental_metrics)
    ...
    def analyze_indices(self) -> List[IndexAnalysisResult]:
        """
        Analyze all indices.

        Returns:
            List of IndexAnalysisResult objects.
        """
        if not self.indices:
            return []

        logger.info(f"Fetching data for {len(self.indices)} indices...")
        index_data = self.data_fetcher.fetch_multiple_stocks(
            self.indices, days=self.days
        )

        index_results = []
        for symbol, df in index_data.items():
            try:
                stock_data_obj = StockData(symbol=symbol, data=df)
                logger.info(f"Analyzing index: {symbol}")
                technical_analysis = TechnicalAnalysis(stock_data_obj.data)
                technical_indicators = technical_analysis.calculate_all_indicators()

                index_info = self.data_fetcher.get_company_info(symbol)
                name = index_info.get('longName', symbol) if index_info else symbol
                ...
                logger.info(f"Completed analysis for index: {symbol}")
            except Exception as e:
                logger.error(f"Error analyzing index {symbol}: {str(e)}", exc_info=True)
                continue
        
        return index_results

    def run_analysis(self, stocks: List[str], indices: List[str], send_email: bool = False, email_recipient: Optional[str] = None):
        stock_results = self.analyze_all()
        index_results = self.analyze_indices()

        # In a real scenario, you'd also analyze mutual funds here
        mf_summaries = []

        report_generator = ReportGenerator(email_recipient=email_recipient)
        report_path, _ = report_generator.generate_stock_report(
            analysis={'stock_results': list(stock_results.values()), 'mutual_fund_summaries': mf_summaries, 'index_results': index_results}
        )

        if send_email:
            report_generator.send_email(file_path=report_path)

        return stock_results, mf_summaries, report_path

    def generate_recommendation(self, symbol: str, technical_indicators: Dict, fundamental_metrics: Dict) -> Recommendation:
        """
        ...
        # 1. P/E Ratio
        pe = fundamental_metrics.get("pe_ratio")
        if pe is not None:
            if pe < 15:
                score += 1
        ...
        de_ratio = fundamental_metrics.get("debt_to_equity")
        ...
        ma50 = technical_indicators.get("ma_50")
        ma200 = technical_indicators.get("ma_200")
