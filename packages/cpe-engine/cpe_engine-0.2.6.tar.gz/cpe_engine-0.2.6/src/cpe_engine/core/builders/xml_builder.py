...
class XmlBuilder(ABC):
    ...
    def _setup_jinja_environment(self) -> Environment:
        ...
        try:
            ...
            # Registrar funciones globales (equivalentes a TwigFunction en Greenter)
            env.globals['get_tributo_afect'] = get_tributo_by_afectacion
            env.globals['get_tributo_code'] = get_tributo_by_code
            
            print(f"[XmlBuilder] Jinja2 configurado con {len(CUSTOM_FILTERS)} filtros personalizados y 2 funciones globales")
            return env
