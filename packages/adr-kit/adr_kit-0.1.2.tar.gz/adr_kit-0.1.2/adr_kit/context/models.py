...
class TaskHint(BaseModel):
    ...
    task_description: str = Field(..., description="Description of what the agent is trying to accomplish")
    changed_files: Optional[List[str]] = Field(None, description="Files being modified")
    technologies_mentioned: Optional[List[str]] = Field(None, description="Technologies mentioned in the task")
...
class ContextualADR(BaseModel):
    ...
    relevance_score: float = Field(..., description="Relevance score for this task (0.0-1.0)")
    relevance_reason: str = Field(..., description="Why this ADR is relevant")
    key_constraints: List[str] = Field(default_factory=list, description="Key constraints from this ADR")
    related_technologies: List[str] = Field(default_factory=list, description="Technologies mentioned in this ADR")


class PlanningGuidance(BaseModel):
    """Contextual guidance for agents based on the current task and constraints."""
    
    guidance_type: str = Field(..., description="Type of guidance: constraint, recommendation, warning, etc.")
    priority: str = Field(..., description="Priority level: low, medium, high, critical")
    message: str = Field(..., description="The guidance message")
    source_adrs: List[str] = Field(default_factory=list, description="ADRs that contributed to this guidance")
    actionable: bool = Field(True, description="Whether this guidance requires specific action")
...
class ContextPacket(BaseModel):
    ...
    hard_constraints: Dict[str, Any] = Field(..., description="Non-negotiable constraints from contract")
    contract_hash: str = Field(..., description="Hash of the constraints contract used")
    
    # Relevant ADRs (curated shortlist)
    relevant_adrs: List[ContextualADR] = Field(..., description="Most relevant ADRs for this task")
    
    # Contextual guidance
    guidance: List[PlanningGuidance] = Field(default_factory=list, description="Specific guidance for this task")
    
    # Metadata
    generated_at: datetime = Field(default_factory=datetime.utcnow, description="When this packet was generated")
    token_estimate: int = Field(..., description="Estimated token count for this packet")
    ...
    def to_agent_prompt(self) -> str:
        ...
        if self.hard_constraints:
            ...
            if self.hard_constraints.get("imports"):
                ...
                if imports.get("disallow"):
                    lines.append(f"❌ Disallowed imports: {', '.join(imports['disallow'])}")
                if imports.get("prefer"):
                    lines.append(f"✅ Preferred imports: {', '.join(imports['prefer'])}")
            
            # Boundary constraints
            if self.hard_constraints.get("boundaries", {}).get("rules"):
                lines.append("🏗️ Architectural boundaries:")
                for rule in self.hard_constraints["boundaries"]["rules"][:3]:  # Limit to top 3
        ...
        high_priority_guidance = [g for g in self.guidance if g.priority in ["high", "critical"]]
...
class RelevanceScore(BaseModel):
    ...
    reasons: List[str] = Field(default_factory=list, description="Reasons for this relevance score")
