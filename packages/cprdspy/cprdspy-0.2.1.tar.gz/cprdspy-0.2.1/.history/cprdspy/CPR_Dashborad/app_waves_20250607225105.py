                                ...
                                html.Div(
                                    [
                                        dcc.Slider(
                                            ...
                                            value=0.5,
                                            marks={
                                                i / 2: str(i / 2) for i in range(1, 5)
                                            },
                                        ),
                                        dcc.Input(
                                            id="amplitude-input",
                                ...
                                html.Div(
                                    [
                                        dcc.Slider(
                                            ...
                                            value=1.0,
                                            marks={
                                                i / 2: str(i / 2) for i in range(1, 3)
                                            },
                                        ),
                                        dcc.Input(
                                            id="opacity-input",
...
# 为每个参数添加同步回调
def create_sync_callbacks(param):
    @callback(
        Output(f"{param}-input", "value"),
        Input(f"{param}", "value"),
        prevent_initial_call=True,
    )
    def sync_to_input(value):
        return value
    ...
    def sync_to_slider(value):
        return value

    return sync_to_input, sync_to_slider
...
for param in [
    ...
    input_callback, slider_callback = create_sync_callbacks(param)
