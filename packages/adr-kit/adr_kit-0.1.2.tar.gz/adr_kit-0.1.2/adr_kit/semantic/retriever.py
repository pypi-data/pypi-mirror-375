...
class SemanticChunker:
    ...
    def __init__(self):
        ...
        self.section_headers = [
            ...
            "status", "background", "rationale", "implementation"
        ]
    ...
    def chunk_adr(self, adr: ADR) -> List[SemanticChunk]:
        ...
        content_chunks = self._chunk_content_by_sections(adr.content, adr.front_matter.id, chunk_counter)
    ...
    def _chunk_content_by_sections(self, content: str, adr_id: str, start_counter: int) -> List[SemanticChunk]:
    ...
    def _split_with_overlap(self, text: str, adr_id: str, section_name: str, start_counter: int) -> List[SemanticChunk]:
...
class SemanticIndex:
    ...
    def build_index(self, adr_dir: str = "docs/adr", force_rebuild: bool = False) -> Dict[str, Any]:
        ...
        for file_path in adr_files:
            try:
                ...
                if not force_rebuild and adr.front_matter.id in self._meta.get('adr_to_chunks', {}):
        ...
        if new_chunks:
            new_embeddings = self._generate_embeddings([chunk.content for chunk in new_chunks])
        ...
        print(f"✅ Semantic index built: {len(self._chunks)} chunks, {len(processed_adrs)} ADRs")
    ...
    def search(self, query: str, k: int = 5, filter_status: Set[str] = None) -> List[SemanticMatch]:
        ...
        for adr_id in adr_scores:
            ...
            excerpt = best_chunk.content[:200] + "..." if best_chunk and len(best_chunk.content) > 200 else best_chunk.content if best_chunk else ""
