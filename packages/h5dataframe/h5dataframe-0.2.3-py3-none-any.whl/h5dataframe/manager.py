...
import itertools
from typing import Any, Callable, Hashable, Literal, cast, override
...
class H5ArrayManager(ArrayManager):  # type: ignore[misc]
    @override
    ...
    def _verify_integrity(self) -> None:
        ...
        for arr in self.arrays:
            if not len(arr) == n_rows:
                raise ValueError(
                    f"Passed arrays should have the same length as the rows Index: {len(arr)} vs {n_rows} rows"
                )
