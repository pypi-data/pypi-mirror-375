...
def initializeStorageUsageHandler(_serviceInfo):
    ...
    global gStorageUsageDumpDB
    gStorageUsageDumpDB = StorageUsageDumpDB()
...
class StorageUsageHandler(RequestHandler):
    ...
    types_removeDirectory = [str, list, tuple]
    ...
    def export_removeDirectory(dirPaths):
        ...
        for dirPath in dirPaths:
            ...
            if not result["OK"]:
                gLogger.error("Could not delete directory", f"{dirPath} : {result['Message']}")
                return result
        return S_OK()
    ...
    types_purgeOutdatedEntries = [str, int]
    ...
    def export_getProblematicDirsSummary(site, problem=False):
        ...
        between the SE dumps and the FC."""
    ...
    types_getSiteTapeUsage = [str]

    @staticmethod
    def export_getSiteTapeUsage(site_name):
        """
        Retrieve the tape usage for a given site.

        :param site_name: The name of the site for which to retrieve the tape usage.
        :type site_name: str
        :return: A dictionary containing the tape usage information for the site and last updatetime.
        :rtype: dict
        """
        try:
            return S_OK(gStorageUsageDumpDB.get_site_usage(site_name, disk=False))
        except Exception as e:
            return S_ERROR(f"Error getting site usage {e!r}")

    types_getSEUsage = [str]

    @staticmethod
    def export_getSEUsage(se_name):
        """
        Retrieve the se usage.

        :param se_name: The name of the SE for which to retrieve the usage.
        :type se_name: str
        :return: A dictionary containing the usage information for the SE and last updatetime.
        :rtype: dict
        """
        try:
            return S_OK(gStorageUsageDumpDB.get_se_usage(se_name))
        except Exception as e:
            return S_ERROR(f"Error getting site usage {e!r}")
