...
# Lazy import SDK to respect runtime env (e.g., ORCHAGENT_TOKEN_FILE)
from typing import TYPE_CHECKING
if TYPE_CHECKING:  # for type hints only
    from orchagent import OrchClient as _OrchClient
...
def _client(base_url: Optional[str]):
    from orchagent import OrchClient  # defer import
    if base_url:
...
def secrets(
    ...
    """
    import sys
    # Load packaged templates via importlib.resources
    try:
        from importlib.resources import files
        res = files('orchcli.templates.secrets').joinpath(f'{provider}.json')
        if not res.is_file():
