...
(function() {
    ...
    FeedbackApp.prototype.initializeManagers = function() {
        ...
        // 14. 初始化 Textarea 自動保存管理器
        self.initializeTextareaAutoSave();
    ...
    /**
     * 初始化 Textarea 自動保存管理器
     */
    FeedbackApp.prototype.initializeTextareaAutoSave = function() {
        console.log('💾 初始化 Textarea 自動保存管理器...');

        try {
            // 檢查 TextareaAutoSave 模組是否已載入
            if (!window.MCPFeedback.TextareaAutoSave) {
                console.warn('⚠️ TextareaAutoSave 模組未載入，跳過初始化');
                return;
            }

            const self = this;

            // 建立 TextareaAutoSave 實例 (手動模式版本)
            this.textareaAutoSave = new window.MCPFeedback.TextareaAutoSave({
                ...
                autoSaveInterval: 0, // 停用定期保存以減少CPU消耗
                enabled: true,
                onSave: function(content, timestamp) {
                    console.log('📝 Textarea 內容已手動保存', {
                        contentLength: content.length,
                        time: new Date(timestamp).toLocaleString()
                    });
                    // 顯示保存成功通知
                    if (window.MCPFeedback.Utils && window.MCPFeedback.Utils.showMessage) {
                        const message = '內容已保存至備份 (' + new Date(timestamp).toLocaleString() + ')';
                        window.MCPFeedback.Utils.showMessage(message, window.MCPFeedback.Utils.CONSTANTS.MESSAGE_SUCCESS);
                    }
                    // 更新還原按鈕可見性
                    self.updateBackupButtons();
                },
                onRestore: function(content, timestamp) {
                    console.log('🔄 Textarea 內容已還原', {
                        contentLength: content.length,
                        savedTime: new Date(timestamp).toLocaleString()
                    });
                    // 顯示還原通知
                    if (window.MCPFeedback.Utils && window.MCPFeedback.Utils.showMessage) {
                        const message = '已還原備份內容 (' + new Date(timestamp).toLocaleString() + ')';
                        window.MCPFeedback.Utils.showMessage(message, window.MCPFeedback.Utils.CONSTANTS.MESSAGE_SUCCESS);
                    }
                },
                onClear: function() {
                    console.log('🧹 暫存的 textarea 內容已清除');
                    // 更新還原按鈕可見性
                    self.updateBackupButtons();
                }
            });
            ...
            // 初始化自動保存功能（但不執行自動還原）
            this.textareaAutoSave.init();
            
            // 設置手動還原和保存按鈕事件
            this.setupBackupButtons();

            console.log('✅ Textarea 自動保存管理器初始化完成');

        } catch (error) {
            console.error('❌ Textarea 自動保存管理器初始化失敗:', error);
        }
    };
    ...
    FeedbackApp.prototype.handleFeedbackReceived = function(data) {
        ...
        if (this.textareaAutoSave) {
            console.log('ℹ️ 提交成功，但保留備份（永不刪除）');
        }
...
`;
    ...
    FeedbackApp.prototype.performAutoSubmit = function() {
        ...
        // 🆕 自動提交前立即備份當前內容（按用戶要求重新啟用）
        if (this.textareaAutoSave) {
            console.log('💾 自動提交前立即保存當前回饋內容...');
            this.textareaAutoSave.manualSave();
            console.log('✅ 自動提交前備份完成');
        }
     ...
     * 設置手動備份按鈕的事件監聽器
     */
    FeedbackApp.prototype.setupBackupButtons = function() {
        const self = this;
        
        // 手動保存按鈕
        const manualSaveBtn = window.MCPFeedback.Utils.safeQuerySelector('#manualSaveBtn');
        if (manualSaveBtn) {
            manualSaveBtn.addEventListener('click', function() {
                if (self.textareaAutoSave) {
                    self.textareaAutoSave.manualSave();
                }
            });
            console.log('✅ 手動保存按鈕事件已設置');
        }

        // 手動還原按鈕
        const restoreBackupBtn = window.MCPFeedback.Utils.safeQuerySelector('#restoreBackupBtn');
        if (restoreBackupBtn) {
            restoreBackupBtn.addEventListener('click', function() {
                if (self.textareaAutoSave) {
                    // 檢查是否有備份可用
                    const backupInfo = self.textareaAutoSave.getBackupInfo();
                    if (backupInfo) {
                        // 顯示確認對話框
                        const confirmMessage = '確定要還原備份內容嗎？\n\n' +
                            '備份時間：' + backupInfo.savedTime + '\n' +
                            '內容長度：' + backupInfo.contentLength + ' 字元\n' +
                            '預覽：' + backupInfo.preview;
                        
                        if (confirm(confirmMessage)) {
                            const success = self.textareaAutoSave.manualRestore();
                            if (!success) {
                                window.MCPFeedback.Utils.showMessage(
                                    '還原失敗，可能沒有可用的備份或備份已過期',
                                    window.MCPFeedback.Utils.CONSTANTS.MESSAGE_ERROR
                                );
                            }
                        }
     ...
     * 更新備份按鈕的可見性和狀態
     */
    FeedbackApp.prototype.updateBackupButtons = function() {
        if (!this.textareaAutoSave) return;

        const restoreBackupBtn = window.MCPFeedback.Utils.safeQuerySelector('#restoreBackupBtn');
        if (restoreBackupBtn) {
            const hasBackup = this.textareaAutoSave.hasBackup();
            if (hasBackup) {
                restoreBackupBtn.style.display = 'inline-flex';
                // 更新按鈕提示文字
                const backupInfo = this.textareaAutoSave.getBackupInfo();
                if (backupInfo) {
                    restoreBackupBtn.title = '還原備份 (' + backupInfo.savedTime + ')';
                }
            } else {
