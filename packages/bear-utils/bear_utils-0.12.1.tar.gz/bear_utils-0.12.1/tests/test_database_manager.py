...
class TestDatabaseConfiguration:
    ...
    def test_database_config_defaults_by_scheme(self) -> None:
        ...
        # PostgreSQL defaults
        pg_config = DatabaseConfig.by_schema(scheme="postgresql")
        ...
        # MySQL defaults
        mysql_config = DatabaseConfig.by_schema(scheme="mysql")
        ...
        # SQLite defaults
        sqlite_config = DatabaseConfig.by_schema(scheme="sqlite")
...
class TestDatabaseManagerOperations:
    ...
    def test_get_all_records_empty(self, clean_db: DatabaseManager):
        ...
        users: list[MockUser] = clean_db.get_all_records(MockUser)
        assert users == []

    def test_get_all_records_with_data(self, db_with_data: DatabaseManager) -> None:
        """Test getting all records with data."""
        users: list[MockUser] = db_with_data.get_all_records(MockUser)
        assert len(users) == 2
    ...
    def test_get_records_by_var_no_match(self, db_with_data: DatabaseManager) -> None:
        ...
        users = db_with_data.get_records_by_var(MockUser, "name", "Charlie")
        assert users == []

    def test_count_records_by_var(self, db_with_data: DatabaseManager) -> None:
        """Test counting records by variable."""
        count = db_with_data.count_records_by_var(MockUser, "name", "Alice")
        assert count == 1

        count = db_with_data.count_records_by_var(MockUser, "name", "NonExistent")
        assert count == 0
...
class TestSessionManagement:
    ...
    def test_close_session(self, clean_db: DatabaseManager) -> None:
        """Test closing sessions."""
        session: Session = clean_db.get_session()
        assert clean_db.session.registry.has() is True
        clean_db.close_session()
        assert clean_db.session.registry.has() is False
...
class TestBaseClassManagement:
    ...
    def test_get_base_creates_default(self) -> None:
        ...
        base: DeclarativeMeta = DatabaseManager.get_base("test_base")
    ...
    def test_set_and_get_base(self) -> None:
        ...
        DatabaseManager.set_base(custom_base, "test_base2")
        retrieved_base: DeclarativeMeta = DatabaseManager.get_base("test_base2")

        assert retrieved_base is custom_base

    def test_clear_bases(self) -> None:
        """Test clearing base classes."""
        DatabaseManager.set_base(declarative_base(), "to_clear")
...
class TestIntegration:
    ...
    def test_basic_crud_workflow(self, clean_db: DatabaseManager) -> None:
        ...
        # READ
        users: list[MockUser] = clean_db.get_records_by_var(MockUser, "name", "CRUD")
        assert len(users) == 1
        user_id = users[0].id
        ...
        # Verify update
        updated_users: list[MockUser] = clean_db.get_records_by_var(MockUser, "email", "updated@test.com")
        ...
        # Verify deletion
        final_count = clean_db.count_records(MockUser)
    ...
    def test_error_recovery(self, db_with_data: DatabaseManager) -> None:
        ...
        final_count: int = db_with_data.count_records(MockUser)
