...
def json_validate(schema: dict, json_data: dict) -> Tuple[bool, ValidationError | None]:
    """
    验证 json 是否符合模板规定的格式
...
def get_func_use_time(logger=None):
    """装饰器：获取函数运行时间"""
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            e_time = time.time()
            msg = f'Function "{func.__name__}" required: {e_time - s_time:.4f} seconds'
            if logger:
                logger.info(msg)
            else:
                print(msg)
