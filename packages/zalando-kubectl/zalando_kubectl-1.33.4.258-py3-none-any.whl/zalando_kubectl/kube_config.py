...
LOCAL_SERVER_PORTS = [18000, 8000, 28000, 38000]
...
def pick_free_port():
    for port in LOCAL_SERVER_PORTS:
        if is_port_available(port):
            return port
    raise Exception(
        f"zkubectl was unable to bind to a local port, as all ports of {LOCAL_SERVER_PORTS} were already\n"
        + "occupied on either IPv4 or IPv6.  Please make sure at least one of them is free.\n\n"
        + f"You can use e.g.  `netstat -ltnp`  on Linux or  `lsof -P -iTCP:{','.join([str(i) for i in LOCAL_SERVER_PORTS])}`\n"
        + "on Linux or Mac to find out who listens on these ports, and then stop one of these services."
    )
...
def get_auth(env: Environment, okta_auth_client_id, force_refresh=False, stdout=None, stderr=None):
    ...
    cmd = [
        ...
        f"--listen-address=localhost:{port}",
    ]
    if force_refresh:
