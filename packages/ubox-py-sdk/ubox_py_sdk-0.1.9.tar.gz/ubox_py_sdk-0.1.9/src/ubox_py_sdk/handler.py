...
def find_optimal_element(selector: Any, match: str) -> Tuple[Optional[Any], bool]:
    """
    智能查找元素，按优先级进行匹配：
    1. 强匹配（精确匹配）
    2. 模糊匹配（包含匹配）
    ...
    # 1. 优先进行强匹配（精确匹配）
    try:
        # 精确匹配：text、value、name、label 属性完全等于 match
    ...
    # 2. 模糊匹配（包含匹配）
    try:
        # 包含匹配：text、value、name、label 属性包含 match
        contains_xpath = f'//*[contains(@text,"{match}") or contains(@value,"{match}") or contains(@name,"{match}") or contains(@label,"{match}")]'
    ...
    # 3. 正则表达式匹配（最后尝试）
    try:
        xpath_regex = f'//*[re:match(@text,"{match}") or ' \
...
class EventHandler:
    ...
    def _process_watchers(self, uitree):
        ...
        for watcher_name, watcher in self.watchers.items():
            ...
            for condition in watcher.conditions:
                xml_element, found = self._check_watcher_condition(xml_tree, condition)
    ...
    def _check_watcher_condition(self, xml_tree, condition):
        """检查watcher条件是否匹配
        ...
        try:
            ...
            if condition.startswith('//'):
                ...
                # 文本匹配
                return find_optimal_element(xml_tree, condition)
        except Exception as e:
