...
from .utils import OutputFormatter, JSONOutputFormatter, ProgressReporter, validate_path, get_project_root
...
class NPMVulnerabilityScanner:
    ...
    def run_scan_command(self, args):
            ...
            for path_str in args.paths:
                try:
                    ...
                    if hasattr(args, 'json') and args.json:
                        json_formatter = JSONOutputFormatter(__version__)
                        print(json_formatter.format_error(str(e)))
                    else:
        ...
        if args.include_global:
            ...
            elif not (hasattr(args, 'json') and args.json):
        ...
        verbose = args.verbose and not (hasattr(args, 'json') and args.json)
        
        # Print headers only if not in JSON mode
        ...
        results = self.scan_paths(paths, verbose=verbose)
        duration = time.time() - start_time
        
        # Output results
        if hasattr(args, 'json') and args.json:
            json_formatter = JSONOutputFormatter(__version__)
            print(json_formatter.format_scan_results(
                results, paths, duration, self.checker.compromised_versions
            ))
        else:
    ...
    def run_check_command(self, args):
        ...
        try:
            ...
            if not package_path.exists():
                error_msg = f"{package_path} does not exist"
                if hasattr(args, 'json') and args.json:
                    json_formatter = JSONOutputFormatter(__version__)
                    print(json_formatter.format_error(error_msg))
                else:
                    print(f"Error: {error_msg}", file=sys.stderr)
                sys.exit(1)
            
            if not package_path.name.endswith('package.json'):
                error_msg = f"{package_path} is not a package.json file"
                if hasattr(args, 'json') and args.json:
                    json_formatter = JSONOutputFormatter(__version__)
                    print(json_formatter.format_error(error_msg))
                else:
        ...
        if hasattr(args, 'json') and args.json:
            json_formatter = JSONOutputFormatter(__version__)
            print(json_formatter.format_scan_results(
                results, [package_path], duration, self.checker.compromised_versions
            ))
        else:
            self.formatter.print_detection_results(results)
    ...
    def run_list_command(self, args):
        ...
        if hasattr(args, 'json') and args.json:
            json_formatter = JSONOutputFormatter(__version__)
            print(json_formatter.format_list_output(self.checker.compromised_versions))
        else:
...
Examples:
  ...
  %(prog)s list
    ...
    parser.add_argument(
        '--json',
        action='store_true',
        help='Output results as JSON (for parsing with jq or other tools)'
    )
