...
class DeliveryAddress(BaseModel):
    organization: str  = Field(description="The name of the receiving organization.")
    attention: str  = Field(
        description="The person or department to address the delivery to."
    )
    address: str  = Field(description="The full street and city address.")



class GeneralSubmissionRequirements(BaseModel):
    deadline: str  = Field(
        description="The exact submission deadline, including date, time, and timezone."
    )
    delivery_address: DeliveryAddress
    number_of_copies: str  = Field(
        description="The required number of physical copies (e.g., '1 original + 2 copies')."
    )
    packaging_instructions: str  = Field(
        description="Instructions on how the proposal should be sealed and labeled."
    )
    pricing_instructions: str  = Field(
        description="Instructions on where and how to submit pricing information."
    )
    binding_instructions: str  = Field(
        description="Requirements for how the proposal should be bound."
    )
    contact_protocol: str  = Field(
        description="Rules for communication with the agency."
    )
...
class InsuranceRequirements(BaseModel):
    ...
    professional_liability: str
    automobile_liability: str



class ComplianceObligations(BaseModel):
    governing_regulations: str
    agency_rights_reservation: str
    insurance_requirements: InsuranceRequirements
    licensing_requirements: str
    diversity_compliance: str
    ethical_certifications: List[str]



class RFPDetails(BaseModel):
    """The root model for all extracted RFP information."""

    general_submission_requirements: GeneralSubmissionRequirements
    tabbed_proposal_submittal: List[Tab]
    required_attachments_checklist: List[str]
...
class GeneratedQuestion(BaseModel):
    question: str  = Field(
        description="The specific, concise, and open-ended question to be asked."
    )
    rationale: str  = Field(
        description="A brief explanation of WHY this question is necessary. E.g., 'To clarify ambiguity in the deliverable timeline' or 'To assess the risk associated with data security compliance'."
    )
    reference: str  = Field(
        description="The exact section, page number, or quote from the RFP that prompted this question. E.g., 'Section 3.1, Page 5, Paragraph 2'."
    )
    priority: str  = Field(
        description="The priority of the question, categorized as 'High', 'Medium', or 'Low'. High-priority questions are critical blockers or address major risks."
    )
...
class QuestionCategory(BaseModel):
    """
    A logical grouping of questions based on a topic from the RFP.
    """

    category: str  = Field(
        description="The name of the category, ideally derived from a section title in the RFP (e.g., 'Technical Specifications', 'Pricing Model', 'Legal Terms')."
    )
    questions: List[GeneratedQuestion]  = Field(
        description="A list of structured questions belonging to this category."
    )
...
class RFPQuestions(BaseModel):
    """
    The root object for all generated questions from an RFP.
    """

    question_categories: List[QuestionCategory]  = Field(
        description="A list of all question categories extracted from the RFP."
    )
