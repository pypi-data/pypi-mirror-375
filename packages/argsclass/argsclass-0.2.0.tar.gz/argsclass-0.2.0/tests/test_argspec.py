...
def test_primitive_boolean_type():
    """Test PrimitiveType with bool - should raise error since booleans are handled by flags."""
    with pytest.raises(ValueError, match="Unsupported primitive type.*Booleans are handled by flags"):
        PrimitiveType(bool)
...
def test_arg_with_choices_constraint():
...
def test_validation_errors():
    ...
    with pytest.raises(TypeError, match="unexpected keyword argument 'aliases'"):
        PositionalArgSpec(
            name="test",
            aliases={"t"}
        )
...
def test_choices_validation():
    ...
    with pytest.raises(ValueError, match="Default value must be one of the choices"):
        OptionArgSpec(
            name="format",
            choices=["json", "xml"],
            default="yaml"
...
def test_cardinality_validation():
    ...
    # Invalid min value
    with pytest.raises(ValueError, match="min must be a non-negative integer"):
        Cardinality(min=-1)
    
    # Invalid max value
    with pytest.raises(ValueError, match="max must be a non-negative integer"):
        Cardinality(min=1, max=-1)
    
    # Max less than min
    with pytest.raises(ValueError, match="max cannot be less than min"):
