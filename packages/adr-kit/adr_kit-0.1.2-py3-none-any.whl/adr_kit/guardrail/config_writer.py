...
try:
    ...
    try:
        ...
        # Fallback for TOML operations if library not available
        toml = None

from .models import ConfigFragment, SentinelBlock, FragmentTarget, FragmentType, ApplyResult, ApplicationStatus
...
class ConfigWriter:
    ...
    def apply_fragments(self, target: FragmentTarget, fragments: List[ConfigFragment]) -> ApplyResult:
        ...
        try:
            ...
            if target.fragment_type in [FragmentType.ESLINT, FragmentType.PRETTIER]:
                updated_content = self._apply_json_fragments(original_content, fragments, target)
            elif target.fragment_type in [FragmentType.RUFF, FragmentType.MYPY, FragmentType.IMPORT_LINTER]:
                updated_content = self._apply_toml_fragments(original_content, fragments, target)
            else:
                updated_content = self._apply_text_fragments(original_content, fragments, target)
    ...
    def _apply_json_fragments(self, content: str, fragments: List[ConfigFragment], target: FragmentTarget) -> str:
    ...
    def _apply_toml_fragments(self, content: str, fragments: List[ConfigFragment], target: FragmentTarget) -> str:
        ...
        except Exception:  # Broad exception handling since different TOML libs have different exceptions
    ...
    def _apply_text_fragments(self, content: str, fragments: List[ConfigFragment], target: FragmentTarget) -> str:
        ...
        managed_section = f"{sentinel.start_marker}\n{fragment_content}\n{sentinel.end_marker}"
        
        # Find and replace existing managed section
        pattern = re.escape(sentinel.start_marker) + r".*?" + re.escape(sentinel.end_marker)
    ...
    def _merge_json_configs(self, base: Dict[str, Any], fragment: Dict[str, Any]) -> Dict[str, Any]:
        """Merge JSON configuration objects."""
        return self._merge_dict_configs(base, fragment)
    
    def _merge_dict_configs(self, base: Dict[str, Any], fragment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in fragment.items():
            if key in result:
                if isinstance(result[key], dict) and isinstance(value, dict):
                    ...
                    seen = set(result[key]) 
                    result[key] = result[key] + [item for item in value if item not in seen]
                else:
