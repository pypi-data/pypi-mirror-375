...
class LicensingManager:
    ...
    def _validate_api_key_format(self, api_key: str) -> bool:
        """Validate API key format for security with enhanced validation"""
        ...
        # Check for sequential characters (security risk)
    ...
    def _make_secure_api_request(self, endpoint: str) -> Dict[str, Any]:
        """Make secure API request to Clyrdia.com backend with enhanced security"""
        import aiohttp
        import ssl
        import asyncio
        import hashlib
        ...
        async def _make_request():
            # Create SSL context for secure connections with enhanced security
            ...
            request_id = hashlib.sha256(f"{self.api_key}{time.time()}".encode()).hexdigest()[:16]
            
            # Add security headers with enhanced protection
            headers = {
                'Authorization': f'Bearer {self.api_key}',
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    ...
                    self._log_security_event("api_request", f"Request to {endpoint} - Status: {response.status}")
        ...
        # Run the async request with enhanced error handling
        try:
        ...
        try:
            ...
            self._log_security_event("api_timeout", f"Request to {endpoint} timed out")
            raise Exception("API request timed out. Please try again.")
        except Exception as e:
            self._log_security_event("api_error", f"Request to {endpoint} failed: {str(e)}")
            raise
        finally:
            if loop.is_running():
                loop.close()
        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    thread.start()
                    thread.join(timeout=15)  # Give it a bit more time than the async timeout
                    return result[0]
                else:
