###
###
### Note: this code is adapted from https://github.com/Crimson-Crow/json-numpy
...
def default(o: Any, *, fallback_default: Callable[[Any],
                dict[str, Any]] | None = None) -> dict[str, Any]:
    """Encodes numpy objects to a JSON-serializable dictionary.

    Args:
        o (object): The object to encode.
        fallback_default (Callable[[Any], dict[str, Any]] | None):
        A fallback encoder function to handle objects that are not numpy objects.

    Returns:
        dict[str, Any]: The JSON-serializable dictionary representation of the
                        numpy object, or the result of the fallback encoder if
                        present and if the object is not a numpy object.
    ...
    if isinstance(o, (ndarray, generic)):
        ...
        return {"__jsnumpy__": b64encode(data).decode(),
...
def object_hook(dct: dict) -> dict | ndarray | generic:
    ...
    """
    if "__jsnumpy__" in dct:
        np_obj = frombuffer(b64decode(dct["__jsnumpy__"]), descr_to_dtype(dct["dtype"]))
...
def _patch_encoder(*args: Any,   default: Callable[[Any], Any] | None = None,
                  user_cls: type[json.JSONEncoder] | None = None,
                  **kwargs: Any) -> json.JSONEncoder:
    ...
    if user_cls is None:
        ...
        return encoder
    return user_cls(*args, default=partial(_default, fallback_default=default), **kwargs)
...
def loads(*args: Any, object_hook: Callable[[dict], Any] | None = None, **kwargs: Any) -> Any:
    return _loads(*args,
                  object_hook=_hook if object_hook is None else lambda dct: _hook(object_hook(dct)),
                 **kwargs)
