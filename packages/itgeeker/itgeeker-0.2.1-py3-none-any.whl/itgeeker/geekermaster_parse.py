...
def generate_ban_company_list():
    ban_company_list = []
    for co in self.env['geeker.ban.company'].search([('active', '=', True)]):
        if co.name:
            ban_company_list.append(co.name)
    _logger.warning("ban_company_list: %s" % ban_company_list)
    return ban_company_list
...
def parse_company_name(text_str):
    ...
    if text_str:
        ...
        if extract_res_list:
            ...
            if found_company_list:
                final_list = []
                ban_company_list = self.generate_ban_company_list()
                for fcl in found_company_list:
                    fcl = fcl.strip()
                    filtered_name = self.filter_co_name(fcl, ban_company_list)
                    if filtered_name:
                    ...
                    #     else:
                    #         final_list.append(fcl)
                _logger.warning("will return final_list: %s", final_list)
                if final_list:
                    return final_list[0]
        return False
    ...
    if text_str:
        ...
        if extract_res_list:
            ...
            if found_school_list:
                final_list = []
                # ban_school_list = self.generate_ban_school_list()
    ...
    if text_str:
        ...
        if extract_res_list:
            ...
            if found_gender_list:
                final_list = []
                # ban_gender_list = self.generate_ban_gender_list()
