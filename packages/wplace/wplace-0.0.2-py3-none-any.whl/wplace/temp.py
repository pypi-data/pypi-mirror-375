...
def decode_country_bitmap(bitmap: str) -> tuple[Country, ...]:
    """Extract list of unlocked country flags from `flagsBitmap`."""
...
def total_needed_pixels(level: int) -> int:
    """Return the total number of placed pixels needed to reach a certain
    level.
    """
...
def needed_pixels(level: int) -> int:
    """Return the number of placed pixels needed to reach the next level."""
