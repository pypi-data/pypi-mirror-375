...
class Client(object):
    ...
    def mplabml_project_download(
        ...
        """Downloads the DCLI, captures, queries, and pipelines of a project.
        
        Args:
            project_name (Optional[str], optional): Name of the project to download.
            rename_project_to (Optional[str], optional): Allows renaming the project downloaded
            outdir (Optional[str], optional): Directory to store project. Defaults to project name.
            max_workers (int, optional): Number of parallel workers to use for downloading files. Defaults to 8.
            batch_size (int, optional): The batch size to use for each worker. Defaults to 10.
        ...
        if project_name is not None:
            self._project = self.projects.get_or_create_project(project_name)
        elif self._project is None:
            raise Exception("No project name given. To perform this action use parameter project_name to set the project or set project with client.project.")
            

        # Set download directory and project name to download as
        if outdir is None and rename_project_to is None:
            outdir = self.project.name
            rename_project_to = self.project.name
        elif outdir is None:
            outdir = rename_project_to
        elif rename_project_to is None:
            rename_project_to = self.project.name
        else:
            outdir = outdir + "\\" + rename_project_to + "\\"
        if not os.path.isdir(outdir):
            os.makedirs(outdir)
        mplabml_project_file = os.path.join(outdir, f"{rename_project_to}_mplabml_project.json")
        

        print("Downloading MPLABML project " + self.project.name + "...")


        # Download DCLI
        dcli = self.project.get_project_dcli()
        dcli_json = json.dumps(dcli)
        ...
        # Download pipelines
        list_of_sandboxes = self._project._sandboxes.get_sandboxes()
        list_of_pipelines = []
        for sandbox in list_of_sandboxes:
            pipeline_details_json = json.dumps(sandbox._pipeline.to_list())
            raw_details = r'{}'.format(pipeline_details_json)
            list_of_pipelines.append(r'{ "pipeline_name":"' + sandbox._name + r'", "pipeline_steps":' + raw_details + r'}')
        ...
        for pipeline in list_of_pipelines:
            ...
            pipeline_string = pipeline_string + pipeline
            i = i + 1
        pipeline_string = pipeline_string + "]"


        # Download Queries
        query_list = self._project._queries.get_queries()
        query_string = "["
        i = 0
        for query in query_list:
            if i > 0:
                query_string = query_string + ", "
            query_string = query_string + str(query._to_dict())
            i = i + 1
        query_string = query_string + "]"
        query_string = query_string.replace(f"'", f'"')
        dcli_string = r'{}'.format(dcli_json)

        
        # Download master file without dcli
        mplabml_project_text = r'{ "project_name": "' + rename_project_to + r'", "pipeline":' + pipeline_string + r', "queries":' + query_string + '}'
        with open(mplabml_project_file, "w") as f:
            f.write(mplabml_project_text)

        # DCLI
        with open(os.path.join(outdir, f"{rename_project_to}_mplabml_project_dcli.json"), "w") as f:
            f.write(dcli_string)

        print("MPLABML project " + self.project.name + " download complete.")
        print("Download location: " + os.path.abspath(outdir))

    def mplabml_project_upload(
        self,
        project_name: Optional[str] = None,
        project_name_to_upload_as: Optional[str] = None,
        project_dir: Optional[str] = None,
        max_workers: int = 8,
        batch_size=10,
    ):
        """Uploads the DCLI, captures, queries, and pipelines of a project.

        Args:
            project_name (Optional[str], optional): Name of project to upload. Can alternatively set client.project.
            project_name_to_upload_as (Optional[str], optional): Project name to upload the project as. Use this to rename a project when uploading.
            project_dir (Optional[str], optional): Directory of mplabml project files to upload. Defaults to project name.
            max_workers (int, optional): Number of parallel workers to use for downloading files. Defaults to 8.
            batch_size (int, optional): The batch size to use for each worker. Defaults to 10.
        ...
        # Set client project, original_project_name
        original_project_name = None
        if project_name_to_upload_as is not None:
            if project_name is not None:
                original_project_name = project_name
            elif self._project is not None:
                original_project_name = self.project.name
            else:
                original_project_name = project_name_to_upload_as
            self._project = self.projects.get_or_create_project(project_name_to_upload_as)
        elif project_name is not None:
            original_project_name = project_name
            self._project = self.projects.get_or_create_project(project_name)
        elif self._project is None:
            raise Exception("ERROR: No project name given. To perform this action use parameter project_name OR set the project using client.project before calling this function. project_name_to_upload_as is only used to rename the project.")
        
        
        # Project names
        if project_name_to_upload_as is None:
            project_name_to_upload_as = self.project.name
        ...
        if project_dir is None:
            ...
            if os.path.isdir(project_dir + "\\" + original_project_name):
                project_dir = project_dir + "\\" + original_project_name
                print("MPLABML project folder found in dir. Setting project_dir to " + os.path.abspath(project_dir))
        if not os.path.isdir(project_dir):
            raise Exception("Project files not found: " + os.path.abspath(project_dir))
        mplabml_project_file = os.path.join(project_dir, f"{original_project_name}_mplabml_project.json")
        dcli_file_path = os.path.join(project_dir, f"{original_project_name}_mplabml_project_dcli.json")


        print("Uploading MPLABML project " + self.project.name + "...")
        
        
        # Read json file
        if os.path.isfile(mplabml_project_file):
            f = open(mplabml_project_file, mode='rt')
            content = f.read()
            mplabml_project = json.loads(content)
            project_name = mplabml_project['project_name']
            pipelines_list = mplabml_project['pipeline']
            queries_list = mplabml_project['queries']
        else:
            raise Exception("ERROR: " + f"{original_project_name}_mplabml_project.json file not found.")
        
        
        # Create project
        self.upload_project_dcli(project_name_to_upload_as, dcli_file_path, True)
        ...
        for filename in os.listdir(project_dir):
            if filename.endswith(".csv"):
                print(filename)
                if 1==1:
                    self.upload_data_file(filename, os.path.join(project_dir, filename), True, False)
                else:
                    self.upload_data_file(filename, os.path.join(project_dir, filename), True, True)
        ...
        for pipeline in pipelines_list:
            ...
            print("pipeline name: " + p_name)
            
            p_steps_list = pipeline['pipeline_steps']
            print("pipeline steps: ")
            print(p_steps_list)

            print(pipeline)
            sandbox_existing = self._project._sandboxes.get_sandbox_by_name(p_name)
            if sandbox_existing != None:
                sandbox_existing.delete()
                print("deleted existing pipeline: " + p_name)
            
            sandbox_created = self._project._sandboxes.create_sandbox(p_name, p_steps_list)
            print("created new pipeline: " + p_name)
        ...
        for query in queries_list:
            print(query)
            q_name = query['name']
            query_existing = self._project._queries.get_query_by_name(q_name)
            if query_existing != None:
                query_existing.delete()
                print("deleted existing query: " + q_name)
            print(query['sensor_columns'])
            
            segmenter_list = self.list_segmenters()
            print("segmenter_list")
            print(segmenter_list)
            print(segmenter_list.at[0, 'id'])
            segmenter_to_use = int(segmenter_list.at[0, 'id'])
            self.create_query(q_name,query['sensor_columns'], query['metadata_columns'], query['metadata_filter'],segmenter=segmenter_to_use,label_column=query['label_column'],capture_configurations=query['capture_configurations'])
            
        print("MPLABML project " + project_name_to_upload_as + " upload complete.")
