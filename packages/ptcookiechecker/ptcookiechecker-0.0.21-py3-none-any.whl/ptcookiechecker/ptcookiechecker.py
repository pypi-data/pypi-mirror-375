...
class PtCookieChecker:
    ...
    def send_request(self, url: str) -> requests.models.Response:
        ...
        try:
            ...
            ptprinthelper.ptprint(f"[{response.history[0].status_code}]", condition=not self.use_json, colortext=False)
            if response.history: # if redirect occured
                final_url = response.url
                if not self.args.redirects:
                    ptprinthelper.ptprint(f"Redirect detected: {response.url}", "ADDITIONS", not self.use_json, colortext=True, indent=4)
                    sys.exit(1)
