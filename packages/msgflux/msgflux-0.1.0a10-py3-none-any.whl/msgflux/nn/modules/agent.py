...
from typing import (
    Any, Callable, List, Literal, Mapping, Optional, Tuple, Union, get_origin
)
...
from msgflux.utils.msgspec import StructFactory, msgspec_dumps
...
class Agent(Module):
    ...
    def __init__(
        ...
        *,
        ...
        examples: Optional[List[Union[Example, Mapping[str, Any]]]] = None,
        ...
        typed_parser: Optional[Literal["typed_xml"]] = None,
        ...
        _annotations: Optional[Mapping[str, type]] = None,
    ):
        """Args:
        name:
        ...
        verbose:
            If True, capture model tool call and response, model output.
        description:
        ...
        if _annotations is None:
            _annotations = {"message": Union[str, Mapping[str, str]], "return": str}
        super().__init__()
    ...
    def forward(
        self, message: Union[str, Mapping[str, Any], Message], **kwargs
    ...
    def _process_tool_flow_control_response(
        ...
        while True:
            ...
            if getattr(raw_response, "current_step", None):  # Step
                step = raw_response.current_step
                actions = step.actions

                if self.verbose:
                    cprint(f"[{self.name}][tool_calls]", bc="br1", ls="b")
                    repr = f"[{self.name}][tool_calls_reasoning] {step.thought}"
                ...
                for act in actions:  # Add results
                    act.result = tool_execution_result.get(act.id)
            ...
            elif getattr(raw_response, "final_answer", None):
    ...
    def _process_tool_call(
        ...
        if self.verbose:
            cprint(f"[{self.name}][tool_responses]", bc="br1", ls="b")
            for id, result in tool_execution_result.responses.items():
                for call in tool_callings:
                    if id == call[0]: # if call_id is equals call_id_response
                        name = call[1]
                        continue
                cprint(f"[{self.name}][tool_response] {name} {result}", ls="b")
    ...
    def _set_generation_schema(
        ...
        elif is_subclass_of(generation_schema, msgspec.Struct):
            if get_origin(generation_schema) is None:  # has TypeVar
                generation_schema = generation_schema[str]  # str as default
            self.register_buffer("generation_schema", generation_schema)
        else:
            raise TypeError(
    ...
    def _set_examples(
        ...
        if isinstance(examples, list) or examples is None:
        ...
        *,
        ...
        if signature is not None:
            ...
            if system_message is None:
                if hasattr(generation_schema, "system_message"):
                    system_message = generation_schema.system_message
                else:
            ...
            if generation_schema is not None:
                # Insert as an TypeVar
                # TODO Check type var
                output_struct = generation_schema[signature_output_struct]
                class Output(output_struct, msgspec.Struct):  
                    pass # Convert typing._GenericAlias to Struct
