...
def test_prepare_single_commits_collects_unique_change_ids(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """
    ...
    def fake_run_cmd(cmd: list[str], **kwargs: Any) -> CommandResult:
        ...
        if cmd[:3] == ["git", "show", "-s"] and "--pretty=format:%an <%ae>" in cmd:
            return CommandResult(returncode=0, stdout="Author <author@example.org>\n", stderr="")
    ...
    monkeypatch.setattr("github2gerrit.core.git_cherry_pick", fake_git_cherry_pick)

    def fake_git_commit_amend(**kwargs: Any) -> None:
        # Called after cherry-pick to ensure trailers exist; we can no-op here.
        pass

    monkeypatch.setattr("github2gerrit.core.git_commit_amend", fake_git_commit_amend)

    # Use a mutable holder to return appropriate trailers for each commit pick
    picked_index = {"i": -1}

    def fake_trailers(keys: list[str] | None = None, **kwargs: Any) -> dict[str, list[str]]:
        # After each pick + amend, core queries trailers; emulate next commit's trailers
        picked_index["i"] += 1
        idx = picked_index["i"]
    ...
    monkeypatch.setattr("github2gerrit.core.git_last_commit_trailers", fake_trailers)
...
def test_prepare_squashed_commit_reuses_change_id_from_comments(
    ...
    def fake_run_cmd(cmd: list[str], **kwargs: Any) -> CommandResult:
        ...
        if cmd[:3] == ["git", "show", "-s"] and "--pretty=format:%an <%ae>" in cmd:
            return CommandResult(returncode=0, stdout="Lead Dev <lead@example.org>\n", stderr="")
        return CommandResult(returncode=0, stdout="", stderr="")
    ...
    monkeypatch.setattr("github2gerrit.core.get_repo_from_env", lambda _c: object())
    monkeypatch.setattr("github2gerrit.core.get_pull", lambda _repo, _num: object())
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("github2gerrit.core.git_commit_new", fake_git_commit_new)
    monkeypatch.setattr("github2gerrit.core.git_commit_amend", fake_git_commit_amend)
    ...
    assert result.commit_shas == []
    # Should have created exactly one new commit; amend may or may not happen depending on presence
