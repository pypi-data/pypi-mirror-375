...
from shai_plugin.core.documentation.schema_doc import docs
...
class PluginConfiguration(BaseModel):
    ...
    class Meta(BaseModel):
        ...
        version: str = Field(
            ...
            "when installing an older plugin to a newer Shai, it's hard to ensure breaking changes never happen,"
            " but at least, Shai can detect it by this field, it knows which version of the manifest is supported.",
        )
        arch: list[PluginArch]
        runner: PluginRunner
        minimum_shai_version: str | None = Field(
            None,
            description="The minimum version of Shai, designed for forward compatibility."
            "When installing a newer plugin to an older Shai, many new features may not be available,"
            "but showing the minimum Shai version helps users understand how to upgrade.",
        )

        @field_validator("minimum_shai_version")
        @classmethod
        def validate_minimum_shai_version(cls, v: Optional[str]) -> Optional[str]:
            if v is None:
                return v
            try:
                Version(v)
