...
def download_single_file(owner, repo, file_path, branch, save_dir=".", force=False, rename=False, rate_limit=float('inf')):
...
def download_github_dir(owner, repo, path, branch="main", save_dir=".", flat=False, force=False, rename=False, rate_limit=float('inf')):
    ...
    if ( response.headers['X-RateLimit-Remaining'] < rate_limit ):
    ...
    for item in response_data:
        if item["type"] == "file":
            ...
            if flat:
                ...
                download_github_dir(owner, repo, item["path"], branch, save_dir, flat, force, rate_limit)
