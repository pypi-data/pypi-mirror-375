...
class SystemMetrics(Base):
    ...
    id = Column(Integer, primary_key=True)
    timestamp = Column(DateTime, default=lambda: datetime.now(hora), nullable=False)
    
    # CPU metrics
    cpu_percent = Column(Float, nullable=True)  # Porcentaje de uso de CPU
    cpu_count_physical = Column(Integer, nullable=True)  # Número de cores físicos
    cpu_count_logical = Column(Integer, nullable=True)  # Número de cores lógicos
    
    # Memory metrics
    memory_total_gb = Column(Float, nullable=True)  # Total de memoria en GB
    memory_available_gb = Column(Float, nullable=True)  # Memoria disponible en GB
    memory_used_gb = Column(Float, nullable=True)  # Memoria usada en GB
    memory_percent = Column(Float, nullable=True)  # Porcentaje de uso de memoria
    
    # Disk metrics
    disk_total_gb = Column(Float, nullable=True)  # Total de disco en GB
    disk_used_gb = Column(Float, nullable=True)  # Disco usado en GB
    disk_free_gb = Column(Float, nullable=True)  # Disco libre en GB
    disk_percent = Column(Float, nullable=True)  # Porcentaje de uso de disco
    
    # GPU metrics (JSON for multiple GPUs)
    gpu_info = Column(JSON, nullable=True)  # Información de GPUs en formato JSON
    
    # System info
    os_info = Column(JSON, nullable=True)  # Información del sistema operativo en JSON
    
    # Process metrics for NovaTrace
    process_memory_mb = Column(Float, nullable=True)  # Memoria usada por el proceso NovaTrace
    process_cpu_percent = Column(Float, nullable=True)  # CPU usada por el proceso NovaTrace
