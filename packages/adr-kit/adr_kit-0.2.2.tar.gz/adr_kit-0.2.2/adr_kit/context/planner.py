...
class PlanningContext:
    ...
    def create_context_packet(self, task_hint: TaskHint) -> ContextPacket:
        ...
        context_packet = ContextPacket(
            ...
            contract_hash=constraints_contract.metadata.hash if constraints_contract else "",
        ...
        for file_path in adr_files:
            try:
                ...
                if adr:
                    ...
                    if not self.config.include_superseded and adr.front_matter.status == ADRStatus.SUPERSEDED:
                        continue
                    adrs.append(adr)
            except ParseError:
                # Skip malformed ADRs
    ...
    def _find_relevant_adrs(self, all_adrs: List[ADR], task_context: TaskContext) -> List[ContextualADR]:
    ...
    def _extract_hard_constraints(self, contract: Optional[Any]) -> Dict[str, Any]:
        ...
        if contract.constraints.imports:
            ...
            if contract.constraints.imports.disallow:
                constraints["imports"]["disallow"] = contract.constraints.imports.disallow
        ...
        if contract.constraints.boundaries:
            ...
            if contract.constraints.boundaries.rules:
                constraints["boundaries"]["rules"] = [
                    {"forbid": rule.forbid} for rule in contract.constraints.boundaries.rules
                ]
        ...
        if adr.front_matter.policy:
            ...
            if policy.imports:
                if policy.imports.disallow:
                    constraints.extend([f"Don't use {item}" for item in policy.imports.disallow[:2]])
                if policy.imports.prefer:
                    constraints.extend([f"Prefer {item}" for item in policy.imports.prefer[:2]])
        ...
        if context_packet.token_estimate > self.config.max_token_budget:
            ...
            if len(context_packet.guidance) > 5:
                ...
                context_packet.guidance = sorted(
                    ...
                    key=lambda g: {"critical": 0, "high": 1, "medium": 2, "low": 3}.get(g.priority, 3)
                )[:5]
        ...
        try:
            ...
            return {
                ...
                "statistics": {
                    ...
                    "accepted_adrs": len([adr for adr in all_adrs if adr.front_matter.status == ADRStatus.ACCEPTED]),
                    "proposed_adrs": len([adr for adr in all_adrs if adr.front_matter.status == ADRStatus.PROPOSED])
                },
