...
#
"""An utility to convert CSV-based calibration data into binary cal data.
"""
import argparse
import glob
import os
...
def csv_to_iq_cal(filename):
    """Convert filename content to IQcal claas.
    ...
    with open(filename, "r") as handle:
        ...
        iq_cal = uhd.usrp.cal.IQCal(metadata["name"], metadata["serial"], metadata["timestamp"])
        for line in handle.readlines():
            lo_freq, corr_real, corr_imag, supp_abs, supp_delta = line.split(",")
...
def main():
    ...
    for filename in file_list:
        ...
        uhd.usrp.cal.database.write_cal_data(key, iq_cal.get_serial(), iq_cal.serialize())
    return 0
