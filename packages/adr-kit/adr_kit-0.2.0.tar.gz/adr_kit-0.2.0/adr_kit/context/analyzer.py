...
class TaskContext(object):
    ...
    def estimated_complexity(self) -> str:
        ...
        complex_words = ["refactor", "migrate", "integrate", "architecture", "framework"]
    ...
    def get_architectural_scope(self) -> List[str]:
        ...
        # Frontend indicators
        if any(tech in self.technologies for tech in ["react", "vue", "angular", "frontend", "ui", "component"]):
            scope.append("frontend")
        
        # Backend indicators  
        if any(tech in self.technologies for tech in ["api", "server", "backend", "database", "service"]):
            scope.append("backend")
        
        # Data indicators
        if any(tech in self.technologies for tech in ["database", "sql", "nosql", "data", "migration"]):
            scope.append("data")
        
        # Integration indicators
        if any(tech in self.technologies for tech in ["api", "webhook", "integration", "service", "external"]):
            scope.append("integration")
        
        # Infrastructure indicators
        if any(tech in self.technologies for tech in ["deploy", "ci", "docker", "kubernetes", "aws", "cloud"]):
...
class TaskAnalyzer:
    ...
    def analyze_task(self, task_hint: TaskHint) -> TaskContext:
        ...
        # Detect technologies
        technologies = self._extract_technologies(description, task_hint.technologies_mentioned or [])
    ...
    def _extract_technologies(self, description: str, mentioned_techs: List[str]) -> Set[str]:
    ...
    def _classify_task_type(self, description: str, explicit_type: Optional[str]) -> TaskType:
    ...
    def _extract_priority_indicators(self, description: str) -> List[str]:
        ...
        priority_words = [
            ...
            "important", "high priority", "deadline", "soon", "quickly"
        ]
    ...
    def _extract_complexity_indicators(self, description: str) -> List[str]:
        ...
        complexity_words = [
            "refactor", "architecture", "migrate", "complex", "integration",
            "framework", "system", "multiple", "across", "entire", "major"
        ]
