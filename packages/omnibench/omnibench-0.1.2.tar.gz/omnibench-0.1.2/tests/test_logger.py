...
from omnibench.logging.logger import (
    LogEntry,
    BenchmarkLog,
    BenchmarkLogger
)
from omnibench.logging.evaluator import (
    BaseEvaluator,
    FloatEvaluator,
    BooleanEvaluator
)
...
class BenchmarkLoggerTests:
    ...
    def test_benchmark_logger_serialization(self) -> TestResult:
        ...
        try:
            ...
            deserialized_logger = BenchmarkLogger.from_json(json_without_eval)
            self.runner.assert_equal(len(deserialized_logger), 1)
            self.runner.assert_true(deserialized_logger.has_log(benchmark_id, objective_id))
            
            return TestResult(
                name="BenchmarkLogger Serialization",
                status=TestStatus.PASS,
                message="✓ Serialization and deserialization work correctly",
                details=f"JSON size with eval: {len(json_with_eval)}, without eval: {len(json_without_eval)}, deserialized logs: {len(deserialized_logger)}",
