...
class S3Segmentor(MircatSegmentor):
    ...
    def _load_nifti_batch(self, nifti_batch: list[Path], temp_dir: Path):
        ...
        bad_niftis = []
        for nifti_path in nifti_batch:
            logger.info('Loading {} from s3', nifti_path)
            try:
    ...
    def _clear_temp_dir(self, temp_dir: Path):
        ...
        if not temp_dir.exists():
            logger.warning('Temporary directory {} does not exist, skipping cleanup', temp_dir)
            return
        for item in temp_dir.iterdir():
            if item.is_dir():
                try:
                    ...
                    logger.error('Error removing directory {}: {}', item, e)
            elif item.is_file():
                try:
                    item.unlink()
                except Exception as e:
                    logger.error('Error removing file {}: {}', item, e)
        logger.debug('Temporary directory {} cleared', temp_dir)
    ...
    def _postprocess_nifti(self, nifti):
        ...
        with threadpool_limits(self.threads_per_process):
            for task, task_files in nifti.task_files.items():
                ...
                if tmp_output_nifti.exists():
                    logger.info('copying {} to {}', tmp_output_nifti, resampled_name)
