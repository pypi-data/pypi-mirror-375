...
impl FixedPoint {
    ...
    /// Get the raw i64 value (for internal use)
    pub fn raw(&self) -> i64 {
        self.0
    }
}
...
mod tests {
    ...
    fn test_from_string() {
        ...
        assert_eq!(FixedPoint::from_str("50000.12345678").unwrap().0, 5000012345678);
        assert_eq!(FixedPoint::from_str("-1.5").unwrap().0, -SCALE - SCALE / 2);
    }
    ...
    fn test_round_trip() {
        ...
        for val in &test_values {
            ...
            let expected = if val.contains('.') {
                format!("{:0<10}", format!("{}.00000000", val.split('.').next().unwrap()))
                    .replace(&format!("{}.00000000", val.split('.').next().unwrap()),
                            &format!("{}.{:0<8}", val.split('.').next().unwrap(), val.split('.').nth(1).unwrap_or("")))
            } else {
                format!("{}.00000000", val)
            };
            
            // For simple cases, just verify round-trip works
