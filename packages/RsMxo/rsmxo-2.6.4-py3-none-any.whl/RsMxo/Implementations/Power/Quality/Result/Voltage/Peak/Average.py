...
class AverageCls:
	...
	def get(self, power=repcap.Power.Default) -> float:
		"""POWer<*>:QUALity:RESult:VOLTage:PEAK:AVERage \n
		Snippet: value: float = driver.power.quality.result.voltage.peak.average.get(power = repcap.Power.Default) \n
		...
		response = self._core.io.query_str(f'POWer{power_cmd_val}:QUALity:RESult:VOLTage:PEAK:AVERage?')
