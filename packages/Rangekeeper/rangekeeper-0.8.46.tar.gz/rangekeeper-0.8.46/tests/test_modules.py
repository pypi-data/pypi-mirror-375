...
class TestStream:
    ...
    def test_stream_validity(self):
        ...
        assert TestStream.flow1.total().magnitude == approx(
            TestStream.stream._resampled_flows[0].sum()
        )
        assert TestStream.flow2.total().magnitude == approx(
            TestStream.stream._resampled_flows[1].sum()
        )
        assert TestStream.flow3.total().magnitude == approx(
            TestStream.stream._resampled_flows[2].sum()
        )
    ...
    def test_stream_resampling(self):
        acq_span = rk.duration.Span(
            name="acq_span",
            start_date=datetime.date(2025, 9, 1),
            end_date=datetime.date(2025, 9, 30),
        )
        prelim_span = rk.duration.Span(
            name="prelim_span",
            start_date=datetime.date(2025, 9, 1),
            end_date=datetime.date(2026, 8, 31),
        )
        constr_span = rk.duration.Span(
            name="constr_span",
            start_date=datetime.date(2026, 9, 1),
            end_date=datetime.date(2027, 8, 31),
        )
        acq_flow = rk.flux.Flow.from_projection(
            name="acq_flow",
            value=-4616497.44,
            proj=rk.projection.Distribution(
                form=rk.distribution.Uniform(),
                sequence=acq_span.to_sequence(frequency=rk.duration.Type.MONTH),
            ),
            units=currency.units,
        )
        prelim_flow = rk.flux.Flow.from_projection(
            name="prelim_flow",
            value=-1996326.19,
            proj=rk.projection.Distribution(
                form=rk.distribution.Uniform(),
                sequence=prelim_span.to_sequence(frequency=rk.duration.Type.MONTH),
            ),
            units=currency.units,
        )
        constr_flow = rk.flux.Flow.from_projection(
            name="constr_flow",
            value=-7875000.00,
            proj=rk.projection.Distribution(
                form=rk.distribution.Uniform(),
                sequence=constr_span.to_sequence(frequency=rk.duration.Type.MONTH),
            ),
            units=currency.units,
        )
        ...
        stream = rk.flux.Stream(
            name="stream",
            flows=[acq_flow, prelim_flow, constr_flow],
            frequency=rk.duration.Type.BIWEEK,
        )
        stream.display()
        # print(f"Stream Index: \n{stream.index}")

        index = stream.index.end_time
        # print(f"Reindexing to: \n{index}")

        resampled = acq_flow.movements.resample(
            rule=rk.duration.Type.offset(rk.duration.Type.DAY),
            origin=stream.index[0].start_time,
        ).sum()
        print(f"\nAcq Flow Resampled: \n{resampled}")

        # print(
        #     acq_flow.movements.reindex(
        #         index=index,
        #         method="ffill",
        #         limit=1,
        #         fill_value=0,
        ...
        # print(
        #     acq_flow.movements.to_period(freq="2W-SUN")
        #     .to_timestamp(freq="2W-SUN", how="end")
        #     .resample(rule="2W-SUN", origin=stream.start_date)
        #     .sum()
        #     .to_period(freq="2W-SUN")
        #     .to_string()
        # )
        # print(
        #     prelim_flow.movements.to_period(freq="2W-SUN")
        #     .to_timestamp(freq="2W-SUN", how="end")
        #     .resample(rule="2W-SUN", origin=stream.start_date)
        #     .sum()
        #     .to_period(freq="2W-SUN")
        ...
        # )
        # print(
        #     constr_flow.movements.to_period(freq="2W-SUN")
        #     .to_timestamp(freq="2W-SUN", how="end")
        #     .resample(rule="2W-SUN", origin=stream.start_date)
        #     .sum()
        #     .to_period(freq="2W-SUN")
        #     .to_string()
        # )
        # print(prelim_flow.to_periods(index=stream.index))
        # print(constr_flow.to_periods(index=stream.index))
