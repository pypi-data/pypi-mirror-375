...
class GateResult:
    ...
    def get_agent_guidance(self) -> str:
        ...
        if self.decision == GateDecision.ALLOWED:
            ...
            return (f"🛑 ADR Required: {self.reasoning}. Please draft an ADR for '{self.choice.name}' "
                   f"and request human approval before proceeding with implementation.")
        
        elif self.decision == GateDecision.BLOCKED:
            return f"❌ Blocked: {self.reasoning}. Do not implement '{self.choice.name}'."
        
        elif self.decision == GateDecision.CONFLICT:
            return (f"⚠️ Conflict: {self.reasoning}. Consider using the recommended alternative "
...
class PolicyGate:
    ...
    def evaluate_from_text(
        self,
        description: str,
        choice_hints: Optional[Dict[str, Any]] = None
    ) -> GateResult:
    ...
    def _parse_choice_description(self, description: str, hints: Dict[str, Any]) -> TechnicalChoice:
        ...
        if any(word in description_lower for word in ["install", "add", "use package", "import", "require"]):
            choice_type = "dependency"
        elif any(word in description_lower for word in ["framework", "library", "adopt", "switch to"]):
            choice_type = "framework"
        else:
        ...
        npm_packages = re.findall(r"@?[a-zA-Z0-9-_]+/[a-zA-Z0-9-_]+|[a-zA-Z0-9-_]+", description)
        ...
        if quoted_names:
            ...
            common_words = {"use", "add", "install", "with", "for", "from", "to", "the", "a", "an"}
                ...
                for preferred in contract.constraints.imports.prefer:
                    if self.engine._are_similar_choices(choice_name, preferred):
                        recommendations["alternatives"].append({
                            ...
                            "source_adr": self._find_source_adr(contract, f"imports.prefer.{preferred}")
                        })
                ...
                for disallowed in contract.constraints.imports.disallow:
                    if choice_name.lower() == disallowed.lower():
                        recommendations["conflicts"].append({
                            ...
                            "source_adr": self._find_source_adr(contract, f"imports.disallow.{disallowed}")
                        })
