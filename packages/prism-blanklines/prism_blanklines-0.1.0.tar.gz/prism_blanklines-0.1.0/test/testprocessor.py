...
"""
    with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
      ...
      result = FileProcessor.processFile(Path(f.name), checkOnly=False, config=BlankLineConfig.fromDefaults())
...
def func():
  pass"""
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
      ...
      result = FileProcessor.processFile(Path(f.name), checkOnly=True, config=BlankLineConfig.fromDefaults())
  ...
  def testFileReadError(self):
    ...
    result = FileProcessor.processFile(nonexistent_path, checkOnly=False, config=BlankLineConfig.fromDefaults())
