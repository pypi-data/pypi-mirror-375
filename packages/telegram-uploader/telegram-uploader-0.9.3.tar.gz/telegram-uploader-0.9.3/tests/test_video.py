...
from telegram_uploader.video import call_ffmpeg, get_video_size, get_video_thumb


class TestcallFfmpeg(unittest.TestCase):
    @patch('telegram_uploader.video.subprocess.Popen', side_effect=FileNotFoundError)
...
class TestGetVideoSize(unittest.TestCase):
    @patch('telegram_uploader.video.call_ffmpeg')
...
class TestGetVideoThumb(unittest.TestCase):
    @patch('telegram_uploader.video.video_metadata')
    @patch('telegram_uploader.video.get_video_size', return_value=[1920, 1080])
    @patch('telegram_uploader.video.call_ffmpeg')
