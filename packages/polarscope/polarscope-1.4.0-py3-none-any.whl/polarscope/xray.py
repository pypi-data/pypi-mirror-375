...
def xray(
    ...
    *,
    ...
    samples : bool, default False
        Include sample metrics (OPT-IN section): n_random.
    ...
    if samples:
        sample_cols = ['n_random']  # Add sample-related columns when implemented
    ...
    if is_expanded:
        ...
        return _build_minimal_gt_table(final_df, df.height, df.width, df, execution_time_ms, corr_target, decimals, sep_mark, dec_mark, compact, title, quality_test, basic_statistics, counts_ratios, distribution)
...
def _build_minimal_gt_table(
    ...
    if basic_statistics:
        basic_cols.extend(["dtype", "count", "mean", "std", "min", "0.25", "0.5", "0.75", "max", "iqr"])
    
    # Counts & Ratios section
    if counts_ratios:
        essential_cols = ["null_count", "null_pct", "zero_count", "zero_pct", "unique_count", "unique_pct", "outlier_count", "outlier_pct", "duplicates_count", "duplicates_pct", "positive_pct", "negative_pct"]
    
    # Distribution section
    if distribution:
        essential_cols.extend(["variance", "skew", "kurtosis", "mad"])
        # Add histogram for Great Tables (first column in Distribution section)
        if "histogram" in summary_df.columns:
            essential_cols.insert(-4, "histogram")  # Insert before variance, skew, kurtosis, mad
...
def _build_expanded_gt_table(
    ...
    gt_table = (
        ...
        .fmt_number(columns=[c for c in ["variance", "skew", "kurtosis"] if c in summary_df.columns], decimals=3, sep_mark=sep_mark, dec_mark=dec_mark)
        ...
        .cols_align(align="left", columns=[str(c) for c in (["Column"] + (["dtype_optimal"] if "dtype_optimal" in summary_df.columns else []) + (["quality_assessment"] if "quality_assessment" in summary_df.columns else []) + (["usability_flags", "recommendation"] if quality_test else []) + test_cols)])
        .tab_options(
