    ...
    *models: Union[nn.Module, Model],
    ...
    if device is None:
        if isinstance(models[0], Model):
            device = models[0].device.type
        else:
            device = "cuda" if torch.cuda.is_available() else "cpu"
    use_fused = is_fused_available() and device == "cuda"
    ...
    return optim.AdamW(
        ...
        betas=betas,
        eps=eps,
        fused=(use_fused and use_fused_if_available) or None,
    )
...
def get_optimizer(
    ...
    **kwargs,
    ...
    return optimizer(
        optim_groups, lr=lr, **filter_kwargs(optimizer, False, ["params"], **kwargs)
    )
