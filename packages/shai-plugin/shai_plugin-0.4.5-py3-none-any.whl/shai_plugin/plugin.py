...
from shai_plugin.config.logger_format import plugin_logger_handler
from shai_plugin.core.entities.message import InitializeMessage
...
from shai_plugin.core.plugin_executor import PluginExecutor
...
from shai_plugin.core.server.io_server import IOServer
from shai_plugin.core.server.router import Router
from shai_plugin.core.server.serverless.request_reader import ServerlessRequestReader
...
class Plugin(IOServer, Router):
    ...
    def _launch_local_stream(self, config: ShaiPluginEnv) -> tuple[RequestReader, ResponseWriter | None]:
        """
        Launch local stream
        """
    ...
    def _launch_remote_stream(self, config: ShaiPluginEnv) -> tuple[RequestReader, ResponseWriter | None]:
        """
    ...
    def _launch_serverless_stream(self, config: ShaiPluginEnv) -> tuple[RequestReader, ResponseWriter | None]:
        """
    ...
    def _execute_request(
        ...
        session = Session(
            ...
            shai_plugin_daemon_url=self.config.SHAI_PLUGIN_DAEMON_URL,
    ...
    @staticmethod
    def _get_remote_install_host_and_port(config: ShaiPluginEnv) -> tuple[str, int]:
        """
        Get host and port for remote installation
        :param config: Shai plugin env config
