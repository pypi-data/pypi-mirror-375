...
class Flow:
    ...
    def to_periods(self, index: pd.PeriodIndex) -> pd.Series:
        """
        Returns a pd.Series (with PeriodIndex) of movements resampled and aligned to the specified index,
        """
        frequency = rk.duration.Type.from_value(index.freqstr)

        resampled = self.movements.resample(
            rule=rk.duration.Type.offset(frequency),
            label="right",
            origin=index[0].start_time,
        ).sum()
        reindexed = resampled.reindex(
            index=index.to_timestamp(how="end"),
            method="ffill",
            limit=1,
        )
        return reindexed.to_period(freq=rk.duration.Type.offset(frequency))
...
class Stream:
    ...
    def _format_flows(
        ...
        for i in range(len(self.flows)):
            ...
            series = self.frame[self.frame.columns[i]]
            formatted_flows.append(
                _format_series(
                    series=series,
                    units=self.flows[i].units,
                    to_datestamps=False,
                    decimals=decimals,
                )
            )
