...
def model_fields_to_controls(
    ...
    :type exclude_list: list
    :param exclude_pk: Необходимость исключить поля первичного ключа из формы
    ...
    if field_list:
        ...
        if exclude_pk:
            exclude_fields.append(model._meta.pk.name)

        is_valid = (lambda fn: lambda x: not fn(x))(
            make_checker(list(exclude_list or ()) + exclude_fields))
