"""
Functionality for sending data to Koito Illumiere.

This module is not part of the core SDK.
"""
import ctypes
import json
import http.client
...
def activate_illumiere_modules(address: str = "localhost") -> None:
    """
    Activate modules in Cobalt Helius to send data to Koito Illumiere.
    """
    ...
    if response.status == 404:
        conn.close()
        raise ConnectionError("Failed to connect to Cobalt Helius.")
    conn.close()
...
class IllumiereObject(ctypes.LittleEndianStructure):
    """
    Represents an object detected by the Illumiere system.
    """
    _pack_ = 1
    _fields_ = [
