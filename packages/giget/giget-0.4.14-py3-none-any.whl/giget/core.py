...
def download_single_file(owner, repo, file_path, branch, save_dir=".", force=False, rename=False, rate_limit=60):
...
def download_github_dir(owner, repo, path, branch="main", save_dir=".", flat=False, force=False, rename=False, rate_limit=60):
    ...
    if ( int(response.headers['X-RateLimit-Remaining']) < rate_limit ):
