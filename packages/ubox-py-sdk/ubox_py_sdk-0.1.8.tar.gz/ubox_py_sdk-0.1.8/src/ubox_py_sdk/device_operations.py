...
from .models import RunMode, DeviceButton
...
class ScreenshotOperation(DeviceOperation):
    ...
    def _post_process(self, response_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        label = kwargs.get('label', '')
        local_file_path = os.path.join(img_path, label)
        file_transfer = FileTransferHandler(self.device)
        file_transfer.pull(str(final_path), local_file_path, "file")
        screenshot_info = {
...
class RecordStartOperation(DeviceOperation):
    ...
    def operation_config(self) -> Dict[str, Any]:
        return {
            ...
            "response_checker": check_record_start_response  # 使用开始录制响应检查器
        }

    def _prepare_request(self, **kwargs) -> Dict[str, Any]:
        record_id = f"{self.udid}_{int(time.time())}"
        """准备开始录制请求参数"""
        return {
            "method": self.operation_config["method"],
            "endpoint": self.operation_config["endpoint"],
    ...
    def _post_process(self, response_data: Dict[str, Any], **kwargs) -> str:
        """处理开始录制响应数据"""
        record_id = self._validate_required_field(response_data, 'record_id', '录制ID')
        return record_id
...
class RecordStopOperation(DeviceOperation):
    ...
    def operation_config(self) -> Dict[str, Any]:
        return {
            ...
            "response_checker": check_standard_response  # 使用标准响应检查器
        }
    ...
    def _prepare_request(self, **kwargs) -> Dict[str, Any]:
        ...
        if not record_id:
            raise UBoxValidationError("录制ID不能为空")

        return {
            "method": self.operation_config["method"],
            "endpoint": self.operation_config["endpoint"],
            "params": self._build_request_params(record_id=record_id)
        }

    def _post_process(self, response_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        """处理停止录制响应数据"""
        # 如果设置了video_path，则下载视频文件
        video_path = self.device.video_path
        if video_path:
            try:
                video_url = _extract_field(response_data, 'videoUrl', '')
                if video_url:
                    self._download_file(video_url, video_path, 300)
                    logger.info(f"视频文件已下载到: {video_path}")
                else:
                    logger.warning("返回数据中没有找到videoUrl，无法下载视频文件")
            except Exception as e:
        ...
        # 返回视频信息
        return {
            "videoUrl": _extract_field(response_data, 'videoUrl', ''),
            "localUrl": _extract_field(response_data, 'localUrl', ''),
            "video_path": video_path,
            "fileKey": _extract_field(response_data, 'fileKey', ''),
            "size": _extract_field(response_data, 'size', 0)
        }
...
class DeviceInfoOperation(DeviceOperation):
    ...
    def _post_process(self, response_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if 'result' in response_data:
            ...
            logger.info(f"设备 {self.udid} 获取到result数据: {device_data}")
            return device_data
        # 如果直接返回设备信息，直接使用
        elif isinstance(response_data, dict):
            logger.info(f"设备 {self.udid} 直接返回设备数据: {response_data}")
            return response_data
        else:
            raise UBoxDeviceError(
                f"无法解析{self.operation_config['name']}返回体: {response_data}"
...
class ClickOperation(DeviceOperation):
    ...
    def _prepare_request(self, **kwargs) -> Dict[str, Any]:
        ...
        for key, value in kwargs.items():
            if key not in ['loc', 'by', 'offset', 'timeout', 'duration', 'times']:
...
class LongClickOperation(DeviceOperation):
    ...
    def _prepare_request(self, **kwargs) -> Dict[str, Any]:
        ...
        # 添加其他参数
        for key, value in kwargs.items():
            if key not in ['loc', 'by', 'offset', 'timeout', 'duration']:
                request_data[key] = value
...
class SlideOperation(DeviceOperation):
    ...
    def _prepare_request(self, **kwargs) -> Dict[str, Any]:
        ...
        # 添加其他参数
        for key, value in kwargs.items():
            if key not in ['loc_from', 'loc_to', 'loc_shift', 'by', 'timeout', 'down_duration', 'up_duration',
                           'velocity']:
                request_data[key] = value
...
class FindOCROperation(DeviceOperation):
    ...
    def _prepare_request(self, **kwargs) -> Dict[str, Any]:
        ...
        # 添加其他参数
        for key, value in kwargs.items():
            if key not in ['word', 'left_word', 'right_word', 'timeout', 'time_interval']:
                request_data[key] = value
...
class MultiFindOperation(DeviceOperation):
    ...
    def _prepare_request(self, **kwargs) -> Dict[str, Any]:
        ...
        # 添加其他参数
        for key, value in kwargs.items():
            if key not in ['ctrl', 'img', 'pos', 'by', 'ctrl_timeout', 'img_timeout']:
                request_data[key] = value
...
class CreateRemoteDirOperation(DeviceOperation):
    ...
    def _post_process(self, response_data: Dict[str, Any], **kwargs) -> Optional[str]:
        """处理清除事件自动处理规则响应数据"""
        if 'temp_dir_path' not in response_data:
            raise UBoxDeviceError(f"无法解析{self.operation_config['name']}返回体: {response_data}")
        return response_data['temp_dir_path']
    ...
    def push_direct(self, file_path: str, target_path: str) -> str:
        ...
        try:
            ...
            if hasattr(response, 'status_code') and response.status_code == 200:
                logger.info(f"文件直接推送成功: {file_path} -> {target_path}")
                return target_path
            else:
    ...
    def pull_direct(self, target_path: str, save_path: str, path_type: str) -> str:
        ...
        if hasattr(response, 'status_code') and response.status_code == 200:
            ...
            try:
                ...
                logger.info(f"文件直接拉取成功: {target_path} -> {save_path}")
                return save_path
