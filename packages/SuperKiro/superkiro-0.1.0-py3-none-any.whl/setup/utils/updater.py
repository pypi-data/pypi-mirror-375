"""
Auto-update checker for SuperKiro Framework
...
class UpdateChecker:
    """Handles automatic update checking for SuperKiro"""
    
    PYPI_URL = "https://pypi.org/pypi/SuperKiro/json"
    CACHE_FILE = Path.home() / ".kiro" / ".update_check"
    ...
    def get_latest_version(self) -> Optional[str]:
        """
        Query PyPI for the latest version of SuperKiro
        ...
        try:
            ...
            req = urllib.request.Request(
                self.PYPI_URL,
                headers={'User-Agent': 'SuperKiro-Updater'}
            )
    ...
    def detect_installation_method(self) -> str:
        """
        Detect how SuperKiro was installed (pip, pipx, etc.)
        ...
        try:
            ...
            if 'SuperKiro' in result.stdout or 'superkiro' in result.stdout:
                return 'pipx'
        except:
            pass
        ...
        try:
            result = subprocess.run(
                [sys.executable, '-m', 'pip', 'show', 'SuperKiro'],
                capture_output=True,
                text=True,
                timeout=2
            )
    ...
    def get_update_command(self) -> str:
        ...
        commands = {
            'pipx': 'pipx upgrade SuperKiro',
            'pip-user': 'pip install --upgrade --user SuperKiro',
            'pip': 'pip install --upgrade SuperKiro',
            'unknown': 'pip install --upgrade SuperKiro'
        }
    ...
    def perform_update(self) -> bool:
        ...
        print(f"{Colors.CYAN}🔄 Updating SuperKiro...{Colors.RESET}")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"{Colors.YELLOW}Please restart SuperKiro to use the new version.{Colors.RESET}")
                return True
            else:
    ...
    def check_and_notify(self, force: bool = False, auto_update: bool = False) -> bool:
        ...
        if os.getenv('SUPERKIRO_NO_UPDATE_CHECK', '').lower() in ['true', '1', 'yes']:
            return False
            
        # Check if auto-update is enabled via environment
        if os.getenv('SUPERKIRO_AUTO_UPDATE', '').lower() in ['true', '1', 'yes']:
            auto_update = True
