...
class IoperatorCls:
	...
	def set(self, frame: str, field: str, operator: enums.OperatorA) -> None:
		"""TRIGger:SBSW:SWIRe:IOPerator \n
		Snippet: driver.trigger.sbsw.swire.ioperator.set(frame = 'abc', field = 'abc', operator = enums.OperatorA.ANY) \n
		...
		self._core.io.write(f'TRIGger:SBSW:SWIRe:IOPerator {param}'.rstrip())

	# noinspection PyTypeChecker
	def get(self) -> enums.OperatorA:
		"""TRIGger:SBSW:SWIRe:IOPerator \n
		Snippet: value: enums.OperatorA = driver.trigger.sbsw.swire.ioperator.get() \n
		No command help available \n
			:return: operator: No help available"""
		response = self._core.io.query_str(f'TRIGger:SBSW:SWIRe:IOPerator?')
