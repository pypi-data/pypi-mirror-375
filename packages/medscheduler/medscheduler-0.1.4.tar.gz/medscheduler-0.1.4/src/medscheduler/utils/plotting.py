...
def plot_future_slot_availability(
    ...
    *,
    ...
    available_col: str = "is_available",
    freq: str = "W",
    limit_future_to_horizon: bool = True,  # noop según el test
    min_bar_px: int = 55,
    label_px_threshold: int = 55,
    min_fig_w_in: float = 7.0,
    max_fig_w_in: float = 16.0,
    ...
    # El test indica que limitar al horizonte es noop por ahora
    df = df[df[date_col] >= ref_date]
    ...
    # 🔒 Clave: si piden 'D', NO auto-agrupar. Bloqueamos allowed a sólo 'D'.
    if freq == "D":
        allowed = ("D",)
    elif freq == "W":
        # Si quieren W, permitimos bajar a D si hace falta, pero nunca subir a M.
        allowed = ("W", "D")
    else:  # freq == "M"
        # Si quieren M, permitimos bajar a W/D si hace falta.
        allowed = ("M", "W", "D")
    ...
    # Sólo aplica a mensual; no es el caso del test, pero mantenemos el guard.
    if figsize[0] >= max_fig_w_in and (n * min_bar_px) / dpi > max_fig_w_in and used_freq == "M":
        return _empty_plot("Too many bars for a readable chart at any granularity.")

    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)

    # Extraer series 1D, alineadas a 'periods'
    ...
    # Si usamos exactamente la freq pedida, NO poner "auto-aggregated"
...
def _extract_group_1d(grouped: Any, key: Any, periods: Sequence) -> np.ndarray:
    """
    Devuelve un vector 1D (len(periods),) para la clave pedida.
    - Si no existe la clave -> vector de ceros.
    - Si es DataFrame con varias columnas -> sumar por filas (axis=1).
    - Alinear siempre al índice 'periods'.
    """
    try:
        obj = grouped.get(key)
    except Exception:
