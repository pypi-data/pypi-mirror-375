...
def _add_task_id_and_filename_to_save_path(
    save_path: Path, task_id: Optional[str]
) -> Path:
    ...
    if save_path.suffix != ".csv":
        if task_id:
            ...
            Path(save_path / f"{task_id}").mkdir(parents=True, exist_ok=True)
            save_path = save_path / f"{task_id}" / DEFAULT_CSV_FILENAME
        else:
            # Create the output directory if it doesn't exist
            save_path.mkdir(parents=True, exist_ok=True)
            save_path = save_path / DEFAULT_CSV_FILENAME
            i = 1
            while save_path.exists():
                save_path = save_path.parent / f"results ({i}).csv"
                i += 1
    else:
        # Create the output directory if it doesn't exist
        save_path.parent.mkdir(parents=True, exist_ok=True)
        filename = save_path.stem
        extension = save_path.suffix  # Includes the dot of the extension
        i = 1
        while save_path.exists():
            save_path = save_path.parent / f"{filename} ({i}){extension}"
            i += 1
...
class _ModellerSide(BaseModellerProtocol):
    ...
    async def run(
        ...
        if SaveLocation.Modeller in self.save_location:
            _save_results(
                modeller_results,
                _add_task_id_and_filename_to_save_path(self.save_path, self.task_id),
            )
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if save_path is None:
            ...
            self.save_path = Path(save_path).expanduser().absolute().resolve()
