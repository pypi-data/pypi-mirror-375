...
class IminCls:
	...
	def set(self, index_min: int, serialBus=repcap.SerialBus.Default, frame=repcap.Frame.Default, field=repcap.Field.Default) -> None:
		"""SBUS<*>:SWIRe:FILTer:FRAMe<*>:FLD<*>:IMIN \n
		Snippet: driver.sbus.swire.filterPy.frame.fld.imin.set(index_min = 1, serialBus = repcap.SerialBus.Default, frame = repcap.Frame.Default, field = repcap.Field.Default) \n
		...
		self._core.io.write(f'SBUS{serialBus_cmd_val}:SWIRe:FILTer:FRAMe{frame_cmd_val}:FLD{field_cmd_val}:IMIN {param}')

	def get(self, serialBus=repcap.SerialBus.Default, frame=repcap.Frame.Default, field=repcap.Field.Default) -> int:
		"""SBUS<*>:SWIRe:FILTer:FRAMe<*>:FLD<*>:IMIN \n
		Snippet: value: int = driver.sbus.swire.filterPy.frame.fld.imin.get(serialBus = repcap.SerialBus.Default, frame = repcap.Frame.Default, field = repcap.Field.Default) \n
		Specifies the index, or sets the start value of an index range. \n
			:param serialBus: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sbus')
			:param frame: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Frame')
			:param field: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Fld')
		...
		frame_cmd_val = self._cmd_group.get_repcap_cmd_value(frame, repcap.Frame)
		field_cmd_val = self._cmd_group.get_repcap_cmd_value(field, repcap.Field)
		response = self._core.io.query_str(f'SBUS{serialBus_cmd_val}:SWIRe:FILTer:FRAMe{frame_cmd_val}:FLD{field_cmd_val}:IMIN?')
