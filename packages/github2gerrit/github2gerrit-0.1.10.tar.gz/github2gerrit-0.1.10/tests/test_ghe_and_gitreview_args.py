...
def test_git_review_args_include_branch_and_repeated_reviewer_flags(
    ...
    monkeypatch.setattr("github2gerrit.core.run_cmd", fake_run_cmd, raising=True)
    ...
    review_calls = [c for c in recorded_cmds if len(c) >= 2 and c[0] == "git" and c[1] == "review"]
    assert review_calls, f"No git-review invocation captured in: {recorded_cmds!r}"
    args = review_calls[-1]
