    ...
    try:
        ...
        if response['Status'] == 'InProgress' and start_time:
            try:
                ...
                # Try to extract estimated time from command comment or previous logs
                # Look for volume information in stdout to estimate progress
                stdout_content = response.get('StandardOutputContent', '')
                
                # Try to find volume information in the stdout
                volume_pattern = r'Starting initialization of (\w+-\w+) \(.*?, (\d+)GB\)'
                volume_matches = re.findall(volume_pattern, stdout_content)
                
                if volume_matches:
                    # Estimate based on volumes found
                    total_gb = sum(int(match[1]) for match in volume_matches)
                    
                    # Rough estimation: assume 100 MB/s average throughput per volume
                    estimated_total_minutes = (total_gb * 1024) / (100 * len(volume_matches))  # MB / (MB/s) = seconds
                    estimated_total_minutes /= 60  # Convert to minutes
                    ...
                    progress_info = {
                        ...
                        "estimated_total_minutes": round(estimated_total_minutes, 1) if estimated_total_minutes > 0 else "Unknown",
