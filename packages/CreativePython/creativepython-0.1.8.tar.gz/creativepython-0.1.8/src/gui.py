...
class EventDispatcher(_QtCore.QObject):
   ...
   def handleMousePress(self, event):
      ...
      i = 0
      while self.draggingItem is None and i < len(candidates):
         item = candidates[i]
         if "mouseDrag" in item._callbackFunctions:  # if item has a mouseDrag event, store it
   ...
   def handleMouseMove(self, event):
      ...
      if self.lastMouseDown is not None:  # mouse is down, so this is a drag event
         ...
         # last, drag events to the dragging item
         if self.draggingItem is not None:
            moveHandled = self.draggingItem._event("mouseDrag", [x,y])
         else:
            moveHandled = False
   ...
   def handleKeyPress(self, event):
      ...
      for item in self.display.items:
         keyDownHandled = item._event("keyDown", [key])
         keyTypeHandled = item._event("keyType", [char])
         if keyDownHandled or keyTypeHandled:  
            eventHandled = True  # if either event got handled by any item, report it
   ...
   def handleKeyRelease(self, event):
      ...
      # last, we send event to each item
      eventHandled = False

      for item in self.display.items:
         keyUpHandled = item._event("keyUp", [key])
         if keyUpHandled:  
            eventHandled = True  # if the event got handled by any item, report it
