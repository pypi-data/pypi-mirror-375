    ...
    try:
        ...
        for network in private_networks:
            if addr1 in network and addr2 in network:
                # 进一步检查是否在同一个/24子网内（通常表示同一个广播域）
                subnet1 = ipaddress.IPv4Network(f'{addr1}/24', strict=False)
                subnet2 = ipaddress.IPv4Network(f'{addr2}/24', strict=False)
...
class TcpP2PManager:
    ...
    def update_peers(self, peer_list: List[Dict]):
        ...
        logger.debug(f"Peers: {list(self.peers.keys())}")
    ...
    async def try_direct_send(self, target_virtual_ip: str, data: bytes) -> bool:
        ...
        if target_virtual_ip not in self.peers:
            logger.debug(f"No peer info for {target_virtual_ip}, using server relay")
            return False
        ...
        if not is_local_network(self.local_ip, private_ip):
            logger.debug(f"Peer {target_virtual_ip}@{private_ip} not in local network, using server relay")
            return False
