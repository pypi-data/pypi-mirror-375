...
# import the config file as cfg, allowing for different locations
# such as src, src.proj.core, src.proj.src
app_src_dir = os.path.dirname(__file__)
wkdir = os.getcwd()
relative_path = os.path.relpath(app_src_dir, wkdir)
# import xxx.config as cfg
cfg = importlib.import_module('.config',relative_path)
...
class ConfigParmsExt(ConfigParms):
    ...
    def set_directories(self,) -> None:
        ...
        if cfg.wkdir is None:
            ...
            config_spec = importlib.util.find_spec(f"{relative_path}.config")
