...
class CategoryRule(BaseModel):
    ...
    category: str = Field(..., description="Category name (e.g., 'runtime_dependency', 'framework')")
    patterns: List[str] = Field(..., description="Regex patterns to match choice names")
    keywords: List[str] = Field(default_factory=list, description="Keywords that indicate this category")
    examples: List[str] = Field(default_factory=list, description="Example choices in this category")
...
class NameMapping(BaseModel):
    ...
    aliases: List[str] = Field(..., description="Alternative names that map to the canonical name")
...
class GateConfig(BaseModel):
    ...
    default_tool_policy: GateDecision = Field(
        ...
        description="Default action for development tools"
    )
    
    # Allow/deny lists
    always_allow: List[str] = Field(
        default_factory=list,
        description="Choices that are always allowed without ADR"
    )
    always_deny: List[str] = Field(
        default_factory=list, 
        description="Choices that are always blocked"
    )
    development_tools: List[str] = Field(
        default_factory=lambda: [
            "eslint", "prettier", "jest", "vitest", "webpack", "vite",
            "typescript", "babel", "rollup", "esbuild", "parcel",
            "pytest", "black", "mypy", "ruff", "pre-commit",
    ...
    name_mappings: List[NameMapping] = Field(
        default_factory=lambda: [
            ...
            NameMapping(canonical_name="@tanstack/react-query", aliases=["react-query", "tanstack-query"]),
    ...
    def categorize_choice(self, choice_name: str) -> Optional[str]:
        ...
        if any(dev_tool in normalized_name for dev_tool in ["test", "lint", "format", "build"]):
            return "development_tool"
