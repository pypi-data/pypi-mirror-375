...
class Utilities:
	...
	Access snippet: ``utils = RsMxo.utilities``"""
	...
	def logger(self) -> ScpiLogger:
		...
		Access snippet: ``logger = RsMxo.utilities.logger``"""
	...
	def get_lock(self) -> threading.RLock:
		...
		By default:
			- If you create standard new RsMxo instance with new VISA session, the session gets a new thread lock. You can assign it to other RsMxo sessions in order to share one physical instrument with a multi-thread access.
			- If you create new RsMxo from an existing session, the thread lock is shared automatically making both instances multi-thread safe.
