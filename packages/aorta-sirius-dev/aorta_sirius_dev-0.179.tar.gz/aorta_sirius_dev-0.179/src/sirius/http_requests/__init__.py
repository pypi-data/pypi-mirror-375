...
from typing import Dict, Any, List, cast, Tuple, BinaryIO
...
class AsyncHTTPSession(HTTPSession):
    ...
    async def post(self, url: str, data: Dict[str, Any] | None = None, headers: Dict[str, Any] | None = None, files: RequestFiles | None = None) -> HTTPResponse:
        ...
        if files:
            form_data = {"payload_json": json.dumps(data)} if data else {}

            processed_files: Dict[str, Tuple[str, BinaryIO, str]] = {}
            for key, (filename, fileobj) in files.items():  # type: ignore[misc, union-attr]
                mime_type, _ = mimetypes.guess_type(filename)  # type: ignore[arg-type]
                processed_files[key] = (filename, fileobj, mime_type or "application/octet-stream")  # type: ignore[assignment]

            http_response = HTTPResponse(await self.client.post(url, data=form_data, files=processed_files, headers=headers))
        else:
