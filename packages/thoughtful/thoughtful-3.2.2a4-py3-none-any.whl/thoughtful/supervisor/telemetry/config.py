...
from thoughtful.supervisor.telemetry.metrics.counters import (
    record_agent_type_workflow_completed,
)
from thoughtful.supervisor.telemetry.metrics.counters import (
    record_agent_type_workflow_failed,
)
from thoughtful.supervisor.telemetry.metrics.counters import (
    record_agent_type_workflow_started,
)
        ...
        try:
            ...
            if context.meter and context.root_span:
                try:
                    ...
                    # Record agent-type workflow started metric if agent_type is available
                    if (
                        config.manifest
                        and hasattr(config.manifest, "agent_type")
                        and config.manifest.agent_type
                    ):
                        record_agent_type_workflow_started(config.manifest.agent_type)
                        logger.debug(
                            "Recorded agent-type workflow started metric for: %s",
                            config.manifest.agent_type,
                        )
...
def shutdown_telemetry_from_context(
    ...
    # Record workflow completion or failure metrics before closing the root span
    if context.meter is not None and context.config.manifest is not None:
        try:
            workflow_name = getattr(context.config.manifest, "uid", "sup")

            # Check if workflow failed (exception occurred)
            if exc_type is not None:
                # Workflow failed - record failure metrics
                logger.debug(
                    "Workflow failed, recording failure metrics for: %s", workflow_name
                )

                # Record agent-type workflow failed metric if agent_type is available
                if (
                    hasattr(context.config.manifest, "agent_type")
                    and context.config.manifest.agent_type
                ):
                    record_agent_type_workflow_failed(
                        context.config.manifest.agent_type
                    )
                    logger.debug(
                        "Recorded agent-type workflow failed metric for: %s",
                        context.config.manifest.agent_type,
                    )
            else:
                ...
                logger.debug(
                    "Recorded workflow completed metric for: %s", workflow_name
                )

                # Record agent-type workflow completed metric if agent_type is available
                if (
                    hasattr(context.config.manifest, "agent_type")
                    and context.config.manifest.agent_type
                ):
                    ...
                    logger.debug(
                        "Recorded agent-type workflow completed metric for: %s",
                        context.config.manifest.agent_type,
                    )
        except Exception as metric_error:
            logger.error(
                "Failed to record workflow completion/failure metric: %s",
                str(metric_error),
            )
            # Continue without workflow completion/failure metric
