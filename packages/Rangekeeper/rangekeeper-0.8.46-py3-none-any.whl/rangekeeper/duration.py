...
class Type(enum.Enum):
    ...
    def from_value(value: str):
        if value in ("10Y", "10YE", "10Y-DEC"):
            return Type.DECADE
        if value in ("5Y", "5YE", "5Y-DEC"):
            return Type.SEMIDECADE
        if value in ("2Y", "2YE", "2Y-DEC"):
            return Type.BIENNIUM
        if value in ("Y", "YE", "YE-DEC", "Y-DEC", "A-DEC"):
            return Type.YEAR
        if value in ("2Q", "2QE", "2Q-DEC"):
            return Type.SEMIYEAR
        if value in ("Q", "QE", "Q-DEC"):
            return Type.QUARTER
        if value in ("M", "ME"):
            return Type.MONTH
        if value in ("2W", "2WE", "2W-SUN"):
            return Type.BIWEEK
        if value in ("W-SUN", "WE", "W"):
            return Type.WEEK
        if value == "D":
            return Type.DAY
        else:
