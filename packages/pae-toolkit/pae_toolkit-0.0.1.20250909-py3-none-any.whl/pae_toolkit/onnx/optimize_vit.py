...
optimize_plans = {
    ...
    "vit_large_patch16_224": ["pad_nz_block"],
}
...
def node_check(node) -> tuple:
    ...
    if isinstance(node, str):
        ...
        raise TypeError(f"Invalid preorder type: {type(node)}!")
...
def pattern_select(
    ...
    preorders: list | None = None,
    successors: list | None = None,
) -> list:
    ret = []
...
def merge_bmm_axis(graph: OnnxGraph, anchor_reshapes: list, anchor_adds: list) -> None:
    ...
    original_shape_init = graph.add_initializer("original_shape", original_shape)
...
def cal_padding_shape(graph: OnnxGraph, merged: bool = False) -> tuple:
    first_concat = graph.get_nodes("Concat")[0]
    bs, hidden_dim2, hidden_dim1 = graph.get_value_info(first_concat.outputs[0]).shape
    bs, hidden_dim2, hidden_dim1 = int(bs), int(hidden_dim2), int(hidden_dim1)
...
def pad_nz_block(
    ...
    new_concat_init = graph.add_initializer("padding_concat_init", np.zeros(padding_shape, dtype=np.float32))
    ...
    for add_node in anchor_adds:
        ...
        graph.add_node(
            ...
            inputs=[
                output_name,
                new_slice_init_starts.name,
                new_slice_init_ends.name,
                new_slice_init_axes.name,
            ],
...
def apply_optimization(onnx_path: str, save_path: str, model_config: str) -> None:
    ...
    assert plan is not None, f"Model config {model_config} not found in optimization plan"
