...
SELECT 
    ...
    ,enc.PatientDurableKey as PATIENT_DURABLE_KEY
    ,enc.EncounterKey as ENCOUNTER_KEY
...
WHERE
    ...
    AND VF.AppointmentStatus {{ filt_encounter_status|inclause_str(set_op = True)}}
    {% endif %}

    {% if filt_encounter_type %}
    AND enc.Type {{ filt_encounter_type|inclause_str(set_op = True) }}
    {% else %}
    AND [IsOutpatientFaceToFaceVisit] = 1
    {% endif %}

    {% if filt_encounter_department %}
    AND Dep.DepartmentName {{ filt_encounter_department|inclause_str(set_op = True)}}
    {% endif %}

    {% if filt_encounter_department_specialty %}
    AND Dep.DepartmentSpecialty {{ filt_encounter_department_specialty|inclause_str(set_op = True) }}
    {% endif %}

    {% if filt_encounter_provider_id %}
    AND prov.ProviderEpicId {{ filt_encounter_provider_id|inclause(set_op = True) }}
    {% endif %}
