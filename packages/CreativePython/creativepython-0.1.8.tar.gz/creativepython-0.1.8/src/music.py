...
class MidiSequence():
   ...
   def play(self, startAtBeat=0.0):
      """Play the MIDI score."""

      # map beats to ms based on base score tempo (ignores per-phrase tempo here on purpose)
      self._positionMs = float(startAtBeat) * (1000.0 * 60.0 / max(1e-9, self.defaultTempo))   # convert beats to milliseconds
      self._positionMs = max(0.0, min(self._positionMs, self._totalDurationMs))   # clamp position to valid range
      self._reindexForPosition()   # position next event index according to current playback position
   ...
   def loop(self, startAtBeat=0.0):
      ...
      self.play(startAtBeat)   # and start playing
   ...
   def stop(self):
      ...
      self._allNotesOff()           # turn off any sounding notes
   ...
   def _reindexForPosition(self):
      """Positions next event index according to current playback position and turns off notes past position."""

      # turn off all currently sounding notes
      Play.allNotesOff()

      # find first event with start >= position using linear scan (events list is small in practice)
      pos = int(self._positionMs)
      self._nextEventIndex = len(self._events)   # default to end of events list

      for i, event in enumerate(self._events):

         if event['startMs'] >= pos:   # is this event at or after current position?
            self._nextEventIndex = i   # yes, so remember its index
            break
   ...
   def _tick(self):
      ...
      if not self._playing:   # is the sequence not playing?
      ...
      while self._nextEventIndex < len(self._events) and self._events[self._nextEventIndex]['startMs'] <= pos:
         event = self._events[self._nextEventIndex]
         transPitch = min(127, max(0, event['pitch'] + self._transposeSemitones))   # apply transpose and clamp to valid range

         Play.setInstrument(event['instrument'], event['channel'])   # set instrument
         Play.setVolume(self.volume, event['channel'])            # set volume

         if event['panning'] != -1:   # is panning specified?
            Play.setPanning(event['panning'], event['channel'])         # yes, so use it
      ...
      if self._positionMs >= self._totalDurationMs:   # have we reached the end?
         ...
         if self._loop and self._totalDurationMs > 0:   # should we loop?
            # wrap position and restart indexing
            self._positionMs = self._positionMs - self._totalDurationMs   # wrap position
            self._nextEventIndex = 0   # restart from beginning
