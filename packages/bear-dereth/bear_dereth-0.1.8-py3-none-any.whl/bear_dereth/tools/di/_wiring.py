...
if TYPE_CHECKING:
    ...
    from ._container import DeclarativeContainer
    ...
    class Provider[T: DeclarativeContainer]:
        ...
        def __class_getitem__(cls, item: Any) -> Provide:
            ...
            if hasattr(item, "service_name"):
                # If item is already a Provide instance (Container.console returns Provide)
                return item
            if hasattr(item, "__name__"):
                # If item is a type/class, use its name
...
def _get_function_signature(func: Callable) -> Signature:
...
def _get_type_hints(name: str, func: Callable) -> Any | None:
...
class Parser:
    ...
    def get_service_instance(service: Any) -> None | type:
        ...
        try:
            ...
            return service  # Return the instance directly
        except Exception:
            return None
    ...
    def _resolve_string_to_type(type_name: str, func: Callable) -> type | None:
        ...
        with suppress(NameError, AttributeError, KeyError, TypeError):
            resolved_type: Any | None = _get_type_hints(type_name, func)
    ...
    def _resolve_annotated(name: str, func: Callable) -> type | None:
        ...
        if origin is not None:
            ...
            if args:
                ...
                # Handle nested generics like Union[Console, str]
    ...
    def _parsing(self) -> Result:
        ...
        if self.is_present:
            cached_instance: Any | None = self.container.get(self.name, expected_type=resolved_type)
            service_instance: None | type = self.get_service_instance(cached_instance)
            if service_instance is not None:
                if self.is_singleton(cached_instance):
                    instance = cached_instance.get()
                    return Result(instance=instance, success=True)
...
def _parse_params(func: Callable, *args, **kwargs) -> tuple[tuple[Any, ...], dict[str, Any]]:
    """Parse the parameters of a function."""
    s: Signature = _get_function_signature(func)
...
def inject[T](func: Callable[..., T]) -> Callable[..., T]:
    ...
    def wrapper(*args, **kwargs) -> T:
        bound_args, bound_kwargs = _parse_params(func, *args, **kwargs)
