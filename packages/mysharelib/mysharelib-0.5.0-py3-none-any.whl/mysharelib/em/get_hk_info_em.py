...
def get_hk_info_em(symbol: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        # 创建基本数据DataFrame
        basic_df = pd.DataFrame(basic_data, columns=["指标名称", "最新数据"])
        ...
        # 创建与上年同期对比的DataFrame
        comparison_df = pd.DataFrame(comparison_data, columns=["指标名称", "最新数据", "上年同期"])
        ...
        print(f"获取数据时出错: {e}")
        # 返回空的DataFrame
        return pd.DataFrame(columns=["指标名称", "最新数据"]), pd.DataFrame(columns=["指标名称", "最新数据", "上年同期"])
