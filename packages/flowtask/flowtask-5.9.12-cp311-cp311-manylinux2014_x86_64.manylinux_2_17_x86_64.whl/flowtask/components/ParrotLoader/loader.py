...
class ParrotLoader(FlowComponent):
    ...
    def _get_loader_by_extension(self, suffix: str, source_path: Path = None) -> AbstractLoader:
        ...
        if hasattr(self, 'loader'):
            ...
            if isinstance(self.path, str) and self.path.startswith(('http://', 'https://', 'ftp://')):
                ...
                if self.path.is_dir():
                        ...
                        for ext in self.extensions:
                            for item in self.path.glob(f'*{ext}'):
                                if item.is_file() and set(item.parts).isdisjoint(self.skip_directories):
                                    loader = self._get_loader_by_extension(item.suffix, item)
                                    docs = await loader.load(item)
                    ...
                    # Process single file
                    if not self.extensions or self.path.suffix.lower() in self.extensions:
                        if set(self.path.parts).isdisjoint(self.skip_directories):
                            loader = self._get_loader_by_extension(self.path.suffix, self.path)
                            documents = await loader.load(self.path)
                else:
                    raise ValueError(
