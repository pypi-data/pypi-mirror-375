...
import pytest
from argsclass.parser import ParserContext, PositionalArgumentParser, OptionArgumentParser, FlagArgumentParser, parse
...
class TestPositionalArgumentParser:
    ...
    def test_parse_invalid_type(self):
        ...
        with pytest.raises(ValueError, match="Error parsing argument"):
            parser.parse(spec, context)
    ...
    def test_parse_wrong_spec_type(self):
        ...
        with pytest.raises(TypeError, match="Expected PositionalArgSpec"):
            parser.parse(spec, context)


class TestFlagArgumentParser:
    ...
    def test_parse_flag_wrong_spec_type(self):
        ...
        with pytest.raises(TypeError, match="Expected FlagArgSpec"):
            parser.parse(spec, context)
    ...
    def test_parse_multiple_flags(self):
        ...
        # Parse verbose flag
        parser.parse(verbose_spec, context)
        assert context["verbose"] is True
        assert context.current_argv == ["--debug", "file.txt"]
        
        # Parse debug flag
        parser.parse(debug_spec, context)
...
class TestOptionArgumentParser:
    ...
    def test_parse_option_invalid_type(self):
        ...
        with pytest.raises(ValueError, match="Error parsing option"):
            parser.parse(spec, context)
    ...
    def test_parse_option_insufficient_values(self):
        ...
        with pytest.raises(ValueError, match="Option '--files' requires a value"):
            parser.parse(spec, context)
    ...
    def test_parse_option_wrong_spec_type(self):
        ...
        with pytest.raises(TypeError, match="Expected OptionArgSpec"):
            parser.parse(spec, context)
    ...
    def test_parse_multiple_options(self):
        ...
        # Parse output option
        parser.parse(output_spec, context)
        assert context["output"] == "file.txt"
        assert context.current_argv == ["--config", "config.json", "positional"]
        
        # Parse config option
        parser.parse(config_spec, context)
    ...
    def test_parse_option_with_exact_count_consecutive_values(self):
        ...
        with pytest.raises(ValueError, match="Not enough values for option 'coords'"):
            parser.parse(spec, context)
    ...
    def test_parse_option_without_value_is_always_error(self):
        ...
        with pytest.raises(ValueError, match="Option '--config' requires a value"):
            parser.parse(spec2, context2)
...
class TestParseFunction:
    ...
    def test_parse_flags_with_case_insensitive_boolean_values(self):
        ...
        for argv, expected in test_cases:
            result = parse(specs, argv)
            assert result == expected, f"Failed for {argv}"
