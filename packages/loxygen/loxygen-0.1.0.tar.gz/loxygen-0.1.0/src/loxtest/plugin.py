...
class Option[T]:
    ...
    value_type: type[T]
    stash_key: pytest.StashKey[T] = field(init=False)

    def __post_init__(self):
        key = pytest.StashKey[self.value_type]()
        object.__setattr__(self, "stash_key", key)
...
OPTIONS: list[Option] = [
    ...
    Option(
        ...
        cli={"type": str, "action": "append"},
        processor=None,
        value_type=list[str],
    ),
]

KEYS = {option.name: option.stash_key for option in OPTIONS}
...
class TestItem(pytest.Item):
    ...
    def runtest(self):
        ...
        failed_lines = tuple(
            expected.lineno
            for output, expected in zip(self.output, self.expected, strict=True)
    ...
    def run_lox(self):
        cmd = self.config.stash[KEYS["interpreter_cmd"]] + [self.path]
        try:
    ...
    def add_result(self) -> list[str]:
        ...
        for result, output in zip(self.expected, self.output, strict=True):
...
class LoxFile(pytest.File):
    ...
    def collect(self):
        ...
        skip_dirs = self.parent.config.stash[KEYS["skip_dirs"]]
...
def pytest_addoption(parser: pytest.Parser):
    for option in OPTIONS:
        ...
        parser.addoption(f"--{option.name}", help=option.help, default=None, **option.cli)
...
def pytest_terminal_summary(
    ...
    if backend_errors:
        ...
        for error, count in Counter(backend_errors).items():
            terminalreporter.line(f"{error} was raised {count} time{'s' if count > 1 else ''}.")
