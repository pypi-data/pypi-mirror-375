    ...
    def generate_rid(
        ...
        if isinstance(resource_type, str):
            try:
                ...
                valid_types = [t.value for t in SthgResourceType]
                raise ValueError(f"无效的资源类型字符串 '{resource_type}'，可选值: {valid_types}")
        ...
        return f"ri.{found_scope.lower()}.{namespace.lower()}.{resource_type.value.lower()}.{rid_uuid}"
