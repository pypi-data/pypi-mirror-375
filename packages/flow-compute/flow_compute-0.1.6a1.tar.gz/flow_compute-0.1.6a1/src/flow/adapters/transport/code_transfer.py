...
class CodeTransferManager:
    ...
    def _transfer_code(
        ...
        # Choose an effective target directory up-front using a lightweight
        # remote writability probe when available. This avoids failing the
        # first rsync attempt on unwritable absolute paths like /workspace.
        ...
        try:
            ...
            probe_target = None
        try:
            if probe_target and probe_target != "~":
                if not self._is_remote_dir_writable(connection, probe_target):
                    # Compute a safer fallback under the user's home
                    target_to_use = self._compute_home_fallback(config)
                    if self.progress_reporter:
                        try:
                            self.progress_reporter.update_status(
                                f"Remote path {probe_target} not writable; using {target_to_use}"
                            )
                        except Exception:
                            pass
        except Exception:
            # If probe fails for any reason, fall back to original behavior
            target_to_use = config.target_dir
