...
async def log_request_processing_time(
    ...
    logger.info(
        ...
        extra_fields={"request_id": request_id, "processing_time": round(processing_time, 4)},
    )
...
async def catch_exceptions_middleware(
    ...
    try:
        ...
        logger.error(
            f"Request error",
            category=LogCategory.API,
            extra_fields={"request_id": request_id, "traceback": traceback.format_exc()},
            exc_info=True,
        )
        ELKLogger.log(
...
async def log_processing_time(
    ...
    # logger.info("Request ID: {request_id} | Path: {request.url.path} | Processing Time: {processing_time:.4f} seconds", category=LogCategory.SYSTEM, extra_fields={"processing_time:.4f": processing_time:.4f}
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: FastAPI, limit: int, period: int) -> None:
        ...
        self.limit = limit
        self.period = period
        self.visits: defaultdict[str, defaultdict[str, list[float]]] = (
