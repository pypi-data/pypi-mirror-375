#!/usr/bin/env python3
"""
Comprehensive test suite for OmniBenchmarker class.
...
from omnibench.objectives.output import (
    StringEqualityObjective, 
    RegexMatchObjective
)
from omnibench.objectives.combined import CombinedBenchmarkObjective
from omnibench.objectives.llm_judge import (
    LLMJudgeObjective,
    DEFAULT_BINARY_PROMPT
)
from omnibench.objectives.path import (
...
class TestRunner:
    ...
    def display_results(self):
        ...
        if not self.console:
            ...
            print("\n" + "="*80)
            print("OMNIBENCHMARKER TEST RESULTS")
            print("="*80)
        ...
        self.console.print("\n")
        self.console.rule("[bold blue]OmniBenchmarker Test Results", style="blue")
        ...
        summary = Panel(
            ...
            title="OmniBenchmarker Test Summary",
...
class OmniBenchmarkerTests:
    ...
    def test_basic_initialization(self) -> TestResult:
        """Test basic OmniBenchmarker initialization."""
        try:
            ...
            return TestResult(
                ...
                message="✓ OmniBenchmarker initialized successfully",
    ...
    def test_invoke_method_specification(self) -> TestResult:
        ...
        try:
            ...
            mixed_benchmarker = OmniBenchmarker(
    ...
    def test_async_method_detection(self) -> TestResult:
        ...
        async def async_test():
            ...
            try:
                ...
                self.runner.assert_isinstance(result, dict, "Result should be a dictionary")
                self.runner.assert_equal(result["agent_type"], "real_async_llm", "Agent type should be real_async_llm")
                self.runner.assert_equal(mock_agent.async_calls, 1, "Should call async method")
                self.runner.assert_equal(mock_agent.sync_calls, 0, "Should not call sync method")
                self.runner.assert_in("status", result, "Result should have status field")
                self.runner.assert_in("message", result, "Result should have message field")
                ...
                return TestResult(
                    ...
                    details=f"Agent type: {result['agent_type']}, Async calls: {mock_agent.async_calls}",
                    expected="async method detected",
                    actual=f"Used async method with agent type: {result['agent_type']}"
                )
            except Exception as e:
                return TestResult(
                    name="Async Method Detection",
    ...
    def test_mixed_benchmark_execution(self) -> TestResult:
        ...
        async def async_test():
            ...
            try:
                ...
                # Either sync or async method could be called depending on implementation
                total_calls = mock_agent.sync_calls + mock_agent.async_calls
                self.runner.assert_greater(total_calls, 0, "Agent should be called")
                
                return TestResult(
                    name="Mixed Benchmark Execution",
                    status=TestStatus.PASS,
                    message="✓ Mixed benchmark execution successful",
                    details=f"Sync calls: {mock_agent.sync_calls}, Async calls: {mock_agent.async_calls}",
                    expected="successful mixed execution",
                    actual=f"Total calls: {total_calls}"
                )
            except Exception as e:
                return TestResult(
                    name="Mixed Benchmark Execution",
    ...
    def test_concurrent_execution(self) -> TestResult:
        ...
        async def async_test():
            ...
            try:
                ...
                # Reset call count for sequential test
                ...
                # Each benchmark has 1 iteration, agent should be called 3 times for each test run
                total_calls = mock_agent.call_count
                self.runner.assert_equal(total_calls, 3, f"Agent should be called 3 times for sequential test, got {total_calls}")
    ...
    def test_agent_error_handling(self) -> TestResult:
        ...
        try:
            ...
            self.runner.assert_isinstance(result, dict, "Result should be a dictionary")
            # Agent should be called twice (one for each benchmark) and should fail
            self.runner.assert_equal(mock_agent.call_count, 2, "Agent should be called twice")
    ...
    def test_objective_error_handling(self) -> TestResult:
        ...
        try:
            ...
            self.runner.assert_isinstance(result, dict, "Result should be a dictionary")
            # Agent should be called twice (one for each benchmark)
            self.runner.assert_equal(mock_agent.call_count, 2, "Agent should be called twice")
    ...
    def test_real_llm_integration(self) -> TestResult:
        ...
        try:
            ...
            from omnibench.objectives.llm_judge import LLMBinaryOutputSchema
    ...
    def test_performance_comparison(self) -> TestResult:
        ...
        async def async_test():
            ...
            try:
                ...
                async_calls = mock_agent.call_count

                # Reset call count for sync test
                mock_agent.call_count = 0
                ...
                sync_calls = mock_agent.call_count
                ...
                # Each benchmark has 1 iteration, should be called 5 times for each test run
                self.runner.assert_equal(async_calls, 5, f"Async agent should be called 5 times, got {async_calls}")
                self.runner.assert_equal(sync_calls, 5, f"Sync agent should be called 5 times, got {sync_calls}")
...
def main():
    """Main test execution function for OmniBenchmarker tests."""
    runner = TestRunner()
    ...
    # Print header
    if runner.console:
        runner.console.rule("[bold cyan]OmniBenchmarker Test Suite", style="cyan")
        runner.console.print("\n[bold]Testing OmniBenchmarker Async/Sync Functionality[/bold]")
        ...
        print("="*80)
        print("OmniBenchmarker Test Suite")
        print("="*80)
        print("Testing OmniBenchmarker Async/Sync Functionality")
    ...
    tests = OmniBenchmarkerTests(runner)
    ...
    if runner.console:
        with Progress(
            ...
            task = progress.add_task("Running OmniBenchmarker tests...", total=len(test_methods))
        ...
        print("Running OmniBenchmarker tests...\n")
    ...
    if runner.console:
        ...
        if exit_code == 0:
            runner.console.print("[bold green]🎉 All tests passed! OmniBenchmarker async/sync functionality is working correctly.[/bold green]")
        else:
        ...
        if exit_code == 0:
            print("🎉 All tests passed! OmniBenchmarker async/sync functionality is working correctly.")
        else:
