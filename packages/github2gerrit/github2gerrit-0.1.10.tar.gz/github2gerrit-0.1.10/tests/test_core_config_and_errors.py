...
def test_resolve_target_branch_prefers_env_gerrit_branch(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("GERRIT_BRANCH", "release-1.2")
    monkeypatch.setenv("GITHUB_BASE_REF", "main")  # should be ignored due to GERRIT_BRANCH
    orch = Orchestrator(workspace=tmp_path)
    assert orch._resolve_target_branch() == "release-1.2"


def test_resolve_target_branch_falls_back_to_github_base_ref(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_resolve_target_branch_defaults_to_master(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.delenv("GERRIT_BRANCH", raising=False)
...
def test_push_to_gerrit_topic_default_prefix_with_pr(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    # Arrange
    orch = Orchestrator(workspace=tmp_path)
    repo = RepoNames(project_gerrit="releng/builder", project_github="releng-builder")
    gerrit = GerritInfo(host="gerrit.example.org", port=29418, project="releng/builder")
    ...
    assert review_cmd is not None, f"Expected git review command in calls: {calls}"
...
def test_push_to_gerrit_topic_custom_prefix_without_pr(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    # Arrange
    orch = Orchestrator(workspace=tmp_path)
    repo = RepoNames(project_gerrit="releng/builder", project_github="releng-builder")
    gerrit = GerritInfo(host="gerrit.example.org", port=29418, project="releng/builder")
...
def test_push_to_gerrit_raises_on_git_review_failure(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    orch = Orchestrator(workspace=tmp_path)
    repo = RepoNames(project_gerrit="releng/builder", project_github="releng-builder")
    gerrit = GerritInfo(host="gerrit.example.org", port=29418, project="releng/builder")
...
def test_configure_git_raises_on_git_review_init_failure(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    orch = Orchestrator(workspace=tmp_path)
    gerrit = GerritInfo(host="gerrit.example.org", port=29418, project="releng/builder")
    ...
    def fake_run_cmd(cmd: list[str], **kwargs: Any) -> CommandResult:
        ...
        if cmd[:2] == ["git", "rev-parse"]:
            return CommandResult(returncode=0, stdout=str(tmp_path) + "\n", stderr="")
