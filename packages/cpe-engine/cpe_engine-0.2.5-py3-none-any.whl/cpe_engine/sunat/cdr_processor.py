...
class CdrProcessor:
    ...
    def _parse_cdr_xml(cdr_xml: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if supplier_party:
                cdr_data['sender'] = {
                    'document_number': CdrProcessor._get_document_number_from_party(supplier_party[0], namespaces),
            ...
            if customer_party:
                cdr_data['receiver'] = {
                    'document_number': CdrProcessor._get_document_number_from_party(customer_party[0], namespaces),
            ...
            if doc_reference:
                cdr_data['document_reference'] = {
                    ...
                    'document_type_code': CdrProcessor._get_document_type_code(doc_reference[0], namespaces),
                }
    ...
    @staticmethod
    def _get_document_number_from_party(party_element, namespaces: Dict[str, str]) -> Optional[str]:
        """
        Obtiene número de documento de elemento Party, manejando diferentes formatos SUNAT.
        
        SUNAT CDRs pueden tener diferentes formatos:
        - <cbc:ID schemeID="6">12345678901</cbc:ID> (con schemeID)
        - <cbc:ID>12345678901</cbc:ID> (sin schemeID)
        
        Args:
            party_element: Elemento Party XML
            namespaces: Namespaces XML
            
        Returns:
            Número de documento (RUC, DNI, etc.) o None si no se encuentra
        """
        try:
            # Primero intentar con schemeID (formato preferido)
            ruc_with_scheme = party_element.xpath('.//cbc:ID[@schemeID="6"]', namespaces=namespaces)
            if ruc_with_scheme and ruc_with_scheme[0].text:
                return ruc_with_scheme[0].text
            
            # Si no tiene schemeID, buscar cualquier ID que parezca RUC
            all_ids = party_element.xpath('.//cbc:ID', namespaces=namespaces)
            for id_element in all_ids:
                if id_element.text and id_element.text.isdigit() and len(id_element.text) in [8, 11]:
    ...
    @staticmethod
    def _get_document_type_code(doc_reference_element, namespaces: Dict[str, str]) -> Optional[str]:
        """
        Obtiene código de tipo de documento de referencia.
        
        Args:
            doc_reference_element: Elemento DocumentReference XML
            namespaces: Namespaces XML
            
        Returns:
            Código de tipo de documento o None
        """
        try:
            # Buscar DocumentTypeCode directamente - enfoque declarativo
            # Solo retornamos lo que SUNAT proporciona explícitamente
            doc_type_code = CdrProcessor._get_text(doc_reference_element, './/cbc:DocumentTypeCode', namespaces)
