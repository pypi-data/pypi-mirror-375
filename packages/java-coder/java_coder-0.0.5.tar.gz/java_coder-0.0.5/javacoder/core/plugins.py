...
class ProjectPlugin(Plugin):
    ...
    def get_project_dependencies(self):
        dependence_dict = {}
        if self.project_pom and self.project_pom.getroot():
    ...
    def add_author_info(self, operator):
        ...
        if operator.class_cursor:
            ...
            if operator.class_cursor.documentation:
                class_documentation = operator.class_cursor.documentation if type(
                    operator.class_cursor.documentation) == str else operator.class_cursor.documentation.documentation
                for k in self.author_map.keys():
                    if '@' + k not in class_documentation:
                        author_info_document = author_info_document + self.get_converted_author_info(k) + '\n'
                document = author_info_document + class_documentation
            else:
