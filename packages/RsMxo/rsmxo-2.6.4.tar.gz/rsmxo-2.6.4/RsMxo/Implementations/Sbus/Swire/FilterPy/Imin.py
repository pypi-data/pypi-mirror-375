...
class IminCls:
	...
	def set(self, frame: str, field: str, data: int, serialBus=repcap.SerialBus.Default) -> None:
		"""SBUS<*>:SWIRe:FILTer:IMIN \n
		Snippet: driver.sbus.swire.filterPy.imin.set(frame = 'abc', field = 'abc', data = 1, serialBus = repcap.SerialBus.Default) \n
		...
		self._core.io.write(f'SBUS{serialBus_cmd_val}:SWIRe:FILTer:IMIN {param}'.rstrip())

	def get(self, serialBus=repcap.SerialBus.Default) -> int:
		"""SBUS<*>:SWIRe:FILTer:IMIN \n
		Snippet: value: int = driver.sbus.swire.filterPy.imin.get(serialBus = repcap.SerialBus.Default) \n
		Specifies the index, or sets the start value of an index range. \n
			:param serialBus: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sbus')
			:return: data: No help available"""
		serialBus_cmd_val = self._cmd_group.get_repcap_cmd_value(serialBus, repcap.SerialBus)
		response = self._core.io.query_str(f'SBUS{serialBus_cmd_val}:SWIRe:FILTer:IMIN?')
