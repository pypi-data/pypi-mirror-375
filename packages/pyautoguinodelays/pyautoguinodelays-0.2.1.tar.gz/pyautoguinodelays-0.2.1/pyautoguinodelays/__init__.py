...
def _mouseMoveDrag(moveOrDrag, x, y, xOffset, yOffset, duration, tween=linear, button=None):
    ...
    if duration > MINIMUM_DURATION:
        ...
        if sleep_amount < MINIMUM_SLEEP:
            num_steps = (duration / MINIMUM_SLEEP)
            sleep_amount = duration / num_steps
...
def failSafeCheck():
    if FAILSAFE and tuple(position()) in FAILSAFE_POINTS:
        raise FailSafeException('PyAutoGUI fail-safe triggered from mouse moving to a corner of the screen. To disable this fail-safe, set pyautoguiDGWEdit.FAILSAFE to False. DISABLING FAIL-SAFE IS NOT RECOMMENDED.')
