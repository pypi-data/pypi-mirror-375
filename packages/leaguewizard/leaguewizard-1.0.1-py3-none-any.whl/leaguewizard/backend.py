...
def get_process_fullname(pid: int) -> str | None:
    """Return full path of process, or None if not accessible."""
    hProcess = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, False, pid)
    if not hProcess:
        return None
    try:
        buf_len = wintypes.DWORD(32768)  # max path
...
def find_process_fullname(target_name: str) -> str | None:
    """Find full path of first process whose basename matches target_name."""
