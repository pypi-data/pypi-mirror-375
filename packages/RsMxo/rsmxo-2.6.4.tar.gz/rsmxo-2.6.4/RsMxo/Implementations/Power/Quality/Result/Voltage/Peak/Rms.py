...
class RmsCls:
	...
	def get(self, power=repcap.Power.Default) -> float:
		"""POWer<*>:QUALity:RESult:VOLTage:PEAK:RMS \n
		Snippet: value: float = driver.power.quality.result.voltage.peak.rms.get(power = repcap.Power.Default) \n
		...
		response = self._core.io.query_str(f'POWer{power_cmd_val}:QUALity:RESult:VOLTage:PEAK:RMS?')
