...
class Column(object):
    ...
    def adjust_width(self, row, max_width=None):
        ...
        if max_width is not None and self.width > max_width:
            row[self.index] = "\n".join(
                textwrap.wrap(row[self.index], width=max_width))
...
class Table(object):
    ...
    def __init__(self, headers, show_headers=True, max_width=None):
    ...
    def adjust_widths(self, row):
        ...
        for col in self.cols:
            col.adjust_width(row, max_width=self.max_width)
            if '\n' in row[col.index]:
                self.simple = False
    ...
    def format_row(self, row):
        ...
        for c in self.cols:
            ...
            for ln in cell.splitlines():
                ...
                lines[c.index].append(ln + (" " *
                                            (c.width - column_width(ln))))
        height = 1
        for c in self.cols:
        ...
        for i in range(height):
            x.append(self.margin + self.colsep.join(
                [' ' + lines[c.index][i] + ' '
                 for c in self.cols]) + self.margin)
        return '\n'.join(x)
