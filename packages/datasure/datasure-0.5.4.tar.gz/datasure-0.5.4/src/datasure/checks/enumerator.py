...
def display_enumerator_summary(
    ...
    styler = summary_df.style.format(subset=perc_cols, formatter="{:.2%}")
...
def display_enumerator_productivity(
    ...
    # Adjust max_elements if data size exceeds 390,000
    styler = productivity_df.style.format(
        subset=format_cols,
        formatter="{:,.0f}",
    ).background_gradient(
        subset=format_cols,
    ...
    if productivity_df.size > 390000:
        styler = styler.set_table_attributes('style="max-elements: none;"')

    productivity_df = styler
...
def display_enumerator_statistics_overtime(
    ...
    if statscol:
        ...
        # Adjust max_elements if data size exceeds 390,000
        styler = stats_overtime_df.style.format(
            subset=format_cols,
            formatter=format,
        ).background_gradient(
            subset=format_cols,
        ...
        if stats_overtime_df.size > 390000:
            styler = styler.set_table_attributes('style="max-elements: none;"')

        stats_overtime_df = styler
