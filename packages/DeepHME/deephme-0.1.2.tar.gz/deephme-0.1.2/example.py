...
def main():
    ...
    estimator = DeepHME(model_name='predict_quantiles3D_DL_v8', channel=ch)
    mass = estimator.predict(event_id=branches['event'],
    ...
    p4 = estimator.predict(event_id=branches['event'],
                           lep1_pt=branches['lep1_pt'], 
                           lep1_eta=branches['lep1_eta'], 
                           lep1_phi=branches['lep1_phi'], 
                           lep1_mass=branches['lep1_mass'],
    ...
    mass_p4 = estimator._compute_mass(p4)
    plt.hist(mass_p4, bins=np.linspace(0, 2500, 100))
    plt.title('Predicted mass')
    plt.ylabel('Count')
    plt.xlabel('Mass')
    plt.grid(True)
    plt.savefig(f'example_p4_{ch}.pdf', bbox_inches='tight')
    plt.clf()
    plt.close()

    plt.hist2d(mass_p4, mass, bins=np.linspace(250, 1500, 100), cmap='viridis')
    plt.title('mass from P4 vs mass')
    plt.ylabel('From mass')
    plt.xlabel('From P4')
    plt.savefig(f'p4_vs_mass_{ch}.pdf', bbox_inches='tight')
    plt.clf()
    plt.close()
