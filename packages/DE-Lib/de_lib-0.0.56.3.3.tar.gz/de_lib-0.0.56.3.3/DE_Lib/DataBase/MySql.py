...
class MYSQL:
    ...
    def Connect(self, str_cnn: dict):
        ...
        try:
            ...
            if self.DRIVER_CONEXAO.upper() == "SQLALCHEMY":
                __dns = f"""{self.DATABASE.lower()}+pymysql://{self.USERNAME}:{self.PASSWORD}@{self.HOST}:{self.PORT}/{self.INSTANCE}"""
                __engine = sqa.create_engine(url=__dns)
                __conn = __engine.connect().connection
                #__conn = __conn.connection
            elif self.DRIVER_CONEXAO.upper() == "MYSQL":
                __conn = mysql.connect(user=self.USERNAME, password=self.PASSWORD, database=self.INSTANCE, host=self.HOST)
            elif self.DRIVER_CONEXAO.upper() == "PYMYSQL":
                __conn = pymysql.connect(user=self.USERNAME, password=self.PASSWORD, database=self.INSTANCE, host=self.HOST)
