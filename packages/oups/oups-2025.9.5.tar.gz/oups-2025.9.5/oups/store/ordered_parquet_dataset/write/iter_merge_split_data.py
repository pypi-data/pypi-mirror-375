...
def iter_merge_split_data(
    ...
    for rg_idx_start, cmpt_ends_excl in merge_sequences:
        ...
        for chunk_countdown, (rg_idx_end_excl, df_idx_end_excl) in enumerate(
            ...
            # chunks.
            opd_chunk = (
                None if rg_idx_start == rg_idx_end_excl else opd[rg_idx_start:rg_idx_end_excl].to_pandas()
            )
            df_chunk = None if df_idx_start == df_idx_end_excl else df.iloc[df_idx_start:df_idx_end_excl]
            chunk = concat(
                [
                    remainder,
                    opd_chunk,
