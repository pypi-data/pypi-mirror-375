...
class GitOperator:
    ...
    def verify_cherry_pick_success(self, source_branch: str):
        """
        ...
        source_branch = process_ref(ref_name=source_branch)
        
        # 执行git cherry命令，对比当前HEAD和源分支
        output = self._run_command(["cherry", "HEAD", source_branch])
        
        if not output:
            self.logger.info("没有需要检查的提交")
            return True
        
        # 按行分割输出
        lines = output.split('\n')
        # 用于匹配git cherry输出格式的正则表达式
        ...
        for line in lines:
            ...
            if match:
                ...
                # 这是本地 HEAD 的commit_hash, 并不是开发分支的 commit_hash
