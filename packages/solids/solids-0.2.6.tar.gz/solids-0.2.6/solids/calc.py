...
class code:
    ...
    def set_EMT(self):
        ...
        return xopt
    def set_GULP(self, block_gulp=block_gulp, gulp_path=None, nproc=nproc, base_name=base_name):
        if gulp_path is None:
            print('Error in the path_exe for GULP')
        from aegon.libcodegulp import calculator_all_gulp
        xopt=calculator_all_gulp(self.list_of_atoms, block_gulp, gulp_path, nproc, base_name)
        return xopt
    def set_TiO2(self, nproc=nproc):
        from solids.libcalc_tio2 import TiO2_parallel
        xopt=TiO2_parallel(self.list_of_atoms, n_jobs = 4)
        return xopt
#------------------------------------------------------------------------------------------
