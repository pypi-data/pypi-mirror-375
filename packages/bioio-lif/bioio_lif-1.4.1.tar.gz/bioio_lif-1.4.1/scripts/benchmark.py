...
# This file is under /scripts while the test resourcess are under /bioio_lif/tests/resources
test_resources_dir = pathlib.Path(__file__).parent.parent / "bioio_lif" / "tests" / "resources"
...
test_files = [
    ...
    if test_file.name.endswith(".lif")
]
print(f"Test files: {[file.name for file in test_files]}")
bioio_base.benchmark.benchmark(bioio_lif.reader.Reader, test_files)
