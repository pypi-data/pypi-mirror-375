...
from typing import Any, Callable, Hashable, Literal, override
...
class H5DataFrame(pd.DataFrame):
    ...
    _internal_names: list[str] = ["_data_file"] + pd.DataFrame._internal_names
    _internal_names_set: set[str] = {"_data_file"} | pd.DataFrame._internal_names_set

    _data_file: ch.H5Dict[Any] | None  # pyright: ignore[reportUninitializedInstanceVariable]
    ...
    def _constructor_sliced_from_mgr(self, mgr: BaseArrayManager, axes: list[pd.Index[Any]]) -> pd.Series[Any]:
        ser: pd.Series = pd.Series._from_mgr(mgr, axes)  # type: ignore[attr-defined]
    ...
    def __init__(
        ...
        if isinstance(data, ch.H5Dict):
            ...
            _columns: pd.Index = pd.Index(data["arrays"].keys(), dtype=data.attributes["columns_dtype"])
        ...
        NDFrame.__init__(self, mgr)  # pyright: ignore[reportArgumentType]

    @override
    def __finalize__(self, other: H5DataFrame, method: str | None = None, **kwargs: Any) -> pd.DataFrame:  # pyright: ignore[reportIncompatibleMethodOverride]
