...
"""Tool for retrieving detailed information about RDS DB Instances (converted from resource)."""
...
DESCRIBE_INSTANCE_DETAIL_TOOL_DOCSTRING = """Get detailed information about a specific Amazon RDS instance.

This tool retrieves comprehensive details about a specific RDS database instance identified by its instance ID, including configuration, status, endpoints, storage details, and security settings.
"""
...
class Instance(BaseModel):
    ...
    def from_DBInstanceTypeDef(cls, instance: DBInstanceTypeDef) -> 'Instance':
        ...
        return cls(
            ...
            resource_uri=f"aws-rds://db-instance/{formatted_data.get('instance_id', '')}",
        )


@mcp.tool(
    name='DescribeDBInstanceDetails',
    description=DESCRIBE_INSTANCE_DETAIL_TOOL_DOCSTRING,
)
@handle_exceptions
async def describe_db_instance_detail(
    instance_id: str = Field(..., description='The instance identifier'),
    aws_region: str = Field(
        ..., description='AWS region for this call (e.g., us-east-1)'
    ),
) -> Instance:
    """Get detailed information about a specific instance as a tool.

    Args:
        instance_id: The unique identifier of the RDS instance.
        aws_region: Region to target for this call (e.g., 'us-east-1').
    """
    logger.info(f'Getting instance detail (tool) for {instance_id}')
