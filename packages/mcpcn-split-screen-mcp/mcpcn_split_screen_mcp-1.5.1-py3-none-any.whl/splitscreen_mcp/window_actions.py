...
def _excluded_list():
    ...
    """
    base = "AIdo,com.silicon-geek.aido"
    env_keys = [
        "SPLITSCREEN_EXCLUDE",
        "COMPUTER_SPLIT_SCREEN_EXCLUDE",
        "MCP_SPLITSCREEN_EXCLUDE",
...
def _is_excluded_windows(hwnd) -> bool:
    ...
    try:
        ...
        # 精确匹配：全名、去扩展名
        if (exe_name and exe_name in ex) or (exe_stem and exe_stem in ex):
            return True
        # 子串匹配：标题已判定；这里对可执行名、去扩展名、完整路径都做子串
        if exe_name and any(e in exe_name for e in ex):
            return True
        if exe_stem and any(e in exe_stem for e in ex):
            return True
        if exe_low and any(e in exe_low for e in ex):
            return True
    except Exception:
        pass
    return False
