...
from mplabml.datamanager.capture_configurations import CaptureConfigurations
from mplabml.datamanager.captures import Captures, BackgroundCaptures
from mplabml.datamanager.capture import BackgroundCapture
from mplabml.datamanager.featurefiles import FeatureFiles
from mplabml.datamanager.knowledgepack import KnowledgePack
from mplabml.datamanager.queries import Queries
from mplabml.datamanager.sandboxes import Sandboxes
from mplabml.method_calls.functioncall import FunctionCall
...
class Project(object):
    ...
    def delete_async(self):
        ...
        url = "project-delete/{0}/".format(self.uuid)
        request = self._connection.request("post", url)
        response, err = utility.check_server_response(request)

        return response

    def delete_async_status(self):
    ...
    def get_project_dcli(self):
        url = "project/{0}/dcli/".format(self.uuid)
        response = self._connection.request("get", url)
        response_data, err = utility.check_server_response(response)
        if err is False:
            return response_data
