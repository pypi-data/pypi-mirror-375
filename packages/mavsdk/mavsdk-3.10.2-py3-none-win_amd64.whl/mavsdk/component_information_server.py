...
class FloatParamUpdate:
    ...
    def __str__(self):
        ...
        struct_repr = ", ".join(
            ["name: " + str(self.name), "value: " + str(self.value)]
        )
...
class ComponentInformationServerResult:
    ...
    class Result(Enum):
        ...
        def translate_to_rpc(self):
            ...
            if (
                self
                == ComponentInformationServerResult.Result.INVALID_PARAM_START_VALUE
            ):
                return component_information_server_pb2.ComponentInformationServerResult.RESULT_INVALID_PARAM_START_VALUE
            if (
                self
                == ComponentInformationServerResult.Result.INVALID_PARAM_DEFAULT_VALUE
            ):
...
class ComponentInformationServer(AsyncBase):
    ...
    def _setup_stub(self, channel):
        """Setups the api stub"""
        self._stub = (
            component_information_server_pb2_grpc.ComponentInformationServerServiceStub(
                channel
            )
        )
    ...
    async def provide_float_param(self, param):
        ...
        if result.result != ComponentInformationServerResult.Result.SUCCESS:
            raise ComponentInformationServerError(
                result, "provide_float_param()", param
            )
