...
DECLSPEC u32 memcat16c (PRIVATE_AS u32 *block, const u32 offset, PRIVATE_AS const u32 *append, const u32 append_len, PRIVATE_AS u32 *digest)
{
  ...
  if (new_len >= 64)
  {
    ...
    sm3_transform (block + 0, block + 4, block + 8, block + 12, digest);
...
KERNEL_FQ KERNEL_FA void m35100_init (KERN_ATTR_TMPS (sm3crypt_tmp_t))
{
  ...
  sm3_transform (block + 0, block + 4, block + 8, block + 12, alt_result);
  ...
  if (block_len >= 56)
  {
    sm3_transform (block + 0, block + 4, block + 8, block + 12, p_bytes);
  ...
  if (block_len >= 56)
  {
    sm3_transform (block + 0, block + 4, block + 8, block + 12, s_bytes);
...
KERNEL_FQ KERNEL_FA void m35100_loop (KERN_ATTR_TMPS (sm3crypt_tmp_t))
{
  ...
  for (u32 i = 0, j = LOOP_POS; i < LOOP_CNT; i++, j++)
  {
    ...
    if (block_len >= 56)
    {
      sm3_transform (block + 0, block + 4, block + 8, block + 12, tmp);
