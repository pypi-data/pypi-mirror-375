...
class Connection(object):
    ...
    def _login_with_oauth2(self, password, attempt=0):
        ...
        if self.username is None or password is None:
            if os.path.exists(self._token_filename):
                ...
                print(
                    "Connect to MPLABML Cloud using your MPLABML Account. Sign up for free at https://mplabml.com/plans/community-edition/"
                )
                self.username = input("Enter username: ")
                password = getpass("Enter password: ")
    ...
    def file_request(
        self, url, file_path, data=None, read_type="rb", method="post", **kwargs
    ):
