...
def kcp_v(
    ...
    # between the two:
    plot_times = [
        (true_times[i] + true_times[i+1]) / 2
        for i in range(len(true_times)-1)
    ]
    ...
    for i, _ in enumerate(detected_change_points):
        ...
        if i < len(detected_change_points) - 1:
            ...
            if np.sum(
                ...
                detected_change_points[i+1] < np.array(interval_end_points)
            ):
        ...
        arlot_crit_diag = (1/(R-L)) * np.sum(
            [np.sum(np.multiply(LR_data[:, j], LR_data[:, j]))
        ...
        for z in range(0, R-L-1):
            ...
            arlot_crit[z] = arlot_crit_diag - (1/(R-L)) * ((1/(C_z-L)) * (np.sum(
                np.matmul(np.transpose(L_data_z), L_data_z)
            )) + (1/(R-C_z)) * (np.sum(np.matmul(np.transpose(R_data_z), R_data_z))))
        ...
        for m in range(0, n_time_series):
            ...
            arlot_crit_diag_zz = (
                1/(R-L))*np.sum([np.sum(np.multiply(LR_data_zz[0, j], LR_data_zz[0, j])) for j in range(0, R-L)])

            # Also calculate the criterion as if there were no change points, for later use.
            arlot_crit_zero_zz[m] = arlot_crit_diag_zz - (1/(R-L)) * (1/(R-L)) * (
            ...
            for z in range(0, R-L-1):
                ...
                arlot_crit_sense[z] = arlot_crit_diag_zz - (1/(R-L)) * ((1/(C_z-L)) * (np.sum(np.matmul(np.transpose(
                    L_data_zz), L_data_zz))) + (1/(R-C_z)) * (np.sum(np.matmul(np.transpose(R_data_zz), R_data_zz))))
        ...
        plt.axvline(
            plot_times[(detected_change_points[i]-1)], ls='--', c='red', lw=1)
        ...
        for m in range(0, n_time_series):
            if (m in keep_indices):
                plt.plot(
                    plot_times[(L+1):R], arlot_crit_sense_array[m, :]-arlot_crit_zero_zz[m], label=m)
            else:
                plt.plot(plot_times[(L+1):R], arlot_crit_sense_array[m,
                         :]-arlot_crit_zero_zz[m], color="black")
