...
def batch_job_wait(cluster_id: int, workspace: str | None, download: str | None):
    ...
    if download and not cluster_errored:
        cluster_filestores = coiled.filestore.wait_until_complete(cluster_id, "result")
        ...
        if cluster_filestores.get("result").get("filestore"):
            coiled.filestore.download_from_filestore_with_ui(
                fs=cluster_filestores.get("result").get("filestore"),
                into=download,
            )

        coiled.filestore.clear_filestores_with_ui(cluster_filestores)
