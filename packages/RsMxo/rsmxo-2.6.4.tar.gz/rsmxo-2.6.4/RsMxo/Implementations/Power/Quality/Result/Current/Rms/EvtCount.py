...
class EvtCountCls:
	...
	def get(self, power=repcap.Power.Default) -> int:
		"""POWer<*>:QUALity:RESult:CURRent:RMS:EVTCount \n
		Snippet: value: int = driver.power.quality.result.current.rms.evtCount.get(power = repcap.Power.Default) \n
		...
		response = self._core.io.query_str(f'POWer{power_cmd_val}:QUALity:RESult:CURRent:RMS:EVTCount?')
