...
class SignedXmlBuilder:
    ...
    def build_sign_and_send(self, document) -> Dict[str, Any]:
        ...
        try:
            ...
            # Agregar información del documento incluyendo hash de firma
            try:
                digest_value = self.xml_signer.extract_digest_value(xml_signed)
                hash_info = {
                    'digest_value': digest_value,
                    'hash_algorithm': 'SHA-256'
                }
            except Exception as e:
                print(f"[SignedXmlBuilder] Warning: No se pudo extraer hash de firma: {e}")
                hash_info = {
                    'digest_value': None,
                    'hash_algorithm': None
                }
