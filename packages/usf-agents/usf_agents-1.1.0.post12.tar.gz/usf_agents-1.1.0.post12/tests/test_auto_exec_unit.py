...
async def test_mode_disable_returns_pending_tool_calls():
    ...
    completion = await run_with_auto_execution(agent, [{"role": "user", "content": "x"}], tools, router, mode="disable")
    ...
    assert isinstance(message.get("tool_calls"), list) and len(message["tool_calls"]) > 0
...
async def test_mode_auto_executes_agent_and_custom():
    ...
    completion = await run_with_auto_execution(agent, [{"role": "user", "content": "x"}], tools, router, mode="auto")
    assert isinstance(completion, dict) and completion.get("object") == "chat.completion"
    choice = (completion.get("choices") or [{}])[0] or {}
    message = choice.get("message") or {}
...
async def test_mode_agent_pauses_on_custom():
    ...
    completion = await run_with_auto_execution(agent, [{"role": "user", "content": "x"}], tools, router, mode="agent")
    assert isinstance(completion, dict) and completion.get("object") == "chat.completion"
    choice = (completion.get("choices") or [{}])[0] or {}
    message = choice.get("message") or {}
    assert isinstance(message.get("tool_calls"), list) and len(message["tool_calls"]) > 0
...
async def test_mode_tool_pauses_on_agent():
    ...
    completion = await run_with_auto_execution(agent, [{"role": "user", "content": "x"}], tools, router, mode="tool")
    assert isinstance(completion, dict) and completion.get("object") == "chat.completion"
    choice = (completion.get("choices") or [{}])[0] or {}
    message = choice.get("message") or {}
    assert isinstance(message.get("tool_calls"), list) and len(message["tool_calls"]) > 0
