...
class KafkaSelfHealing:
    ...
    def detect_and_heal_issues(self, issue_context: Optional[str] = None) -> bool:
        ...
        try:
            ...
            self.logger.info(f"🔧 Starting self-healing diagnostics (context: {issue_context or 'unknown'})")
            
            # Issue 1: Multiple consumer processes
            if self._detect_multiple_consumers():
                self.logger.warning("🔧 Detected multiple consumers, cleaning up")
                if self._heal_multiple_consumers():
                    self.stats.record_healing_attempt(True, 'multiple_consumers')
                    self.logger.info("✅ Multiple consumer cleanup successful")
                    time.sleep(5)  # Wait for cleanup
                else:
                    self.stats.record_healing_attempt(False, 'multiple_consumers')
                    self.logger.error("❌ Multiple consumer cleanup failed")
                    return False
                    
            # Issue 2: Network connectivity
            if not self._test_kafka_connectivity():
                self.logger.error("❌ Kafka connectivity failed - infrastructure issue")
                self.stats.record_healing_attempt(False, 'connectivity')
                return False
                
            # Issue 3: Stale consumer group (if connection context suggests it)
            if issue_context and 'connection' in issue_context.lower():
                self.logger.warning("🔧 Connection context suggests stale consumer group, resetting")
                if self._heal_consumer_group():
                    self.stats.record_healing_attempt(True, 'consumer_group_reset')
                    self.logger.info("✅ Consumer group reset successful")
                else:
                    self.stats.record_healing_attempt(False, 'consumer_group_reset')
                    self.logger.error("❌ Consumer group reset failed")
                    return False
...
class SelfHealingMixin:
    ...
    def _attempt_with_self_healing(self, operation_func: Callable, operation_name: str = "operation") -> bool:
        ...
        while self._connection_attempts < self._max_connection_attempts:
            ...
            logger.info(f"🔄 Attempting {operation_name} (attempt {self._connection_attempts}/{self._max_connection_attempts})")
            ...
            # Operation failed
            logger.warning(f"❌ {operation_name} failed (attempt {self._connection_attempts})")
