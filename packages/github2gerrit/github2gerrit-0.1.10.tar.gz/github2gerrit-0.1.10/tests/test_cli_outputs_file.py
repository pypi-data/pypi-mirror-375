...
class _DummyResult:
    def __init__(self, urls: list[str], nums: list[str], shas: list[str] | None = None) -> None:
...
class _DummyOrchestratorSingle:
    ...
    def execute(self, *, inputs: Any, gh: Any) -> Any:
        # Simulate the orchestrator also exporting commit sha(s) in the environment
...
def test_single_pr_path_writes_outputs_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert mapping.get("gerrit_change_request_url") == "https://gerrit.example.org/c/repo/+/101"
    assert mapping.get("gerrit_change_request_num") == "101"
    # commit sha is written because our dummy orchestrator exported GERRIT_COMMIT_SHA
    assert mapping.get("gerrit_commit_sha") == "deadbeefcafebabe"


def test_multi_pr_url_mode_writes_aggregated_outputs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Provide a repository URL to enter URL mode and bulk path (SYNC_ALL_OPEN_PRS)
    ...
    assert "gerrit_commit_sha" not in mapping or mapping["gerrit_commit_sha"] == ""
