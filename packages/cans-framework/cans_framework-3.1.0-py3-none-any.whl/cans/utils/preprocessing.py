...
class DataPreprocessor:
    ...
    def process_tabular_data(self,
                           ...
                           graph_type: str = "global") -> CANSDataset:
        """
        ...
        Args:
            ...
            graph_type: Graph construction type ('global', 'individual', 'temporal')
        ...
        try:
            ...
            # Build graphs based on graph_type
            if graph_type == "global":
                # Single global graph with all samples as nodes
                node_features, edge_index = self.graph_builder.build_from_features(features)
                graph_data = [Data(x=node_features, edge_index=edge_index) for _ in range(len(data))]
            
            elif graph_type == "individual":
                # Each sample is its own single-node graph (original approach)
                graph_data = []
                for i in range(len(data)):
                    single_node_features = torch.tensor(features[i:i+1], dtype=torch.float)
                ...
                # Create temporal graphs (assumes data is time-ordered)
                graph_data = self._build_temporal_graphs(features, data)
    ...
    def _build_temporal_graphs(self, features: np.ndarray, data: pd.DataFrame) -> List[Data]:
        """Build temporal graphs where each sample connects to previous time steps"""
        graph_data = []
        window_size = min(5, len(features))  # Use sliding window of size 5
        ...
        for i in range(len(features)):
            ...
            window_features = torch.tensor(features[start_idx:end_idx], dtype=torch.float)
            
            # Create temporal edges (each node connects to previous time step)
            edges = []
            for t in range(1, len(window_features)):
                edges.append([t-1, t])  # Connect previous to current
            ...
            graph = Data(x=window_features, edge_index=edge_index)
            graph_data.append(graph)
