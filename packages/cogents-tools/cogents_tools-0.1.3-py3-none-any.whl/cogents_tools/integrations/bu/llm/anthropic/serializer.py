    ...
    def _serialize_content(
        ...
        if isinstance(content, str):
            if use_cache:
                return [
                    TextBlockParam(
                        text=content, type="text", cache_control=CacheControlEphemeralParam(type="ephemeral")
                    )
                ]
            else:
                return content
            ...
            if message.content is not None:
                if isinstance(message.content, str):
                    ...
                    for part in message.content:
                        if part.type == "text":
                            blocks.append(
                                AnthropicMessageSerializer._serialize_content_part_text(part, use_cache=message.cache)
                            )
