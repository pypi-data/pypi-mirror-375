...
class Flow:
    ...
    def to_periods(self, index: pd.PeriodIndex) -> pd.Series:
        """
        Returns a pd.Series (with PeriodIndex) of movements summed to specified frequency.
        """
        ...
        # Check if frequency is multiple (e.g. '2W', '3M', etc) or singular (e.g. 'W', 'M', etc)
        if not bool(re.search(r"\d", index.freqstr)):
