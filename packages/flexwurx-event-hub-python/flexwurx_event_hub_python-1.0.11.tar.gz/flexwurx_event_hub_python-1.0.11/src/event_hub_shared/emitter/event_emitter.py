...
class EventEmitter:
    ...
    async def emit(
        ...
        """
        logger.info(f"Emitting event: {event_type} to {hub_name}")
        producer = await self._get_producer(hub_name)
        
        logger.info(f"Got producer for {hub_name}")
        event_data = {
        ...
        logger.info(f"Event data for {event_type}")
        ...
        async with producer:
            logger.info(f"Creating batch for {hub_name}")
            event_data_batch = await producer.create_batch()
            logger.info(f"Adding event to batch for {hub_name}")
            event_data_batch.add(event)
            logger.info(f"Sending batch for {hub_name}")
            await producer.send_batch(event_data_batch)
            logger.info(f"Sent batch for {hub_name}")
