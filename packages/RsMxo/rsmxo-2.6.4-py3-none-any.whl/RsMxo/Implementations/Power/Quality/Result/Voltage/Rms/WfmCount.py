...
class WfmCountCls:
	...
	def get(self, power=repcap.Power.Default) -> int:
		...
		Snippet: value: int = driver.power.quality.result.voltage.rms.wfmCount.get(power = repcap.Power.Default) \n
		...
		response = self._core.io.query_str(f'POWer{power_cmd_val}:QUALity:RESult:VOLTage:RMS:WFMCount?')
