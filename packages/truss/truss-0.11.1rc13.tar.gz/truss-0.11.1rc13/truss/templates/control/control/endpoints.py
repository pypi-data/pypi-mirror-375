...
async def forward_to_client(client_ws: WebSocket, server_ws: AsyncWebSocketSession):
    while True:
        try:
            ...
            logger.info(
                f"PROXY: Received WebSocketDisconnect from server - Code: {e.code}, Reason: {e.reason}"
            )
            await _safe_close_ws(client_ws, logger, e.code or 1000, e.reason)
            logger.info("PROXY: Client connection closed successfully")
            return  # Make sure to return!
        except Exception as e:
            logger = client_ws.app.state.logger
            logger.error(
                f"PROXY: Unexpected error in forward_to_client: {e}", exc_info=True
            )
            await _safe_close_ws(client_ws, logger, 1011, "Proxy error")
            return  # Make sure to return!
