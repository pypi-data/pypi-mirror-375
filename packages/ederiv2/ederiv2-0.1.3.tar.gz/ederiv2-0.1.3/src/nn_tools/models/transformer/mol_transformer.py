...
from src.input_tools.file_loaders.smiles_loader import SmilesLoader
...
if __name__=="__main__":
    ...
    dataset_filenames = {
        ...
        "gdb-fragments-like-10smi": "gdb_fragments-like_smi10_smiles.pkl",
        # "gdb-fragments-like-09smi": "gdb_fragments-like_smi09_smiles.pkl",
        # "zinc-all": "df_zinc_dedup_fragments_final.pkl",
    }  

    save_run.log_training_info(f"Dataset files to load: {dataset_filenames}")   
    
    smiles_loader=SmilesLoader(BASE_PATH)
    
    frag_smiles=smiles_loader.from_pickle_df_files(dataset_filenames, target_column='fragments')
    ...
    batch_size=2048
    save_run.log_metric("batch_size", batch_size)
    dataset = ActionSequenceDataset(dataset_actions, actions_vocab)
    save_run.log_training_info(f"ActionSequenceDataset created with {len(dataset)} samples.")
    ...
    save_run.log_training_info(f"Dataset split into {n_train} training and {n_test} testing samples.")
    collate = CollateFunctionFactory.get_collate_function("transformer_nested")
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, collate_fn=collate)
    test_loader = DataLoader(test_ds, batch_size=batch_size, shuffle=False, collate_fn=collate)

    save_run.log_training_info(f"DataLoaders created with batch size {batch_size}.")
    ...
    step_params={"clip_gradients": True}
    params_str = ", ".join(f"{k}={v}" for k, v in sorted(step_params.items()))
    save_run.log_training_info(f"Training parameters: {params_str}")
    save_run.log_training_info("Starting training...")
    training_losses=transformer_trainer.train(train_loader, epochs=training_hyperparams["total_epochs"], 
                                              save_run=save_run, step_params=step_params, test_loader=test_loader)
