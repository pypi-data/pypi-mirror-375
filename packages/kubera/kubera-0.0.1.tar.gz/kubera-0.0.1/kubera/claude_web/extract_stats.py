#!/usr/bin/env python3
"""
Extract statistics from Claude web conversation trace data.
...
def extract_statistics(csv_file: str) -> Dict[str, Any]:
    """Extract comprehensive statistics from the Claude web trace CSV file."""
    ...
    with open(csv_file, "r") as f:
        ...
        for row in reader:
            ...
            start_timestamp = parse_timestamp(row["start_timestamp"])
            stop_timestamp = parse_timestamp(row["stop_timestamp"])

            if start_timestamp:
                stats["conversations"][session_uuid]["timestamps"].append(
                    start_timestamp
                )

            # Calculate response time for assistant messages
            if row["role"] == "assistant" and start_timestamp and stop_timestamp:
                response_time = (stop_timestamp - start_timestamp).total_seconds()
                stats["conversations"][session_uuid]["response_times"].append(
                    response_time
                )
    ...
    # Calculate conversation durations, chain lengths, and response times
    ...
    for session_uuid, conv_data in stats["conversations"].items():
        ...
        all_response_times.extend(conv_data["response_times"])

        # Remove timestamps and response_times from final output
        del conv_data["timestamps"]
        del conv_data["response_times"]
    ...
    if all_response_times:
        stats["timing_analysis"]["avg_response_time_seconds"] = sum(
            all_response_times
        ...
        for response_time in all_response_times:
            if response_time < 5:
                stats["timing_analysis"]["response_time_distribution"][
                    "<5 seconds"
                ] += 1
    ...
    stats["timing_analysis"]["response_time_distribution"] = dict(
        stats["timing_analysis"]["response_time_distribution"]
    )
...
def print_statistics(stats: Dict[str, Any]):
    ...
    print("\n" + "=" * 60)
    print("CLAUDE WEB CONVERSATION ANALYSIS")
    print("=" * 60)
    ...
    print("-" * 40)
    if stats["timing_analysis"]["avg_response_time_seconds"] > 0:
        print(
            f"Average Response Time: {stats['timing_analysis']['avg_response_time_seconds']:.1f} seconds"
        )

        print("\nResponse Time Distribution:")
        total_responses = sum(
            stats["timing_analysis"]["response_time_distribution"].values()
        )
        if total_responses > 0:
            for time_range, count in stats["timing_analysis"][
                "response_time_distribution"
            ].items():
                percentage = (count / total_responses) * 100
                print(f"  {time_range}: {count:,} ({percentage:.1f}%)")
    else:
        print("No response timing data available")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract statistics from Claude web conversation trace data."
    )
    parser.add_argument(
        "--input-file",
        type=str,
    ...
    parser.add_argument(
        "--output-file",
        type=str,
        default="./data/stats/claude_web_stats.json",
    ...
    # Extract statistics
    print(f"Extracting statistics from Claude web trace data: {args.input_file}")
