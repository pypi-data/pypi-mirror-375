...
def _cleanupOldNotes():
   ...
   global notesCurrentlyPlaying

   if len(notesCurrentlyPlaying) > _MAX_NOTES_ON:
      # calculate how many notes to remove (keep only the most recent ones)
      notesToRemove = len(notesCurrentlyPlaying) - int(_MAX_NOTES_ON * 0.75)  # keep 75% of max

      # remove the oldest notes (first in the list)
      for _ in range(min(notesToRemove, len(notesCurrentlyPlaying))):

         if notesCurrentlyPlaying:
            pitch, channel = notesCurrentlyPlaying.pop(0)   # remove oldest note
