...
# {
    ...
    def choose_starting_solution(self, N_trials=20):
        """
        Generate a starting solution and calculate the starting temperature (tI)
        to approximately accept half of the solutions.

        Parameters:
        - N_trials: Number of trial perturbations to perform for temperature calculation.

        Returns:
        - sol: The generated starting solution.
        """
        print(f"SA[{str(self.idnum)}] - Generating a starting solution")
        attempts = 0
        delta_Es = []  # Store ΔE values for temperature calculation

        while attempts < 100:
            self.v_print(f"SA[{str(self.idnum)}]. Attempt={attempts}")
            sol = self.generate_solution()  # Generate a solution
            self.repair_solution_for_clarity(sol)
            sol, converged = self.evaluate(sol)  # Evaluate the solution

            if converged:
                # Perform trial perturbations to calculate temperature
                ...
                # Break out of the loop with a valid starting solution
                break

            attempts += 1

        if not delta_Es:
            # Fallback if no worse solutions are found during trials
            ...
            print(f"Calculated starting temperature: tI = {self.tI}")
    ...
    # {
        ...
        try:
            ...
            prob_i = [(crit[1] * (after[i] - before[i]) / self.t)
