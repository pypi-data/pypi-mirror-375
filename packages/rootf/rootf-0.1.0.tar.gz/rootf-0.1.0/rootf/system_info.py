...
def get_system_info(show_public_ip=False):
    info = {}
    try:
        info["user_host"] = f"{os.getlogin()}@{socket.gethostname()}"
    except:
        info["user_host"] = "unknown"
    ...
    try:
        shell_version = subprocess.check_output([shell, "--version"], text=True, stderr=subprocess.DEVNULL).splitlines()[0]
        info["shell"] = f"{shell} {re.search(r'\d+\.\d+\.\d+', shell_version).group()}"
    except:
        info["shell"] = shell
    ...
    if platform.system() == "Linux":
        try:
            if os.path.exists("/data/data/com.termux/files/usr/bin/pkg"):
                packages += int(subprocess.check_output("pkg list-installed | wc -l", shell=True, text=True, stderr=subprocess.DEVNULL))
            elif os.path.exists("/usr/bin/dpkg"):
                packages += int(subprocess.check_output("dpkg -l | wc -l", shell=True, text=True, stderr=subprocess.DEVNULL))
            elif os.path.exists("/usr/bin/rpm"):
                packages += int(subprocess.check_output("rpm -qa | wc -l", shell=True, text=True, stderr=subprocess.DEVNULL))
            elif os.path.exists("/usr/bin/pacman"):
                packages += int(subprocess.check_output("pacman -Q | wc -l", shell=True, text=True, stderr=subprocess.DEVNULL))
            elif os.path.exists("/usr/bin/apk"):
                packages += int(subprocess.check_output("apk info | wc -l", shell=True, text=True, stderr=subprocess.DEVNULL))
        except:
            pass
