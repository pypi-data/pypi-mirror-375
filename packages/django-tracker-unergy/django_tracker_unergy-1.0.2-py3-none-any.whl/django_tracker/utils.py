...
class M2MAuditUtils:
    ...
    @staticmethod
    def get_m2m_field_names(model_class_or_instance) -> List[str]:
        """Obtiene los nombres de todos los campos ManyToMany de un modelo"""
        # Si es una clase, usar la metaclass directamente
        if isinstance(model_class_or_instance, type):
            meta = model_class_or_instance._meta
        else:
            # Si es una instancia, usar su metaclass
            meta = model_class_or_instance._meta
