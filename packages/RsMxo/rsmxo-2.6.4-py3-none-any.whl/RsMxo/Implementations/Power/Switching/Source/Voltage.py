...
class VoltageCls:
	...
	def set(self, source_voltage: enums.SignalSource, power=repcap.Power.Default) -> None:
		...
		Snippet: driver.power.switching.source.voltage.set(source_voltage = enums.SignalSource.C1, power = repcap.Power.Default) \n
		...
		self._core.io.write(f'POWer{power_cmd_val}:SWITching:SOURce:VOLTage {param}')

	# noinspection PyTypeChecker
	def get(self, power=repcap.Power.Default) -> enums.SignalSource:
		"""POWer<*>:SWITching:SOURce[:VOLTage] \n
		Snippet: value: enums.SignalSource = driver.power.switching.source.voltage.get(power = repcap.Power.Default) \n
		Selects the voltage source waveform. Active analog channels and math waveforms can be used. \n
			:param power: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Power')
			:return: source_voltage: No help available"""
		power_cmd_val = self._cmd_group.get_repcap_cmd_value(power, repcap.Power)
		response = self._core.io.query_str(f'POWer{power_cmd_val}:SWITching:SOURce:VOLTage?')
