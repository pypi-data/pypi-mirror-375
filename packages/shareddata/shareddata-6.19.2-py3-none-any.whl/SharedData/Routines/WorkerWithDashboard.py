...
def worker_main(dashboard_args):
    ...
    subprocess.run(["git", "config", "--global", "user.name", os.environ['GIT_USER']])
    subprocess.run(["git", "config", "--global", "user.email", os.environ['GIT_EMAIL']])
    subprocess.run(["git", "config", "--global", "credential.helper", "!f() {{ echo username=\\$GIT_USER; echo password=\\$GIT_TOKEN; }};f"])
