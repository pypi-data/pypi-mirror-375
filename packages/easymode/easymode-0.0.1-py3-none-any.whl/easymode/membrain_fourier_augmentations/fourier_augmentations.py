...
class MissingWedgeMaskAndFourierAmplitudeMatchingCombined:
    """
    ...
    def randomize(self) -> None:
        ...
        self._do_mw_transform = np.random.uniform(0.0, 1.0) < self.missing_wedge_prob
        self._do_amp_transform = np.random.uniform(0.0, 1.0) < self.amplitude_prob
        self._do_sample_kernel = np.random.uniform(0.0, 1.0) < self.sample_kernel_prob
    ...
    def __call__(self, img):
        ...
        if self.missing_wedge_aug and self._do_mw_transform:
            missing_wedge_mask = wedge_mask(img.shape, self.missing_angle)
        ...
        if self._do_sample_kernel:
            ...
            real_patch = self.run_interpolation(img, real_patch, kernel)
