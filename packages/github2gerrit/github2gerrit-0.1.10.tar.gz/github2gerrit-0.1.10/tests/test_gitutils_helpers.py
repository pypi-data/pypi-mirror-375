...
def test_mask_text_replaces_tokens_and_ignores_empty() -> None:
    text = "token=ABC and again ABC; mixed abc should not change case-sensitively"
    out = mask_text(text, masks=["ABC", ""])
    assert out == "token=*** and again ***; mixed abc should not change case-sensitively"
...
"""
    ...
    res = run_cmd_with_retries([sys.executable, "-c", code], cwd=tmp_path, env=_no_cov_env())
...
def test_run_cmd_with_retries_does_not_retry_on_non_transient_error(
    ...
    code = 'import sys; print("permanent failure", file=sys.stderr); sys.exit(2)'
...
def test_git_raises_on_non_transient_error(
    ...
    def fake_run_cmd(cmd: list[str], **kwargs: object) -> CommandResult:
        return CommandResult(returncode=2, stdout="", stderr="permanent failure")
...
def test_git_quiet_suppresses_failure_logging(
    ...
    def fake_run_cmd(cmd: list[str], **kwargs: object) -> CommandResult:
        return CommandResult(returncode=1, stdout="", stderr="config key not found")
    ...
    failure_logs = [record for record in debug_logs if "Command failed" in record.getMessage()]
