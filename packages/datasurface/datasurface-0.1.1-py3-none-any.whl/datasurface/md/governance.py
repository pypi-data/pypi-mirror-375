...
class Dataset(ANSI_SQL_NamedObject, Documentable, JSONable):
    ...
    def checkForBackwardsCompatibility(self, other: object, vTree: ValidationTree) -> bool:
        ...
        if (not isinstance(other, Dataset)):
            ...
            return False
        _ = super().checkForBackwardsCompatibility(other, vTree)
        if (self.originalSchema is None):
            ...
            _ = self.originalSchema.checkForBackwardsCompatibility(other.originalSchema, vTree)
...
class CaptureMetaData(UserDSLObject):
    ...
    @abstractmethod
    def getAllCredentials(self) -> list[Credential]:
        """The CMD should return a list of any/all credentials used by the CMD"""
        pass
...
class DataTransformerOutput(CaptureMetaData):
    ...
    def getAllCredentials(self) -> list[Credential]:
        """The DataTransformerOutput should return a list of any/all credentials used by the DataTransformerOutput"""
        return []
...
class IngestionMetadata(CaptureMetaData):
    ...
    def getAllCredentials(self) -> list[Credential]:
        """The IngestionMetadata should return a list of any/all credentials used by the IngestionMetadata"""
        return [self.credential] if self.credential else []
...
class PlatformServicesProvider(UserDSLObject):
    ...
    @abstractmethod
    def getAllNormalCredentials(self) -> list[Credential]:
        """The PlatformServicesProvider should return a list of normalcredentials used by the PlatformServicesProvider"""
        pass

    @abstractmethod
    def getAllSuperCredentials(self) -> list[Credential]:
        """This returns any credentials which has wide read/write permissions to data managed by the PSP"""
        pass
...
class SuperCredentialUsedAsNormalCredential(ValidationProblem):
    def __init__(self, superCredential: Credential, normalCredential: Credential):
        super().__init__(f"Super credential {superCredential.name} used as normal credential {normalCredential.name}", ProblemSeverity.WARNING)

    def __eq__(self, other: object) -> bool:
        return super().__eq__(other) and isinstance(other, SuperCredentialUsedAsNormalCredential)
...
class Ecosystem(GitControlledObject, JSONable):
    ...
    def __init__(self, name: str, repo: Repository,
        ...
        self.normalCredentials: dict[str, Credential] = dict[str, Credential]()
        self.superCredentials: dict[str, Credential] = dict[str, Credential]()
    ...
    def gatherAllCredentials(self) -> None:
        """This generates the list of all credentials, normal and super, used by the ecosystem"""
        self.normalCredentials.clear()
        self.superCredentials.clear()
        # Credentials are on IngestMetaData or PlatformServiceProviders primary data containers.
        for gz in self.zones.authorizedObjects.values():
            for team in gz.teams.defineAllObjects():
                for store in team.dataStores.values():
                    if store.cmd is not None:
                        credentials = store.cmd.getAllCredentials()
                        for credential in credentials:
                            self.normalCredentials[credential.name] = credential
        # Now add the credentials from the platform services providers
        for psp in self.platformServicesProviders:
            credentials = psp.getAllNormalCredentials()
            for credential in credentials:
                self.normalCredentials[credential.name] = credential

        # Now add the super credentials from the platform services providers
        for psp in self.platformServicesProviders:
            credentials = psp.getAllSuperCredentials()
            for credential in credentials:
                self.superCredentials[credential.name] = credential
    ...
    def lintAndHydrateCaches(self) -> ValidationTree:
        ...
        self.gatherAllCredentials()
        # Check if super credentials are used by any of the normal credentials
        # Ideally, we do not want super credentials to be used by any of the normal credentials
        for superCredential in self.superCredentials.values():
            if superCredential.name in self.normalCredentials:
                ecoTree.addRaw(SuperCredentialUsedAsNormalCredential(superCredential, self.normalCredentials[superCredential.name]))
    ...
    def createGraphForDataTransformer(self, dt: DataTransformerOutput, exportStep: ExportNode) -> None:
        ...
        try:
            ...
            if w.dataContainer:
                ...
                dtStep: DataTransformerNode = self.findExistingOrCreateStep(DataTransformerNode(w, self.platform))
                triggerStep: TriggerNode = self.findExistingOrCreateStep(TriggerNode(w, self.platform))
                ...
                for dsgR in self.roots:
                    if dsgR.workspace == w:
                        for sink in dsgR.dsg.sinks.values():
                            dsrExportStep: ExportNode = self.findExistingOrCreateStep(
