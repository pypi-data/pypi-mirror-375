...
class CalibrationResult:
    ...
    class Result(Enum):
        ...
        def translate_from_rpc(rpc_enum_value):
            ...
            if (
                rpc_enum_value
                == calibration_pb2.CalibrationResult.RESULT_CONNECTION_ERROR
            ):
            ...
            if (
                rpc_enum_value
                == calibration_pb2.CalibrationResult.RESULT_COMMAND_DENIED
            ):
...
class Calibration(AsyncBase):
    ...
    async def calibrate_gyro(self):
        ...
        try:
            async for response in calibrate_gyro_stream:
                ...
                if "NEXT" in [
                    return_code.name for return_code in CalibrationResult.Result
                ]:
    ...
    async def calibrate_accelerometer(self):
        ...
        calibrate_accelerometer_stream = self._stub.SubscribeCalibrateAccelerometer(
            request
        )
    ...
    async def calibrate_magnetometer(self):
        ...
        calibrate_magnetometer_stream = self._stub.SubscribeCalibrateMagnetometer(
            request
        )
    ...
    async def calibrate_level_horizon(self):
        ...
        calibrate_level_horizon_stream = self._stub.SubscribeCalibrateLevelHorizon(
            request
        )
    ...
    async def calibrate_gimbal_accelerometer(self):
        ...
        calibrate_gimbal_accelerometer_stream = (
            self._stub.SubscribeCalibrateGimbalAccelerometer(request)
        )
