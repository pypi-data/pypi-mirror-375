...
class AppointmentScheduler:
    ...
    working_hours : list[tuple[int, int]] | tuple[int, int] | list[tuple[str, str]] | tuple[str, str], default [(8, 18)]
        ...
        Accepts integer hours (e.g., (8, 16)) or strings "HH" / "HH:MM" with minutes == 00 (e.g., ("08:00","16:00")).
        Multiple blocks allowed; they must be non-overlapping and sorted automatically.
    ...
    @staticmethod
    def _parse_hour_like(x: Union[int, str]) -> int:
        """
        Parse an hour-like value into an integer hour [0..23].
        Accepts:
          - int (0..23)
          - str "HH" or "HH:MM" (minutes must be 00 for now)
        """
        if isinstance(x, int):
            if 0 <= x <= 23:
                return x
        ...
        if isinstance(x, str):
            ...
            if m != 0:
                raise ValueError(
                    f"Minutes must be 00 for now (got {m:02d}) in {x!r}. "
                    "If you need minute-level blocks, open an issue or extend the parser safely."
                )
            return h

        raise TypeError(f"Unsupported hour-like type: {type(x)} for value {x!r}")
    ...
    def _normalize_working_hours(
        ...
        """
        Normalize working_hours into a sorted, non-overlapping list of (start_hour, end_hour) integer tuples.
        Accepts:
          - None  -> defaults to [(8, 18)]
          - (start, end)
          - [(start, end), ...]
        Each start/end can be int hour or str "HH"/"HH:MM" with minutes == 00.
        """
        if value is None:
            return [(8, 18)]

        # Allow a single tuple instead of list
        ...
        for i, block in enumerate(value):
            ...
            h_start = AppointmentScheduler._parse_hour_like(start_raw)
            h_end = AppointmentScheduler._parse_hour_like(end_raw)
            if h_start >= h_end:
                raise ValueError(
                    f"`working_hours[{i}]` invalid interval: start >= end ({h_start} >= {h_end})"
                )
            norm.append((h_start, h_end))
        ...
        for j in range(1, len(norm)):
            ...
            if cur[0] < prev[1]:
                raise ValueError(
                    f"`working_hours` blocks overlap: {prev} and {cur}. "
    ...
    def __init__(
        ...
        if ref_date is None:
            self.ref_date: datetime = overall_end.replace(hour=0, minute=0, second=0, microsecond=0)
        else:
            ref_dt = self._to_datetime(ref_date).replace(hour=0, minute=0, second=0, microsecond=0)
        ...
        self.earliest_scheduling_date: datetime = overall_start
        ...
        self._effective_date_ranges: list[tuple[datetime, datetime]] = self._extend_last_range(
            self.date_ranges, self.booking_horizon
        )
        ...
        # ============================
        self.working_hours: list[tuple[int, int]] = self._normalize_working_hours(working_hours)
    ...
    def _iter_working_dates(self) -> list[pd.Timestamp]:
        """Fechas laborables sobre los rangos *efectivos* (capacidad),
        filtradas por working_days, en orden cronológico."""
    ...
    def _weighted_sample_past_slots(self, n: int) -> pd.DataFrame:
        ...
        # Tamaño inválido → devolver DF vacío con columnas mínimas
        if n <= 0:
            return pd.DataFrame(columns=cols)

        # Asegurar que haya slots internos; si no hay, intentar generarlos
            ...
            if hasattr(self, "generate_slots"):
                try:
                    ...
                    if gen is not None and hasattr(gen, "empty") and not gen.empty:
                        # Persistir en el scheduler para usos posteriores
        ...
        df = slots_df.copy()
        # Normalizar fechas y disponibilidad
        ...
        if "is_available" in df.columns:
            ...
            # Si no existe la columna, asumimos todo disponible (fallback defensivo)
            df["is_available"] = True

        # Inferir ref_date si no está seteada: max(fecha)+1d ⇒ todo “pasado”
        ...
        # Filtrar pasado + disponibles
        past = df[(df["appointment_date"] < ref_date) & (df["is_available"])].copy()

        # Si igual no hay pasado (p. ej. todas ≥ ref_date), usar disponibles de todo el rango
        if past.empty:
            past = df[df["is_available"]].copy()
            if past.empty:
                return pd.DataFrame(columns=cols)

        # Pesos calendario (mes × día semana), con fallback a uniforme si algo sale mal
        try:
            ...
            weights = None  # uniforme

        # Si hay menos filas que n, permitir reemplazo para cumplir exactamente n (el test lo exige)
        replace = len(past) < n

        # Semilla reproducible si existe
