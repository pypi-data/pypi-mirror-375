...
"""
        ...
        comment_data = {
            "type": "ebs_initialization",
            "estimated_minutes": round(estimated_minutes, 1) if estimated_minutes > 0 else 0,
            "volumes_count": len(volume_info),
        ...
        ssm_response = ssm.send_command(
            ...
            Comment=f'EBS Volume Initialization - {len(volume_info)} volumes | {comment_json}'
        )
...
def check_initialization_status(command_id: str, region: str = DEFAULT_REGION) -> str:
    ...
    try:
        ...
        estimation_data = {}
        try:
            # Get command details to extract comment
            command_details = ssm.describe_commands(CommandId=command_id)
            if command_details['Commands']:
                comment = command_details['Commands'][0].get('Comment', '')
                # Look for JSON data in comment after the '|' separator
                if '|' in comment:
                    json_part = comment.split('|', 1)[1].strip()
                    estimation_data = json.loads(json_part)
        except Exception as e:
            ...
            try:
                ...
                if estimation_data and estimation_data.get('estimated_minutes', 0) > 0:
                    ...
                    progress_info = {
                        ...
                        "instance_type": estimation_data.get('instance_type', 'Unknown'),
