...
class Order:
    ...
    def check_trailing_stop_v2(self, row: DataRow) -> bool:
        ...
        if self.position_side == OrderPositionSide.LONG:
            ...
            # 고점 갱신하고 양봉인 경우 1분봉 기준 강한 추세가 많기에 추가 조건
            if is_new_highest:
                if row.open < row.close:
                    if row.close < self.limit_price:
                        is_closed = True
                    else:
                        return is_closed
                    
            # 고점 갱신하고 음봉이거나 일반적인 경우
...
class BacktesterABS_v2(ABC):
    ...
    def __init__(self, test_id, symbols, test_start_date='2023-01-01', test_end_date='2024-06-30', 
                data_type='futures', params=None ,pyramiding=1, leverage=1, slippage=0.0005, ptc=0.0005,initial_balance=10000,save_trades=True,plot_results=True, is_hedge=True):
        self.test_id = test_id
    ...
    def check_entry_signals(self, ts, row):
        ...
        while self.sig_ptr < self.sig_len and self.signals['ts'][self.sig_ptr] == ts:
            ...
            if not self.is_hedge:
                is_reverse_position_side = False
                for order in self.active_orders:
                    if order.symbol == self.get_symbol_name(s['sym_id']):
                        if order.position_side != position_side:
                            is_reverse_position_side = True
                            break
                if is_reverse_position_side:
                    continue
