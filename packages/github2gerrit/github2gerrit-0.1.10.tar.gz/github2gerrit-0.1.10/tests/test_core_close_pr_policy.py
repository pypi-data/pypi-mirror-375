...
def test_close_pr_skipped_when_preserve_github_prs_true(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _fail(*args: Any, **kwargs: Any) -> Any:
        raise AssertionError("GitHub API should not be called when PRESERVE_GITHUB_PRS is true")
...
def test_close_pr_not_invoked_for_non_target_event(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _fail(*args: Any, **kwargs: Any) -> Any:
        raise AssertionError("GitHub API should not be called for non-target events")
...
def test_close_pr_invoked_for_pull_request_target_event(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("github2gerrit.core.build_client", lambda: DummyClient())
    monkeypatch.setattr("github2gerrit.core.get_repo_from_env", lambda _c: DummyRepo())
