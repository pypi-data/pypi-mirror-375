...
class IoperatorCls:
	...
	def set(self, index_operator: enums.OperatorA, frame=repcap.Frame.Default, field=repcap.Field.Default) -> None:
		"""TRIGger:SBSW:SWIRe:FRAMe<*>:FLD<*>:IOPerator \n
		Snippet: driver.trigger.sbsw.swire.frame.fld.ioperator.set(index_operator = enums.OperatorA.ANY, frame = repcap.Frame.Default, field = repcap.Field.Default) \n
		...
		self._core.io.write(f'TRIGger:SBSW:SWIRe:FRAMe{frame_cmd_val}:FLD{field_cmd_val}:IOPerator {param}')

	# noinspection PyTypeChecker
	def get(self, frame=repcap.Frame.Default, field=repcap.Field.Default) -> enums.OperatorA:
		"""TRIGger:SBSW:SWIRe:FRAMe<*>:FLD<*>:IOPerator \n
		Snippet: value: enums.OperatorA = driver.trigger.sbsw.swire.frame.fld.ioperator.get(frame = repcap.Frame.Default, field = repcap.Field.Default) \n
		No command help available \n
			:param frame: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Frame')
			:param field: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Fld')
		...
		frame_cmd_val = self._cmd_group.get_repcap_cmd_value(frame, repcap.Frame)
		field_cmd_val = self._cmd_group.get_repcap_cmd_value(field, repcap.Field)
		response = self._core.io.query_str(f'TRIGger:SBSW:SWIRe:FRAMe{frame_cmd_val}:FLD{field_cmd_val}:IOPerator?')
