...
def update_figure(theme, wave_type, A, F, P, theta, R, width, opacity, color, custom_color):
    # 设置主题
    set_theme(theme)
    ...
    # 如果自定义颜色有效，则使用自定义颜色
    import re
    if custom_color and re.match(r'^#([A-Fa-f0-9]{6})
...
, custom_color):
    ...
    # 创建波形图
    fig = wave_func(
        A=A, F=F, P=P, color=selected_color, theta=theta, R=R, width=width, opacity=opacity
    )
