    ...
    def run_analysis(self, send_email: bool = False) -> Tuple[List[Dict], List[Dict], Optional[Path]]:
        ...
        if send_email and self.email_recipient:
            ...
            if email_content_data:
                ...
                # Split email_recipient by commas and strip whitespace
                ...
                email_sent = self.email_sender.send_email(
                    to_email=recipients,
                    subject=f"[v1.0.16-debug] Stock and Mutual Fund Analysis Report - {datetime.now().strftime('%Y-%m-%d')}",
                    html_content=html_content,
                    attachments=attachments,
                    context={
                        "stock_results": email_stock_results,
                ...
                if email_sent:
                    ...
                    logger.warning(f"Email report to {', '.join(recipients)} was not sent due to configuration issues.")
