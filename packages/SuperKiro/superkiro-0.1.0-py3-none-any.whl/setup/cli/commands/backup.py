...
def register_parser(subparsers, global_parser=None) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        "backup",
        help="Backup and restore SuperKiro installations",
        description="Create, list, restore, and manage SuperKiro installation backups",
        epilog="""
Examples:
  SuperKiro backup --create               # Create new backup
  SuperKiro backup --list --verbose       # List available backups (verbose)
  SuperKiro backup --restore              # Interactive restore
  SuperKiro backup --restore backup.tar.gz  # Restore specific backup
  SuperKiro backup --info backup.tar.gz   # Show backup information
  SuperKiro backup --cleanup --force      # Clean up old backups (forced)
        """,
