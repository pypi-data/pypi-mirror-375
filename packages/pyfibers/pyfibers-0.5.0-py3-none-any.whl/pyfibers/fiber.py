...
class Fiber:
    ...
    def resample_potentials(
        ...
        **Note**: Unlike 3D fibers, 1D fiber shifting is temporary and only affects the potential resampling.
        ...
        # Apply shift to fiber coordinates if specified
        if shift != 0 or shift_ratio is not None:
            shifted_start = _shift_fiber(self.length, self.delta_z, shift, shift_ratio, center)
            target_coords = self.longitudinal_coordinates + shifted_start
        else:
        ...
        if not center:
            ...
            target_coords = target_coords - (target_coords[0] + target_coords[-1]) / 2
