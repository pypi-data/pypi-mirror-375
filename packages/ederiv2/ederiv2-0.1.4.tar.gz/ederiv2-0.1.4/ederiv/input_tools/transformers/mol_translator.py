...
class MolTranslator:
    ...
    def parse_token(self, token):
        ...
        - 'downgrade_aromatic_bond_C1_C2' -> ('downgrade', 'aromatic', 'C1', 'C2')  # ignored
        - 'upgrade_aromatic_bond_C1_C2' -> ('upgrade', 'aromatic', 'C1', 'C2')  # ignored
        """
        if token.startswith("add_"):
            ...
            parts = token.split("_")
            bond_type = parts[1]  # should be 'single', 'double', 'triple', or 'aromatic'
            atom1 = parts[3]
            atom2 = parts[4]
            return 'downgrade', bond_type, atom1, atom2
        elif token.startswith("upgrade_"):
            parts = token.split("_")
            bond_type = parts[1]  # should be 'single', 'double', or 'aromatic'
            atom1 = parts[3]
            atom2 = parts[4]
        ...
        for token in instruction_list:
            ...
            if parsed:
                ...
                if action == 'add':
                    ...
                    # Ignore aromatic bond operations
...
if __name__ == "__main__":
    ...
    test_instructions = ['<START>', 'add_single_bond_C1_C2', 
                        ...
                        'upgrade_single_bond_C3_C4',  # Upgrade C3-C4 to double bond
                        'downgrade_double_bond_C3_C4',
                        'downgrade_aromatic_bond_C1_C2',
                        'upgrade_aromatic_bond_C1_C2',
                        '<END>']

    translator4 = MolTranslator()
    smiles4 = translator4.to_smiles(test_instructions, remove_isolated=False)
    print("New Translator4 functionality SMILES:", smiles4)
