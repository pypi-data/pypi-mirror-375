...
class Gestionnaire(ctypes.Structure):
    _fields_ = [
        ...
        ("plein_ecran", c_bool),
        ("temps_frame", c_int),  # Uint32 → c_int
...
# chargement dll dans le dossier de la bibli dans le dossier dll
_pkg_dir = path.dirname(__file__)
dll_path = path.join(_pkg_dir, "dll", "jeu.dll")
...
class _PyCgame:
    ...
    #touches
    def key_just_pressed(self, key_name):
        if not self._g:
            return False
    ...
    # iamges
    def ajouter_image(self, id_, x, y, w, h, id_num,sens=0, rotation=0):
        return jeu.ajouter_image_au_tableau(self._g, id_.encode("utf-8"),
                                            x, y, w, h, sens, id_num, rotation)

    def supprimer_image(self, id_):
        jeu.supprimer_images_par_id(self._g, id_)

    def modifier_image(self, x, y, w, h, id_num,sens=0, rotation=0):
        jeu.modifier_images(self._g, x, y, w, h, sens, id_num, rotation)

    def modifier_texture(self, lien, id_):
        jeu.modifier_texture_image(self._g, lien.encode("utf-8"), id_)
    ...
    def redimensionner_fenetre(self):
        """Redimensionne la fenêtre selon la configuration actuelle."""
        if not self._g:
