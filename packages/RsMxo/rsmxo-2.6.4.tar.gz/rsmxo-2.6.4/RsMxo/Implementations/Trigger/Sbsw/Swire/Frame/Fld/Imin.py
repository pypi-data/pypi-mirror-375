...
class IminCls:
	...
	def set(self, index_min: int, frame=repcap.Frame.Default, field=repcap.Field.Default) -> None:
		"""TRIGger:SBSW:SWIRe:FRAMe<*>:FLD<*>:IMIN \n
		Snippet: driver.trigger.sbsw.swire.frame.fld.imin.set(index_min = 1, frame = repcap.Frame.Default, field = repcap.Field.Default) \n
		...
		self._core.io.write(f'TRIGger:SBSW:SWIRe:FRAMe{frame_cmd_val}:FLD{field_cmd_val}:IMIN {param}')

	def get(self, frame=repcap.Frame.Default, field=repcap.Field.Default) -> int:
		"""TRIGger:SBSW:SWIRe:FRAMe<*>:FLD<*>:IMIN \n
		Snippet: value: int = driver.trigger.sbsw.swire.frame.fld.imin.get(frame = repcap.Frame.Default, field = repcap.Field.Default) \n
		No command help available \n
			:param frame: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Frame')
			:param field: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Fld')
		...
		frame_cmd_val = self._cmd_group.get_repcap_cmd_value(frame, repcap.Frame)
		field_cmd_val = self._cmd_group.get_repcap_cmd_value(field, repcap.Field)
		response = self._core.io.query_str(f'TRIGger:SBSW:SWIRe:FRAMe{frame_cmd_val}:FLD{field_cmd_val}:IMIN?')
