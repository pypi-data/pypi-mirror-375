...
class RateLimiter:
    ...
    async def __call__(self, request: Request=None, response: Response=None):
        ...
        if self.timeout:
            try:
                await asyncio.wait_for(self._exec_rate_limit(request, response), self.timeout)
            except asyncio.TimeoutError:
                print("rate limit timeout")
        else:
            await self._exec_rate_limit(request, response)
        
    async def _exec_rate_limit(self, request: Request, response: Response):
        route_index = 0
        dep_index = 0
