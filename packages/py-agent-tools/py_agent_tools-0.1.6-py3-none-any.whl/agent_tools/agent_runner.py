        ...
        **kwargs: Any,
        ...
        try:
            if stream is False:
                ...
                self.elapsed_time += time.perf_counter() - start_time
                # Ensure result is a non-None string
                self.result = _result.output or ""
            else:
                ...
                async with agent.run_stream(
                    ...
                    async for message in stream_result.stream_text(debounce_by=3):
                        if message:
                            collected_chunks.append(message)
                    # Concatenate all streamed chunks as the final result
                    self.result = "".join(collected_chunks)
