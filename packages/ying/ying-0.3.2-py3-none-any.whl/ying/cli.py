...
class ProgressLog(object):
    def plog(self, cmd, mode):
        # Lazy import to avoid importing heavy dependencies at startup
...
def main():
    ...
    # Provide a lightweight version flag that does not import the package
    try:
        pkg_version = get_version("ying")
    except Exception:
        pkg_version = "unknown"
    parser.add_argument("-V", "--version", action="version", version=f"ying {pkg_version}")
    ...
    if args.subcommand == "notify":
        # Lazy import to avoid importing dynaconf unless needed
