...
class TextmodeConsole:
    def __init__(self, cols: int = 16, rows: int = 16):
        ...
        self._skip_dialogue: bool = False
    ...
    # --------------- Dialogue helpers -----------------
    def setSkipDialogue(self, skip: bool) -> None:
        """Enable or disable skip-dialogue mode.

        When enabled, dialogue() will not render frames or wait for input;
        it will queue the dialogue text and return immediately.
        """
        self._skip_dialogue = bool(skip)
    ...
    async def choices(self, options: List[str], y: int = 0, width: int = 6,
