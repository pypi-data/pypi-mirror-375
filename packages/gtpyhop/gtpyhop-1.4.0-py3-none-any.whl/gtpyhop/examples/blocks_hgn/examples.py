...
""")
    ...
    # Skip the very large examples for brevity in legacy mode
    print("Skipping large IPC examples for brevity...")

    print("""
Call run_lazy_lookahead on a simple problem:
""")
...
def main_session(do_pauses=True, verbose=1):
    ...
    """
    print(f"\n=== Running blocks_hgn examples with PlannerSession (verbose={verbose}) ===")
    ...
    with gtpyhop.PlannerSession(domain=domain_to_use, verbose=verbose) as session:
        with session.isolated_execution():
            ...
            result = session.find_plan(state1, [('pos','b','hand')])
            plan = result.plan if (result and result.success) else False
            th.check_result(plan, False)
...
""")
    ...
    with gtpyhop.PlannerSession(domain=domain_to_use, verbose=verbose) as session:
        with session.isolated_execution():
            ...
            result = session.find_plan(state1, [('unstack','a','b')])
            plan = result.plan if (result and result.success) else None
            th.check_result(plan, [('unstack','a', 'b')])

            result = session.find_plan(state1, [('pos','a','b')])
            plan = result.plan if (result and result.success) else None
            th.check_result(plan, [])

            result = session.find_plan(state1, [('pos','a','hand')])
            plan = result.plan if (result and result.success) else None
            th.check_result(plan, [('unstack','a', 'b')])

            result = session.find_plan(state1, [('pos','c','hand')])
            plan = result.plan if (result and result.success) else None
            th.check_result(plan, [('pickup','c')])
    ...
    print("""
A Multigoal is a data structure that specifies desired values for some of the
state variables. Below, goal1a says we want the blocks in the
configuration "c on b on a on the table".
""")
...
def main_with_args(argv=None):
    ...
    """
    parser = argparse.ArgumentParser(description="Run blocks_hgn examples")
