...
from vansarah.model import KModelForONNX

def export_onnx(model, output):
    onnx_file = output + "/" + "vansarah.onnx"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Export vansarah Model to ONNX", add_help=True)
    parser.add_argument("--inference", "-t", help="test vansarah.onnx model", action="store_true")
    parser.add_argument("--check", "-m", help="check vansarah model", action="store_true")
    parser.add_argument(
        "--config_file", "-c", type=str, default="checkpoints/config.json", help="path to config file"
    )
    parser.add_argument(
        "--checkpoint_path", "-p", type=str, default="checkpoints/vansarah-v1_0.pth", help="path to checkpoint file"
    )
    parser.add_argument(
