...
from mathmaker.core.env import find_tex_format
...
class LaTeX(Structure.Structure):
    ...
    def write_preamble(self, variant='default', required_pkg=None,
                       required_macros=None):
        # fmtfile can only be used for article class, so far.
        fmt_file_available = find_tex_format() is not None
        if required_pkg is None:
            required_pkg = []
        if required_macros is None:
            required_macros = []
        ...
        lxfonts = r"""
% Replace letters by lxfonts
\setmathfont[range={it/{Latin,latin,Greek,greek}}]{lcmmi8mod.otf}
\setmathfont[range={up/{Latin,latin,Greek,greek}}]{lcmmi8mod.otf}
""" if settings.round_letters_in_math_expr else ''
        ...
        variouspkg = [p
                      for p in [amssymb, amsmath, fancyvrb, eurosym,
...
\setmathfont[range={{
  "0030-"0039,                            % Figures 0-9
  "002B, "002D, "00D7, "00F7, "003D,      % + - × ÷ =
  "0028-"0029, "005B-"005D, "007B-"007D,  % ( ) [ ] {{ }}
  "002E, "002C, "003C, "003E, "0025,      % . , < > %
  "00B0, "002F, "2212,                    % ° / −
  "0391, "2212-"2341                      % patch for digits and - sign
}}]{{{font_name}}}LXFONTS
""".format(font_name=settings.font).replace('LXFONTS', lxfonts)
        # language
...
{sisetup}
        ...
        # TikZ setup
        tikz_setup = ''
        if 'handhighlight' in required_macros:
            required.tikz_library['calc'] = True
            required.tikz_library['backgrounds'] = True
        ...
        if required.package.get('cellspace', False):
            ...
            # TEMPORARY BUG FIX (TEXLIVE 2025):
            cellspace += r'''
% temporary fix to TL 2025 compilation failing
% with cellspace and m{} or p{} tabular columns
\makeatletter
...
'''.format(french_parallel_sign_comment=_('Redefinition of ' r'"\parallel" '
                                          'command to draw the parallel '
                                          'symbol slanted as usual in french '
                                          'notations'))
        if 'handhighlight' in required_macros:
            commoncmd += r'''
\NewDocumentCommand{\handhighlight}{ O{} m }{%
  \tikz[baseline=(X.base)]{%
    \node[inner sep=0pt,outer sep=0pt,anchor=base] (X) {#2};%
    \begin{scope}[on background layer]
       \path[fill=YellowOrange!60,opacity=0.6,rounded corners=0.6ex,#1]
        ($(X.north west)+(-0.25em,0.9ex)$) rectangle '''\
        r'''($(X.south east)+(0.25em,-0.7ex)$);
    \end{scope}%
  }%
}
'''
        ...
        preamble = r"""
{documentclass}{symbols}
{font_patch}{sourcecodepro}
{polyglossia}
{language_setup}{decimalcomma}
{siunitx}{xcolor}{tikz_setup}{hyperref}{cancel}{multicol}{placeins}{ulem}"""\
r"""{textcomp}{array}{cellspace}{textpos}{specificpackages}

{specificcommands}{commoncommands}
""".format(documentclass=dc, symbols=variouspkg, sourcecodepro=sourcecodepro,
    ...
    def write_new_line(self, check='', check2='', check3='', check4=''):
        output_str = '\\newline \n'
        if (check == r'\]' or 'addvspace' in check2 or 'newpage' in check3
    ...
    def write_out(self, latex_document: str, pdf_output=False):
        ...
        if pdf_output:
            with NamedTemporaryFile(mode='r+t') as tmp_file:
                ...
                fmt_path = find_tex_format()

                cmd = ['lualatex', '-interaction', 'nonstopmode',
                       tmp_file.name]
                if fmt_path is not None:
                    cmd = ['lualatex', '-interaction', 'nonstopmode',
                           '-fmt', fmt_path, tmp_file.name]
                p = subprocess.Popen(cmd, cwd=settings.outputdir,
                                     stdout=sys.stderr)
                errorcode = p.wait()
                if errorcode:
    ...
    def create_table(self, size, content, **options):
        ...
        if 'justify' in options and type(options['justify']) is list:
            ...
            extra_last_column = "@{}m{0pt}@{}"
            extra_col_sep = " & "
            justify = [r">{\centering}" for _ in range(n_col)]
    ...
    def insert_dashed_hline(self, **options):
        ...
        return "\\begin{tikzpicture}[x=2cm]" \
               + r"\draw[black,line width=0.5pt,dashed] (0,0)--(9,0);" \
               + r"\end{tikzpicture}" + "\n"
