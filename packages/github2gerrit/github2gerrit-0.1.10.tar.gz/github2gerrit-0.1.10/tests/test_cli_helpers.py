...
def test_extract_pr_number(evt: dict[str, object], expected: int | None) -> None:
...
def test_read_github_context_reads_event_and_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_read_github_context_falls_back_to_PR_NUMBER_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_read_github_context_handles_missing_event_file(
    ...
    assert ctx.event_path is not None  # Path is created from the string env var even if it doesn't exist
