...
class ConsoleManager:
    ...
    def _show_banned_users(self):
        ...
        for ban in bans:
            ...
            banned_at = ban.get('timestamp', 'N/A')
            table.add_row(
    ...
    def _export_data(self):
        ...
        filename = inquirer.text(message="Enter filename:", default=f"ffcapcha_export_{time.strftime('%Y%m%d_%H%M%S')}")
        
        if export_type == 'JSON':
            data = {
                'requests': self.api.get_recent_reqs(1000),
                'bans': self.api.get_banned_users(),
                'stats': self.api.get_stats(),
