...
from .exceptions import RenderError, ComponentNotFoundError, JavaScriptEngineError
...
class NodeRenderer:
    ...
    def __init__(self, components_dir: str = 'components', cache_enabled: bool = True,
                 node_executable: str = 'node', timeout: int = 30):
        """
    ...
    def _check_node_availability(self):
        ...
        try:
            ...
            if result.returncode != 0:
                raise JavaScriptEngineError(f"Node.js not found or not working: {self.node_executable}")
    ...
    def render_component(self, component_name: str, props: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            process = subprocess.run(
                [self.node_executable, str(self.ssr_script_path), component_path, props_json, cache_enabled],
                capture_output=True,
                text=True,
            ...
            if process.returncode != 0:
                ...
                raise RenderError(f"Node.js process failed: {error_msg}. Debug info: {debug_info}")
            ...
            raise RenderError(f"Component rendering timed out after {self.timeout} seconds")
