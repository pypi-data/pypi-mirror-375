...
"""A script for generating the atomic coordinates of a model of a :math:`25
\times 25 \times 1 \ \text{nm}^3` large superblock of amorphous carbon (C), that
is subsequently truncated to the same lateral dimensions of a model of
:math:`\text{MoS}_2` generated in another script, and to :math:`0.5 \ \text{nm}`
along the :math:`z`-axis. The superblock, prior to truncation, is generating by
tiling various transformations of a model of a :math:`5 \times 5 \times 1 \
\text{nm}^3` large block of amorphous C. The atomic coordinates of the :math:`5
\times 5 \times 1 \ \text{nm}^3` large block of amorphous C were taken from
Ref. [Ricolleau1]_.
...
The current script takes as input the output data generated by another script,
namely the script ``../../MoS2/scripts/execute_action.py``, hence one must
execute the latter script before executing the former script. See the summary
documentation of the latter script for details on the correct form of the
command to run said script. Upon successful completion of the current script,
the atomic coordinates of the model of the truncated superblock of amorphous C
are stored in the file at the file path ``../data/atomic_coords.xyz``. The
