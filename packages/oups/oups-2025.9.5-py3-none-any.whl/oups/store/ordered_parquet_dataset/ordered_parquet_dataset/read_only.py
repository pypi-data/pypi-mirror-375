...
def file_ids_in_directory(dirpath: str) -> list[int]:
    ...
    """
    return [int(FILE_ID_FROM_REGEX.match(entry.name)["i"]) for entry in scandir(dirpath) if entry.is_file()]
...
class ReadOnlyOrderedParquetDataset(OrderedParquetDataset):
    ...
    This class inherits all functionality from OrderedParquetDataset but
    ...
    def _from_instance(cls, opd):
        """
        Create ReadOnlyOrderedParquetDataset from OrderedParquetDataset instance.

        This internal constructor bypasses normal __init__ to create a read-only
        version of any existing OrderedParquetDataset instance.

        Parameters
        ----------
        opd : OrderedParquetDataset
            Any OrderedParquetDataset instance to make read-only.
        ...
        instance_dict = {key: value for key, value in opd.__dict__.items() if key not in CACHED_PROPERTIES}
        ...
        # At this point there are two objects sharing the same lock:
        #   1) 'opd' (the ephemeral view created by '__getitem__')
        #   2) 'instance' (the read-only view created here)
        # Both will call '__del__' independently and decrement the counter,
        # so we must increment once per object that holds a reference to the lock.
    ...
    def max_file_id(self) -> int:
        ...
        Note: Read-only view scans the filesystem because its metadata may be partial.
    ...
    def _remove_row_group_files(self, file_ids, sort_row_groups=True, key_value_metadata=None):
        ...
        file_ids : list[int]
            File ids to remove.
        sort_row_groups : bool | None, default True
            If `True`, sort row groups after removing files.
        key_value_metadata : dict[str, str] | None, default None
