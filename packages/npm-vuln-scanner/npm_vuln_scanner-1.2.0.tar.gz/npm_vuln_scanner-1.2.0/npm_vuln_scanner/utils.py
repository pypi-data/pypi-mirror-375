...
class OutputFormatter:
    ...
    def print_summary(self, results: Dict[str, List[DetectionResult]], scan_paths: List[Path], duration: float):
        ...
        if critical_count > 0:
            ...
            print("  2. DO NOT run 'npm install' or 'yarn install' until specs are fixed")
            print("  3. First, pin versions to safe releases (before Sept 8, 2025 13:00 UTC)")
            print("  4. Then generate new lock files with the safe versions")
            print("  5. Use 'npm ls' to check installed versions without installing new ones")
...
class JSONOutputFormatter:
    ...
    def format_scan_results(self, results: Dict[str, List[DetectionResult]], 
                           scan_paths: List[Path], duration: float,
                           compromised_versions: Dict[str, List[str]]) -> str:
        """
        ...
        output = {
            ...
            "summary": {
                ...
                "compromised_packages_checked": sorted(compromised_versions.keys()),
                "exit_code": 1 if sum(len(r) for r in results.values()) > 0 else 0
            },
    ...
    def _format_findings(self, results: Dict[str, List[DetectionResult]]) -> Dict:
        ...
        for severity in ['critical', 'high', 'medium', 'warning']:
            ...
            if severity in results:
                for detection in results[severity]:
                    finding = {
                        "package_name": detection.package_name,
                        "file_path": str(detection.file_path),
                        "version_spec": detection.package_version,
                    ...
                    if detection.actual_version:
                        finding["actual_version"] = detection.actual_version
                    
                    if detection.parent_package:
                        finding["parent_package"] = detection.parent_package
                    
                    findings[severity].append(finding)
        
        return findings
    
    def _get_recommendations(self, results: Dict[str, List[DetectionResult]]) -> Dict:
        ...
        if critical_count > 0:
            recommendations["immediate_actions"] = [
                "CRITICAL: Confirmed compromised packages are installed",
                "These packages contain malicious code targeting crypto wallets",
                "Immediately downgrade or remove these packages",
                "Check for suspicious transactions if your app handles crypto",
                "Rotate any exposed credentials or keys"
            ]
            recommendations["risk_level"] = "CRITICAL"
        elif high_count > 0:
            recommendations["immediate_actions"] = [
                "Your version specifications could install compromised versions",
                "DO NOT run 'npm install' or 'yarn install' until specs are fixed",
                "First, pin versions to safe releases (before Sept 8, 2025 13:00 UTC)",
                "Then generate new lock files with the safe versions",
                "Consider using 'npm ls' to check currently installed versions without installing"
            ]
            recommendations["risk_level"] = "HIGH"
        elif total_count > 0:
            recommendations["immediate_actions"] = [
                "Packages from the compromised list are present but using safe versions",
            ...
            recommendations["immediate_actions"] = []
            recommendations["risk_level"] = "NONE"
    ...
    def format_list_output(self, compromised_versions: Dict[str, List[str]]) -> str:
        """
        Format the list of compromised packages as JSON.
        
        Returns:
            JSON string with compromised packages list
        """
        output = {
            "metadata": {
                "scanner_version": self.scanner_version,
                "generated_at": datetime.now().isoformat(),
                "description": "NPM packages compromised after September 8, 2025 13:00 UTC"
            },
            "compromised_packages": {},
            "summary": {
                "total_packages": len(compromised_versions),
                "total_versions": sum(len(versions) for versions in compromised_versions.values())
            }
        }
        
        # Add package details
        for package_name in sorted(compromised_versions.keys()):
            output["compromised_packages"][package_name] = {
                "versions": compromised_versions[package_name],
                "version_count": len(compromised_versions[package_name])
            }
