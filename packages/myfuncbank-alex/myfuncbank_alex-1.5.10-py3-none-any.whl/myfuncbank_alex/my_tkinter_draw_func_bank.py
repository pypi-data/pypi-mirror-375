...
def readExcel2dataframe(sheetname,filepath,skiprows):
...
def update_chart_show(cursheet,df,ax,fig,title,**kwargs):
    matplot_draw(cursheet,df,ax,fig,title,**kwargs)
    fig.canvas.draw_idle()

    height_Mouse(cursheet,df,ax,fig,**kwargs)



def matplot_draw(cursheet,df,ax,fig,title,**kwargs):
    ...
    selected_df=df.loc[str(cursheet.begin_date):str(cursheet.end_date)]
    x = selected_df.index
    lines=[]

    # 判断当前的dataframe是否有缺失值，如果有，则使用线性填充方法补齐缺失区域数据
    has_missing_values = selected_df.isna().any().any()
    if has_missing_values:
        selected_df=selected_df.astype(float) #将wind为object类型的数据转换为float型
        # 对包含数据缺失的所有列进行线性插值
        selected_df.interpolate(method='linear', limit_area='inside', inplace=True)
    ...
    for column in selected_df.columns:
        if "【副轴】"in column:
            ...
            if "【柱状图】" in column:
                ...
                selected_df.rename(columns={column : new_colname},inplace=True)
                bar=ax_twin.bar(x, selected_df[new_colname], width=10, label=new_colname,color='green')
                lines.append(bar)
                ...
                selected_df[new_colname] = pd.to_numeric(selected_df[new_colname], errors='coerce')
                # 取该列的最小值,指定面积图填充从0.8倍下限值开始向上填充，否则默认下限是从0值开始填充屏幕
                y_min = selected_df[new_colname].min()
                area = ax_twin.fill_between(x, selected_df[new_colname], 0.8 * y_min, alpha=0.5, label=new_colname,
                                            color='green')
                lines.append(area)
                ...
                if "【虚线】" in column:
                    ...
                    line,=ax_twin.plot(x, selected_df[new_colname], label=new_colname, color='orange', linestyle='--')
                    lines.append(line)
                else:
                    line, = ax_twin.plot(x, selected_df[column], label=column, color='green')
                    lines.append(line)
            ...
            if "【柱状图】" in column:
                ...
                new_colname=column.replace('【柱状图】',"")
                selected_df.rename(columns={column : new_colname},inplace=True)
                bar=ax.bar(x, selected_df[new_colname], width=10, label=new_colname)
                lines.append(bar)
                ...
                # 取该列的最小值,指定面积图填充从0.8倍下限值开始向上填充，否则默认下限是从0值开始填充屏幕
                y_min = selected_df[new_colname].min()
                area = ax.fill_between(x, selected_df[new_colname], 0.8 * y_min, alpha=0.5, label=new_colname)
                lines.append(area)
                ...
                if "【虚线】" in column:
                    ...
                    line,=ax.plot(x, selected_df[new_colname], label=new_colname, linestyle='--')
                    lines.append(line)
                else:
                    line, = ax.plot(x, selected_df[column], label=column)
                    lines.append(line)
...
def height_Mouse(cursheet,df,ax,fig,**kwargs): # 实现当鼠标悬浮在图形线条上某个位置时，动态显示出此点在x,y轴上的取值
    #解除绑定之前的所有事件处理函数
    ...
    selected_df = df.loc[str(cursheet.begin_date):str(cursheet.end_date)]
    pure_df=selected_df.dropna(how='all') #删除全是nan的行
...
def insert_timespan_button(sheet,df,fatherFrame,years):
    def on_button_click():
        reverse_flag = False
        ls = df.index.tolist()
        if ls[0] > ls[len(ls)-1]:
