...
from .models import GetStorage, AddStorage, RemoveStorage, UpdateStorage
...
async def send_message(request: AddStorage):
    key = request.key
    data = request.data
...
async def send_message(request: RemoveStorage):
    key = request.key
    data = request.data
    ...
    for item in data:
        item_id = string_to_uuid(str(item))
...
async def send_message(request: UpdateStorage):
    key = request.key
    data = request.data
    ...
    item_id = string_to_uuid(str(data))
