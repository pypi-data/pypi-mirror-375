...
class OracleThinConnectionHelper(object):
    """
    Oracle connection string handling/parsing class. This class handles all of the common logic for connecting to Oracle databases with both oracledb and sqlalchemy.

    This version leaves out the call to `oracledb.init_oracle_client()` to prevent thick
    mode from being enabled.
    """

    def __init__(self, login, password, ldap_server, oracle_context):
        """
        Constructor, chops up login string into individual parts to be used to create connections.
        :param login:    the database login string
        :param password: the database password
        ...
        try:
            if re.match(short_form_regex, login):
                ...
                with ldap3.Connection(server) as conn:
                    ...
                    dsn = conn.entries[0].orclNetDescString.value
            else:
                # parse out the port, host, and dsn
                username = login_parts[0]
                server = login_parts[1]
            ...
            raise Exception("Invalid login string.") from exc
    ...
    def get_sql_alchemy_engine(self, **kwargs):
        """
        :return: A sqlalchemy engine object
        """
        return create_engine(self.__engine_string, **kwargs)
