...
class PythonRuleExtractor:
    ...
    def extract_from_adr(self, adr) -> Dict[str, Any]:
            ...
            for match in matches:
                if isinstance(match, tuple):
                    if len(match) == 2:
                        ...
                        preferred_lib = self._normalize_python_library(preferred.strip())
        ...
        if "architecture" in tags or "layering" in tags:
            rules["architectural_rules"].extend(self._extract_architectural_rules(content, adr))
...
def generate_ruff_config(adr_directory: Union[Path, str] = "docs/adr") -> str:
    ...
    if all_banned_imports:
        ...
        ruff_config["flake8-import-conventions"] = {
            "banned-imports": banned_list
        }
...
def generate_pyproject_ruff_section(adr_directory: Union[Path, str] = "docs/adr") -> Dict[str, Any]:
