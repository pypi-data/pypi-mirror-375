"""Configuration settings for Forklift application."""
...
class CacheConfig(BaseModel):
    ...
    cache_dir: str = Field(default=".forklift/cache", description="Cache directory")
...
class LoggingConfig(BaseModel):
    ...
    file_path: str = Field(default="forklift.log", description="Log file path")
...
class ForkliftConfig(BaseSettings):
    """Main configuration for Forklift application."""
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "ForkliftConfig":
...
def load_config(config_path: str | Path | None = None) -> ForkliftConfig:
    """Load configuration from file or environment variables."""
    if config_path:
        config = ForkliftConfig.from_file(config_path)
    else:
        ...
        possible_paths = [
            ...
            Path("forklift.yml"),
            Path("forklift.json"),
            Path(".forklift/config.yaml"),
            Path(".forklift/config.yml"),
            Path(".forklift/config.json"),
        ]

        config = None
        for path in possible_paths:
            if path.exists():
                config = ForkliftConfig.from_file(path)
                break
