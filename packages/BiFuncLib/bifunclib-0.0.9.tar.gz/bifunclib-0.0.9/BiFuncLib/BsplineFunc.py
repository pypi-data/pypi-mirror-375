            ...
            if n > nbasis or lambda_ > 0:
                    ...
                    if lambda_ > 0:
                        ...
                        if eigvals[neiglow] <= 0:
                            raise ValueError("Eigenvalue(NBASIS-NDERIV) of penalty matrix is not positive.")
                        penfac = eigvecs[:, :neiglow] @ np.diag(np.sqrt(eigvals[:neiglow]))
                    ...
                    if covariates is not None:
                        ...
                        if not onewt:
                            if matwt:
                                basismat_aug[ind1[:, None], ind2] = wtfac @ covariates
                            else:
