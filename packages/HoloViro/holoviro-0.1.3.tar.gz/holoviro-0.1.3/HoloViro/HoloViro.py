...
def _pipInstall(args, quiet=False):
    """
    Run pip in-process (no subprocess, no extra console window).
    `args` should be a list like: ["install", "--disable-pip-version-check", "<spec>", ...]
    Raises RuntimeError on nonzero exit.
    """
    _bootstrapPip()

    # Prefer the official in-process entrypoint
    def _call_pip_main():
        from pip._internal.cli.main import main as pip_main  # type: ignore
        return pip_main(args)

    # Fallback: execute `python -m pip` semantics in-process
    def _call_pip_runpy():
        old_argv = sys.argv[:]
        sys.argv = ["pip"] + args
        try:
    ...
    rc = 0
    if quiet:
        # Silence pip output to match DEVNULL behavior
    ...
    if rc != 0:
        raise RuntimeError(f"pip exited with status {rc}")
...
def _ensurePackagesCore(requires, quiet=True, envVarOptOut="NO_AUTO_PIP", failHard=False):
    ...
    with _PIP_LOCK:
        ...
        for import_name, pip_spec in missing:
            try:
                # In-process pip (no subprocess, no window flash)
