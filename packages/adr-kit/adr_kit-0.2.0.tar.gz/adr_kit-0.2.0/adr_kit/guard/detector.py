...
class PolicyViolation:
    ...
    violation_type: str  # 'import_disallowed', 'import_not_preferred', 'boundary_violated'
    severity: str        # 'error', 'warning', 'info'
    message: str         # Human-readable description
    file_path: str       # File where violation occurred
    line_number: Optional[int] = None    # Line number if applicable
    adr_id: str = None                   # ADR that defines the violated policy
    adr_title: str = None               # Title of the relevant ADR
...
class SemanticPolicyMatcher:
    ...
    def find_relevant_adrs(self, file_changes: Dict[str, List[str]], 
                          context_lines: List[str] = None) -> List[SemanticMatch]:
...
class GuardSystem:
    ...
    def __init__(self, project_root: Path = None, adr_dir: str = "docs/adr"):
    ...
    def analyze_diff(self, diff_text: str, build_index: bool = True) -> CodeAnalysisResult:
        ...
        for file_path, imports in file_changes.items():
            ...
            for adr_id, policy_info in self._policy_cache.items():
                ...
                all_adrs_for_violation_check.append(type('MockMatch', (), {
                    'adr_id': adr_id,
    ...
    def _check_file_violations(self, file_path: str, imports: List[str], 
                              relevant_adrs: List[SemanticMatch]) -> List[PolicyViolation]:
        ...
        for adr_match in relevant_adrs[:5]:  # Check top 5 most relevant
            ...
            violations.extend(self._check_import_violations(
                file_path, imports, adr, policy, language
            ))
            
            # Check boundary violations (simplified for now)
            violations.extend(self._check_boundary_violations(
                file_path, imports, adr, policy
            ))
        
        return violations
    
    def _check_import_violations(self, file_path: str, imports: List[str], 
                               adr: ADR, policy: Any, language: str) -> List[PolicyViolation]:
            ...
            for disallowed in disallowed_imports:
                if self._import_matches_pattern(import_name, disallowed):
                    violations.append(PolicyViolation(
                        ...
                        suggested_fix=self._suggest_import_alternative(import_name, policy),
        ...
        if policy.imports and policy.imports.prefer:
            for import_name in imports:
                ...
                preferred_alternative = self._find_preferred_alternative(import_name, policy.imports.prefer)
    ...
    def _check_boundary_violations(self, file_path: str, imports: List[str],
                                 adr: ADR, policy: Any) -> List[PolicyViolation]:
        ...
        for rule in policy.boundaries.rules:
            if "cross-layer" in rule.forbid.lower() or "layer" in rule.forbid.lower():
                ...
                for import_name in imports:
                    ...
                    if file_layer and import_layer and self._violates_layer_rule(file_layer, import_layer):
                        violations.append(PolicyViolation(
    ...
    def _suggest_import_alternative(self, import_name: str, policy: Any) -> Optional[str]:
    ...
    def _find_preferred_alternative(self, import_name: str, preferred_imports: List[str]) -> Optional[str]:
    ...
    def _are_similar_imports(self, import1: str, import2: str) -> bool:
        ...
        for base, alternatives in common_alternatives.items():
            if import1.startswith(base) and any(import2.startswith(alt) for alt in alternatives):
                return True
            if import2.startswith(base) and any(import1.startswith(alt) for alt in alternatives):
                return True
    ...
    def _generate_summary(self, violations: List[PolicyViolation], 
                         file_changes: Dict[str, List[str]], 
                         relevant_adrs: List[SemanticMatch]) -> str:
        ...
        if warning_count > 0:
            summary_parts.append(f"⚠️ {warning_count} warning{'s' if warning_count != 1 else ''}")
