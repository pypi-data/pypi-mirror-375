...
Graph.prototype = {
     ...
     *              representation
     */
    addNode: function(id, content, shapeType='ellipse') {
        /* testing if node is already existing in the graph */
        if(this.nodes[id] == undefined) {
            this.nodes[id] = new Graph.Node(id, content, shapeType);
        }
 ...
 * Node
 */
Graph.Node = function(id, node, shape){
    node = node || {};
    node.id = id;
    node.edges = [];
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawNode: function(node) {
        ...
        if(node.shape) {
            ...
            console.log("opoint")
            console.log(opoint)
            console.log("Math.round(point[0] - opoint.x) => "+Math.round(point[0] - opoint.x))
            console.log("Math.round(point[1] - opoint.y) => "+Math.round(point[1] - opoint.y))
        ...
        if(!node.render) {
            node.render = function(r, node) {
                ...
                if(node.shapeType == 'ellipse'){
                    elem = r.ellipse(0, 0, 50, 30).attr({fill: color, stroke: color, "stroke-width": 2});
                    fontSz = 22;
                }
                else if(node.shapeType == 'rectangle'){
                    elem = r.rect(0, 0, 100, 20).attr({fill: color, stroke: color, "stroke-width": 2});
                    fontSz = 13;
                }
                
                /* set DOM node ID */
                elem.node.id = node.label || node.id;
                shape = r.set().
                    push(elem).
        ...
        // if (xTrans < 5){ xTrans = 5;}
        // if (xTrans > $(document).width()-500){ xTrans = $(document).width()-500;}
        // if (yTrans < 5){ yTrans = 5;}
        // if (yTrans > $(document).height()-250){ yTrans = $(document).height()-250;}

        shape.translate(xTrans, yTrans);

        console.log("AAA => "+xTrans)
        console.log("BBB => "+yTrans)
