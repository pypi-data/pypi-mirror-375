...
class AsyncHTTPSession(HTTPSession):
    ...
    async def post(self, url: str, data: Dict[str, Any] | None = None, headers: Dict[str, Any] | None = None, is_form_url_encoded: bool = False, files: RequestFiles | None = None) -> HTTPResponse:
        data_string: str | None = None
        if data is not None:
            data_string = json.dumps(data)
        ...
        http_response: HTTPResponse = HTTPResponse(await self.client.post(url, data=data, headers=headers, files=files)) if is_form_url_encoded else HTTPResponse(await self.client.post(url, data=data_string, headers=headers, files=files))  # type: ignore[arg-type]
        if not http_response.is_successful:
            AsyncHTTPSession.raise_http_exception(http_response)
