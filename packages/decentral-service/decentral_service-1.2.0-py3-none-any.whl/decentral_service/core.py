...
class DecentralService:
    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                self._running = True
                if self.debug:
                    self.logger.debug(f"Service configuration: host={self.host}, port={self.port}, workers={self.max_workers}, api={self.enable_api}")
    ...
    def submit_task(self, 
        ...
        if self.debug:
            self.logger.debug(f"Task {task_id} submitted: type={task_type}, priority={priority}, overwrite={overwrite}")
