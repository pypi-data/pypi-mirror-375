...
(function() {
    ...
    SessionDataManager.prototype.cleanupExpiredSessions = function() {
        if (!this.settingsManager) {
            console.log('📊 設定管理器未初始化，跳過會話清理');
            return;
        }

        const retentionHours = this.settingsManager.get('sessionHistoryRetentionHours', 720); // 預設30天
        console.log('📊 使用會話保留時間:', retentionHours, '小時 (', Math.round(retentionHours / 24), '天)');
        
        const retentionMs = retentionHours * 60 * 60 * 1000;
        const now = TimeUtils.getCurrentTimestamp();

        const originalCount = this.sessionHistory.length;
        console.log('📊 開始清理會話，當前會話數量:', originalCount);
        ...
        this.sessionHistory = this.sessionHistory.filter(function(session) {
            ...
            const shouldKeep = sessionAge < retentionMs;
            
            if (!shouldKeep) {
                console.log('📊 移除過期會話:', session.session_id, '年齡:', Math.round(ageInHours), '小時');
            }
        ...
        if (cleanedCount > 0) {
            console.log('📊 清理了', cleanedCount, '個過期會話，剩餘', this.sessionHistory.length, '個會話');
            this.saveToServer();
        } else {
            console.log('📊 沒有過期會話需要清理，當前會話數量:', this.sessionHistory.length);
        }
    };
