...
class BloggerYear(object):
    ...
    def __init__(self, env):
        ...
        for (dirpath, dirnames, filenames) in os.walk(top):
            ...
            currentday = None
            docnames = sorted(
                [fn[:-4] for fn in filenames if fn.endswith('.rst')])
...
class MainBlogIndexDirective(InsertInputDirective):
    ...
    def get_rst(self):
        ...
        if index != 'index':
            raise Exception(
                "Allowed only inside file named `<blogname>/index.rst`")
        text = ''
        years = get_blogger_years(env, blogname)
...
.. rubric:: {0}""".format("All entries:")
        ...
        for day in days:
            date_text = format_date(day.date,
                                    format="long",
                                    locale=self.language).strip()
            text += ("\n\n**{}** --- ".format(date_text))
            text += ", ".join(
                [":doc:`{}`".format(docname) for docname in day.docnames])
...
"""
    ...
    # %02d%02d""" % (day.month, day.day)
        ...
        retval = tpl.render(calendar=text, intro=intro, year=blogger_year.year)
        # days=blogger_year.days)
        # print("="*80)
        # print(retval)
        # print("="*80)
