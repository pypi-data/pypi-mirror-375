...
class ZlBusSdk(Thread):
    def __init__(self, advNameStr: str = '', searchStr: str = 'zl', bleScannerPass: bool = False):
        ...
        self.pkt.setFlowIdFormat(zlapi.e_Tk_FLOW_FORMAT.TK_FLOW_ID_FORMAT_8)

        # step 3, 手动设置上传数据的数据格式，用于解包格式识别，错误的数据格式，无法解包成功，包数据直接丢弃
        # self.pkt.setDataFormat(0, zlapi.e_Upload_DataFormat.NEW_UPLOAD_DATA_TIME | zlapi.e_Upload_DataFormat.NEW_UPLOAD_DATA_QUATERNION|
        #                    zlapi.e_Upload_DataFormat.NEW_UPLOAD_DATA_GYRO | zlapi.e_Upload_DataFormat.NEW_UPLOAD_DATA_LIN_ACC)
        # or
        # 通过蓝牙获取, 见listenData函数中的如下调用 await client.write_gatt_char(CmdCtrl_WriteNotify_UUID, zlapi.ul_getDataFormat(), response=True)
            ...
            while self.pkt.count() > 0:
                ...
                if block != None:
                    if isinstance(block, zlb.ImuDataBlock):
                        ...
                        if state:
                            print("电池电量:", level, "%")
                    elif isinstance(block, zlapi.ul_UploadDataFormatBlock):
                        format = block.uploadDataFormat
                        print("上报数据格式:", hex(format))
                    elif isinstance(block, zlapi.hl_FlowIdFormatBlock):
    ...
    async def listenData(self):
        try:
            ...
            async with BleakClient(self.blueAddr) as self.client:
                ...
                await self.client.write_gatt_char(CmdCtrl_WriteNotify_UUID, zlapi.ul_getDataFormat(), response=True)
                await asyncio.sleep(0.1)
                # await self.client.write_gatt_char(CmdCtrl_WriteNotify_UUID, zlapi.hl_configOutDataPort(zlapi.e_DataOutPort.TK_RF_PORT | zlapi.e_DataOutPort.TK_UART_PORT), response=True)
                await self.client.write_gatt_char(CmdCtrl_WriteNotify_UUID, zlapi.hl_configOutDataPort(zlapi.e_DataOutPort.TK_RF_PORT), response=True)
                await asyncio.sleep(0.1)
...
def bleDemo(advNameStr: str = '', bleScannerPass: bool = False):
    icDev = ZlBusSdk(advNameStr, bleScannerPass)
