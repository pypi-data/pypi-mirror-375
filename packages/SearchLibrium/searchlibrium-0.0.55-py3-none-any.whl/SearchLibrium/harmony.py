    ...
    def remove_non_unique_solutions(self):
        ...
        for sol in self.memory:
            try:
                ...
                print(f"sol: {sol}")
                print(f"crit: {self.criteria}")

                # Create the tuple based on the criteria
                sol_tuple = tuple([sol[self.criteria[0]], sol[self.criteria[1]]])  # Adjust self.criteria as needed
            except KeyError as e:
                # Handle missing keys
                print(f"KeyError: {e} - sol: {sol}, criteria: {self.criteria}")
