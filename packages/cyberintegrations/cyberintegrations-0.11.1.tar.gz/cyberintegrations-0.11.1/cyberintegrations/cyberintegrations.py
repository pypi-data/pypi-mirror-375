...
class Poller(object):
    ...
    def send_request(
        self, endpoint, method="GET", data=None, params=None, decode=True, **kwargs
    ):
    ...
    def set_product(
        ...
        self._session.headers["User-Agent"] = (
            "{product_type}/{product}/{integration}/{username}/{library}".format(
                ...
                library=_merge(
                    TechnicalConsts.library_name, TechnicalConsts.library_version
                ),
            )
        )
...
class TIPoller(Poller):
    ...
    def execute_action_by_id(
        self, collection_name, feed_id, action, request_params=None, decode=True
    ):
    ...
    def get_hunting_rules_collections(self):
        ...
        for item in response:
            if item.get("huntingRulesUsed"):
                ...
                if (
                    collection_name in CollectionConsts.TI_COLLECTIONS_INFO.keys()
                    or collection_name in CollectionConsts.ONLY_SEARCH_COLLECTIONS
                ):
...
class DRPPoller(Poller):
    ...
    def change_status(self, feed_id, status):
        ...
        if violation:
            if (
                ...
                logger.exception(
                    AttributeError("Can not change the status of the selected issue")
                )
    ...
    def get_seq_update_dict(
        ...
        fmt_str = r"%Y-%m-%d"
        timestamp = (
            datetime.strptime(date, fmt_str).replace(tzinfo=timezone.utc).timestamp()
        )
...
class DRPUpdateFeedGenerator(FeedGenerator):
    ...
    def _get_params(self):
        ...
        if self.generator_info.approve_states:
            params.update({"approveState[]": self.generator_info.approve_states[0]})
            ...
            if filter_map:
                for _filter in filter_map:
                    ...
                    if ignore:
                        ...
                        if self._keys_found(
                            feed=feed, keys_road=_keys_road, check_list=_check_list
                        ):
                            scip_flag = True
                            continue
