...
def parse_dynamics_data(dynamic_item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    """
    try:
        card_type = dynamic_item.get("type")
        ...
        text_content = ""
        if module_dynamic.get("desc") and module_dynamic["desc"].get("text"):
            text_content = module_dynamic["desc"]["text"]
        ...
        return {
            ...
            "url": f"https://t.bilibili.com/{dynamic_item.get('id_str')}" if dynamic_item.get('id_str') else None
        }
