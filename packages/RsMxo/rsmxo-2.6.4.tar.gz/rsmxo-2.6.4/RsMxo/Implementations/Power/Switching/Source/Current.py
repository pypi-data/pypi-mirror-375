...
class CurrentCls:
	...
	def set(self, source_current: enums.SignalSource, power=repcap.Power.Default) -> None:
		"""POWer<*>:SWITching:SOURce:CURRent \n
		Snippet: driver.power.switching.source.current.set(source_current = enums.SignalSource.C1, power = repcap.Power.Default) \n
		...
		self._core.io.write(f'POWer{power_cmd_val}:SWITching:SOURce:CURRent {param}')

	# noinspection PyTypeChecker
	def get(self, power=repcap.Power.Default) -> enums.SignalSource:
		"""POWer<*>:SWITching:SOURce:CURRent \n
		Snippet: value: enums.SignalSource = driver.power.switching.source.current.get(power = repcap.Power.Default) \n
		Selects the current source waveform. Active analog channels and math waveforms can be used. \n
			:param power: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Power')
			:return: source_current: No help available"""
		power_cmd_val = self._cmd_group.get_repcap_cmd_value(power, repcap.Power)
		response = self._core.io.query_str(f'POWer{power_cmd_val}:SWITching:SOURce:CURRent?')
