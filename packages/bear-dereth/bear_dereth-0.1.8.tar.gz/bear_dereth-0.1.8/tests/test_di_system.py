...
from bear_dereth.tools.di import HAS_DEPENDENCY_INJECTOR
from bear_dereth.tools.di._container import DeclarativeContainer
from bear_dereth.tools.di._resources import Singleton
from bear_dereth.tools.di._wiring import Provide, Provider, _get_function_signature, _parse_params, inject
...
def console_container() -> type[SimpleTestContainer]:
    ...
    Provide.set_container(SimpleTestContainer)
...
def multi_service_container() -> type[MultiTestContainer]:
    ...
    Provide.set_container(MultiTestContainer)
    return MultiTestContainer


def test_dependency_injector_installed() -> None:
    """Test that dependency_injector is not installed for these tests since we are testing our own implementation."""
    assert not HAS_DEPENDENCY_INJECTOR, "dependency_injector package is not installed."
...
class TestDeclarativeContainerMeta:
    ...
    def test_captures_service_annotations(self) -> None:
        ...
        assert "console" in TestContainer._service_types
        assert "database" in TestContainer._service_types
        assert "_private_attr" not in TestContainer._service_types
        assert TestContainer._service_types["console"] == Console
...
class TestProvider:
    ...
    def test_provide_class_getitem_with_type(self, console_container) -> None:
        """Test Provide[SomeClass] uses the class name."""
        Provide.set_container(console_container)
        result: Provider = Provide.__class_getitem__(Console)
        assert isinstance(result, Provider)
        assert result.service_name == "console"

    def test_provide_class_getitem_with_string(self, console_container) -> None:
...
class TestParseParams:
    ...
    def test_parse_params_creates_missing_service(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        args, kwargs = _parse_params(test_func)
    ...
    def test_parse_params_ignores_non_provide_defaults(self) -> None:
        ...
        args, kwargs = _parse_params(test_func, TestContainer)
...
class TestServiceMetadata:
    """Test service metadata tracking functionality."""

    def test_class_registration_metadata(self) -> None:
        """Test that class registration creates proper metadata."""

        class TestContainer(DeclarativeContainer):
            console: Console

        TestContainer.register("console", Console)
        metadata = TestContainer._service_metadata["console"]

        assert metadata["type_name"] == "Console"
        assert metadata["module"] == "rich.console"
        assert metadata["is_class"] is True
        assert "id" in metadata

    def test_instance_registration_metadata(self) -> None:
        """Test that instance registration creates proper metadata."""

        class TestContainer(DeclarativeContainer):
            console: Console

        console_instance = Console()
        TestContainer.register("console", console_instance)

        container = TestContainer()

        metadata: FrozenDict = container._service_metadata["console"]

        assert metadata["type_name"] == "Console"
        assert metadata["module"] == "rich.console"
        assert metadata["is_class"] is False
        assert metadata["id"] == id(console_instance)

    def test_metadata_is_frozen(self) -> None:
        """Test that metadata dictionaries are frozen for caching."""
        ...
        TestContainer.register("console", Console())
        container = TestContainer()
        metadata = freeze(container.get_all_metadata())

        assert hash(metadata) is not None  # Should be hashable (frozen)
...
class TestCachingBehavior:
    ...
    def test_function_signature_caching(self) -> None:
        ...
        sig1: Signature = _get_function_signature(test_func)
        # Second call should hit cache
        sig2: Signature = _get_function_signature(test_func)
...
class TestEdgeCases:
    ...
    def test_empty_container(self) -> None:
        ...
        assert len(EmptyTestContainer.get_all_metadata()) == 0
...
class TestTeardown:
    def test_teardown_callbacks_run_after_shutdown(self) -> None:
        ...
        MockContainer.register("service", Service())
