"""Core exceptions for Forklift application."""
...
class ForkliftError(Exception):
    """Base exception for all Forklift errors."""
...
class ForkliftConfigurationError(ForkliftError):
...
class ForkliftValidationError(ForkliftError):
...
class ForkliftNetworkError(ForkliftError):
...
class ForkliftAuthenticationError(ForkliftError):
...
class ForkliftOutputError(ForkliftError):
...
class ErrorHandler:
    ...
    def handle_error(self, error: Exception, context: str = "") -> None:
        ...
        """
        # Determine exit code
        if isinstance(error, ForkliftError):
    ...
    def _format_error_message(self, error: Exception, context: str) -> str:
        ...
        if self.debug and not isinstance(error, ForkliftError):
            import traceback
    ...
    def validate_unicode_text(self, text: str, context: str = "text") -> str:
        ...
        Raises:
            ForkliftUnicodeError: If text cannot be processed
        """
