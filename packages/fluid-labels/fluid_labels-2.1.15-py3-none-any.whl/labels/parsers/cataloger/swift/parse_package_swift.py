...
def get_dependencies(
    ...
    for package_node in adj(graph, dep_node, depth=2):
        if is_valid_package_node(graph, package_node):
            source_url, package_version = extract_package_details(graph, package_node)
            if source_url and package_version:
                pkg = create_package(graph, package_node, source_url, package_version, reader)
                if pkg:
                    packages.append(pkg)
...
def create_package(
    ...
    return new_swift_package_manager_package(
        source_url=source_url, version=package_version, location=new_location, revision=None
    )
