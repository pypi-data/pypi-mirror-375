...
class SearchIndex(BasePlugin):
    ...
    def _flatten_meta(self, meta, exclude_paths=None, current_path=''):
        """Recursively extract all string values from meta, excluding dot-paths in exclude_paths (ignoring list indices)."""
        if exclude_paths is None:
            exclude_paths = set()
        result = []
        def strip_indices(path):
            # Remove [number] from path for matching
            return '.'.join([p.split('[')[0] for p in path.split('.')])
        if isinstance(meta, dict):
            for k, v in meta.items():
                path = f"{current_path}.{k}" if current_path else k
                if strip_indices(path) in exclude_paths:
                    continue
                result.extend(self._flatten_meta(v, exclude_paths, path))
        elif isinstance(meta, list):
            for idx, item in enumerate(meta):
                path = f"{current_path}[{idx}]" if current_path else f"[{idx}]"
                result.extend(self._flatten_meta(item, exclude_paths, path))
        elif isinstance(meta, str):
            result.append(meta)
        return result

    def _add_entry(self, title, text, loc, meta=None):
        text = text.replace('\u00a0', ' ')
        text = re.sub(r'[ \t\n\r\f\v]+', ' ', text.strip())
        entry = {
            'title': title,
        ...
        if meta:
            entry['meta'] = meta
            exclude_paths = set(meta.get('exclude_meta_keys', []))
            entry['meta_text'] = ' '.join(self._flatten_meta(meta, exclude_paths))
