...
class SimpleLinksClient:
    ...
    def __init__(self, host: str, port: int, server_secret: str, subnet: str = "default",
        ...
        try:
            self.p2p_manager = TcpP2PManager(local_port=20002)
            self.logger.info("P2P direct connection enabled (TCP port 20002)")
        except Exception as e:
            self.logger.warning(f"P2P initialization failed: {e}, using server relay only")
    ...
    async def setup_linux_tun(self) -> bool:
        ...
        try:
            ...
            os.system(f"ip addr add {self.virtual_ip}/{prefix_len} dev slink0")
    ...
    async def tun_reader(self):
        ...
        while self.running and self.connected:
            try:
                ...
                if self.p2p_manager:
                    try:
                        self.logger.debug(f"CLIENT: Trying P2P send to {target_ip} ({len(data)} bytes)")
                        sent_via_p2p = await self.p2p_manager.try_direct_send(target_ip, data)
                        if sent_via_p2p:
                            self.stats['p2p_sent'] += 1
                            self.logger.info(f"CLIENT: Sent packet via P2P to {target_ip} ({len(data)} bytes)")
                        else:
                            self.logger.debug(f"CLIENT: P2P send to {target_ip} failed, using server relay")
                    except Exception as e:
                        self.logger.debug(f"CLIENT: P2P send exception: {e}, falling back to server relay")
                else:
                    self.logger.debug(f"CLIENT: No P2P manager, using server relay for {target_ip}")
                ...
                if not sent_via_p2p:
                    ...
                    self.logger.debug(f"Forwarded packet via server to {target_ip} ({len(data)} bytes)")
            ...
            async for message in self.websocket:
                try:
                    ...
                    if msg_type == 'packet':
                        ...
                        await self.handle_peer_list(data)
                    else:
    ...
    async def handle_peer_list(self, data: dict):
        """处理来自服务器的P2P peer列表"""
        try:
            peer_list = data.get('peers', [])
            self.logger.info(f"CLIENT: Received peer list with {len(peer_list)} peers")
            
            for peer in peer_list:
                vip = peer.get('virtual_ip')
                pip = peer.get('private_ip')
                hostname = peer.get('hostname', 'unknown')
                self.logger.debug(f"CLIENT: Peer {vip}@{pip} ({hostname})")
            
            if self.p2p_manager and peer_list:
                self.p2p_manager.update_peers(peer_list)
                self.logger.info(f"CLIENT: Updated P2P peer list: {len(peer_list)} peers")
            elif not self.p2p_manager:
                self.logger.warning(f"CLIENT: Received peer list but P2P manager not available")
            else:
                self.logger.debug(f"CLIENT: Empty peer list received")
                
        except Exception as e:
            self.logger.error(f"CLIENT: Error handling peer list: {e}")
    ...
    async def p2p_packet_processor(self):
        ...
        while self.running and self.connected:
            try:
                # 使用阻塞等待数据包，避免轮询开销
                packet = await self.p2p_manager.wait_for_packet(timeout=0.1)
                if packet:
                    ...
                    if hasattr(self, 'tun_fd') and self.tun_fd is not None:
                        try:
                            ...
                            self.logger.debug(f"Received packet via P2P from {peer_virtual_ip} ({len(data)} bytes)")
                        except (OSError, BrokenPipeError) as e:
                            if self.running:
                                self.logger.debug(f"TUN write error: {e}")
                
            except Exception as e:
                if self.running and self.connected:
                    self.logger.error(f"P2P packet processor error: {e}")
    ...
    async def handle_incoming_packet(self, data: dict):
        ...
        try:
            ...
            self.logger.debug(f"Received packet from {source_ip} via server relay")
            
            # 直接写入TUN设备，避免线程池开销
            if hasattr(self, 'tun_fd') and self.tun_fd is not None:
                try:
                    os.write(self.tun_fd, bytes.fromhex(packet_data))
            ...
            try:
                ...
                if self.p2p_manager:
                    try:
                        ...
                        self.logger.info(f"P2P manager started - Private IP: {self.local_ip}, TCP port: {self.p2p_manager.local_port}")
                        # 启动P2P数据包处理任务
                        asyncio.create_task(self.p2p_packet_processor())
                    except Exception as e:
                        self.logger.warning(f"P2P manager start failed: {e}, using server relay only")
                        self.p2p_manager = None
    ...
    async def disconnect(self):
        ...
        if self.p2p_manager:
            try:
                ...
                self.logger.debug("P2P manager stopped")
            except Exception as e:
                self.logger.debug(f"P2P manager stop error: {e}")
        ...
        if total_sent > 0 or total_received > 0:
            ...
            self.logger.info(f"Sent - P2P: {self.stats['p2p_sent']}, Relay: {self.stats['relay_sent']}")
            self.logger.info(f"Received - P2P: {self.stats['p2p_received']}, Relay: {self.stats['relay_received']}")
            
            if total_sent > 0:
                p2p_percentage = (self.stats['p2p_sent'] / total_sent) * 100
                self.logger.info(f"P2P efficiency: {p2p_percentage:.1f}% of sent traffic")
...
def info() -> None:
    ...
    if not _client_instance:
        print(f"SimpleLinks Client v{__version__}: Not running")
        return
    
    client_status = status()
    
    print(f"=== SimpleLinks Client v{__version__} Status ===")
