...
{
	if (region.eP.d7 === region.dG.d7)
	{
		return 'on line ' + region.eP.d7;
	}
	return 'on lines ' + region.eP.d7 + ' through ' + region.dG.d7;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { fQ: 'hidden', fp: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { fQ: 'mozHidden', fp: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { fQ: 'msHidden', fp: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { fQ: 'webkitHidden', fp: 'webkitvisibilitychange' }
		: { fQ: 'hidden', fp: 'visibilitychange' };
}
...
function _Browser_getViewport()
{
	return {
		...
		e0: {
			...
			e5: _Browser_doc.documentElement.clientWidth,
...
function _Browser_getScene()
{
	...
	return {
		e5: Math.max(body.scrollWidth, body.offsetWidth, elem.scrollWidth, elem.offsetWidth, elem.clientWidth),
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		function done(response) {
			callback(toTask(request.fL.a(response)));
		}

		var xhr = new XMLHttpRequest();
		xhr.addEventListener('error', function() { done($elm$http$Http$NetworkError_); });
		xhr.addEventListener('timeout', function() { done($elm$http$Http$Timeout_); });
		xhr.addEventListener('load', function() { done(_Http_toResponse(request.fL.b, xhr)); });
		$elm$core$Maybe$isJust(request.dd) && _Http_track(router, xhr, request.dd.a);

		try {
			xhr.open(request.cX, request.gZ, true);
		} catch (e) {
			return done($elm$http$Http$BadUrl_(request.gZ));
		}
...
function _Http_configureRequest(xhr, request)
{
	...
	xhr.responseType = request.fL.d;
	xhr.withCredentials = request.fb;
}
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {dN: fragment, dU: host, em: path, eo: port_, eu: protocol, ev: query};
	});
	...
	function (toResult, response) {
		switch (response.$) {
			...
			case 3:
				...
				return $elm$core$Result$Err(
					$elm$http$Http$BadStatus(metadata.gO));
			default:
				var body = response.b;
				return A2(
...
var $elm$http$Http$onEffects = F4(
	function (router, cmds, subs, state) {
		return A2(
			...
			A3($elm$http$Http$updateReqs, router, cmds, state.ez));
	});
			...
			return $elm$http$Http$Request(
				{
					fb: r.fb,
					cy: r.cy,
					fL: A2(_Http_mapExpect, func, r.fL),
					cS: r.cS,
					cX: r.cX,
					dc: r.dc,
					dd: r.dd,
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{fb: false, cy: r.cy, fL: r.fL, cS: r.cS, cX: r.cX, dc: r.dc, dd: r.dd, gZ: r.gZ}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{cy: $elm$http$Http$emptyBody, fL: r.fL, cS: _List_Nil, cX: 'GET', dc: $elm$core$Maybe$Nothing, dd: $elm$core$Maybe$Nothing, gZ: r.gZ});
};
...
var $author$project$Formulas$getSeriesInfo = function (model) {
	return $elm$http$Http$get(
		{
			fL: $elm$http$Http$expectString($author$project$Formulas$GotFormulas),
			gZ: A3(
...
var $author$project$Formulas$Formula = F5(
	function (name, imeta, meta, source, kind) {
		return {cU: imeta, f1: kind, f8: meta, aC: name, gN: source};
	});
var $author$project$Formulas$Imeta = F7(
	function (formula, tzaware, index_type, value_type, contenthash, index_dtype, value_dtype) {
		return {fw: contenthash, cM: formula, fX: index_dtype, fY: index_type, gW: tzaware, g$: value_dtype, g0: value_type};
	});
...
var $author$project$Formulas$checkWordsInFormula = F2(
	function (listWord, formula) {
		var checkWordInString = function (word) {
			return A2($elm$core$String$contains, word, formula.cU.cM) || A2($elm$core$String$contains, word, formula.aC);
		};
		return A2(
			$elm$core$List$all,
			function (bool) {
			...
			_List_fromArray(
				[
					...
					A3(
						...
						_List_fromArray(
							[
								A2($elm$url$Url$Builder$string, 'name', formula.aC)
							])))
				]),
			_List_fromArray(
				[
...
var $elm$parser$Parser$Advanced$DeadEnd = F4(
	function (row, col, problem, contextStack) {
		return {fs: col, fx: contextStack, gy: problem, gI: row};
	});
...
var $elm$parser$Parser$Advanced$fromState = F2(
	function (s, x) {
		return A2(
			...
			A4($elm$parser$Parser$Advanced$DeadEnd, s.gI, s.fs, x, s.i));
	});
...
var $elm$parser$Parser$Advanced$keyword = function (_v0) {
	...
	return function (s) {
		var _v1 = A5($elm$parser$Parser$Advanced$isSubString, kwd, s.c, s.gI, s.fs, s.b);
		var newOffset = _v1.a;
		var newRow = _v1.b;
		var newCol = _v1.c;
		return (_Utils_eq(newOffset, -1) || (0 <= A3(
			...
			progress,
			0,
			{fs: newCol, i: s.i, j: s.j, c: newOffset, gI: newRow, b: s.b});
	};
};
...
var $elm$parser$Parser$Advanced$bumpOffset = F2(
	function (newOffset, s) {
		return {fs: s.fs + (newOffset - s.c), i: s.i, j: s.j, c: newOffset, gI: s.gI, b: s.b};
	});
	...
	function (invalid, expecting, intSettings, floatSettings, intPair, s) {
		...
		if (floatOffset < 0) {
			return A2(
				$elm$parser$Parser$Advanced$Bad,
				true,
				A4($elm$parser$Parser$Advanced$fromInfo, s.gI, s.fs - (floatOffset + s.c), invalid, s.i));
		} else {
		...
		return $elm$parser$Parser$Advanced$number(
			{
				...
				d$: $elm$core$Result$Ok($elm$core$Basics$toFloat),
				f$: invalid,
...
var $elm$parser$Parser$Advanced$token = function (_v0) {
	...
	return function (s) {
		var _v1 = A5($elm$parser$Parser$Advanced$isSubString, str, s.c, s.gI, s.fs, s.b);
		var newOffset = _v1.a;
		var newRow = _v1.b;
		var newCol = _v1.c;
		...
		return $elm$parser$Parser$Advanced$number(
			{
				...
				dT: $elm$core$Result$Err(invalid),
				d$: $elm$core$Result$Ok($elm$core$Basics$identity),
				f$: invalid,
				ej: $elm$core$Result$Err(invalid)
			});
	});
	...
	function (isGood, offset, row, col, src, indent, context) {
		...
		while (true) {
			...
			if (_Utils_eq(newOffset, -1)) {
				return {fs: col, i: context, j: indent, c: offset, gI: row, b: src};
			} else {
...
var $elm$parser$Parser$Advanced$variable = function (i) {
	return function (s) {
		var firstOffset = A3($elm$parser$Parser$Advanced$isSubChar, i.eP, s.c, s.b);
		if (_Utils_eq(firstOffset, -1)) {
			return A2(
				$elm$parser$Parser$Advanced$Bad,
				false,
				A2($elm$parser$Parser$Advanced$fromState, s, i.dJ));
		} else {
			var s1 = _Utils_eq(firstOffset, -2) ? A7($elm$parser$Parser$Advanced$varHelp, i.d_, s.c + 1, s.gI + 1, 1, s.b, s.j, s.i) : A7($elm$parser$Parser$Advanced$varHelp, i.d_, firstOffset, s.gI, s.fs + 1, s.b, s.j, s.i);
			var name = A3($elm$core$String$slice, s.c, s1.c, s.b);
			return A2($elm$core$Set$member, name, i.eB) ? A2(
				$elm$parser$Parser$Advanced$Bad,
				false,
				A2($elm$parser$Parser$Advanced$fromState, s, i.dJ)) : A3($elm$parser$Parser$Advanced$Good, true, name, s1);
		}
	};
};
var $elm$parser$Parser$variable = function (i) {
	return $elm$parser$Parser$Advanced$variable(
		{dJ: $elm$parser$Parser$ExpectingVariable, d_: i.d_, eB: i.eB, eP: i.eP});
};
	...
	return A2(
		...
		A2(
			...
			$elm$parser$Parser$variable(
				{
					d_: $elm$core$Basics$neq(quotechar),
					eB: $elm$core$Set$empty,
					eP: $elm$core$Basics$always(true)
				}),
			$elm$parser$Parser$symbol($author$project$Lisp$quote)));
}();
		...
		while (true) {
			...
			if (_Utils_eq(newOffset, -1)) {
				return A3(
					...
					0,
					{fs: col, i: s0.i, j: s0.j, c: offset, gI: row, b: s0.b});
			} else {
				if (_Utils_eq(newOffset, -2)) {
					var $temp$isGood = isGood,
						$temp$offset = offset + 1,
...
var $elm$parser$Parser$Advanced$chompWhile = function (isGood) {
	return function (s) {
		return A5($elm$parser$Parser$Advanced$chompWhileHelp, isGood, s.c, s.gI, s.fs, s);
	};
};
...
var $elm$parser$Parser$DeadEnd = F3(
	function (row, col, problem) {
		return {fs: col, gy: problem, gI: row};
	});
var $elm$parser$Parser$problemToDeadEnd = function (p) {
	return A3($elm$parser$Parser$DeadEnd, p.gI, p.fs, p.gy);
};
...
var $elm$parser$Parser$Advanced$run = F2(
	function (_v0, src) {
		var parse = _v0;
		var _v1 = parse(
			{fs: 1, i: _List_Nil, j: 1, c: 0, gI: 1, b: src});
		if (!_v1.$) {
			var value = _v1.b;
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							A2($author$project$Info$layoutFormula, model, formula.cU.cM)
						]))
				]));
	};
	var filtreFormulas = $elm$core$List$isEmpty(listWords) ? model.bM : A2(
		$elm$core$List$filter,
...
var $author$project$Formulas$main = function () {
	var init = function (input) {
		var model = {aR: input.aR, dH: _List_Nil, bM: _List_Nil, aC: input.aC, b$: ''};
	...
	return $elm$browser$Browser$element(
		{
			...
			gX: $author$project$Formulas$update,
			g1: $author$project$Formulas$view
		});
}();
