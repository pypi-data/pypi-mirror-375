...
class RelevanceRanker:
    ...
    def __init__(self, strategy: RankingStrategy = RankingStrategy.HYBRID):
        ...
        self.tech_categories = {
            "frontend": ["react", "vue", "angular", "javascript", "typescript", "css", "html", "ui", "ux", "component"],
            "backend": ["api", "server", "service", "nodejs", "python", "java", "go", "database", "sql"],
            "data": ["database", "sql", "nosql", "mongodb", "postgres", "mysql", "redis", "schema", "migration"],
            "testing": ["test", "testing", "jest", "pytest", "spec", "coverage", "unit", "integration"],
            "deployment": ["deploy", "ci", "cd", "docker", "kubernetes", "aws", "cloud", "infrastructure"],
            "security": ["auth", "authentication", "authorization", "security", "oauth", "jwt", "encryption"]
        }
    
    def rank_adrs_for_task(self, adrs: List[ADR], task_context: TaskContext) -> List[RelevanceScore]:
    ...
    def _calculate_relevance_score(self, adr: ADR, task_context: TaskContext) -> RelevanceScore:
        ...
        if status_score > 0:
            status_val = adr.front_matter.status.value if hasattr(adr.front_matter.status, 'value') else str(adr.front_matter.status)
        ...
        task_type_score, task_type_reasons = self._score_task_type_alignment(adr, task_context)
    ...
    def _score_technology_overlap(self, adr: ADR, task_context: TaskContext) -> Tuple[float, List[str]]:
        ...
        if len(overlap) <= 3:
            ...
            reasons.append(f"Shared technologies: {', '.join(top_overlap)} and {len(overlap)-3} more")
        
        return tech_score, reasons
    
    def _score_keyword_overlap(self, adr: ADR, task_context: TaskContext) -> Tuple[float, List[str]]:
    ...
    def _score_policy_relevance(self, adr: ADR, task_context: TaskContext) -> Tuple[float, List[str]]:
        ...
        if policy.imports and task_context.task_type.value in ["dependency", "feature", "refactor"]:
            score += 0.3
            reasons.append("Has import/dependency policies")
        
        # Boundary policies relevant for architectural tasks
        if policy.boundaries and any(word in task_context.keywords for word in ["architecture", "structure", "layer"]):
            score += 0.2
            reasons.append("Has architectural boundary policies")
        
        return min(score, 0.5), reasons  # Cap policy score
    
    def _score_task_type_alignment(self, adr: ADR, task_context: TaskContext) -> Tuple[float, List[str]]:
    ...
    def _compute_weighted_score(self, factors: Dict[str, float]) -> float:
        ...
        weighted_sum = sum(weights.get(factor, 0) * value for factor, value in factors.items())
    ...
    def get_top_n_relevant(self, adrs: List[ADR], task_context: TaskContext, n: int = 5) -> List[Tuple[ADR, RelevanceScore]]:
        ...
        top_scores = scores[:n]
        return [(adr_lookup[score.adr_id], score) for score in top_scores if score.adr_id in adr_lookup]