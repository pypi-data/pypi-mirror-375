...
from tools.clickhouse_tools import ClickHouseConnection, ClickHouseToolExecutor, CLICKHOUSE_TOOLS
...
class ClickHouseAgent:
    ...
    def __init__(self, config: ClickHouseConfig):
        ...
        self.current_database = config.database  # Track current database
                    ...
                    if tool_calls and len(tool_calls) > 0:
                        ...
                        for tool_call in tool_calls:
                            ...
                            try:
                                ...
                                # Update current database if switch_database was called
                                if tool_name == "switch_database":
                                    try:
                                        result_data = json.loads(result)
                                        if "current_database" in result_data:
                                            self.current_database = result_data["current_database"]
                                            # Update system prompt with new database
                                            system_message = self._build_system_prompt()
                                            self.conversation_history[0] = {"role": "system", "content": system_message}
                                    except:
    ...
    async def _execute_tool(self, tool_name: str, tool_input: Dict[str, Any]) -> str:
        ...
        try:
            if tool_name == "list_databases":
                ...
                return await self.tool_executor.get_current_database()
                ...
                return await self.tool_executor.show_config()
...
CURRENT DATABASE: {self.current_database}
...
- list_databases: See all available databases
- switch_database: Change to a different database
- list_tables: Find available data in current database
...
DATABASE CONTEXT:
- You are currently in the '{self.current_database}' database
- Use switch_database to change databases when needed
- Always consider which database you're working in
...
Agentic Behavior is very important.
Bad example:
User asks you to do X, and for best results to do X you need to lets say call and reason over the results of A B C then the bad example would be you do A then ask if user wants you to continue with B then you do B and wait for confirmation to contnue with C, This is a bad example.
Good example:
You do A B C directly without waiting for the user to ask for it AND YOU DONT REPEAT THE SAME TOOL CALL AGAIN AND AGAIN.
