...
class Action:
    ...
    def package_manager_options(self):
        ...
        if self.plugin_package_manager == 'pip':
            defaults = ['--disable-pip-version-check', '--no-cache-dir']
            # Add defaults if not already present
            options_list = defaults.copy()
            for option in user_options:
                if option not in options_list:
                    options_list.append(option)
        elif self.plugin_package_manager == 'uv':
            defaults = ['--no-cache']
            # Add defaults if not already present
            options_list = defaults.copy()
            for option in user_options:
        ...
        # Return the options in the format expected by Ray
        if self.plugin_package_manager == 'pip':
