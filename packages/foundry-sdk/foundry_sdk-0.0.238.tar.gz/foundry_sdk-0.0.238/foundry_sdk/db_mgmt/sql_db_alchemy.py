    ...
    def handle_insertion_multi_line_copy(
        ...
        *,
        ...
        try:
            ...
            # For UPDATE mode, we'll use DELETE + INSERT optimization instead of ON CONFLICT
            if mode == "RAISE":
            ...
            else:  # UPDATE mode - use DELETE + INSERT optimization
            ...
            for offset in range(0, data.height, chunk_size):
                ...
                if use_on_conflict:
                    # Traditional approach for RAISE and IGNORE modes
                    insert_sql = (
                    ...
                    # DELETE + INSERT optimization for UPDATE mode
                    pk_col_list = ", ".join(f'"{c}"' for c in pks)
                    
                    if non_pk_cols:
                        # 1) Create temp table with rows that need updating
                        rows_to_update_table = f"temp_rows_to_update_{batch_count}"
                        # Create properly qualified column list for SELECT
                        qualified_non_id_col_list = ", ".join(f't."{c}"' for c in non_id_cols)
                        cursor.execute(
                            f'CREATE TEMP TABLE "{rows_to_update_table}" AS '  # noqa: S608
                            f'SELECT {qualified_non_id_col_list} FROM "{temp_table}" t '
                            f'INNER JOIN "{table}" r ON {" AND ".join(f"t.\"{c}\" = r.\"{c}\"" for c in pks)}'
                        )
                        
                        # 2) Delete existing rows that will be replaced
                        cursor.execute(
                            f'DELETE FROM "{table}" '  # noqa: S608
                            f'WHERE ({pk_col_list}) IN ('
                            f'SELECT {pk_col_list} FROM "{rows_to_update_table}"'
                            f')'
                        )
                        
                        # 3) Insert new rows (those not in existing table)
                        cursor.execute(
                            f'INSERT INTO "{table}" ({non_id_col_list}) '  # noqa: S608
                            f'SELECT {qualified_non_id_col_list} FROM "{temp_table}" t '
                            f'LEFT JOIN "{rows_to_update_table}" u ON {" AND ".join(f"t.\"{c}\" = u.\"{c}\"" for c in pks)} '
                            f'WHERE u."{pks[0]}" IS NULL'
                        )
                        
                        # 4) Insert updated rows
                        cursor.execute(
                            f'INSERT INTO "{table}" ({non_id_col_list}) '  # noqa: S608
                            f'SELECT {non_id_col_list} FROM "{rows_to_update_table}"'
                        )
                        
                        # 5) Clean up update temp table
                        cursor.execute(f'DROP TABLE IF EXISTS "{rows_to_update_table}"')
                    else:
                        # If no non-pk columns, just insert new records (ignore existing)
                        qualified_non_id_col_list = ", ".join(f't."{c}"' for c in non_id_cols)
                        cursor.execute(
                            f'INSERT INTO "{table}" ({non_id_col_list}) '  # noqa: S608
                            f'SELECT {qualified_non_id_col_list} FROM "{temp_table}" t '
                            f'LEFT JOIN "{table}" r ON {" AND ".join(f"t.\"{c}\" = r.\"{c}\"" for c in pks)} '
                            f'WHERE r."{pks[0]}" IS NULL'
                        )
    ...
    *,
    ...
    try:
        ...
        if mode == "RAISE":
            ...
            # First insert new records (same as before)
            insert_sql = (
                f'INSERT INTO "{table}" ({non_id_col_list}) '  # noqa: S608
                f'SELECT {non_id_col_list} FROM "{temp_table}" AS t '
                f'WHERE NOT EXISTS (SELECT 1 FROM "{existing_keys_table}" AS e WHERE {" AND ".join(f't."{c}" = e."{c}"' for c in pks)})'
            ...
            # For existing records: DELETE then INSERT (faster than UPDATE)
            non_pk_cols = [c for c in cols if c not in pks]
            if non_pk_cols:
                # 1) Create temp table with rows that need updating (exist in both tables)
                rows_to_update_table = f"temp_rows_to_update_worker_{offset}"
                # Create properly qualified column list for SELECT
                qualified_non_id_col_list = ", ".join(f't."{c}"' for c in non_id_cols)
                cursor.execute(
                    f'CREATE TEMP TABLE "{rows_to_update_table}" AS '  # noqa: S608
                ...
                # 3) Insert updated rows from temp table
                cursor.execute(
                    f'INSERT INTO "{table}" ({non_id_col_list}) '  # noqa: S608
                    f'SELECT {non_id_col_list} FROM "{rows_to_update_table}"'
                )
                
                # 4) Clean up update temp table
                cursor.execute(f'DROP TABLE IF EXISTS "{rows_to_update_table}"')
