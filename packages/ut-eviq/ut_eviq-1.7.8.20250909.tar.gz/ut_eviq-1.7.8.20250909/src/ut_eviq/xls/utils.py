...
from ut_xls.pd.ioipathwb import PdIoiPathnmWb
...
class EvexIoc:
    ...
    def read_wb_exp_to_df(cls, kwargs: TyDic) -> TnPdDf:
        _pddf: TnPdDf = PdIoiPathnmWb.read_wb_to_df(Cfg.InPath.evex, Cfg.sheet_exp, kwargs)
        return _pddf

    @classmethod
    def read_wb_exp_to_aod(cls, kwargs: TyDic) -> TnAoD:
        _aod: TnAoD = PdIoiPathnmWb.read_wb_to_aod(Cfg.InPath.evex, Cfg.sheet_exp, kwargs)
        return _aod
