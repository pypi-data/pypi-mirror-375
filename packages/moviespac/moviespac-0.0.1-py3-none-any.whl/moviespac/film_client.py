...
# Literal[...] -> restreint une valeur à quelques chaînes précises
# Union[A, B] -> valeur de type A ou B

from .schemas import MovieSimple, MovieDetailed, RatingSimple, TagSimple, LinkSimple, AnalyticsResponse, MovieScore, GenreStat
from .film_config import MovieConfig

OutputFormat = Literal["pydantic", "dict", "pandas"]

class MovieClient: # point d'entree unique  pour consommer l'api
    ...
    def _format_output(self, data, model, output_format: OutputFormat):
    ...
    def get_top_rated_movies(self, limit: int = 10, min_votes: int = 50, output_format: OutputFormat = "pydantic"):
        url = f"{self.movie_base_url}/movies/top-rated"
        params = {"limit": limit, "min_votes": min_votes}
        reponse = httpx.get(url, params=params)
        return self._format_output(reponse.json(), MovieScore, output_format)


    def get_popular_genres(self, limit: int = 15, output_format: OutputFormat = "pydantic"):
        url = f"{self.movie_base_url}/analytics/popular-genres"
        params = {"limit": limit}
        reponse =httpx.get(url, params=params)
        return self._format_output(reponse.json(), GenreStat, output_format)
