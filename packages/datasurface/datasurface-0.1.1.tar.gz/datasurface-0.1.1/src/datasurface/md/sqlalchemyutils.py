    ...
    if exists:  # type: ignore[attr-defined]
        ...
        try:
            if is_postgres:
                ...
                rc: Optional[Row[Any]] = result.fetchone()
                if rc is None:
                    raise Exception("View definition not found")
                else:
                    currentViewDef = rc[0]
            elif is_mssql:
...
def reconcileViewSchemas(
        engine: Engine, store: Datastore,
        viewNameMapper: Callable[[Dataset], str],
        underlyingTableMapper: Callable[[Dataset], str]) -> dict[str, bool]:
    ...
    Args:
        ...
        viewNameMapper: Function that takes a dataset and returns the view name "def view_name_mapper(dataset: Dataset) -> str:"
