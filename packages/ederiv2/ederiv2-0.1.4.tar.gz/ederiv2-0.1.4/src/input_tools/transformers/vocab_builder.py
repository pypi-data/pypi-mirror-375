...
class ActionVocab:
    ...
    def _replace_prefix(self, old_prefix, new_prefix):
        """
        Replace actions starting with `old_prefix` with a new action
        that starts with `new_prefix`.
        """
        new_actions = []
        for action in self.idx2action:
            if action.startswith(old_prefix):
                new_action = action.replace(old_prefix, new_prefix, 1)
                new_actions.append(new_action)
        return new_actions

    def replace_prefix_in_vocab(self, old_prefix, new_prefix):
        """
        Public method to replace actions starting with `old_prefix` with
        `new_prefix` and update the vocabulary.
        """
        new_actions = self._replace_prefix(old_prefix, new_prefix)
        self.idx2action.extend(new_actions)
        self.action2idx.update({action: idx for idx, action in enumerate(self.idx2action)})
...
if __name__=="__main__":
    ...
    dataset_actions=smiles_dataset_action_gen.generate(frag_smiles, dfs_kwargs)
