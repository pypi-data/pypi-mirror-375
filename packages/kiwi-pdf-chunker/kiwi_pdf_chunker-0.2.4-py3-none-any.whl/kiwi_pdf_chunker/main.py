...
class PDFParser: 
    ...
    def __init__(self, 
                 ...
                 azure_openai_endpoint_embedding=None): 
                            ...
                            if structure_type == 'good':
                                ...
                                try:
                                    ...
                                    parsed_tables[table_id] = {
                                        "parsed_data": tree, 
                                        "coordinates": yolo_coordinates  # Now matches YOLO format
                                    }
                                ...
                                if plumber_bbox:
                                    ...
                                    parsed_string = self._parse_bad_table_with_vision(table_image)
    ...
    def _run_docling_parser(self, pdf_path: str) -> Optional[dict]: 
        ...
        """
        Runs Docling in-process (formerly via external run_docling.py).
        Mirrors run_docling.py behavior and returns its JSON as a dict.
        """
        ...
        try:
            ...
            # Match run_docling.py: use .dict()
            doc_dict = docling_doc.dict()

            # Inject markdown text per table, matching run_docling.py
            if 'tables' in doc_dict:
                for i, table_obj in enumerate(docling_doc.tables):
                    markdown_text = table_obj.export_to_markdown()
                    doc_dict['tables'][i]['markdown_text'] = markdown_text
            ...
            try:
                import traceback
                logger.error(f"Error processing {pdf_path} with docling:\n{traceback.format_exc()}")
            except Exception:
                logger.error(f"Error running Docling in-process: {e}")
            return None
    ...
    def _classify_and_summarize_table_content(self, parsed_tables, table_categories):
        ...
        for table_id, table_data in parsed_tables.items():
            try:
                ...
                payload_1 = {
                    "messages": [{"role": "system", "content": [{"type": "text", "text": TABLE_CLASSIFICATION_SYSTEM_PROMPT}]}, {"role": "user", "content": [{"type": "text", "text": TABLE_CLASSIFICATION_USER_PROMPT}]}],
                    "temperature": 0.7
                }
                ...
                payload_2 = {
                    "messages": [{"role": "system", "content": [{"type": "text", "text": TABLE_SUMMARY_SYSTEM_PROMPT}]}, {"role": "user", "content": [{"type": "text", "text": TABLE_SUMMARY_USER_PROMPT}]}],
                    "temperature": 1.0
                }
                ...
                payload_3 = {
                    "messages": [{"role": "system", "content": [{"type": "text", "text": TABLE_ID_COLUMN_SYSTEM_PROMPT}]}, {"role": "user", "content": [{"type": "text", "text": TABLE_ID_COLUMN_USER_PROMPT}]}],
                    "temperature": 0.7
                }
                ...
                try:
                    output_list = json.loads(output_id_columns)  # This should work for the list format
    ...
    def _classify_table_structure(self, pdfplumber_bbox, pdf_page):
        """
        Classifies a table's structure as 'good' or 'bad' based on layout uniformity.
        'Good' tables have uniform, single-line rows.
        'Bad' tables have complex, multi-line rows that break a simple grid structure.

        Args:
            table_bbox_coords (list): The bounding box of the table [x1, y1, w, h] from YOLO in PIXELS.
            pdf_page (pdfplumber.page.Page): The pdfplumber page object.
        ...
        try:
            ...
            # --- Text Presence Check ---
            if not table_crop.chars or len(table_crop.chars) < 30:
                logger.debug(f"Classified as BAD - Not enough text characters found ({len(table_crop.chars)}).")
                return "bad"

            # --- Row Uniformity Check ---
            lines = defaultdict(list)
            for char in table_crop.chars:
                rounded_top = round(char['top'], 1)
                lines[rounded_top].append(char)
            
            if len(lines) <= 1:
                logger.debug("Classified as BAD - Only one or zero text lines found.")
                return "bad"

            sorted_line_tops = sorted(lines.keys())
            gaps = [sorted_line_tops[i+1] - sorted_line_tops[i] for i in range(len(sorted_line_tops) - 1)]
            gaps = [g for g in gaps if g > 1.0]

            if not gaps:
                logger.debug("Classified as BAD - No significant gaps between text lines found.")
                return "bad"

            median_gap = np.median(gaps)
            if median_gap == 0:
                logger.debug("Classified as BAD - Median gap is zero.")
                return "bad"

            std_dev_gap = np.std(gaps)
            
            if std_dev_gap / median_gap > 0.5:
                logger.debug(f"Classified as BAD - High variance in line spacing (std_dev/median = {std_dev_gap / median_gap:.2f}).")
                return "bad"

            # --- Structural Integrity Check ---
            table_settings = {"vertical_strategy": "lines", "horizontal_strategy": "lines"}
            if not table_crop.find_tables(table_settings):
                logger.debug("Classified as BAD - Text is uniform, but no line-based structure was found.")
                return "bad"

        except Exception as e:
            logger.error(f"ERROR: An error occurred during table classification: {e}", exc_info=self.debug_mode)
            return "bad"

        logger.debug("Classified as GOOD - Text is present, line spacing is uniform, and grid lines were found.")
        return "good"
    ...
    def _construct_table_grid(self, table: dict) -> list[list[dict]]:
        ...
        # # Fallback to creating a grid from the table cells
        # cells = (table.get("table_cells") or table.get("data", {}).get("table_cells") or [])

        # if not cells:
        #     return []

        # # Helper: read either of two possible field names
        # def gx(c: dict, k1: str, k2: str):
        #     return c.get(k1, c.get(k2))

        # # Determine matrix size using end indices
        # # Scan all cells to find the furthest bottom/right positions seen
        # max_row_end = 0
        # max_col_end = 0
        # for c in cells:
        #     r_end = gx(c, "end_row_idx", "end_row_offset_idx")
        #     c_end = gx(c, "end_col_idx", "end_col_offset_idx")
        #     if r_end is None or c_end is None:
        #         continue
        #     max_row_end = max(max_row_end, int(r_end))
        #     max_col_end = max(max_col_end, int(c_end))

        # num_rows = max_row_end
        # num_cols = max_col_end

        # # Prefill with explicit empties (placeholders)
        # empty = {"text": "", "is_placeholder": True}
        # grid = [[dict(empty) for _ in range(num_cols)] for _ in range(num_rows)]

        # for c in cells:

        #     r0 = gx(c, "start_row_idx", "start_row_offset_idx")
        #     c0 = gx(c, "start_col_idx", "start_col_offset_idx")
        #     r1 = gx(c, "end_row_idx", "end_row_offset_idx")
        #     c1 = gx(c, "end_col_idx", "end_col_offset_idx")

        #     if r0 is None or c0 is None:
        #         continue

        #     # Normalize types to int
        #     r0, c0 = int(r0), int(c0)

        #     # Default to single cell if end indices missing
        #     r1 = int(r1) if r1 is not None else r0
        #     c1 = int(c1) if c1 is not None else c0

        #     # If Docling produced an out-of-range start index, skip it
        #     if not (0 <= r0 < num_rows and 0 <= c0 < num_cols):
        #         continue

        #     anchor = dict(c)  # Shallow copy
        #     anchor.setdefault("text", "")
        #     anchor["is_placeholder"] = False
        #     anchor["rowspan"] = max(1, r1 - r0 + 1)
        #     anchor["colspan"] = max(1, c1 - c0 + 1)

        #     # Drop non-dict bboxes so .get(...) is always safe
        #     if "bbox" in anchor and not isinstance(anchor["bbox"], dict):
        #         anchor.pop("bbox", None)

        #     # Place the anchor at the top-left position in the grid, replacing the initial empty placeholder
        #     grid[r0][c0] = anchor

        #     # Fill the spanned area (if any) with span placeholders
        #     if anchor["rowspan"] > 1 or anchor["colspan"] > 1:
        #         for rr in range(r0, min(r1 + 1, num_rows)):
        #             for cc in range(c0, min(c1 + 1, num_cols)):
        #                 if rr == r0 and cc == c0:
        #                     continue
        #                 grid[rr][cc] = {
        #                     "text": "",
    ...
    def _parse_bad_table_with_vision(self, table_image: Image.Image) -> Optional[str]:
        """
