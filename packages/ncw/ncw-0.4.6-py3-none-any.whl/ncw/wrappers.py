...
class Structure:
    ...
    def items(self) -> Iterator[tuple[str, ValueType]]:
        """Iterator over (canoical endpoint, value) tuples"""
        for segments, value in iter_items(self.data):
            yield self.parsing_cache.canonical(segments), value
        #

    def iter_canonical_endpoints(self) -> Iterator[str]:
        """Iterate over the canonical endpoints of the data structure"""
        for canonical_address, _ in self.items():
            yield canonical_address
        #

    @classmethod
    def from_canonical_items(
        cls, sources: Iterable[tuple[str, ValueType]], separator: str = DOT
    ):
        """Constructor method from an iterable of items, inverse function
        of the .items() method for Structures containing no lists
        """
        interim_parsing_cache = SINGLETON_CACHES[separator]
        low_level_items = [
            (interim_parsing_cache[canonical_path], value)
            for canonical_path, value in sources
        ]
        return cls(collection_from_items(low_level_items), separator=separator)
