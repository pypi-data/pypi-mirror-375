...
class EventEmitter:
    ...
    async def emit(
        ...
        """
        print(f"Emitting event: {event_type} to {hub_name}")
        producer = await self._get_producer(hub_name)
        
        print(f"Got producer for {hub_name}")
        event_data = {
        ...
        print(f"Event data for {event_type}")
        ...
        async with producer:
            print(f"Creating batch for {hub_name}")
            event_data_batch = await producer.create_batch()
            print(f"Adding event to batch for {hub_name}")
            event_data_batch.add(event)
            print(f"Sending batch for {hub_name}")
            await producer.send_batch(event_data_batch)
            print(f"Sent batch for {hub_name}")
        
        print(f"Emitted event: {event_type} to {hub_name}")
    ...
    async def emit_batch(
        ...
        print(f"Emitted batch of {len(events)} events to {hub_name}")
